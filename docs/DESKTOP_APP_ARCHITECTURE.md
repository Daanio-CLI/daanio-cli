# Daanio Desktop Architecture Direction

Status: Proposed
Updated: 2026-04-25

This document captures the initial direction for a desktop application for Daanio under these constraints:
...
The goal is to make the desktop client a first-class Daanio surface without forking the Daanio runtime or turning the app into a heavyweight IDE clone.
...
Build Daanio Desktop as a small Rust desktop client with a custom GPU-rendered UI. The app should connect to a local Daanio server/daemon that owns sessions, tools, agent execution, persistence, and permissions.
...
| Backend/runtime | Existing Rust Daanio server/session runtime |
| Process model | Desktop frontend + local Daanio daemon/server |
...
Daanio Desktop should not start as a full IDE and should not look like a conventional chatbot.
...
1. Existing Daanio server channel, if compatible with desktop needs.
...
A debug HUD should exist in the prototype before real Daanio integration is considered complete.
...
- connects to local Daanio server/daemon
- lists sessions
...
```text
crates/
  daanio-desktop-protocol/   # shared protocol/event types if not already covered by server types
  daanio-desktop-ui/         # UI tree, layout, text/cache abstractions, renderer-agnostic pieces
  daanio-desktop-renderer/   # wgpu renderer and GPU resources
  daanio-desktop/            # app shell, platform window, protocol client, product UI
```
...
- small serialization/protocol crates already consistent with Daanio
...
4. Reuse the existing Daanio server protocol or introduce a desktop-specific event protocol crate?
...
Only after that should the real Daanio event stream be connected.
