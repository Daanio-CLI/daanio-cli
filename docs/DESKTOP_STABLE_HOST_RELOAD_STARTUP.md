...
The current desktop hot reload path starts a replacement `daanio-desktop` process. The replacement process creates a new `winit` window, waits behind a reload handoff marker, then the old process exits. This is enough to preserve session continuity, but it cannot reliably preserve the physical OS window.
...
Today `crates/daanio-desktop/src/main.rs` owns almost everything in one process:

```text
daanio-desktop process
  run()
...
```mermaid
flowchart LR
  Launcher[daanio-desktop launcher] --> Host[daanio-desktop-host]
  Host --> Window[winit window]
  Host --> Renderer[wgpu renderer + font/text atlases]
  Host <-->|input, display updates, snapshots| Worker[daanio-desktop-app worker]
  Worker <-->|session protocol| Server[daanio server/daemon]
```
...
- `daanio-desktop-host --daemon` starts at login or after first launch.
...
- `daanio-desktop --host`
- `daanio-desktop --app-worker --ipc <fd-or-socket>`
...
```text
crates/daanio-desktop-protocol   # host/worker messages and scene types
crates/daanio-desktop-renderer   # Canvas/rendering/display-list renderer
crates/daanio-desktop-host       # window/event loop/worker lifecycle
crates/daanio-desktop-app        # product UI worker
crates/daanio-desktop            # launcher or compatibility wrapper
```
...
The recommended path is a stable `daanio-desktop-host` plus a reloadable `daanio-desktop-app` worker communicating via a typed display-list protocol. It avoids Wayland window-position limitations, avoids Rust plugin ABI hazards, and creates a natural path to both smooth hot reloads and better perceived cold-start performance.

## Current implementation snapshot

As of 2026-05-24, the first stable-host slice exists in the current `daanio-desktop` binary:
...
- `cargo test -p daanio-desktop desktop_`
- `cargo check -p daanio-desktop`
- `selfdev build target=desktop`
- runtime smoke: `cargo run -p daanio-desktop --bin daanio-desktop -- --desktop-process-role stable-host --startup-benchmark --startup-log`

## Crate-split and cold-start assessment

The next split should not be a separate binary yet. The code is functionally split, but `crates/daanio-desktop/src/main.rs` is still a large 10k+ line compilation unit that imports both heavy renderer dependencies (`wgpu`, `glyphon`) and app/session logic. Splitting crates before moving more code out of `main.rs` would add build-system complexity without materially reducing cold-start work.

Recommended order:

1. **Module split inside `daanio-desktop` first.** Move stable-host/window/reload code, worker-process loop, and canvas/rendering code out of `main.rs` into focused modules. This reduces merge conflicts and makes crate boundaries obvious without changing packaging.
2. **Promote protocol and scene types to a small crate.** `desktop_protocol`, `desktop_scene`, and IPC framing are already mostly independent. A future `daanio-desktop-protocol` crate can compile quickly and be shared by host and worker without pulling WGPU.
