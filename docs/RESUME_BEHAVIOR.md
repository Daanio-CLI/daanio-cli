...
- Daanio sessions switch the current workspace/client to that session.
- Importable external sessions are first converted to a Daanio session, then resumed in place.
...
- If terminal launch is unavailable, the UI prints manual `daanio --resume <id>` commands.
