...
Status: draft
Owner: daanio
Audience: daanio core, browser bridge authors, adapter authors

## Why this exists

daanio should expose a single first-class `browser` tool while remaining compatible with multiple browser automation backends:
...
The protocol in this document defines the **normalized contract** between daanio and a browser provider.

This is intentionally **not** a demand that every bridge speak exactly the same native command language. Instead:

- daanio defines a **core semantic layer** it can rely on
...
1. **One first-class tool in daanio**
...
3. **Capability negotiation**
   - daanio should know what each provider can and cannot do.
...
- **browser tool**: the user/model-facing daanio tool.
...
- **browser session**: the provider's isolated session or attachment scope for a daanio session.
...
- direct Rust trait calls inside daanio
...
daanio should not care whether a provider uses tabs, contexts, profiles, or remote targets internally.
It only needs a stable handle it can reuse.

### `session.ensure`

Creates or reuses a browser session for a daanio session.

Request:

```json
{
  "client_session_id": "daanio_session_123",
  "browser_preference": "auto",
  "isolation": "per_daanio_session",
  "attach": "prefer",
  "persist": true,
  "metadata": {
    "owner": "agent",
...
daanio must not assume identifier shape or encode browser semantics into them.

---

## Normalized core methods

These are the semantics daanio can rely on.
...
{
  "snapshot": {
    "format": "daanio.page_snapshot.v1",
    "root": {
      "node_id": "n1",
      "role": "document",
      "name": "Example Domain",
...
### Rule 3: daanio core should only rely on normalized methods by default
...
- a daanio-side adapter knows how to use them safely
...
Semantics or qualities that influence daanio behavior:
...
## Recommended daanio integration policy

The daanio `browser` tool should:
...
4. Should `page.snapshot` support multiple named formats beyond `daanio.page_snapshot.v1`?
