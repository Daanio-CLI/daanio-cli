...
`daanio provider-doctor` is a user-facing diagnostic that answers one question:
...
tracks (`daanio provider-test-coverage`), but as an interactive command you can run
...
```bash
# Validate daanio's own wiring for a provider, no API key, no spend:
daanio provider-doctor cerebras --tier offline

# Validate the key + live model catalog (needs a key, negligible spend):
daanio provider-doctor cerebras --tier catalog

# Full readiness, including real chat, streaming, and tool calls (spends balance):
daanio provider-doctor cerebras --tier full

# Pin a specific model and emit JSON for scripting/CI:
daanio provider-doctor cerebras --model gpt-oss-120b --tier full --json
```
...
| --- | --- | --- | --- | --- |
| `offline` | no | no | daanio-side wiring against a synthetic catalog | catalog reload, picker rendering, fallback labeling, and model-switch routing bugs for this provider |
...
12. `real_daanio_tool_smoke` - an end-to-end tool smoke passed (full tier)
...
`daanio provider-test-coverage` shows a cumulative "Recorded spend" footer
...
1. **"My picker is broken / shows the wrong models."**
   ...
   `model_switch_route` fail, it's a daanio-side routing bug for that provider:
...
2. **"It won't connect / says auth failed."**
   ...
   `daanio login --provider <provider>`.
...
`daanio provider-test-coverage`. Lighter tiers record the API-dependent
checkpoints as skipped, so they never over-credit a pair.

`daanio provider-test-coverage` renders the same 11 checkpoints as an 11-stage
...
```
  ...
  6/11   nvidia-nim / gemma-4-31b  failed at `streaming reply`; run `daanio provider-doctor nvidia-nim --model gemma-4-31b --tier full`; last tested 2 days ago ...
```
