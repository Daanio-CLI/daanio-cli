...
Daanio. The root `daanio` crate is still the integration shell, but stable leaf
...
2. Avoid cyclic domain crates. Root `daanio` may depend on leaf crates, but leaf
...
5. Preserve compatibility facades during migration. `crate::storage::*` can
   re-export `daanio-storage::*` while callers move gradually.

## Current first step

`daanio-storage` is now a leaf crate for app paths, permission hardening, atomic
...
- `cargo check -p daanio-storage`: ~0.9s after initial dependencies were built.
- `cargo check -p daanio --lib`: ~14s in the current warm-cache state.

## Recommended next extractions

1. `daanio-provider-anthropic`: move Anthropic request/stream translation out of
   root `src/provider/anthropic.rs` and depend only on `daanio-provider-core`,
   `daanio-message-types`, and serde/reqwest primitives.
2. `daanio-provider-openai`: same for OpenAI request/stream handling. This
   reduces rebuilds when editing server/TUI code and makes provider tests cheap.
3. `daanio-session-core`: move session storage paths, journal metadata, and
   memory-profile pure transforms once dependencies on root prompt/logging are
   cut behind callbacks.
4. `daanio-tui-app-state`: split key/input/navigation state transitions from
   rendering. Keep ratatui rendering in `daanio-tui-render`/root while state tests
   compile without the whole root crate.
5. `daanio-server-protocol-runtime`: split websocket/client event fanout glue from
   agent execution so server tests do not rebuild TUI/provider internals.

## Anti-patterns to avoid

- Extracting crates that depend on root `daanio`. That preserves the compile-time
