...
This document describes a proposed evolution of daanio's UI architecture from the
...
Today, daanio is effectively organized like this:
...
- a independent daanio window attached to one session
...
- current independent daanio window
...
2. Client spawns a independent daanio client attached to the same session.
...
This can initially be provided through existing daanio control channels such as:
...
- Preserve current `daanio --resume <session>` workflow.
...
- Should workspace mode be a new command (`daanio workspace`) or a runtime mode of
  the normal client?
...
This gives daanio a portable built-in multi-session workspace without sacrificing
