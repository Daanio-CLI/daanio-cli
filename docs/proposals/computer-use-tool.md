...
This mirrors the existing **`browser`** tool (`crates/daanio-app-core/src/tool/browser.rs`):
one registered tool, an `action: String` that selects a sub-operation, with optional
typed params. It gives daanio a closed control loop (*see screen → decide → act*)
...
```
crates/daanio-macos-control/        (new) cfg(target_os = "macos") platform crate
...
crates/daanio-app-core/src/tool/computer.rs   (new) ComputerTool
  └─ thin dispatch layer: parse input -> call daanio-macos-control -> ToolOutput
  └─ registered in crates/daanio-app-core/src/tool/mod.rs base_tools()
```
...
1. `AXIsProcessTrustedWithOptions([kAXTrustedCheckOptionPrompt: true])` — shows the
   system dialog *and auto-adds daanio to the Accessibility list* (toggled off).
...
1. `daanio-macos-control` crate: permissions, screenshot, AX read, AX action,
