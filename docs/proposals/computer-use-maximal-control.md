...
`AXIsProcessTrustedWithOptions(prompt=true)` (adds daanio to the list + shows the
...
Important: the permission attaches to the **host binary/terminal** running daanio.
For a stable experience we likely want a signed daanio.app with a fixed bundle id so
...
6. Signed daanio.app bundle for durable permissions.
...
2. **Requests** what it can programmatically:
   - `AXIsProcessTrustedWithOptions(prompt=true)` — shows the Accessibility dialog
     and pre-adds daanio to the list (toggled off).
...
- Ship/install daanio as a **signed `.app` bundle with a stable bundle id**
  (e.g. `com.daanio.app`) and a Designated Requirement, so the Accessibility /
