...
The existing `daanio-plan` graph code is the foundation; this is an evolution of
it, not a rewrite.
...
The pressures above are implemented as hard engine rules in `daanio-plan`'s
...
## 10. Data model changes (against `daanio-plan`)
