...
A human-in-the-loop safety layer for unmonitored agent operations. Designed as an independent subsystem that any daanio feature can integrate with. Currently the only consumer is ambient mode, but the system is intentionally decoupled so it can be reused for future features.
...
sequenceDiagram
    ...
    NF->>US: Email: "daanio ambient wants to create a PR"
...
url = "https://example.com/daanio-safety"
secret = "..."
...
- Extracted 3 memories from crashed session daanio-red-fox-1234
...
```bash
daanio safety review           # Interactive review of pending requests
daanio safety list             # List all pending requests
daanio safety approve <id>     # Approve a specific request
daanio safety deny <id>        # Deny a specific request
daanio safety log              # View decision history
```
...
The safety system exposes a simple API for any daanio feature to use:
...
- [ ] TUI review panel
- [ ] CLI commands (`daanio safety review/list/approve/deny/log`)
