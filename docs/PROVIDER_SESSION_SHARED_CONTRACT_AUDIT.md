...
This document audits the current provider, session, and shared-contract seams in the daanio workspace and recommends the next **realistic** crate moves that improve modularity without creating high-churn dependency cycles.
...
1. **Add a small `daanio-shared-contracts` crate** for the serde-only protocol/session overlap types that already act like shared contracts.
2. **After that, add a narrow `daanio-session-contracts` crate** for session metadata/replay/view structs that are widely reused but do not need the full `Session` runtime.
3. **If we want one more provider-side move before a larger provider refactor, extract the pure provider identity/selection layer** into `daanio-provider-core` or a small `daanio-provider-selection` crate.
...
- `daanio-provider-core`: no local workspace deps
- `daanio-provider-metadata`: no local workspace deps
- `daanio-provider-openrouter`: no local workspace deps
- `daanio-provider-gemini`: no local workspace deps
...
- `daanio-provider-metadata` holds stable login/profile catalog data
- `daanio-provider-core` holds route/cost/shared HTTP client/core value types
- `daanio-provider-openrouter` holds OpenRouter-specific catalog/cache/ranking/model-spec support
- `daanio-provider-gemini` holds Gemini Code Assist schema/types/support helpers
...
- either a new `crates/daanio-provider-selection`
- or a small `provider_identity` / `selection` module inside `daanio-provider-core`
...
### Option B: narrow `daanio-session-contracts`
...
### Option C: add `daanio-shared-contracts`
...
Create `crates/daanio-shared-contracts`.
...
Create `crates/daanio-session-contracts`.

Do this only after Phase 1, so session replay types can point at `daanio_shared_contracts::*` instead of `crate::protocol::*` or `crate::plan::*`.
...
1. Add `crates/daanio-shared-contracts` with serde-only types from `plan.rs` and the small protocol/session overlap set.
...
3. Re-measure touched-file compile times for:
   ...
   - `src/protocol.rs`
   - `src/provider/mod.rs`
4. If the new seam stays clean, follow with a narrow `daanio-session-contracts` extraction.
