...
The first implementation step for Daanio Desktop should be **Phase 0: a fullscreen blank white canvas**.
...
```text
packaging/linux/daanio-desktop.desktop
```

It expects a `daanio-desktop` binary to be available on `PATH`. For local testing after installing or copying the binary somewhere your desktop launcher can execute, copy the entry to your user applications directory:

```bash
mkdir -p ~/.local/share/applications
cp packaging/linux/daanio-desktop.desktop ~/.local/share/applications/
