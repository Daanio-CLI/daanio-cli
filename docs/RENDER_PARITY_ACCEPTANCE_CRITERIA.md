...
Status: adopted for the `daanio-render-core` -> TUI switchover.
Harness: `crates/daanio-tui-markdown/src/render_core_adapter_tests.rs`
...
|------|---------------------------|------|--------------------------------------------------|
| CI (default) | 5000 (L1, L2), 3000×3 widths = 9000 renders (L3) | must pass on every PR touching `daanio-tui-markdown`, `daanio-render-core` | p < 6.0e-4 per generated document (L1/L2); p < 1.0e-3 per doc per width (L3) |
| Pre-switchover deep run | `DAANIO_MD_FUZZ_ITERS=100000` | must pass once, on the exact commit proposed for switchover | p < 3.0e-5 per generated document |
| Nightly (optional soak) | `DAANIO_MD_FUZZ_ITERS=25000`, rotating `DAANIO_MD_FUZZ_SEED` (e.g. epoch-day) | failures file an issue with repro seed | accumulates coverage across seeds over time |
...
- **Reproducibility:** each failure reports the iteration index `i`; the
  ...
  reproducible with `DAANIO_MD_FUZZ_SEED=<base_seed> DAANIO_MD_FUZZ_ITERS=<i+1>`
  (or by re-deriving the single seed). `base_seed` defaults are fixed
  constants per suite and overridable via `DAANIO_MD_FUZZ_SEED`.
...
cargo test -p daanio-tui-markdown --lib render_core_adapter

# Deep run (switchover gate; ~80s on an XPS 13)
DAANIO_MD_FUZZ_ITERS=100000 \
  cargo test -p daanio-tui-markdown --lib render_core_adapter::tests::fuzz

# Alternate-seed confirmation
DAANIO_MD_FUZZ_ITERS=100000 DAANIO_MD_FUZZ_SEED=20260713 \
  cargo test -p daanio-tui-markdown --lib render_core_adapter::tests::fuzz
```
