...
//!   notice that it renders daanio poorly and suggest a modern terminal (Ghostty).
//! - Linux: create a .desktop launcher file.
//!
//! Each nudge can be dismissed permanently with "Don't ask again".
//! State is persisted in `~/.daanio/setup_hints.json`.
...
use macos_terminal::{
    ...
    launch_command_for_macos_terminal, paused_daanio_shell_command, save_preferred_macos_terminal,
};
#[cfg(windows)]
use windows_setup::{
...
///   connection.
/// - 3: register three launch hotkeys instead of one. `Cmd+;` opens daanio in
///   `$HOME`, `Cmd+'` opens it in the last project directory, and `Cmd+Shift+'`
///   opens a self-dev session in the last daanio repo. Existing users are
...
///   `daanio-terminal-launch`, avoiding the generated shell-script hop on hotkey
///   press. Scripts/plan are still written for compatibility and diagnostics.
/// - 6: direct launches pass `--spawn-hotkey` into the new Daanio process so
...
impl SetupHintsState {
    fn path() -> Result<PathBuf> {
        Ok(storage::daanio_dir()?.join("setup_hints.json"))
    }
...
fn mac_hotkey_support_dir() -> Result<PathBuf> {
    Ok(storage::daanio_dir()?.join("hotkey"))
}

/// File holding the last project directory daanio was launched from. The `Cmd+'`
/// global hotkey reads this at fire time to reopen daanio there.
#[cfg(any(test, target_os = "macos", target_os = "linux", windows))]
fn mac_hotkey_last_dir_file() -> Result<PathBuf> {
    Ok(mac_hotkey_support_dir()?.join("last_dir"))
}

/// File holding the last daanio *repository* directory the user worked in. The
...
/// Load the `[launch_hotkeys]` table from `~/.daanio/config.toml`.
///
...
fn load_launch_hotkeys_config() -> daanio_config_types::LaunchHotkeysConfig {
    #[derive(serde::Deserialize, Default)]
    struct Wrapper {
        #[serde(default)]
        launch_hotkeys: daanio_config_types::LaunchHotkeysConfig,
    }
    let Ok(dir) = storage::daanio_dir() else {
...
/// its own `Cmd+;` hotkey). When `dir` is inside a daanio repo, the repo root is
...
pub fn record_launch_dirs(dir: &std::path::Path, repo_dir: Option<&std::path::Path>) {
    #[cfg(any(target_os = "macos", target_os = "linux", windows))]
    {
        if let Err(err) = record_launch_dirs_inner(dir, repo_dir) {
            daanio_logging::warn(&format!("failed to record launch dirs for hotkeys: {err}"));
        }
    }
...
<dict>
    <key>Label</key>
    <string>com.daanio.hotkey</string>
    ...
    <dict>
        <key>DAANIO_PREFERRED_TERMINAL</key>
...
/// Launch a new daanio window in the user's preferred macOS terminal, passing
/// `extra_args` (e.g. `["--resume", "<session-id>"]`) to the daanio invocation.
///
...
#[cfg(target_os = "macos")]
pub fn launch_daanio_in_macos_terminal(extra_args: &[String]) -> Result<()> {
    let terminal = effective_macos_terminal();
    let exe = std::env::current_exe()?;
    let exe_path = exe.to_string_lossy().into_owned();
    let shell_command =
        macos_terminal::paused_daanio_shell_command_with_args(&exe_path, extra_args);
    ...
    let command = match macos_terminal::no_automation_launch(terminal, &shell_command) {
        ...
        macos_terminal::NoAutomationLaunch::CommandFile { app } => {
            let dir = storage::daanio_dir()?.join("launcher");
    ...
    let status = std::process::Command::new("sh")
        ...
        .arg(&command)
        .status()
        .context("failed to launch terminal for daanio")?;
    if !status.success() {
        anyhow::bail!(
...
fn install_macos_hotkey_listener(
    ...
    let status = std::process::Command::new("launchctl")
        ...
        .status()
        .context("failed to load daanio LaunchAgent")?;
    if !status.success() {
...
fn startup_hints_for_launch(_state: &SetupHintsState) -> Option<StartupHints> {
    ...
    let spawn_notice = if !_state.hotkey_configured || _state.startup_spawn_hint_dismissed {
        ...
        Some(format!(
            "Cmd+; launches a new daanio in your home directory from anywhere, system-wide (opens in {}). Cmd+' reopens your last project; Cmd+Shift+' opens a self-dev session.",
...
fn macos_terminal_notice(
    ...
    let message = "The built-in macOS Terminal.app renders daanio poorly (slow, limited colors, no inline images). Consider a modern terminal such as Ghostty, iTerm2, or Alacritty for a much better experience.".to_string();

    Some(StartupHints::with_status_and_display(
        "Tip: Terminal.app renders daanio poorly. Try Ghostty, iTerm2, or Alacritty.".to_string(),
        "Terminal",
        message,
    ))
}
...
/// Manual `daanio setup-hotkey` command.
///
...
pub fn run_setup_hotkey(
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        let terminal = effective_macos_terminal();
        eprintln!("\x1b[1mdaanio setup-hotkey\x1b[0m");
        eprintln!();
        eprintln!("  Preferred terminal: {}", terminal.label());
        eprintln!("  Installing a LaunchAgent with three system-wide daanio launch hotkeys.");
        eprintln!();
        ...
        match install_macos_hotkey_listener(Some(terminal)) {
            Ok(installed_terminal) => {
                ...
                let _ = state.save();
                eprintln!(
                    "  \x1b[32m✓\x1b[0m Created launch hotkeys → {} + daanio",
                    installed_terminal.label()
                );
                eprintln!();
                eprintln!("  Press these anywhere, system-wide:");
                eprintln!("    \x1b[1mCmd+;\x1b[0m       new daanio in your home directory");
                eprintln!(
                    "    \x1b[1mCmd+'\x1b[0m       new daanio in your last project directory"
                );
                eprintln!(
                    "    \x1b[1mCmd+Shift+'\x1b[0m new daanio self-dev session (last daanio repo)"
                );
        ...
        if let Some(comp) = detect_linux_compositor() {
            ...
            match install_linux_launch_hotkeys(comp) {
                Ok(changed) => {
                    if changed {
                        eprintln!(
                            "  \x1b[32m✓\x1b[0m Installed daanio launch hotkeys into your {} config.",
                            comp.name()
                        );
                    } else {
                        eprintln!(
                            "  \x1b[32m✓\x1b[0m daanio launch hotkeys already up to date in your {} config.",
                            comp.name()
                        );
                    }
                    state.hotkey_configured = true;
...
pub(crate) fn install_cli_launch_hints_notice() {
    match cli_launch_hints::install_available() {
        ...
        Ok(_) => {}
        Err(err) => daanio_logging::warn(&format!(
...
pub(crate) fn active_primary_launch_hotkey() -> Option<(String, String)> {
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        let exe_path = std::env::current_exe()
            ...
            .unwrap_or_else(|_| "daanio".to_string());
...
fn run_macos_hotkey_listener() -> Result<()> {
    use daanio_terminal_launch::{TerminalCommand, spawn_command_in_new_terminal_with};
    ...
    // launches daanio on key-down, then hand the thread to the event loop so the
    ...
    // standard daanio log plus the plist's StandardOut/ErrorPath.
    daanio_logging::init();
    macos_hotkey_log("starting macOS daanio launch hotkey listener");
    ...
    // to a cwd + daanio argv. Older versions dispatched through generated shell
    ...
    if launch_for_id.is_empty() {
        anyhow::bail!("failed to register any daanio launch hotkey");
    }
    ...
    GlobalHotKeyEvent::set_event_handler(Some(move |event: GlobalHotKeyEvent| {
        ...
        if let Some(launch) = launch_for_id.get(&event.id) {
            ...
            let command = TerminalCommand::new(&exe_path, args)
                .fresh_spawn()
                .kind("hotkey")
                .spawn_env("DAANIO_SPAWN_LABEL", launch.label.clone());
            match spawn_command_in_new_terminal_with(&command, &cwd, |cmd| cmd.spawn().map(|_| ()))
            {
                Ok(true) => {}
                Ok(false) => {
                    macos_hotkey_log("failed to launch daanio: no terminal candidate worked")
                }
                Err(err) => macos_hotkey_log(&format!("failed to launch daanio: {err}")),
            }
        }
    }));

    macos_hotkey_log("macOS daanio launch hotkeys registered; entering event loop");
    ...
    macos_hotkey_log("macOS daanio launch hotkey event loop exited");
    Ok(())
}
...
/// Log a hotkey-listener diagnostic to both the daanio log and stderr.
///
...
fn macos_hotkey_log(message: &str) {
    daanio_logging::info(message);
    eprintln!("[daanio hotkey] {message}");
}
...
pub fn maybe_show_setup_hints() -> Option<StartupHints> {
            ...
            && state.launch_hotkey_tracking_version < LAUNCH_HOTKEY_TRACKING_VERSION
        {
            match windows_setup::refresh_windows_launch_hotkeys() {
                Ok(()) => {
                    state.launch_hotkey_tracking_version = LAUNCH_HOTKEY_TRACKING_VERSION;
                    let _ = state.save();
                    daanio_logging::info("Migrated Windows launch hotkeys to usage tracking");
                }
                Err(err) => daanio_logging::warn(&format!(
    ...
    // `daanio setup-launcher` explicitly.
...
fn macos_launch_hotkeys_notice(state: &SetupHintsState) -> Option<StartupHints> {
    ...
    Some(StartupHints::with_status_and_display(
        ...
        "Launch hotkeys",
        format!("Configured Daanio launch hotkeys:\n{}", lines.join("\n")),
    ))
}
...
/// Config file daanio manages for a flat (`#`-commented) compositor config.
...
/// Directory for daanio's hidden KDE launcher desktop files.
#[cfg(target_os = "linux")]
...
/// The config file daanio would manage for the *current* session's compositor.
#[cfg(target_os = "linux")]
...
/// The sentinel that marks daanio's managed region in `path` for `comp`.
#[cfg(target_os = "linux")]
...
/// Whether daanio's launch hotkeys are already installed for `comp`.
#[cfg(target_os = "linux")]
fn linux_hotkeys_installed(comp: linux_env::LinuxCompositor) -> bool {
    use linux_env::LinuxCompositor;
    match comp {
        LinuxCompositor::Gnome => gnome_keybinding_list().contains("/daanio-launch-"),
        LinuxCompositor::Cinnamon => {
            dconf_read("/org/cinnamon/desktop/keybindings/custom-list").contains("daanio-launch-")
        }
        LinuxCompositor::Mate => {
            dconf_list("/org/mate/desktop/keybindings/").contains("daanio-launch-")
        }
        LinuxCompositor::Xfce => xfce_shortcut_commands_text().contains("/launch_daanio_"),
        LinuxCompositor::Kde => kde_globalshortcutsrc_path()
            .and_then(|p| std::fs::read_to_string(p).ok())
            .map(|text| text.contains("[services][daanio-launch-"))
            .unwrap_or(false),
...
/// Pick a terminal emulator to launch daanio in on Linux. Honors `$TERMINAL`,
...
/// scripts written under `~/.daanio/hotkey/`, so the config never embeds shell
...
/// xfconf-query. Stale daanio entries bound to accelerators we no longer use
...
fn install_xfce_launch_hotkeys() -> Result<bool> {
    ...
    // Remove stale daanio entries bound to accelerators we no longer use.
    for line in existing.lines() {
        let Some((prop, value)) = line.split_once(char::is_whitespace) else {
            continue;
        };
        if value.trim().contains("/launch_daanio_") && !wanted.iter().any(|(p, _)| p == prop) {
...
/// `~/.daanio/hotkey/` and return the chord -> script binds. The scripts `cd`
/// into the target (with `$HOME` fallback for stale/dynamic dirs) and exec the
/// user's terminal running daanio, so compositor bind lines stay trivial.
#[cfg(target_os = "linux")]
fn write_linux_launch_scripts() -> Result<Vec<linux_env::ScriptBind>> {
    ...
    for entry in resolved {
        ...
        let script_body = format!(
            "#!/bin/sh\n# Auto-generated by daanio setup-hotkey; re-run it to refresh.\n{cd}exec {exec}\n",
            cd = entry.cd_prefix,
        );
        let script_path = hotkey_dir.join(&entry.script_file_name);
...
/// Timestamped backup matching the `.bak-daanio-*` convention, taken before
...
fn backup_compositor_config(config_path: &std::path::Path) {
    ...
    let backup = config_path.with_file_name(format!("{file_name}.bak-daanio-hotkeys-{ts}"));
...
fn reload_compositor_config(comp: linux_env::LinuxCompositor) {
        ...
        .status()
    {
        ...
        Ok(status) => daanio_logging::warn(&format!(
            "{} exited with {status} while reloading hotkey binds",
            cmd[0]
        )),
        Err(err) => daanio_logging::warn(&format!("failed to run {} reload: {err}", cmd[0])),
    }
}
...
fn linux_launch_hotkeys_notice(state: &SetupHintsState) -> Option<StartupHints> {
    ...
    Some(StartupHints::with_status_and_display(
        ...
        "Launch hotkeys",
        format!(
            "Configured Daanio launch hotkeys ({}): {} {}",
            comp.name(),
            lines.join("; "),
            footer
        ),
...
/// - Once the user has learned at least one binding and has launched daanio at
...
/// Check whether daanio's keybindings conflict with shortcuts owned by the
...
pub fn maybe_show_keymap_conflict_hint(
    keybindings: &daanio_config_types::KeybindingsConfig,
) -> Option<StartupHints> {
    if !io::stdin().is_terminal() || !io::stderr().is_terminal() {
        return None;
    }
...
/// Whether the current terminal triggers daanio's glyph-safe color quantization
/// (macOS VS Code integrated terminal / Apple Terminal). Mirrors the detection
/// in `daanio-tui-style`'s color module and `daanio-app-core::perf` so the
/// disclosure fires exactly when the behavior is active. Overridable with
/// `DAANIO_GLYPH_SAFE_MODE=on|off`.
fn glyph_safe_mode_active() -> bool {
    if let Ok(raw) = std::env::var("DAANIO_GLYPH_SAFE_MODE") {
...
/// shown the first time daanio launches in a fragile-glyph terminal. Discloses
...
its glyph cache under daanio's full-color animations, rendering letters as boxes. \
daanio automatically quantizes colors to the 256-palette here to keep text readable; \
the only tradeoff is slightly reduced color fidelity. Animations still run. \
For full color, use Ghostty, iTerm2, kitty, or WezTerm, or set DAANIO_GLYPH_SAFE_MODE=off."
        .to_string();
    (
        Some(StartupHints::with_status_and_display(
...
/// Manual `daanio setup-launcher` command.
pub fn run_setup_launcher() -> Result<()> {
    #[cfg(target_os = "macos")]
    {
        let mut state = SetupHintsState::load();
        eprintln!("\x1b[1mdaanio setup-launcher\x1b[0m");
        eprintln!();
        ...
        match install_macos_app_launcher() {
            Ok((app_dir, terminal)) => {
                ...
                eprintln!(
                    "  \x1b[32m✓\x1b[0m Spotlight/Launchpad/Dock will launch daanio in {}",
                    terminal.label()
                );
                eprintln!();
                eprintln!("  Tip: pin Daanio.app to your Dock or launch it with Cmd+Space.");
                return Ok(());
            }
            Err(e) => {
                eprintln!("  \x1b[31m✗\x1b[0m Failed: {}", e);
    ...
    #[cfg(windows)]
    {
        ...
        match create_windows_desktop_shortcut(&mut state) {
            Ok(()) => {
                eprintln!("  \x1b[32m✓\x1b[0m Created desktop shortcut: daanio.lnk");
                return Ok(());
            }
            Err(e) => {
                eprintln!("  \x1b[31m✗\x1b[0m Failed: {}", e);
...
/// Create a desktop shortcut/launcher for daanio.
///
/// - macOS: creates a daanio.app bundle in ~/Applications/
/// - Windows uses [`windows_setup::create_windows_desktop_shortcut`] via
///   `daanio setup-launcher` instead (PowerShell/COM is too slow for the
///   startup path).
...
fn create_desktop_shortcut(state: &mut SetupHintsState) -> Result<()> {
    #[cfg(any(test, target_os = "macos"))]
    {
        ...
        daanio_logging::info(&format!("Created macOS app bundle: {}", app_dir.display()));
    }
...
pub fn reinstall_launch_hotkeys_after_config_change() {
    #[cfg(target_os = "macos")]
    {
        ...
        match install_macos_hotkey_listener(preferred) {
            Ok(terminal) => daanio_logging::info(&format!(
                "Reinstalled launch hotkeys after config change for {}",
                terminal.label()
            )),
            Err(err) => daanio_logging::warn(&format!("failed to reinstall launch hotkeys: {err}")),
        }
    }
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        match install_linux_launch_hotkeys(comp) {
            Ok(true) => daanio_logging::info(&format!(
