...
fn first_three_launches_can_include_hotkey_notice_too() {
    ...
    // The notice should make clear the hotkey works globally, not just inside daanio.
...
fn default_resolved_hotkeys_match_legacy_three() {
    ...
    let resolved = launch_hotkeys::resolve_launch_hotkeys(
        &daanio_config_types::LaunchHotkeysConfig::default(),
        "/usr/local/bin/daanio",
        "/home/u/.daanio/hotkey/last_dir",
        "/home/u/.daanio/hotkey/last_repo",
    );
    ...
    let home = launch_hotkeys::shell_command_for(&resolved[0], "/usr/local/bin/daanio");
    assert!(home.starts_with("cd \"$HOME\"; "));
    assert!(!home.contains("self-dev"));

    let last_dir = launch_hotkeys::shell_command_for(&resolved[1], "/usr/local/bin/daanio");
    assert!(last_dir.contains("cat '/home/u/.daanio/hotkey/last_dir'"));
    assert!(last_dir.contains("cd \"$HOME\""));

    let selfdev = launch_hotkeys::shell_command_for(&resolved[2], "/usr/local/bin/daanio");
    assert!(selfdev.contains("cat '/home/u/.daanio/hotkey/last_repo'"));
    assert!(selfdev.contains("'/usr/local/bin/daanio' 'self-dev';"));
}
...
fn baked_repo_hotkey_cds_into_fixed_dir() {
    ...
    let config = daanio_config_types::LaunchHotkeysConfig {
        enabled: Some(true),
        imported: true,
        entries: vec![daanio_config_types::LaunchHotkeyEntry {
            chord: "cmd+[".to_string(),
            dir: "/Users/jeremy/daanio-github".to_string(),
            label: "daanio-github".to_string(),
            self_dev: false,
        }],
    };
    let resolved = launch_hotkeys::resolve_launch_hotkeys(
    ...
    let cmd = launch_hotkeys::shell_command_for(&resolved[0], "/usr/local/bin/daanio");
    assert!(cmd.contains("/Users/jeremy/daanio-github"));
...
fn mac_hotkey_launch_agent_plist_uses_valid_xml_quotes() {
    let plist = mac_hotkey_launch_agent_plist(
        "/Applications/Daanio.app/Contents/MacOS/daanio",
        "/tmp/daanio-hotkey.out.log",
        "/tmp/daanio-hotkey.err.log",
        "ghostty",
    );
...
#[test]
fn paused_daanio_shell_command_keeps_failures_visible() {
    let command = paused_daanio_shell_command("/tmp/daanio");
    assert!(command.contains("Press Enter to close"));
    assert!(command.contains("Daanio exited with status"));
    assert!(command.contains("daanio executable not found"));
}
...
fn macos_terminal_notice_only_fires_for_default_terminal_app() {
    ...
    assert_eq!(
        ...
        Some("Tip: Terminal.app renders daanio poorly. Try Ghostty, iTerm2, or Alacritty.")
    );
    let (title, message) = hints.display_message.expect("expected display message");
    assert_eq!(title, "Terminal");
    assert!(message.contains("Terminal.app renders daanio poorly"));
...
fn keymap_conflict_hint_full_path_debounces_and_persists_signature() {
    ...
    use daanio_config_types::KeybindingsConfig;
...
fn glyph_safe_notice_shows_once_then_debounces() {
    ...
    assert!(body.contains("DAANIO_GLYPH_SAFE_MODE=off"));
