use super::{
    ...
    escape_shell_single_quotes, launch_command_for_macos_terminal, paused_daanio_shell_command,
    save_preferred_macos_terminal,
};
use anyhow::{Context, Result};
use std::path::{Path, PathBuf};

const MACOS_APP_ICON_FILE_NAME: &str = "Daanio.icns";
const MACOS_APP_ICON_BYTES: &[u8] = include_bytes!(concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/../../assets/app-icons/Daanio.icns"
));
...
pub(super) fn install_macos_app_launcher() -> Result<(PathBuf, MacTerminalKind)> {
    ...
    let launcher_path = macos_dir.join("daanio-launcher");
...
<dict>
    ...
    <string>Daanio</string>
    <key>CFBundleDisplayName</key>
    <string>Daanio</string>
    <key>CFBundleIdentifier</key>
    <string>com.daanio.launcher</string>
    <key>CFBundleVersion</key>
    <string>{version}</string>
    <key>CFBundleShortVersionString</key>
    <string>{version}</string>
    <key>CFBundleExecutable</key>
    <string>daanio-launcher</string>
...
"#,
        version = daanio_build_meta::version(),
...
fn macos_app_launcher_dir() -> Result<PathBuf> {
    ...
    Ok(home.join("Applications").join("Daanio.app"))
}
...
/// `Path::exists()` on `daanio.app` also matches `Daanio.app`. The legacy-bundle
...
fn macos_launcher_script(terminal: MacTerminalKind, exe_path: &str, app_dir: &Path) -> String {
    ...
    let shell_command = paused_daanio_shell_command(exe_path);
    ...
    let missing_message = escape_applescript_text(&format!(
        "Daanio could not launch because the executable was not found.\n\nExpected path:\n{}\n\nTry reinstalling daanio or rerun:\ndaanio setup-launcher",
        exe_path
    ));
    let terminal_failure_message = escape_applescript_text(&format!(
        "Daanio could not open {}.\n\nTry rerunning:\ndaanio setup-launcher\n\nLauncher log:\n~/.daanio/launcher/macos-launcher.log",
        terminal.label()
    ));
...
LOG_DIR="$HOME/.daanio/launcher"
LOG_FILE="$LOG_DIR/macos-launcher.log"
mkdir -p "$LOG_DIR" >/dev/null 2>&1 || true

show_missing_executable() {{
  /usr/bin/osascript <<'APPLESCRIPT' >/dev/null 2>&1 || true
display alert "Daanio launch failed" message "{missing_message}" as critical
APPLESCRIPT
}}

show_terminal_launch_failure() {{
  /usr/bin/osascript <<'APPLESCRIPT' >/dev/null 2>&1 || true
display alert "Daanio launch failed" message "{terminal_failure_message}" as critical
APPLESCRIPT
}}
