//! Render and install global "launch a new daanio" hotkeys on Linux/niri.
//!
...
//!     Alt+Semicolon hotkey-overlay-title="daanio: home" { spawn "sh" "-c" "..."; }
//!     // <<< daanio launch hotkeys (managed) <<<
//! ```
...
pub(crate) const NIRI_BLOCK_BEGIN: &str = "// >>> daanio launch hotkeys (managed) >>>";
/// Closing sentinel for the managed bind region inside `binds { }`.
pub(crate) const NIRI_BLOCK_END: &str = "// <<< daanio launch hotkeys (managed) <<<";
...
pub(crate) struct NiriHotkey {
    pub chord: KeyChord,
    /// Concrete directory to launch daanio in (already resolved from any
...
/// Map a daanio modifier+key chord onto niri's KDL key syntax.
///
/// niri uses `+`-joined modifiers followed by an XKB key name, e.g.
/// `Alt+Semicolon`, `Super+Shift+Apostrophe`. We translate daanio's `cmd`
...
pub(crate) fn chord_to_niri_bind(chord: &KeyChord) -> Option<String> {
    ...
    // daanio `cmd` == macOS Command == Wayland Super.
    if chord.cmd {
        parts.push("Super");
    }
    if chord.ctrl {
...
/// Translate a canonical daanio key token into the XKB key name niri expects.
...
/// Build the `sh -c` command string a bind runs to open daanio in `dir`.
///
/// We `cd` into the directory (falling back to `$HOME` if it has since been
/// removed), then launch daanio via the user's terminal. The terminal is chosen
/// by `terminal` (e.g. `kitty`); we pass it the daanio executable directly.
fn launch_shell_command(
    ...
    // cd with $HOME fallback, then exec the terminal running daanio.
    format!(
...
pub(crate) fn render_niri_bind_line(
    ...
    let title = if hotkey.self_dev {
        format!("daanio: {} (self-dev)", hotkey.label)
    } else {
        format!("daanio: {}", hotkey.label)
    };
...
mod tests {
    ...
    fn renders_bind_line_with_cd_and_terminal() {
        let line = render_niri_bind_line(
            &hk("alt+;", "/home/jeremy/daanio", "daanio", true),
            "/home/jeremy/.local/bin/daanio",
            "kitty",
            "    ",
        )
        .unwrap();
        ...
        assert!(line.contains("hotkey-overlay-title=\"daanio: daanio (self-dev)\""));
        assert!(line.contains("spawn \"sh\" \"-c\""));
        assert!(line.contains("/home/jeremy/daanio"));
    ...
    fn render_block_wraps_sentinels() {
        ...
        .unwrap();
        assert!(block.starts_with("    // >>> daanio launch hotkeys (managed) >>>"));
        assert!(
            block
                .trim_end()
                .ends_with("// <<< daanio launch hotkeys (managed) <<<")
        );
    ...
    fn shell_command_cds_and_self_devs() {
        let s = launch_shell_command("/bin/daanio", "kitty", "/home/u/proj", "cmd+shift+'", true);
        assert!(s.contains("cd '/home/u/proj'"));
        assert!(s.contains("exec 'kitty' '/bin/daanio' --spawn-hotkey"));
