//! Resolve the global "launch a new daanio" hotkeys from config into a concrete
...
//!   [`ResolvedLaunchHotkey`]s. An empty config reproduces daanio's historical
...
/// Build a daanio launch snippet that pauses on non-zero exit so the user can
...
#[cfg(any(test, target_os = "macos"))]
fn paused_daanio_shell_command_with_args(exe_path: &str, args: &[String]) -> String {
...
pub(crate) struct ResolvedLaunchHotkey {
    /// daanio-style chord string, e.g. `cmd+;`.
    pub chord: String,
    ...
    /// fallback) before launching daanio.
    ...
    pub dir: String,
    /// Extra CLI args passed to daanio (e.g. `self-dev`).
    pub args: Vec<String>,
...
/// `launch_daanio_cmd_bracketleft.sh`. Collisions are avoided by appending the
...
fn script_name_for(chord: &str, index: usize) -> String {
    ...
    format!("launch_daanio_{index}_{slug}.sh")
}

/// The built-in default entries, used when config has none. Mirrors daanio's
...
pub(crate) fn shell_command_for(entry: &ResolvedLaunchHotkey, exe_path: &str) -> String {
    format!(
        "{}{}",
        entry.cd_prefix,
        paused_daanio_shell_command_with_args(exe_path, &entry.args)
    )
}

/// Map a daanio key token onto a `global_hotkey` `Code`. Returns `None` for
...
mod tests {
    ...
    fn shell_command_includes_cd_and_exe() {
        ...
        let cmd = shell_command_for(&resolved[0], "/bin/daanio");
        assert!(cmd.contains("/Users/jeremy/proj"));
        assert!(cmd.contains("/bin/daanio"));
    }
}
