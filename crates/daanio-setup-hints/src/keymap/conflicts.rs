//! Detect conflicts between daanio's own key bindings and the bindings
//! discovered on the machine (terminal emulator + macOS system shortcuts).
//!
//! The flow is:
//!   1. enumerate daanio's configured bindings as `(field, label, KeyChord)`
//!      from [`daanio_config_types::KeybindingsConfig`] ([`daanio_bindings`]),
...
/// One configured daanio binding, tied back to its config field.
#[derive(Debug, Clone)]
pub struct DaanioBinding {
...
/// A detected conflict between a daanio binding and something on the machine.
#[derive(Debug, Clone)]
pub struct Conflict {
    /// The daanio binding that may not reach the app.
    pub daanio: DaanioBinding,
...
/// Enumerate daanio's configured bindings as comparable chords. Bindings that are
...
/// chord.
pub fn daanio_bindings(cfg: &KeybindingsConfig) -> Vec<DaanioBinding> {
...
/// Find conflicts between daanio's configured bindings and the discovered
/// machine bindings in `snapshot`.
///
/// Conflicts are deduplicated per `(daanio field, interceptor chord, source)` so
...
pub fn detect_conflicts(cfg: &KeybindingsConfig, snapshot: &KeymapSnapshot) -> Vec<Conflict> {
    ...
    for daanio in daanio_bindings(cfg) {
        let Some(interceptors) = by_chord.get(&daanio.chord) else {
            continue;
        };
...
mod tests {
    ...
    fn detects_ghostty_ctrl_tab_conflict() {
        ...
        let fields: Vec<&str> = conflicts.iter().map(|c| c.daanio.field.as_str()).collect();
    ...
    fn detects_window_manager_prompt_jump_conflict() {
        ...
        // focus, shadowing daanio's built-in prompt navigation fallback.
        let cfg = KeybindingsConfig::default();
        let snapshot = snapshot_with(vec![
        ...
        assert!(
            conflicts
                .iter()
                .all(|c| c.daanio.field.contains("built-in fallback")),
    ...
    fn no_conflict_when_chords_differ() {
        ...
        // daanio has no cmd+t binding by default.
    ...
    fn disabled_binding_is_not_reported() {
        ...
        assert!(
            !conflicts
                .iter()
                .any(|c| c.daanio.field == "keybindings.model_switch_next")
        );
    }
