...
//! (macOS system shortcuts + terminal emulator bindings) so daanio can detect
//! when one of them intercepts a key daanio wants to use.
//!
...
//!
//! Conflict detection against daanio's own bindings is built on top of this in a
//! later layer.
...
pub use conflicts::{
    Conflict, DaanioBinding, conflict_signature, daanio_bindings, detect_conflicts,
};
...
pub fn snapshot_path() -> anyhow::Result<std::path::PathBuf> {
    Ok(daanio_storage::daanio_dir()?.join("keymap-snapshot.json"))
}

/// Collect a fresh snapshot and persist it to `~/.daanio/keymap-snapshot.json`.
...
pub fn refresh_and_save() -> KeymapSnapshot {
    ...
    if let Ok(path) = snapshot_path()
        ...
        && let Err(err) = daanio_storage::write_json_fast(&path, &snapshot)
    {
        daanio_logging::warn(&format!("keymap snapshot write failed: {err}"));
    }
    snapshot
}
