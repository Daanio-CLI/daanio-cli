...
fn macos_launcher_script_shows_alerts_and_uses_terminal_launcher() {
    let script = macos_launcher_script(
        ...
        "/tmp/daanio",
        Path::new("/Users/test/Applications/Daanio.app"),
    );
    assert!(script.contains("display alert \"Daanio launch failed\""));
    assert!(script.contains("daanio setup-launcher"));
...
fn macos_launcher_refreshes_when_new_bundle_missing() {
    ...
    let app_dir = temp.path().join("Daanio.app");
    let legacy_app_dir = temp.path().join("daanio.app");
