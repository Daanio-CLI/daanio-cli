...
use daanio_message_types::ToolCall;
...
// `ResumeTarget` is pure data and now lives in `daanio-session-types` so the
// foundation/import layer can use it without depending on this UI crate. It is
// re-exported here so existing `daanio_tui_session_picker::ResumeTarget` paths
// keep working.
pub use daanio_session_types::ResumeTarget;
...
mod tests {
    ...
    fn source_predicates_cover_provider_and_model_fallbacks() {
        ...
        assert!(session_is_pi(SessionSource::Daanio, Some("pi-main"), None));
