...
//!
//! Run with:
//!   cargo test -p daanio-tui-markdown --test highlight_backend_probe -- --ignored --nocapture

use daanio_tui_markdown::highlight_line;
...
fn workload() -> Vec<(&'static str, &'static str)> {
    vec![
        ...
        (
            "sh",
            "for pid in $(pgrep -f daanio); do awk '/VmRSS/{print $2}' /proc/$pid/status; done",
        ),
        (
            "toml",
