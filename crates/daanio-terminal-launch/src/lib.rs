...
pub struct TerminalCommand {
    ...
    /// Exported as `DAANIO_SPAWN_KIND` to spawn hooks and spawned terminals.
    pub kind: Option<String>,
    /// The daanio session this terminal will run, when known.
    /// Exported as `DAANIO_SPAWN_SESSION_ID`.
    pub session_id: Option<String>,
    /// Extra metadata env entries (e.g. `DAANIO_SPAWN_SWARM_ID`) exported to
    /// spawn hooks and spawned terminals. Applied after the first-class
    /// `DAANIO_SPAWN_*` keys, so entries here win on key collisions.
    ...
    /// and are also exported under a `DAANIO_CLIENT_*` prefix so spawn/focus
...
///
/// The daanio server process is long-lived and captures these at *its* startup,
...
fn resume_terminal_candidates_with_client_env(
    ...
    // DAANIO_TERMINAL and configured spawn hooks still take precedence.
...
pub fn resume_terminal_candidates() -> Vec<String> {
    let configured_terminal = std::env::var("DAANIO_TERMINAL").ok();
...
pub fn resume_terminal_candidates() -> Vec<String> {
    let mut candidates = Vec::new();
    if let Ok(term) = std::env::var("DAANIO_TERMINAL") {
        push_unique_terminal(&mut candidates, term);
    }
...
/// The `DAANIO_SPAWN_*` metadata env exported to spawn hooks and to terminals
/// launched by the built-in fallback:
///
/// - `DAANIO_SPAWN_KIND`: why this spawn happened ("resume", "selfdev",
///   "swarm-agent", ...), when known.
/// - `DAANIO_SPAWN_SESSION_ID`: the daanio session the window will run.
/// - `DAANIO_SPAWN_TITLE`: the suggested window/tab title.
/// - `DAANIO_SPAWN_CWD`: the working directory for the session.
/// - `DAANIO_SPAWN_PROGRAM`: path of the daanio binary to execute.
/// - `DAANIO_SPAWN_COMMAND`: the full command line, shell-escaped, for hooks
///   (like tmux) that take a single shell-command string.
///
/// `TerminalCommand::extra_env` entries (e.g. `DAANIO_SPAWN_SWARM_ID`,
/// `DAANIO_SPAWN_COORDINATOR_SESSION_ID`) are appended last and win collisions.
fn spawn_metadata_env(command: &TerminalCommand, cwd: &Path) -> Vec<(String, String)> {
    let mut env: Vec<(String, String)> = Vec::new();
    if let Some(kind) = &command.kind {
        env.push(("DAANIO_SPAWN_KIND".to_string(), kind.clone()));
    }
    if let Some(session_id) = &command.session_id {
        env.push(("DAANIO_SPAWN_SESSION_ID".to_string(), session_id.clone()));
    }
    if let Some(title) = &command.title {
        env.push(("DAANIO_SPAWN_TITLE".to_string(), title.clone()));
    }
    env.push((
        "DAANIO_SPAWN_CWD".to_string(),
        cwd.to_string_lossy().into_owned(),
    ));
    env.push((
        "DAANIO_SPAWN_PROGRAM".to_string(),
        command.program.to_string_lossy().into_owned(),
    ));
    env.push((
        "DAANIO_SPAWN_COMMAND".to_string(),
    ...
    // `DAANIO_CLIENT_<NAME>` alias so hooks can explicitly distinguish the
    // client's terminal from the server's.
    for (key, value) in &command.client_terminal_env {
        env.push((key.clone(), value.clone()));
        env.push((format!("DAANIO_CLIENT_{key}"), value.clone()));
    }
...
/// convention), so `hook --flag` becomes `hook --flag <daanio> <args...>`.
/// The hook runs in the session working directory with the full
/// `DAANIO_SPAWN_*` metadata env set (see [`spawn_metadata_env`]); hooks that
/// need a single shell-command string (tmux, kitty `@ launch`) can use
/// `$DAANIO_SPAWN_COMMAND` instead of the appended argv.
...
fn build_spawn_command(term: &str, command: &TerminalCommand, cwd: &Path) -> Option<Command> {
    let title = command.title.as_deref().unwrap_or("daanio");
    ...
    match term {
        ...
        "terminal" => {
            ...
            // means split/resume spawns silently fail to launch daanio. Use AppleScript's
    ...
    // not inherit this env, matching the existing DAANIO_FRESH_SPAWN caveat.
    for (key, value) in spawn_metadata_env(command, cwd) {
        cmd.env(key, value);
    }
...
mod tests {
    ...
    fn spawn_metadata_env_reexports_client_terminal_env_with_native_and_client_keys() {
        ...
        // inherited/stale value) and under a `DAANIO_CLIENT_*` alias so hooks can
        // distinguish the client's terminal from the server's.
        let command = TerminalCommand::new("/usr/local/bin/daanio", vec!["--resume".to_string()])
            .kind("swarm-agent")
            .session_id("ses_405")
        ...
        assert_eq!(
            lookup("DAANIO_CLIENT_ZELLIJ_SESSION_NAME").as_deref(),
            Some("sessionB")
        );
        assert_eq!(lookup("DISPLAY").as_deref(), Some(":1"));
        assert_eq!(lookup("DAANIO_CLIENT_DISPLAY").as_deref(), Some(":1"));
        // The first-class spawn metadata still flows through.
        assert_eq!(lookup("DAANIO_SPAWN_KIND").as_deref(), Some("swarm-agent"));
    }
    ...
    fn spawn_metadata_env_without_client_env_has_no_client_keys() {
        ...
        let env = spawn_metadata_env(&command, Path::new("/tmp/work"));
        assert!(
            !env.iter().any(|(k, _)| k.starts_with("DAANIO_CLIENT_")),
            "no client terminal env should produce no DAANIO_CLIENT_* keys"
        );
    }
    ...
    fn missing_tmux_binary_falls_back_to_detected_emulator() {
        let _guard = ENV_LOCK.lock().unwrap();
        let previous_terminal = std::env::var_os("DAANIO_TERMINAL");
        unsafe {
            std::env::remove_var("DAANIO_TERMINAL");
        }
        let command =
            TerminalCommand::new("/usr/local/bin/daanio", vec![]).client_terminal_env(vec![
                (
                    "TMUX".to_string(),
                    "/tmp/tmux-1000/default,123,0".to_string(),
                ),
        ...
        match previous_terminal {
            Some(value) => unsafe { std::env::set_var("DAANIO_TERMINAL", value) },
            None => unsafe { std::env::remove_var("DAANIO_TERMINAL") },
        }
    ...
    fn shell_command_quotes_arguments() {
        let shell = shell_command(&["daanio".to_string(), "it's ok".to_string()]);
        #[cfg(unix)]
        assert_eq!(shell, "'daanio' 'it'\"'\"'s ok'");
    }
    ...
    #[test]
    #[cfg(not(unix))]
    fn windows_cmd_fallback_runs_daanio_under_cmd_k() {
        let command = TerminalCommand::new(
            std::path::PathBuf::from(r"C:\Program Files\daanio\daanio.exe"),
        ...
        let args: Vec<String> = cmd
            ...
            .map(|arg| arg.to_string_lossy().into_owned())
            .collect();
        assert_eq!(&args[..5], ["/C", "start", "daanio", "cmd.exe", "/K"]);
        assert!(args[5].contains(r#""C:\Program Files\daanio\daanio.exe""#));
        assert!(args[5].contains("self-dev"));
    }

    #[test]
    #[cfg(unix)]
    fn macos_terminal_inner_script_runs_daanio() {
        let command = TerminalCommand::new(
            std::path::PathBuf::from("/usr/local/bin/daanio"),
            vec!["--resume".to_string(), "abc-123".to_string()],
        );
        let script = macos_terminal_inner_script(&command, Path::new("/work/dir"));
        assert_eq!(
            script,
            "cd '/work/dir' && exec '/usr/local/bin/daanio' '--resume' 'abc-123'"
        );
        // Must actually exec daanio, not the broken `open -a Terminal <file>` form.
        assert!(script.contains("exec '/usr/local/bin/daanio'"));
    }
    ...
    fn macos_terminal_inner_script_injects_fresh_spawn() {
        let command =
            TerminalCommand::new(std::path::PathBuf::from("/usr/local/bin/daanio"), vec![])
                .fresh_spawn();
        let script = macos_terminal_inner_script(&command, Path::new("/tmp"));
        assert_eq!(
            script,
            "cd '/tmp' && exec env DAANIO_FRESH_SPAWN=1 '/usr/local/bin/daanio'"
        );
    }
    ...
    fn macos_terminal_applescript_uses_do_script() {
        ...
        assert!(applescript.contains("'/usr/local/bin/daanio'"));
    }
    ...
    fn hook_spawn_command_appends_program_args_and_exports_metadata() {
        ...
        .title("🦊 daanio ses_abc")
        .kind("swarm-agent")
        .session_id("ses_abc")
        .spawn_env("DAANIO_SPAWN_SWARM_ID", "swarm-1")
        .fresh_spawn();
        ...
        assert_eq!(
            args,
            vec!["--flag", "/usr/local/bin/daanio", "--resume", "ses_abc"]
        );
        assert_eq!(
            cmd.get_current_dir(),
        ...
        assert_eq!(
            env_value(&cmd, "DAANIO_SPAWN_KIND").as_deref(),
            Some("swarm-agent")
        );
        assert_eq!(
            env_value(&cmd, "DAANIO_SPAWN_SESSION_ID").as_deref(),
            Some("ses_abc")
        );
        assert_eq!(
            env_value(&cmd, "DAANIO_SPAWN_TITLE").as_deref(),
            Some("🦊 daanio ses_abc")
        );
        assert_eq!(
            env_value(&cmd, "DAANIO_SPAWN_CWD").as_deref(),
            Some("/work/dir")
        );
        assert_eq!(
            env_value(&cmd, "DAANIO_SPAWN_PROGRAM").as_deref(),
            Some("/usr/local/bin/daanio")
        );
        #[cfg(unix)]
        assert_eq!(
            env_value(&cmd, "DAANIO_SPAWN_COMMAND").as_deref(),
            Some("'/usr/local/bin/daanio' '--resume' 'ses_abc'")
        );
        assert_eq!(
            env_value(&cmd, "DAANIO_SPAWN_SWARM_ID").as_deref(),
            Some("swarm-1")
        );
        assert_eq!(env_value(&cmd, "DAANIO_FRESH_SPAWN").as_deref(), Some("1"));
    }
