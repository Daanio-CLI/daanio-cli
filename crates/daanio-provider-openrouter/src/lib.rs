...
fn configured_cache_namespace() -> String {
    let raw = std::env::var("DAANIO_OPENROUTER_CACHE_NAMESPACE")
        .ok()
...
pub fn parse_provider_routing_from_env() -> ProviderRouting {
    ...
    if let Ok(providers) = std::env::var("DAANIO_OPENROUTER_PROVIDER") {
    ...
    if std::env::var("DAANIO_OPENROUTER_NO_FALLBACK").is_ok() {
