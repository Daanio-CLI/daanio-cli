...
pub struct NamedProviderConfig {
    ...
    /// Must be a JSON object; keys here override daanio-generated body fields.
    ...
    /// `reasoning_effort` request field (`/effort` support). When unset, daanio
...
pub struct AuthConfig {
    /// External auth source ids that the user has approved daanio to read/use.
...
pub struct AgentsConfig {
    ...
    /// local. Env override: `DAANIO_MEMORY_EMBEDDING_BACKEND`.
    ...
    /// Unset = `text-embedding-3-small`. Env: `DAANIO_MEMORY_EMBEDDING_MODEL`.
    #[serde(default)]
    ...
    /// Env: `DAANIO_MEMORY_EMBEDDING_BASE_URL`.
    #[serde(default)]
...
/// pane by the built-in launcher. `DAANIO_TERMINAL` explicitly selects a terminal
/// emulator instead.
#[derive(Debug, Clone, Serialize, Deserialize, Default)]
#[serde(default)]
pub struct TerminalConfig {
    ...
    /// opening a terminal emulator itself, with `DAANIO_SPAWN_*` metadata env
    ...
    /// tmux window in the current server. If the hook fails to launch, daanio
    ...
    ///
    /// When set, daanio runs the hook (instead of wmctrl/xdotool) whenever it
    /// wants to bring a session's window to the foreground, with
    /// `DAANIO_FOCUS_SESSION_ID` and `DAANIO_FOCUS_TITLE` env vars. Pair this
    ...
    /// which terminal daanio launches into and is preferred over the legacy
    /// `~/.daanio/preferred_terminal.json` file. Re-run `daanio setup-hotkey`
...
/// Lifecycle hooks: external commands daanio runs at well-defined points.
///
/// Hook commands are parsed shell-style (quotes work) but executed directly,
/// with `DAANIO_HOOK_*` env vars describing the event (`DAANIO_HOOK_EVENT`,
/// `DAANIO_HOOK_SESSION_ID`, `DAANIO_HOOK_CWD`, event-specific fields, and a
/// `DAANIO_HOOK_PAYLOAD` JSON mirror). Hook processes get
/// `DAANIO_HOOKS_DISABLED=1` so nested daanio invocations don't recurse.
///
/// All hooks except `pre_tool` are observers: detached, fire-and-forget,
/// failures only logged. `pre_tool` is a gate: daanio waits for it and exit
...
pub struct HooksConfig {
    ...
    /// ("chat"/"resume"/"ambient"). Env override: DAANIO_HOOK_TURN_START.
...
pub struct SponsorsConfig {
    ...
    /// `discover_tools` tool is not registered, and daanio never contacts the
    /// discovery endpoint.
    pub enabled: bool,
...
pub struct KeybindingsConfig {
    ...
    /// Spawn a fresh daanio session in a new terminal window (default: unbound).
    /// Example: "alt+enter".
...
pub struct DisplayConfig {
    ...
    /// adapts daanio's palette for light backgrounds. Default: auto.
    #[serde(default)]
    pub theme: String,
...
pub enum WebSearchEngine {
    ...
    /// `DAANIO_SEARXNG_URL` env var) to point at a SearXNG instance. Useful on
...
impl Default for WebSearchConfig {
    fn default() -> Self {
        Self {
            ...
            bing_api_key: None,
            bing_api_key_env: "DAANIO_BING_API_KEY".to_string(),
            bing_market: "en-US".to_string(),
            searxng_url: None,
            searxng_url_env: "DAANIO_SEARXNG_URL".to_string(),
        }
    }
}
...
pub struct ProviderConfig {
    ...
    /// Whether daanio should automatically try another account on the same provider
    ...
    /// Overridable per-launch via `DAANIO_STREAM_IDLE_TIMEOUT_SECS`.
...
pub struct SafetyConfig {
    ...
    /// SMTP password (prefer DAANIO_SMTP_PASSWORD env var)
    ...
    /// Jade relay bearer token (prefer DAANIO_JADE_RELAY_TOKEN env var)
...
pub struct PowerConfig {
    /// Prevent automatic system sleep while any daanio session is actively
    ...
    ///
    /// Honored by the shared `daanio serve` daemon. The `DAANIO_DISABLE_POWER_INHIBIT`
...
/// A single global launch hotkey: a chord plus the directory it opens daanio in.
///
...
/// - `$LAST_DIR` -> the most recent non-home project directory daanio ran in.
/// - `$LAST_REPO` -> the most recent daanio repo (for self-dev).
///
...
pub struct LaunchHotkeyEntry {
    /// daanio-style chord string, e.g. `cmd+;`, `cmd+[`, `cmd+shift+'`.
    pub chord: String,
...
/// Configuration for the global "launch a new daanio" hotkeys (macOS).
///
/// When `entries` is empty, daanio uses its built-in defaults (`Cmd+;` -> home,
