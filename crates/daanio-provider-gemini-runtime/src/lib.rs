...
//! key), moved out of `daanio-base` so provider edits compile only this crate
...
impl GeminiProvider {
    fn persisted_catalog_path() -> Result<std::path::PathBuf> {
        Ok(daanio_base::storage::app_config_dir()?.join("gemini_models_cache.json"))
    }
    ...
    pub fn new() -> Self {
        let model = std::env::var("DAANIO_GEMINI_MODEL").unwrap_or_else(|_| DEFAULT_MODEL.into());
        let provider = Self {
    ...
    /// cloudcode-pa tier. Set `DAANIO_GEMINI_FORCE_OAUTH=1` to pin OAuth even when
    /// a key is present.
    fn auth_mode() -> GeminiAuthMode {
        let force_oauth = std::env::var("DAANIO_GEMINI_FORCE_OAUTH")
            .map(|value| {
...
fn gemini_http_client() -> reqwest::Client {
    reqwest::Client::builder()
        ...
        .build()
        .unwrap_or_else(|_| daanio_provider_core::shared_http_client())
}
...
fn is_transient_gemini_transport_error(err: &reqwest::Error) -> bool {
    ...
    err.is_connect()
        || err.is_timeout()
        || daanio_provider_core::is_transient_transport_error(&err.to_string())
}
