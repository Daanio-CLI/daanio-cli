...
// flip `DAANIO_NO_TELEMETRY` / `DO_NOT_TRACK`, while the session tests drive the
...
fn test_opt_out_env_var() {
    ...
    daanio_core::env::set_var("DAANIO_NO_TELEMETRY", "1");
    assert!(!is_enabled());
    daanio_core::env::remove_var("DAANIO_NO_TELEMETRY");
}

#[test]
fn test_disabled_without_configured_endpoint() {
    let _guard = lock_test_env();
    let previous = std::env::var_os(TELEMETRY_ENDPOINT_ENV);
    daanio_core::env::remove_var(TELEMETRY_ENDPOINT_ENV);
    assert!(!is_enabled());
    if let Some(previous) = previous {
        daanio_core::env::set_var(TELEMETRY_ENDPOINT_ENV, previous);
    }
}
...
fn test_do_not_track() {
    let _guard = lock_test_env();
    daanio_core::env::set_var("DO_NOT_TRACK", "1");
    assert!(!is_enabled());
    daanio_core::env::remove_var("DO_NOT_TRACK");
}
...
fn test_is_ci_detects_ci_env() {
    ...
    daanio_core::env::set_var("CI", "true");
    assert!(
        is_ci(),
        "CI env var should mark the run as CI (gates install skip)"
    );
    daanio_core::env::remove_var("CI");
    assert!(!is_ci());
}
...
fn test_onboarding_step_milestone_key_includes_provider_and_method() {
    assert_eq!(
        onboarding_step_milestone_key("auth_success", Some("daanio"), Some("API key")),
        "auth_success_daanio_api_key"
    );
    assert_eq!(
...
fn test_install_marker_tracks_current_telemetry_id() {
    ...
    if let Some(prev_home) = prev_home {
        daanio_core::env::set_var("DAANIO_HOME", prev_home);
    } else {
        daanio_core::env::remove_var("DAANIO_HOME");
    }
}
