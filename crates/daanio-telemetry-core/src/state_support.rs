...
pub(super) fn telemetry_id_path() -> Option<PathBuf> {
    storage::daanio_dir().ok().map(|d| d.join("telemetry_id"))
}
...
pub(super) fn telemetry_state_path(name: &str) -> Option<PathBuf> {
    storage::daanio_dir().ok().map(|d| d.join(name))
}
...
pub(super) fn version() -> String {
    daanio_build_meta::pkg_version().to_string()
}
...
pub(super) fn is_daanio_repo_dir(dir: &Path) -> bool {
    ...
    std::fs::read_to_string(cargo_toml)
        .map(|content| content.contains("name = \"daanio\""))
        .unwrap_or(false)
}

fn find_daanio_repo_in_ancestors(start: &Path) -> Option<PathBuf> {
    start
        .ancestors()
        .find(|dir| is_daanio_repo_dir(dir))
        .map(Path::to_path_buf)
}

fn telemetry_daanio_repo_dir() -> Option<PathBuf> {
    if let Ok(path) = std::env::var("DAANIO_REPO_DIR") {
        let path = PathBuf::from(path);
        if is_daanio_repo_dir(&path) {
            return Some(path);
        }
    }

    let manifest_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    if let Some(repo) = find_daanio_repo_in_ancestors(&manifest_dir) {
        return Some(repo);
    }
    ...
    if let Ok(exe) = std::env::current_exe()
        && let Some(repo) = exe
            ...
            .and_then(Path::parent)
            .filter(|dir| is_daanio_repo_dir(dir))
    {
    ...
    std::env::current_dir()
        .ok()
        .and_then(|cwd| find_daanio_repo_in_ancestors(&cwd))
}
...
pub(super) fn build_channel() -> String {
    ...
    if telemetry_daanio_repo_dir().is_some() {
...
pub(super) fn is_git_checkout() -> bool {
    telemetry_daanio_repo_dir().is_some()
}
