use daanio_logging as logging;
use daanio_storage as storage;
mod lifecycle;
mod state_support;
use chrono::{DateTime, NaiveDate, Utc};
use daanio_usage_types::{
...
pub use daanio_usage_types::{ErrorCategory, SessionEndReason};
...
const TELEMETRY_ENDPOINT_ENV: &str = "DAANIO_TELEMETRY_ENDPOINT";
...
pub fn is_enabled() -> bool {
    if std::env::var("DAANIO_NO_TELEMETRY").is_ok() || std::env::var("DO_NOT_TRACK").is_ok() {
        logging::debug("telemetry disabled by environment");
        return false;
    }
    if let Ok(dir) = storage::daanio_dir()
        && dir.join("no_telemetry").exists()
    {
        logging::debug("telemetry disabled by no_telemetry marker");
        return false;
    }
    if std::env::var(TELEMETRY_ENDPOINT_ENV)
        .ok()
        .is_none_or(|endpoint| endpoint.trim().is_empty())
    {
        logging::debug("telemetry disabled because no endpoint is configured");
        return false;
    }
    true
}
...
fn detect_project_profile() -> ProjectProfile {
    ...
    profile.repo_present = root.join(".git").exists() || is_daanio_repo_dir(root);
...
fn post_payload(payload: serde_json::Value, timeout: Duration) -> bool {
    ...
    let client = TELEMETRY_HTTP_CLIENT.get_or_init(|| {
        reqwest::blocking::Client::builder()
            .user_agent(daanio_provider_core::DAANIO_USER_AGENT)
            .build()
            .expect("telemetry HTTP client should build")
    });
    let Ok(endpoint) = std::env::var(TELEMETRY_ENDPOINT_ENV) else {
        return false;
    };
    match client.post(endpoint).timeout(timeout).json(&payload).send() {
    ...
    F: FnMut(Value) + Send + 'static,
{
    ...
    std::thread::Builder::new()
        .name("daanio-telemetry".to_string())
        .spawn(move || {
...
pub fn record_install_if_first_run() {
    ...
    // fresh ~/.daanio (so a new telemetry_id) on every job, which would otherwise
...
pub fn record_auth_started(provider: &str, method: &str) {
    daanio_logging::auth_event("auth_started", provider, &[("method", method)]);
...
pub fn record_auth_cancelled(provider: &str, method: &str) {
    daanio_logging::auth_event("auth_cancelled", provider, &[("method", method)]);
...
pub fn record_auth_surface_blocked(provider: &str, method: &str) {
    daanio_logging::auth_event("auth_surface_blocked", provider, &[("method", method)]);
...
pub fn record_auth_success(provider: &str, method: &str) {
    daanio_logging::auth_event("auth_success", provider, &[("method", method)]);
    if !is_enabled() {
        return;
    }
    let Some(id) = get_or_create_id() else {
...
fn show_first_run_notice() {
    ...
    let (dim, reset) = if daanio_core::console::stderr_supports_ansi() {
        ("\x1b[90m", "\x1b[0m")
    } else {
        ("", "")
    };
    eprintln!("{dim}");
    eprintln!("  daanio collects anonymous usage statistics (install count, version, OS,");
    eprintln!("  session activity, tool counts, and crash/exit reasons). No code, filenames,");
    eprintln!("  prompts, or personal data is sent.");
    eprintln!("  To opt out: export DAANIO_NO_TELEMETRY=1");
    eprintln!("  Details: see TELEMETRY.md in the Daanio source tree");
    eprintln!("{reset}");
}
