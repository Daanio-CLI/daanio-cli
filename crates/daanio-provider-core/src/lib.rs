...
pub use reasoning::{
    DEEPSEEK_SELECTABLE_EFFORTS, GEMINI_SELECTABLE_EFFORTS, OPENAI_SELECTABLE_EFFORTS,
    OPENROUTER_SELECTABLE_EFFORTS, XAI_SELECTABLE_EFFORTS, canonical_reasoning_effort,
...
pub trait Provider: Send + Sync {
    ...
    /// Returns true if daanio should use its own compaction for this provider.
    fn supports_compaction(&self) -> bool {
        false
    }

    /// Returns true if daanio should proactively run its own summary-based compaction.
...
impl CredentialMode {
    /// Resolve the runtime-env pin (DAANIO_*_AUTH / route aliases) for a
...
/// Native tool result to send back to provider bridges that delegate tool execution to daanio.
...
/// Canonical User-Agent for generic outbound Daanio HTTP requests.
pub const DAANIO_USER_AGENT: &str = concat!("daanio/", env!("CARGO_PKG_VERSION"));
    ...
    CLIENT
        .get_or_init(|| {
            reqwest::Client::builder()
                ...
                .build()
                .unwrap_or_else(|err| {
                    eprintln!("daanio: failed to build shared provider HTTP client: {err}");
                        ...
                        .build()
                    {
                        ...
                        Err(fallback_err) => {
                            eprintln!(
                                "daanio: failed to build fallback provider HTTP client: {fallback_err}"
                            );
                            reqwest::Client::new()
                        }
                    }
...
impl RuntimeKey {
    pub fn from_api_method(api_method: &ModelRouteApiMethod, _provider_label: &str) -> Self {
        match api_method {
            ModelRouteApiMethod::DaanioSubscription => Self::DaanioSubscription,
    ...
    pub fn stable_id(&self) -> String {
        match self {
            Self::DaanioSubscription => "daanio-subscription".to_string(),
...
impl RouteSelection {
    ...
    pub fn routed_model_spec(&self) -> String {
        ...
        match &self.runtime_key {
            RuntimeKey::DaanioSubscription => model.to_string(),
...
/// through. Mirrors `daanio_base::provider::openrouter_catalog_model_id` but
...
impl ModelRouteApiMethod {
    ...
    pub fn parse(value: &str) -> Self {
        ...
        match lower.as_str() {
            "daanio-subscription" => Self::DaanioSubscription,
    ...
    pub fn display_label(&self) -> String {
        match self {
            Self::DaanioSubscription => "subscription".to_string(),
...
mod tests {
    ...
    #[test]
    fn canonical_user_agent_identifies_daanio() {
        assert!(DAANIO_USER_AGENT.starts_with("daanio/"));
    }
