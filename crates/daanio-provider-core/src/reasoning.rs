...
//! same ordered values. `swarm` and `swarm-deep` are Daanio UI sentinels rather
//! than wire-level provider values, but they belong in the selectable ladder.

/// OpenAI Responses API effort levels, followed by Daanio's swarm modes.
...
/// Direct DeepSeek effort levels, followed by Daanio's swarm modes.
