...
//!
//! This is safe for daanio's HTTP providers because tools are only executed by
...
use anyhow::Result;
use daanio_message_types::StreamEvent;
use std::sync::Arc;
...
mod tests {
    ...
    fn status_events_are_not_replay_visible() {
        assert!(!stream_event_is_replay_visible(
            &StreamEvent::ConnectionPhase {
                phase: daanio_message_types::ConnectionPhase::Connecting,
            }
        ));
