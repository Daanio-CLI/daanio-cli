...
impl LogLevel {
    ...
    fn is_enabled(self) -> bool {
        !matches!(self, Self::Debug) || std::env::var("DAANIO_TRACE").is_ok()
    }
}
...
fn log_dir() -> Option<PathBuf> {
    daanio_storage::logs_dir().ok()
}

impl Logger {
    fn new() -> Option<Self> {
        let log_dir = log_dir()?;
        daanio_storage::ensure_dir(&log_dir).ok()?;

        // Use date-based log file
        let date = Local::now().format("%Y-%m-%d");
        let path = log_dir.join(format!("daanio-{}.log", date));
    ...
    fn write(&mut self, level: &str, message: &str) {
        ...
        if let Err(err) = self.file.write_all(line.as_bytes()) {
            eprintln!("daanio logger write failed: {err}");
            return;
        }
        if let Err(err) = self.file.flush() {
            eprintln!("daanio logger flush failed: {err}");
        }
    }
}
...
pub fn init() {
    ...
    // `daanio-*.log` files, so it is safe to run unconditionally.
    if !CLEANUP_STARTED.swap(true, Ordering::SeqCst) {
        std::thread::Builder::new()
            .name("daanio-log-cleanup".to_string())
...
/// Log a debug message (only if DAANIO_TRACE is set)
#[expect(
    clippy::collapsible_if,
    reason = "Debug logging keeps env gating and logger access explicit"
)]
pub fn debug(message: &str) {
    if std::env::var("DAANIO_TRACE").is_ok() {
        if let Ok(mut guard) = LOGGER.lock() {
            if let Some(logger) = guard.as_mut() {
...
fn structured_json_enabled() -> bool {
    matches!(
        std::env::var("DAANIO_LOG_JSON").as_deref(),
...
pub fn log_path() -> Option<PathBuf> {
    let log_dir = log_dir()?;
    let date = Local::now().format("%Y-%m-%d");
    Some(log_dir.join(format!("daanio-{}.log", date)))
}

/// Remove daily `daanio-*.log` / `daanio-desktop-*.log` files older than 7 days.
///
...
fn cleanup_old_logs_in(log_dir: &std::path::Path, now: chrono::DateTime<Local>) {
    ...
    for entry in entries.flatten() {
        ...
        let is_daanio_log = (name.starts_with("daanio-") || name.starts_with("daanio-desktop-"))
            && name.ends_with(".log");
        if !is_daanio_log {
            continue;
        }
            ...
            && let Ok(modified) = metadata.modified()
        {
            ...
            {
                eprintln!("daanio logger cleanup failed: {err}");
            }
        }
    }
}
...
fn truncate(s: &str, max_len: usize) -> String {
    if s.len() > max_len {
        format!("{}...", daanio_core::util::truncate_str(s, max_len))
    } else {
        s.to_string()
    }
}
...
mod tests {
    ...
    #[test]
    fn cleanup_removes_only_old_daanio_logs() {
        use std::time::{Duration, SystemTime};

        let dir = std::env::temp_dir().join(format!(
            "daanio-log-cleanup-test-{}-{}",
            std::process::id(),
            SystemTime::now()
        ...
        let old_log = write("daanio-2000-01-01.log", true);
        let old_desktop = write("daanio-desktop-2000-01-01.log", true);
        // Recent log file that SHOULD survive.
        let new_log = write("daanio-2099-01-01.log", false);
        ...
        assert!(!old_log.exists(), "old daanio log should be deleted");
        assert!(!old_desktop.exists(), "old desktop log should be deleted");
        assert!(new_log.exists(), "recent daanio log must survive");
