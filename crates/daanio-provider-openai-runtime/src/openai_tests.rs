...
impl LiveOpenAITestEnv {
    fn new() -> Result<Option<Self>> {
        let lock = daanio_base::storage::lock_test_env();
        let Some(source_auth) = real_codex_auth_path() else {
            return Ok(None);
        };

        let temp = tempfile::Builder::new()
            .prefix("daanio-openai-live-")
            .tempdir()?;
        ...
        let daanio_home = EnvVarGuard::set_path("DAANIO_HOME", temp.path());
        let transport = EnvVarGuard::set("DAANIO_OPENAI_TRANSPORT", "https");

        Ok(Some(Self {
            _lock: lock,
            _daanio_home: daanio_home,
...
async fn live_openai_catalog() -> Result<Option<daanio_base::provider::OpenAIModelCatalog>> {
    let Some(_env) = LiveOpenAITestEnv::new()? else {
        return Ok(None);
    };
    let creds = daanio_base::auth::codex::load_credentials()?;
    ...
    Ok(Some(
        daanio_base::provider::fetch_openai_model_catalog(&token).await?,
    ))
}
...
fn openai_credential_mode_runtime_provider_identity_round_trips() {
    ...
    daanio_base::env::set_var("DAANIO_RUNTIME_PROVIDER", "openai");
    assert_eq!(
    ...
    daanio_base::env::set_var("DAANIO_RUNTIME_PROVIDER", "openai-api");
    assert_eq!(
        OpenAICredentialMode::from_runtime_env(daanio_provider_core::DualAuthProvider::OpenAI),
