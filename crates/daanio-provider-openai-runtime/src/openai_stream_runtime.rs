...
pub(super) fn effective_ws_completion_timeout_secs() -> u64 {
    WEBSOCKET_COMPLETION_TIMEOUT_SECS.max(daanio_base::provider::stream_idle_timeout().as_secs())
}
...
pub(super) async fn stream_response(
    ...
    if !response.status().is_success() {
        ...
        {
            daanio_base::provider::record_model_unavailable_for_account(model_name, &reason);
            daanio_base::logging::warn(&format!(
        ...
        return Err(OpenAIStreamFailure::Other(
            daanio_provider_core::retry_after::error_with_retry_after(msg, retry_after),
        ));
    }
    ...
    // (issue #434).
    let idle_timeout = daanio_base::provider::stream_idle_timeout();
...
pub(super) async fn try_persistent_ws_continuation(
    ...
    if state.connected_at.elapsed() >= Duration::from_secs(WEBSOCKET_PERSISTENT_MAX_AGE_SECS) {
        daanio_base::logging::info("Persistent WS connection too old; forcing reconnect");
        *guard = None;
        log_openai_stream_lifecycle(
            daanio_base::logging::LogLevel::Info,
    ...
    if incremental_items.is_empty() {
        daanio_base::logging::info("No incremental items to send; need fresh request");
        *guard = None;
        log_openai_stream_lifecycle(
            daanio_base::logging::LogLevel::Info,
            "persistent_state_reset",
    ...
    let continuation_payload = serde_json::json!({
        ...
        "previous_response_id_hash": daanio_provider_core::fingerprint::stable_hash_str(&previous_response_id),
        ...
        match result {
            Ok(WsMessage::Text(text)) => {
                ...
                if !logged_first_server_event {
                    emit_connection_phase(tx, daanio_message_types::ConnectionPhase::Streaming)
                        .await;
                    daanio_base::logging::info(&format!(
    ...
    if let Some(resp_id) = new_response_id {
        ...
        daanio_base::logging::warn("Persistent WS: no response_id in response; breaking chain");
        *guard = None;
        log_openai_stream_lifecycle(
            daanio_base::logging::LogLevel::Warn,
            "persistent_state_reset",
...
fn maybe_record_runtime_model_unavailable_from_stream_error(model: &str, message: &str) {
    ...
    if let Some(reason) = reason {
        daanio_base::provider::record_model_unavailable_for_account(model, &reason);
        daanio_base::logging::warn(&format!(
