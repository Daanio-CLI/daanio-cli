...
//! persistent WebSocket), moved out of `daanio-base` so provider edits compile
...
//! [`OpenAIProvider`] with `daanio_base::provider::external` at startup.
//!
//! Model-catalog/account-availability state stays in `daanio_base::provider`
//! (it is shared vocabulary for routing), as does the pure request shaping in
//! `daanio_base::provider::openai_request`.

use anyhow::{Context, Result};
use async_trait::async_trait;
use daanio_base::auth::codex::CodexCredentials;
use daanio_base::auth::oauth;
use daanio_base::provider::openai_request::{build_responses_input, build_tools};
#[cfg(test)]
use daanio_message_types::TOOL_OUTPUT_MISSING_TEXT;
use daanio_message_types::{Message as ChatMessage, StreamEvent, ToolDefinition};
...
const DEFAULT_MODEL: &str = daanio_provider_core::DEFAULT_OPENAI_MODEL;
const ORIGINATOR: &str = "codex_cli_rs";

pub(crate) const CHATGPT_WEB_MODEL: &str = daanio_provider_core::CHATGPT_WEB_MODEL;
...
/// `DAANIO_OPENAI_WS_IDLE_RECONNECT_SECS` (0 disables the idle reconnect entirely,
...
static WEBSOCKET_PERSISTENT_IDLE_RECONNECT_SECS: LazyLock<Option<u64>> = LazyLock::new(|| {
    match std::env::var("DAANIO_OPENAI_WS_IDLE_RECONNECT_SECS") {
        Ok(raw) => match raw.trim().parse::<u64>() {
            Ok(0) => {
                daanio_base::logging::info(
                    "OpenAI persistent WS idle reconnect disabled (DAANIO_OPENAI_WS_IDLE_RECONNECT_SECS=0); relying on healthcheck + max-age",
                );
                None
            }
            Ok(secs) => {
                daanio_base::logging::info(&format!(
                    "OpenAI persistent WS idle reconnect threshold set to {}s (DAANIO_OPENAI_WS_IDLE_RECONNECT_SECS)",
                    secs
                ));
                Some(secs)
            }
            Err(_) => {
                daanio_base::logging::info(&format!(
                    "Warning: invalid DAANIO_OPENAI_WS_IDLE_RECONNECT_SECS '{}'; using default {}s",
        ...
        match raw.trim().to_ascii_lowercase().as_str() {
            ...
            other => {
                daanio_base::logging::warn(&format!(
                    "Unknown DAANIO_OPENAI_TRANSPORT '{}'; using auto. Use: auto, websocket, or https.",
                    other
                ));
                Self::Auto
            }
...
/// Shared dual-auth credential pin (see `daanio_provider_core::CredentialMode`).
/// The OpenAI-specific alias is kept so existing call sites read naturally.
pub(crate) use daanio_provider_core::CredentialMode as OpenAICredentialMode;
...
pub(crate) fn load_credentials_for_mode(mode: OpenAICredentialMode) -> Result<CodexCredentials> {
    match mode {
        OpenAICredentialMode::Auto => daanio_base::auth::codex::load_credentials(),
        OpenAICredentialMode::OAuth => daanio_base::auth::codex::load_oauth_credentials(),
        OpenAICredentialMode::ApiKey => daanio_base::auth::codex::load_api_key_credentials(),
    }
}
...
fn log_openai_stream_lifecycle(
    level: daanio_base::logging::LogLevel,
    phase: &str,
    ...
    daanio_base::logging::event(level, "PROVIDER_STREAM_LIFECYCLE", owned);
}
...
async fn emit_connection_phase(
    ...
    phase: daanio_message_types::ConnectionPhase,
) {
        ...
        loop {
                ...
                >= Duration::from_secs(WEBSOCKET_PERSISTENT_MAX_AGE_SECS)
            {
                log_openai_stream_lifecycle(
                    daanio_base::logging::LogLevel::Info,
            ...
            match ensure_persistent_ws_is_healthy(state, false).await {
                ...
                Err(error) => {
                    ...
                    log_openai_stream_lifecycle(
                        daanio_base::logging::LogLevel::Warn,
                        "persistent_state_reset",
                        vec![
                            ("model", model.clone()),
...
impl OpenAIProvider {
    pub(crate) fn supports_extended_prompt_cache_retention(model_id: &str) -> bool {
        daanio_base::provider::openai::supports_extended_prompt_cache_retention(model_id)
    }
    ...
    fn new_inner(credentials: CodexCredentials, browser_only: bool) -> Self {
        let credential_mode = if browser_only {
            ...
            OpenAICredentialMode::from_runtime_env(daanio_provider_core::DualAuthProvider::OpenAI)
        };
        ...
        let mut model = if browser_only {
            ...
            std::env::var("DAANIO_OPENAI_MODEL")
                .unwrap_or_else(|_| DEFAULT_MODEL.to_string())
                .trim()
                .to_string()
        };
        if !is_chatgpt_web_model(&model)
            && !daanio_base::provider::known_openai_model_ids()
                .iter()
        ...
        let prompt_cache_key = std::env::var("DAANIO_OPENAI_PROMPT_CACHE_KEY")
            .ok()
            .map(|v| v.trim().to_string())
            .filter(|v| !v.is_empty());
        let prompt_cache_retention = std::env::var("DAANIO_OPENAI_PROMPT_CACHE_RETENTION")
            .ok()
            .map(|v| v.trim().to_string())
            .filter(|v| !v.is_empty());
        let prompt_cache_retention = match prompt_cache_retention.as_deref() {
            ...
            Some(other) => {
                daanio_base::logging::info(&format!(
                    "Warning: Unsupported DAANIO_OPENAI_PROMPT_CACHE_RETENTION '{}'; expected 'in_memory' or '24h'",
                    other
                ));
                None
            }
        ...
        let native_compaction_threshold_tokens = daanio_base::config::config()
            .provider
            .openai_native_compaction_threshold_tokens
            .max(1000);
        let model_reasoning_efforts =
            daanio_base::provider::cached_openai_reasoning_efforts().unwrap_or_default();
    ...
    pub(crate) fn set_credential_mode(&self, mode: OpenAICredentialMode) -> Result<()> {
        ...
        if let Some(route) = mode.auth_route(daanio_provider_core::DualAuthProvider::OpenAI) {
    ...
    fn reload_cached_reasoning_efforts(&self) {
        let cached = daanio_base::provider::cached_openai_reasoning_efforts().unwrap_or_default();
    ...
    async fn clear_persistent_ws(&self, reason: &str) {
        ...
        if persistent_ws.is_some() {
            daanio_base::logging::info(&format!("Clearing persistent OpenAI WS state: {}", reason));
        }
        *persistent_ws = None;
    }
    ...
    fn revalidate_reasoning_effort(&self) {
        ...
        if daanio_base::prompt::is_swarm_effort(&current)
    ...
    fn api_reasoning_effort(&self, effort: Option<&str>) -> Option<String> {
        match effort {
            Some(e) if daanio_base::prompt::is_swarm_effort(e) => {
                let available = self.available_efforts();
                daanio_provider_core::OPENAI_SELECTABLE_EFFORTS
                    .iter()
                    .rev()
                    .find(|candidate| {
                        !daanio_base::prompt::is_swarm_effort(candidate)
    ...
    fn parse_max_output_tokens(raw: Option<&str>) -> Option<u32> {
        ...
        match raw.parse::<u32>() {
            ...
            Err(_) => {
                daanio_base::logging::warn(&format!(
                    "Invalid DAANIO_OPENAI_MAX_OUTPUT_TOKENS='{}'; using default {}",
    ...
    fn load_max_output_tokens() -> Option<u32> {
        let raw = std::env::var("DAANIO_OPENAI_MAX_OUTPUT_TOKENS").ok();
        let parsed = Self::parse_max_output_tokens(raw.as_deref());
        if raw.is_some() {
            match parsed {
                Some(value) => daanio_base::logging::info(&format!(
                    "OpenAI max_output_tokens configured to {}",
                    value
                )),
                None => daanio_base::logging::info(
                    "OpenAI max_output_tokens disabled (DAANIO_OPENAI_MAX_OUTPUT_TOKENS=0)",
                ),
            }
        }
        parsed
    ...
    /// `DAANIO_OPENAI_API_BASE`, `OPENAI_BASE_URL`, `OPENAI_API_BASE`.
    ///
    ...
    pub(crate) fn resolve_api_base() -> String {
        daanio_base::provider::openai::resolve_api_base()
    }
    ...
    async fn model_id(&self) -> String {
        ...
        let availability = daanio_base::provider::model_availability_for_account(&current);

        match availability.state {
            daanio_base::provider::AccountModelAvailabilityState::Unavailable => {
                ...
                if let Some(fallback) = daanio_base::provider::get_best_available_openai_model()
                    && fallback != current
                {
                    daanio_base::logging::info(&format!(
            ...
            daanio_base::provider::AccountModelAvailabilityState::Unknown => {
                if daanio_base::provider::should_refresh_openai_model_catalog()
                    && daanio_base::provider::begin_openai_model_catalog_refresh()
                {
                    let is_chatgpt_mode = {
                    ...
                    daanio_base::provider::refresh_openai_model_catalog_in_background(
                        token,
                        is_chatgpt_mode,
            ...
            daanio_base::provider::AccountModelAvailabilityState::Available => {}
        }
