...
//!    wipes the real shared daanio mermaid cache dir, which is fine for an
...
//! legacy SVG-retarget backend (`DAANIO_MMDR_SIZE_API_DISABLE=1` is a
//! compile-time toggle consumed by build.rs, so it triggers a rebuild):
//!   cargo test -p daanio-tui-mermaid --test layout_cache_cross_width_parity -- --ignored --nocapture
//!   DAANIO_MMDR_SIZE_API_DISABLE=1 cargo test -p daanio-tui-mermaid --test layout_cache_cross_width_parity -- --ignored --nocapture
