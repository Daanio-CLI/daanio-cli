...
//!
//! Run with:
//!   cargo test --release -p daanio-tui-mermaid --test layout_cache_resize_probe -- --ignored --nocapture
//!
//! NOTE: uses the real daanio mermaid cache dir (like the crate's unit tests);
...
use daanio_tui_mermaid::{RenderResult, debug_stats, render_mermaid_untracked};

/// Deterministic ~40-node / 45-edge flowchart. There is no `infra` fixture in
/// this crate (checked `crates/daanio-tui-mermaid/` for fixtures/), so this is
