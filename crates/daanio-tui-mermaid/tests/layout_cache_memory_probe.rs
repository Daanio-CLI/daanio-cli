...
//! `cargo test -p daanio-tui-mermaid --test layout_cache_memory_probe -- --ignored --nocapture`
...
fn layout_cache_stays_bounded_under_50_distinct_renders() {
    ...
    daanio_tui_mermaid::reset_debug_stats();
    let stats_before = daanio_tui_mermaid::debug_stats();
    ...
    for idx in 0..RENDERS {
        ...
        match daanio_tui_mermaid::render_mermaid_untracked(&content, Some(90)) {
            daanio_tui_mermaid::RenderResult::Image { .. } => {}
            daanio_tui_mermaid::RenderResult::Error(error) => {
                render_errors += 1;
    ...
    let stats = daanio_tui_mermaid::debug_stats();
    let memory = daanio_tui_mermaid::debug_memory_profile();
