//! Stress probe: build the mermaid source that `daanio_plan::mermaid::
...
//!
//!   scripts/dev_cargo.sh run -p daanio-tui-mermaid --features renderer \
//!       --example swarm_plan_stress
//!
//! Cases:
//!   (a) real deep-mode plan fixture (examples/swarm_plan_fixture.json,
//!       snapshot of ~/.daanio/state/swarm/_home_jeremy_daanio__git.json)
...
use daanio_plan::PlanItem;
use daanio_plan::mermaid::swarm_plan_mermaid;
...
fn probe(name: &str, src: &str) -> bool {
    match daanio_tui_mermaid::render_mermaid_untracked(src, Some(100)) {
        ...
        daanio_tui_mermaid::RenderResult::Error(err) => {
