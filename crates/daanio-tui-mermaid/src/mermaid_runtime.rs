...
/// Terminal multiplexers / agent-multiplexers that sit between daanio and the
...
/// Detect whether daanio is running inside a known multiplexer, using the same
...
///
/// * `probe_override` is the parsed value of `DAANIO_MERMAID_PICKER_PROBE`
...
/// Parse only the explicit `DAANIO_MERMAID_PICKER_PROBE` override into a mode,
...
pub(crate) fn prewarm_svg_font_db_async() {
    SVG_FONT_DB_PREWARM_STARTED.get_or_init(|| {
        let _ = std::thread::Builder::new()
            .name("daanio-mermaid-fontdb-prewarm".to_string())
            .spawn(|| {
                let _ = &*SVG_FONT_DB;
            });
    });
...
/// By default daanio uses environment-based detection and never blocks startup
/// on terminal capability responses. Set DAANIO_MERMAID_PICKER_PROBE=1 to run an
...
pub fn init_picker() {
    PICKER.get_or_init(|| {
        ...
        let probe_override = std::env::var("DAANIO_MERMAID_PICKER_PROBE")
            .ok()
            .as_deref()
