...
//!
//! This powers `daanio provider-doctor`: it walks the same strict provider/model
...
//! - [`DoctorTier::Offline`]: no API key, no network, no spend. Validates daanio's
...
use daanio_base::protocol::{AuthChanged, CatalogNamespace, RuntimeProviderKey};
use daanio_base::provider::ModelRoute;
use daanio_base::provider_catalog::OpenAiCompatibleProfile;

/// How much of the strict pipeline to exercise.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum DoctorTier {
    /// No key, no network, no spend. Validates daanio-side wiring only.
    Offline,
...
const FULL_PIPELINE_LABELS: &[(&str, &str)] = &[
    ...
    (
        checkpoints::REAL_DAANIO_TOOL_SMOKE,
        "Real Daanio tool smoke",
    ),
...
fn tool_stage_detail(stage: &daanio_base::live_tests::LiveVerificationStage) -> String {
...
/// doctor report.
fn reasoning_stage_detail(stage: &daanio_base::live_tests::LiveVerificationStage) -> String {
    match stage
        .evidence
...
///
/// The predicate itself lives in `daanio_base::auth::doctor` so base-internal
...
pub use daanio_base::auth::doctor::native_doctor_supports_provider;
...
pub async fn run_claude_native_e2e(
    ...
    use daanio_base::provider::Provider;
    use daanio_provider_anthropic_runtime::AnthropicProvider;

    let normalized = daanio_base::auth::lifecycle::normalized_auth_provider_id(Some(provider_id))
        .unwrap_or("claude");
    let provider_label = daanio_base::auth::lifecycle::provider_display_label(Some(normalized))
    ...
    // `DAANIO_RUNTIME_PROVIDER=claude-api` would silently test the API-key path
        ...
        checks.push(DoctorCheck::failed(
            ...
            format!(
                "could not select the Claude OAuth credential path: {error}. \
                 Run `daanio login --provider claude` to mint a fresh OAuth token."
            ),
        ));
        return Ok(finish_report(
            provider_id,
    ...
    let catalog_models: Vec<String> = if tier.requires_api_key() {
        match provider_runtime.fetch_live_model_ids_for_doctor().await {
            ...
            Ok(_) => {
                ...
                let fallback = daanio_base::provider::known_anthropic_model_ids();
                checks.push(DoctorCheck::passed(
                    checkpoints::MODEL_CATALOG_LIVE_ENDPOINT,
                    label_for(checkpoints::MODEL_CATALOG_LIVE_ENDPOINT),
                    format!(
...
/// Google OAuth tokens minted by `daanio login --provider antigravity`; the
...
/// accepts every schema construct daanio emits), then any Gemini model, then any
...
pub async fn run_antigravity_native_e2e(
    ...
    use daanio_base::provider::Provider;
    use daanio_provider_antigravity_runtime::AntigravityProvider;
    ...
    let _ = daanio_base::auth::lifecycle::normalized_auth_provider_id(Some(provider_id));
    let provider_label = daanio_base::auth::lifecycle::provider_display_label(Some("antigravity"))
                ...
                checks.push(DoctorCheck::failed(
                    ...
                    format!(
                        "could not resolve an Antigravity credential: {error}. \
                         Run `daanio login --provider antigravity` to sign in."
                    ),
                ));
                return Ok(finish_report(
                    provider_id,
...
impl NativeProviderKind {
    ...
    fn spec(self) -> NativeProviderSpec {
        match self {
            Self::OpenAi => NativeProviderSpec {
                ...
                auth_env_key: None,
                login_hint: "daanio login --provider openai",
            },
            Self::Gemini => NativeProviderSpec {
                ...
                auth_env_key: None,
                login_hint: "daanio login --provider gemini",
            },
            Self::Cursor => NativeProviderSpec {
                ...
                login_hint: "daanio login --provider cursor",
            },
            Self::Copilot => NativeProviderSpec {
                ...
                auth_env_key: None,
                login_hint: "daanio login --provider copilot",
            },
            Self::Bedrock => NativeProviderSpec {
                ...
                login_hint: "daanio login --provider bedrock",
            },
            Self::Daanio => NativeProviderSpec {
                provider_id: "daanio",
                label: "Daanio Subscription",
                // The transport is OpenAI-compatible internally, but the public
                // route identity is the managed Daanio subscription. Model
                // switches use a bare model id so they stay on that runtime.
                contract: WiringContract {
                    api_method: daanio_base::subscription_catalog::DAANIO_ROUTE_API_METHOD
                        .to_string(),
                    route_provider: daanio_base::subscription_catalog::DAANIO_PROVIDER_DISPLAY_NAME
                        .to_string(),
                    expected_runtime: "daanio",
                    expected_namespace: None,
                    switch_prefix: String::new(),
                },
                auth_source: "Daanio subscription API key (DAANIO_API_KEY)",
                auth_env_key: Some("DAANIO_API_KEY"),
                login_hint: "daanio login --provider daanio",
            },
            Self::Azure => NativeProviderSpec {
                ...
                login_hint: "daanio login --provider azure",
            },
        }
    }
    ...
    fn build_runtime(self) -> anyhow::Result<std::sync::Arc<dyn daanio_base::provider::Provider>> {
        use anyhow::Context as _;
        use daanio_base::provider::Provider;
        let runtime: std::sync::Arc<dyn Provider> = match self {
            Self::OpenAi => {
                let credentials =
                    daanio_base::auth::codex::load_credentials().unwrap_or_else(|_| {
                        daanio_base::auth::codex::CodexCredentials {
                ...
                std::sync::Arc::new(daanio_provider_openai_runtime::OpenAIProvider::new(
                    credentials,
                ))
            }
            Self::Gemini => {
                std::sync::Arc::new(daanio_provider_gemini_runtime::GeminiProvider::new())
            }
            Self::Cursor => {
                std::sync::Arc::new(daanio_provider_cursor_runtime::CursorCliProvider::new())
            }
            Self::Copilot => {
                ...
                daanio_base::env::set_var("DAANIO_COPILOT_PREFETCH_STARTUP_GRACE_MS", "0");
                let runtime = match daanio_provider_copilot_runtime::CopilotApiProvider::new() {
                    Ok(runtime) => runtime,
                    Err(_) => daanio_provider_copilot_runtime::CopilotApiProvider::new_with_token(
                        String::new(),
                    ),
                };
            ...
            Self::Bedrock => {
                std::sync::Arc::new(daanio_base::provider::bedrock::BedrockProvider::new())
            }
            Self::Daanio => {
                std::sync::Arc::new(daanio_base::provider::daanio::DaanioProvider::new())
            }
            Self::Azure => {
                ...
                daanio_base::auth::azure::apply_runtime_env()
                    .context("apply Azure OpenAI runtime env")?;
                let runtime = daanio_provider_openrouter_runtime::OpenRouterProvider::new()
                ...
                if let Some(model) = daanio_base::auth::azure::load_model() {
    ...
    async fn resolve_credential(self) -> anyhow::Result<String> {
        ...
        match self {
            Self::OpenAi => {
                let credentials = daanio_base::auth::codex::load_credentials()
                    .context("load OpenAI credentials (run `daanio login --provider openai`)")?;
            ...
            Self::Gemini => {
                let tokens = daanio_base::auth::gemini::load_or_refresh_tokens()
                    .await
            ...
            Self::Cursor => {
                let key = daanio_base::auth::cursor::load_api_key()
                    .context("load Cursor credential (run `daanio login --provider cursor`)")?;
            ...
            Self::Copilot => {
                let token = daanio_base::auth::copilot::load_github_token()
                    .context("load GitHub Copilot token (run `daanio login --provider copilot`)")?;
            ...
            Self::Bedrock => {
                if !daanio_base::provider::bedrock::BedrockProvider::has_credentials() {
                    anyhow::bail!(
            ...
            Self::Daanio => {
                if !daanio_base::subscription_catalog::has_credentials() {
                    anyhow::bail!(
                        "no Daanio subscription credential found (set DAANIO_API_KEY or run \
                         `daanio login --provider daanio`)"
                    );
                }
                Ok("Daanio subscription credential resolved".to_string())
            }
            Self::Azure => {
                if !daanio_base::auth::azure::has_configuration() {
                    anyhow::bail!(
                        "Azure OpenAI is not fully configured (need AZURE_OPENAI_ENDPOINT plus an \
                         API key or Entra ID); run `daanio login --provider azure`"
                    );
                }
                Ok(format!(
                    "Azure OpenAI configured ({})",
                    daanio_base::auth::azure::method_detail()
                ))
            }
        }
    }
    ...
    fn cheapest_model(self, catalog: &[String]) -> Option<String> {
        ...
        let cheap_markers: &[&str] = match self {
            ...
            Self::Daanio => &["mini", "flash", "haiku", "lite", "nano"],
...
struct NativeProviderSpec {
    ...
    label: &'static str,
    /// daanio-side routing/activation contract for the wiring checkpoints.
    contract: WiringContract,
    ...
    /// `daanio login` hint surfaced when the credential cannot be resolved.
...
async fn run_generic_native_api_checks(
    provider: &dyn daanio_base::provider::Provider,
    selected: &str,
    label: &str,
    checks: &mut Vec<DoctorCheck>,
    spend: &mut DoctorSpend,
...
/// The daanio-side wiring a given compat profile is expected to activate.
///
...
/// and daanio remaps them to their native runtimes. The doctor must assert the
...
fn wiring_contract(profile: OpenAiCompatibleProfile) -> WiringContract {
    match daanio_base::auth::lifecycle::normalized_auth_provider_id(Some(profile.id)) {
...
fn run_wiring_checks_for_contract(
    ...
    let auth = AuthChanged {
        provider: daanio_base::protocol::AuthProviderId::new(auth_provider_id),
...
mod tests {
    ...
    fn native_provider_kind_maps_every_generic_id() {
        for (id, expected) in [
            ...
            ("daanio", NativeProviderKind::Daanio),
    ...
    fn native_provider_specs_are_self_consistent() {
        ...
        for kind in [
            ...
            if kind == NativeProviderKind::Daanio {
                assert!(
                    spec.contract.switch_prefix.is_empty(),
                    "Daanio switches must use bare managed model ids"
                );
            } else {
                assert!(
    ...
    /// `native_doctor_supports_provider` lives in `daanio_base::auth::doctor`
    ...
    #[test]
    fn native_daanio_contract_uses_managed_subscription_identity() {
        let contract = NativeProviderKind::Daanio.spec().contract;
        assert_eq!(
            contract.api_method,
            daanio_base::subscription_catalog::DAANIO_ROUTE_API_METHOD
        );
        assert_eq!(
            contract.route_provider,
            daanio_base::subscription_catalog::DAANIO_PROVIDER_DISPLAY_NAME
        );
        assert_eq!(contract.expected_runtime, "daanio");
        assert!(contract.expected_namespace.is_none());
