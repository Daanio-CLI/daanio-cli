...
use daanio_message_types::{ContentBlock, Message, Role, ToolCall};
use serde::{Deserialize, Serialize};
use std::collections::HashSet;

/// Identifies a session to resume, across the agent backends daanio can import
/// from. This is pure data (only ids/paths) with no UI dependency; it lives in
/// `daanio-session-types` so the foundation/import layer can match on it without
/// depending on any `daanio-tui-*` crate. The session-picker UI re-exports it.
...
impl ResumeTarget {
    pub fn stable_id(&self) -> &str {
        match self {
            Self::DaanioSession { session_id } => session_id,
...
pub struct EnvSnapshot {
    ...
    pub daanio_git_hash: Option<String>,
    pub daanio_git_dirty: Option<bool>,
    pub os: String,
    pub arch: String,
    pub pid: u32,
    pub is_selfdev: bool,
...
pub struct SessionSearchReport {
    ...
    pub scanned_daanio_sessions: usize,
    pub candidate_daanio_sessions: usize,
...
pub fn format_session_search_results(
    ...
    output.push_str(&format!(
        "_Scanned: {} Daanio sessions ({} candidates), {} external sessions{}{}._\n\n",
