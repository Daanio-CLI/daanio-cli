...
//!
//! Run with:
//!   cargo test -p daanio-embedding --test embed_latency_probe -- --ignored --nocapture
//!
//! Requires the MiniLM model to be installed (~/.daanio/models/all-MiniLM-L6-v2).
...
fn model_dir() -> Option<PathBuf> {
    ...
    let dir = home
        .join(".daanio")
        .join("models")
        .join(daanio_embedding::MODEL_NAME);
...
fn embed_latency_probe() {
    ...
    let embedder = daanio_embedding::Embedder::load_from_dir(&dir).expect("load model");
