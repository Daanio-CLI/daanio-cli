...
/// Get the daanio repository directory
pub fn get_repo_dir() -> Option<PathBuf> {
    if let Ok(path) = std::env::var("DAANIO_REPO_DIR") {
        let path = PathBuf::from(path);
        if is_daanio_repo(&path) {
            return Some(path);
        }
    }
...
///
/// Release archives install a tiny `daanio` wrapper script alongside the real
/// `daanio-<platform>.bin` payload (the wrapper sets `LD_LIBRARY_PATH` and execs
...
pub fn selfdev_build_command_for_target(
    ...
    let specs = match target {
        SelfDevBuildTarget::Tui => vec![("daanio", "daanio")],
        SelfDevBuildTarget::Desktop => vec![("daanio-desktop", "daanio-desktop")],
        SelfDevBuildTarget::All | SelfDevBuildTarget::Auto => {
            vec![("daanio", "daanio"), ("daanio-desktop", "daanio-desktop")]
        }
    };
...
fn infer_selfdev_build_target(repo_dir: &Path) -> SelfDevBuildTarget {
    ...
    for line in text.lines() {
        ...
        if path == "Cargo.toml" || path == "Cargo.lock" || path.starts_with(".cargo/") {
            desktop = true;
            other = true;
        } else if path.starts_with("crates/daanio-desktop/") {
            desktop = true;
...
///
/// Defaults to `~/.local/bin` on Unix, `%LOCALAPPDATA%\daanio\bin` on Windows.
/// Overridable with `DAANIO_INSTALL_DIR`.
pub fn launcher_dir() -> Result<PathBuf> {
    if let Some(custom) = non_empty_env_path("DAANIO_INSTALL_DIR") {
        return Ok(custom);
    }

    if let Some(sandbox_home) = non_empty_env_path("DAANIO_HOME") {
    ...
    #[cfg(windows)]
    {
        if let Ok(local) = std::env::var("LOCALAPPDATA") {
            return Ok(PathBuf::from(local).join("daanio").join("bin"));
        }
        Ok(home_dir()?
            .join("AppData")
            .join("Local")
...
/// Path to the launcher binary (`~/.local/bin/daanio` by default).
...
/// Check if a directory is the daanio repository
pub fn is_daanio_repo(dir: &Path) -> bool {
    // Check for Cargo.toml with name = "daanio"
...
mod tests {
    ...
    fn repo_fixture(git_file: bool) -> tempfile::TempDir {
        ...
        if git_file {
            std::fs::write(temp.path().join(".git"), "gitdir: /tmp/daanio-test-git\n")
                .expect("git file");
        } else {
    ...
    fn find_repo_in_ancestors_finds_workspace_from_crate_dir() {
        ...
        let crate_dir = repo.path().join("crates").join("daanio-build-support");
    ...
    #[test]
    fn is_daanio_repo_accepts_git_file_for_worktree() {
        let repo = repo_fixture(true);
        assert!(is_daanio_repo(repo.path()));
    }

    /// Build a release-style install dir: `daanio` wrapper script + payload.
    fn release_install_fixture() -> (tempfile::TempDir, PathBuf, PathBuf) {
        let temp = tempfile::TempDir::new().expect("temp install");
        let wrapper = temp.path().join("daanio");
        let payload = temp.path().join("daanio-linux-x86_64.bin");
        std::fs::write(
            &wrapper,
    ...
    fn resolve_binary_payload_follows_channel_symlink_to_wrapper() {
        // The real layout: builds/<channel>/daanio -> versions/<v>/daanio (wrapper).
        ...
        let link = channel_dir.join("daanio");
        #[cfg(unix)]
    ...
    fn resolve_binary_payload_keeps_real_binary() {
        ...
        let temp = tempfile::TempDir::new().expect("temp install");
        let binary = temp.path().join("daanio");
        std::fs::write(&binary, vec![0x7fu8; 8192]).expect("binary");
        std::fs::write(temp.path().join("daanio-linux-x86_64.bin"), [0u8; 8]).expect("bin");
        assert_eq!(
    ...
    fn resolve_binary_payload_keeps_small_script_without_payload() {
        let temp = tempfile::TempDir::new().expect("temp install");
        let script = temp.path().join("daanio");
    ...
    fn resolve_binary_payload_refuses_ambiguous_payloads() {
        ...
        std::fs::write(temp.path().join("daanio-macos-aarch64.bin"), [0u8; 8]).expect("second bin");
        assert_eq!(
            resolve_binary_payload(&wrapper),
