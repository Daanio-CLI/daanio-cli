...
pub fn builds_dir() -> Result<PathBuf> {
    let dir = resolve_builds_dir(
        std::env::var_os("DAANIO_HOME").map(PathBuf::from),
...
fn resolve_builds_dir(
    daanio_home: Option<PathBuf>,
    local_app_data: Option<PathBuf>,
    default_daanio_dir: PathBuf,
    is_windows: bool,
) -> PathBuf {
    if let Some(daanio_home) = daanio_home {
        return daanio_home.join("builds");
    }

    if is_windows && let Some(local_app_data) = local_app_data {
        // Keep runtime channel discovery aligned with scripts/install.ps1 and
        // the supported Windows layout under %LOCALAPPDATA%\daanio\builds.
        // Durable user state and logs still live under ~/.daanio.
        return local_app_data.join("daanio").join("builds");
    }

    default_daanio_dir.join("builds")
}
...
mod tests {
    ...
    fn windows_builds_use_local_app_data() {
        let resolved = resolve_builds_dir(
            ...
            PathBuf::from("/home/test/.daanio"),
            true,
        );

        assert_eq!(resolved, PathBuf::from("/local-app-data/daanio/builds"));
    }

    #[test]
    fn daanio_home_override_wins_on_windows() {
        let resolved = resolve_builds_dir(
            Some(PathBuf::from("/isolated-daanio")),
            Some(PathBuf::from("/local-app-data")),
            PathBuf::from("/home/test/.daanio"),
            true,
        );

        assert_eq!(resolved, PathBuf::from("/isolated-daanio/builds"));
    }

    #[test]
    fn non_windows_builds_stay_under_daanio_home() {
        let resolved = resolve_builds_dir(
            None,
        ...
        assert_eq!(resolved, PathBuf::from("/home/test/.daanio/builds"));
    }
}
