...
pub use paths::{
    ...
    find_repo_in_ancestors, get_repo_dir, is_daanio_repo, launcher_binary_path, launcher_dir,
...
pub fn smoke_test_server_binary(binary: &Path) -> Result<()> {
    ...
    let socket_path = temp.path().join("daanio-smoke.sock");
    let stderr_path = temp.path().join("daanio-smoke.stderr.log");
