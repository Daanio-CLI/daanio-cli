...
fn with_temp_daanio_home<T>(f: impl FnOnce() -> T) -> T {
    ...
    daanio_core::env::set_var("DAANIO_HOME", temp_home.path());
    let result = f();
...
fn test_binary_version_hash_mismatch_rejects_publish_candidate() {
    ...
    let error =
        validate_binary_version_matches_source_report(&report, Path::new("daanio"), &source)
...
fn test_find_repo_in_ancestors_walks_upward() {
    let temp = tempfile::tempdir().expect("tempdir");
    let repo = temp.path().join("daanio-repo");
...
#[test]
fn launcher_dir_uses_sandbox_bin_when_daanio_home_is_set() {
    with_temp_daanio_home(|| {
        let launcher_dir = launcher_dir().expect("launcher dir");
        let expected = storage::daanio_dir().expect("daanio dir").join("bin");
...
fn update_launcher_symlink_stays_inside_sandbox_home() {
    with_temp_daanio_home(|| {
        ...
        let expected_launcher = storage::daanio_dir()
            .expect("daanio dir")
            .join("bin")
            .join(binary_name());
...
fn daemon_reload_target_version() -> Option<String> {
    ...
    // versions/<version>/daanio -> <version>
    canonical
        .parent()
...
/// Write a distinct, real binary into `versions/<version>/daanio` with an
