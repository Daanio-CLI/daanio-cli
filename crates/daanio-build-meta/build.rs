...
// Build metadata generator for the daanio workspace.
//
// This is the single source of truth for the DAANIO_* compile-time values that
// previously lived in the root `daanio` crate's build.rs. It is hosted in the
// leaf `daanio-build-meta` crate so every workspace crate can read identical
// values (via `daanio_build_meta::*`) without duplicating this script.
//
// NOTE: because this crate's own package version is unrelated to daanio's, we
...
fn main() {
    ...
    let dirty = match std::env::var("DAANIO_BUILD_GIT_DIRTY") {
        Ok(value) => matches!(
    ...
    let raw_log = std::env::var("DAANIO_BUILD_CHANGELOG_RAW")
        .ok()
    ...
    println!("cargo:rustc-env=DAANIO_GIT_HASH={}", git_hash);
    println!("cargo:rustc-env=DAANIO_GIT_DATE={}", git_date);
    println!("cargo:rustc-env=DAANIO_VERSION={}", version);
    println!("cargo:rustc-env=DAANIO_SEMVER={}", build_semver);
    println!("cargo:rustc-env=DAANIO_BASE_SEMVER={}", base_semver);
    println!("cargo:rustc-env=DAANIO_UPDATE_SEMVER={}", update_semver);
    println!("cargo:rustc-env=DAANIO_GIT_TAG={}", git_tag);
    println!("cargo:rustc-env=DAANIO_CHANGELOG={}", changelog);
    println!("cargo:rustc-env=DAANIO_PKG_VERSION={}", pkg_version);

    // Forward DAANIO_RELEASE_BUILD env var if set (CI sets this for release binaries)
    if std::env::var("DAANIO_RELEASE_BUILD").is_ok() {
        println!("cargo:rustc-env=DAANIO_RELEASE_BUILD=1");
    }
    ...
    // Since `daanio-build-meta` sits at the bottom of the crate graph
    ...
    //   * Release/dist builds set DAANIO_RELEASE_BUILD=1 and DAANIO_BUILD_SEMVER,
    ...
    println!("cargo:rerun-if-env-changed=DAANIO_RELEASE_BUILD");
    println!("cargo:rerun-if-env-changed=DAANIO_BUILD_SEMVER");
    // Allow callers to force a metadata refresh (e.g. install scripts) without a
    // full clean, by bumping this env var.
    println!("cargo:rerun-if-env-changed=DAANIO_BUILD_GIT_HASH");
    println!("cargo:rerun-if-env-changed=DAANIO_BUILD_GIT_DATE");
    println!("cargo:rerun-if-env-changed=DAANIO_BUILD_GIT_DIRTY");
    println!("cargo:rerun-if-env-changed=DAANIO_BUILD_GIT_TAG");
}

/// Workspace root, derived from this crate's manifest dir (`crates/daanio-build-meta`).
fn repo_root() -> PathBuf {
    let manifest_dir = std::env::var("CARGO_MANIFEST_DIR")
        .map(PathBuf::from)
        .unwrap_or_else(|_| PathBuf::from("."));
    // crates/daanio-build-meta -> crates -> <repo root>
    manifest_dir
        .parent()
        .and_then(|p| p.parent())
...
fn resolve_build_semver(base_version: (u32, u32, u32)) -> Result<String, String> {
    ...
    // function of HEAD, so the emitted DAANIO_SEMVER/DAANIO_VERSION only change when
    ...
    // turn invalidated `daanio-build-meta` and force-recompiled the entire crate
...
fn metadata_value(key: &str) -> Option<String> {
    let path = std::env::var("DAANIO_BUILD_METADATA_FILE").ok()?;
