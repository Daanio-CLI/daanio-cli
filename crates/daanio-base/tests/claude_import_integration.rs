...
//!
//! These drive the real `daanio_base::import` pipeline against synthetic
//! `.jsonl` fixtures written into a sandboxed `DAANIO_HOME`, so they never touch
...
//! environment (`DAANIO_HOME`).

use daanio_base::import::{
    import_session, import_session_from_file, list_claude_code_sessions,
    list_claude_code_sessions_lazy,
};
use daanio_base::message::ContentBlock;
use daanio_base::session::Session;
use daanio_import_core::imported_claude_code_session_id;
use std::path::{Path, PathBuf};
use std::sync::Mutex;

/// Serializes tests that mutate `DAANIO_HOME`. This test binary has its own
...
impl Drop for EnvVarGuard {
    fn drop(&mut self) {
        if let Some(prev) = self.prev.take() {
            daanio_base::env::set_var(self.key, prev);
        } else {
...
/// Create the sandboxed Claude project dir under `$DAANIO_HOME/external/...`.
...
fn import_array_tool_result_with_image_uses_placeholder_not_base64() {
    ...
    let saved = std::fs::read_to_string(daanio_session_file(temp.path(), "img-session")).unwrap();
    assert!(
        !saved.contains(&big),
...
/// Path to where daanio persists imported sessions under the sandbox.
/// With `DAANIO_HOME` set, `storage::daanio_dir()` returns it directly, and
/// sessions live at `<DAANIO_HOME>/sessions/<id>.json`.
fn daanio_session_file(home: &Path, claude_id: &str) -> PathBuf {
