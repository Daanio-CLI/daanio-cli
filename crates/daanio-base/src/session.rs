...
///
/// Wraps the on-disk streaming marker from `daanio-storage` (cleared on every
    ...
    pub fn new(session_id: impl Into<String>) -> Self {
        Self {
            ...
            sleep_assertion: crate::platform::PowerAssertion::prevent_user_idle_system_sleep(
                "Daanio streaming model response",
            ),
        }
    }
}
...
fn default_is_test_session() -> bool {
    env_flag_enabled("DAANIO_TEST_SESSION")
}
...
pub fn derive_session_provider_key(provider_name: &str) -> Option<String> {
    ...
    if normalized_name == "daanio" {
        return Some("daanio".to_string());
    }
    ...
    if let Ok(active) = std::env::var("DAANIO_ACTIVE_PROVIDER") {
...
request in this new forked session, using the inherited conversation only as context.\n</system-reminder>",
    ...
    /// Check if this session is working on the daanio repository
    pub fn is_self_dev(&self) -> bool {
        if let Some(ref dir) = self.working_dir {
            // Check if working dir contains daanio source
            let path = std::path::Path::new(dir);
            path.join("Cargo.toml").exists()
                && path.join("src/main.rs").exists()
                && std::fs::read_to_string(path.join("Cargo.toml"))
                    .map(|s| s.contains("name = \"daanio\""))
                    .unwrap_or(false)
        } else {
            false
        }
    ...
    pub fn strip_oversized_images(&mut self, target_total_chars: usize) -> usize {
        ...
        let stripped = daanio_compaction_core::strip_large_images_in_contents(
            &mut contents,
            target_total_chars,
        );
        if stripped > 0 {
