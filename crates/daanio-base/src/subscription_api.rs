//! Typed client for the Daanio account and subscription API.
//!
...
use crate::subscription_catalog::{self, DaanioTier};
...
pub const MODELS_FETCH_TIMEOUT: Duration = Duration::from_secs(10);
...
impl SubscriptionMe {
    pub fn parsed_tier(&self) -> Option<DaanioTier> {
        DaanioTier::parse(&self.tier)
    }
...
impl fmt::Display for AccountApiError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::Unauthorized => write!(f, "the Daanio account key is revoked or expired"),
            Self::Forbidden => write!(f, "the Daanio account request was denied"),
            Self::LegacyBackend => write!(
                f,
                "the configured Daanio API uses the legacy email-based login contract; update the backend to the current device authorization contract"
            ),
            Self::Http { status, code } => match code {
                Some(code) => write!(f, "Daanio account API returned HTTP {status} ({code})"),
                None => write!(f, "Daanio account API returned HTTP {status}"),
            },
            Self::InvalidResponse(detail) => {
                write!(
                    f,
                    "Daanio account API returned an invalid response: {detail}"
                )
            }
        }
    }
...
struct ModelCatalogWire {
    data: Vec<ModelCatalogEntryWire>,
}
...
pub fn configured_api_base() -> String {
    subscription_catalog::configured_api_base()
        .unwrap_or_else(|| subscription_catalog::DEFAULT_DAANIO_API_BASE.to_string())
        .trim_end_matches('/')
        .to_string()
}
...
pub async fn fetch_available_models_with(
    ...
    let wire: ModelCatalogWire = serde_json::from_str(&body)
        .map_err(|_| AccountApiError::InvalidResponse("malformed /v1/models JSON"))?;
...
pub async fn fetch_subscription_me() -> Result<SubscriptionMe> {
    let api_key = subscription_catalog::configured_api_key()
        .context("no Daanio account credential configured (run `daanio account login`)")?;
...
mod tests {
    ...
    fn subscription_me_parses_expected_shape() {
        let json = r#"{
            ...
            "manage_url": "https://dashboard.example.com/account"
        }"#;
        let me: SubscriptionMe = serde_json::from_str(json).expect("parse");
        assert_eq!(me.parsed_tier(), Some(DaanioTier::Flagship));
        assert!(me.has_active_paid_plan());
        assert_eq!(
            me.manage_url.as_deref(),
            Some("https://dashboard.example.com/account")
        );
    }
    ...
    async fn device_request_uses_new_contract_and_parses_public_urls() {
        let base = spawn_server(vec![(
            200,
            vec![],
            r#"{"device_code":"secret","flow_id":"public-flow","verification_uri":"https://dashboard.example.com/account","verification_uri_complete":"https://dashboard.example.com/account?flow=public-flow","verify_url":"https://dashboard.example.com/account?flow=public-flow","expires_in":600,"interval":3}"#.to_string(),
        )]);
        let result = request_device_authorization(&client(), &base, Some(DaanioTier::Pro))
            .await
            .expect("device auth");
    ...
    async fn legacy_device_response_is_explained_without_echoing_body() {
        ...
        let error = request_device_authorization(&client(), &base, Some(DaanioTier::Pro))
            .await
    ...
    #[tokio::test]
    async fn models_catalog_is_authenticated_trimmed_and_deduplicated() {
        let base = spawn_server(vec![(
            200,
            vec![],
    ...
    #[tokio::test]
    async fn models_catalog_accepts_an_authoritative_empty_list() {
        let base = spawn_server(vec![(
            200,
            vec![],
            r#"{"object":"list","success":true,"data":[]}"#.to_string(),
        )]);
        let models = fetch_available_models_with(&client(), &base, "browser-issued-key")
            .await
            .expect("empty models catalog");
