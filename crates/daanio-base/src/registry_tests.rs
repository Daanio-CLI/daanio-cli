...
fn find_server_by_socket_sync_returns_matching_server() {
    ...
    let prev_home: Option<OsString> = std::env::var_os("DAANIO_HOME");
...
async fn cleanup_stale_preserves_live_socket_paths() {
    ...
    let socket = temp_runtime.path().join("daanio.sock");
    let debug_socket = temp_runtime.path().join("daanio-debug.sock");
