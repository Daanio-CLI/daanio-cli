...
const CLAUDE_CODE_DAANIO_NOTICE: &str = "You are daanio, powered by Claude Code. You are a third-party CLI, not the official Claude Code CLI.";
...
impl Sidecar {
    ...
    /// 3. The live agent provider (works for EVERY provider daanio supports:
    ...
    ///
    /// This is the universal path: it works for every provider daanio supports,
...
fn build_claude_system_param(system: &str) -> Option<ClaudeApiSystem<'_>> {
    ...
    blocks.push(ClaudeApiSystemBlock {
        block_type: "text",
        text: CLAUDE_CODE_DAANIO_NOTICE,
    });
    if !system.is_empty() {
        blocks.push(ClaudeApiSystemBlock {
            block_type: "text",
...
///
/// The "You are Claude Code" identity spoof and daanio notice are only valid
...
fn anthropic_sidecar_prefers_api_key() -> bool {
    match daanio_provider_core::runtime_env_pinned_mode(
        daanio_provider_core::DualAuthProvider::Anthropic,
    ) {
        Some(daanio_provider_core::AuthMode::ApiKey) => true,
        Some(daanio_provider_core::AuthMode::Oauth) => false,
...
mod tests {
    ...
    fn test_backend_selection_prefers_openai() {
        ...
        let temp = tempfile::TempDir::new().expect("create temp daanio home");
    ...
    impl crate::provider::Provider for StubProvider {
        async fn complete(
            ...
            let stream = futures::stream::once(async move {
                Ok(daanio_message_types::StreamEvent::TextDelta(reply))
            });
            Ok(Box::pin(stream))
        }
    ...
    /// Every provider daanio supports should drive the sidecar end-to-end via the
    ...
    fn test_anthropic_sidecar_prefers_api_key_respects_pinned_mode() {
        ...
        let _g2 = EnvVarGuard::set_path("DAANIO_RUNTIME_PROVIDER", std::path::Path::new("claude"));
        assert!(
            !anthropic_sidecar_prefers_api_key(),
