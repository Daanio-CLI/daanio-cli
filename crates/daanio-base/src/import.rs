...
/// Convert Claude Code content blocks to daanio ContentBlocks
...
fn truncate_import_text(text: String, max_bytes: usize) -> String {
    ...
    format!(
        "{}{}",
        daanio_core::util::truncate_str(&text, prefix_budget),
        suffix
    )
}
...
fn reuse_existing_imported_session(session_id: &str) -> bool {
    // An imported snapshot becomes a normal daanio continuation as soon as the
    ...
    // doing so can discard daanio-only turns and journal state. New imports are
...
pub fn imported_session_id_for_target(
    target: &daanio_session_types::ResumeTarget,
) -> Option<String> {
    match target {
        daanio_session_types::ResumeTarget::DaanioSession { session_id } => {
            Some(session_id.clone())
        }
        daanio_session_types::ResumeTarget::ClaudeCodeSession { session_id, .. } => {
            Some(imported_claude_code_session_id(session_id))
        }
        daanio_session_types::ResumeTarget::CodexSession { session_id, .. } => {
            Some(imported_codex_session_id(session_id))
        }
        daanio_session_types::ResumeTarget::PiSession { session_path } => {
            Some(imported_pi_session_id(session_path))
        }
        daanio_session_types::ResumeTarget::OpenCodeSession { session_id, .. } => {
            Some(imported_opencode_session_id(session_id))
        }
        daanio_session_types::ResumeTarget::CursorSession { session_id, .. } => {
            Some(imported_cursor_session_id(session_id))
        }
    }
}

pub fn resolve_resume_target_to_daanio(
    target: &daanio_session_types::ResumeTarget,
) -> Result<daanio_session_types::ResumeTarget> {
    use daanio_session_types::ResumeTarget;
    ...
    let session_id = match target {
        ResumeTarget::DaanioSession { session_id } => {
            return Ok(ResumeTarget::DaanioSession {
                session_id: session_id.clone(),
            });
        }
    ...
    Ok(ResumeTarget::DaanioSession { session_id })
}
...
fn import_session_from_file_with_target(
    ...
    let mut session = Session::create_with_id(daanio_session_id, None, title);
...
/// Explicitly hand a currently-running Claude Code session over to Daanio.
///
/// This is deliberately separate from normal resume. It first imports the
/// current transcript into a fresh durable Daanio session, then gracefully stops
...
/// A stop failure rolls back the staged Daanio session and leaves ordinary
...
fn take_over_live_claude_session_with_timeout(
    ...
    // Use a normal memorable Daanio session ID so the handed-off conversation
    ...
    if stop_outcome == crate::claude_live::StopLiveClaudeOutcome::ExitUnconfirmed {
        crate::session_list_cache::invalidate();
        anyhow::bail!(
            "Claude Code process {} was asked to exit, but its exit was not confirmed; prepared Daanio session {} was preserved and can be resumed",
            live.pid,
            takeover_id
        );
    }
    ...
    if let Some(err) = final_refresh_error {
        ...
        return Err(err).with_context(|| {
            format!(
                "Claude Code exited, but its final transcript could not be refreshed; prepared Daanio session {takeover_id} was preserved"
            )
        });
    }

    crate::logging::info(&format!(
        "Claude takeover complete: source_session={session_id} source_pid={} daanio_session={takeover_id}",
        live.pid
    ));
    crate::session_list_cache::invalidate();
    Ok(ResumeTarget::DaanioSession {
...
fn finalize_imported_session(
    ...
    // Never overwrite a daanio-side continuation with a shorter external
...
pub fn import_cursor_session_from_path(
    ...
    let created_at =
        daanio_import_core::file_modified_datetime(session_path).unwrap_or_else(Utc::now);
