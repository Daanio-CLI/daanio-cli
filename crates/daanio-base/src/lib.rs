//! `daanio-base`: foundational layer of the daanio application core.
//!
//! This crate holds the downward-closed set of modules that the upper
//! server/tool/agent layer (`daanio-app-core`) depends on: provider, auth,
...
//!
//! `daanio-app-core` re-exports this crate via `pub use daanio_base::*`, so every
