...
//! doesn't expose a usage API. Data persists to ~/.daanio/copilot_usage.json.
...
pub use daanio_usage_types::{AllTimeUsage, CopilotUsageTracker, DayUsage, MonthUsage};
...
mod tests {
    ...
    #[test]
    fn usage_path_respects_daanio_home() {
    ...
    #[test]
    fn save_and_load_roundtrip_under_daanio_home() {
        let _env_lock = lock_env();
        clear_tracker();
        let temp = tempfile::tempdir().expect("tempdir");
        let _home = EnvVarGuard::set("DAANIO_HOME", temp.path().as_os_str());
