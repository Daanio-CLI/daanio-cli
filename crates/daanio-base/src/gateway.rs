...
//! This lets iOS/web clients interact with daanio sessions identically
//! to TUI clients.
//!
//! Architecture:
...
pub use daanio_gateway_types::{PairedDevice, PairingCode};
...
async fn handle_pair_request(
    ...
    // Reload from disk - pairing codes are generated by `daanio pair` CLI
    *reg = DeviceRegistry::load();
    ...
    let body = serde_json::json!({
        "token": token,
        "server_name": "daanio",
        "server_version": daanio_build_meta::version(),
    });
    http_response(200, "OK", &body.to_string())
}
