...
fn test_openai_live_catalog_replaces_static_fallback_list() {
    ...
    assert_eq!(
        ...
        [
            ...
            daanio_provider_core::CHATGPT_WEB_MODEL.to_string()
        ]
    );
    ...
    for extra in &models[2..] {
        assert!(
            daanio_provider_core::is_openai_api_only_pro_model(extra),
...
fn test_remote_daanio_subscription_fallback_keeps_managed_route_identity() {
    ...
    let routes = remote_model_routes_fallback(
        Some(crate::subscription_catalog::DAANIO_PROVIDER_DISPLAY_NAME),
        &models,
    );
