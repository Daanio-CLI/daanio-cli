...
#[cfg(test)]
use daanio_provider_core::FailoverDecision;
...
pub use daanio_provider_core::attempt_tracker;
pub use daanio_provider_core::cli_provider_arg_for_session_key;
pub use daanio_provider_core::{
    ALL_CLAUDE_MODELS, ALL_OPENAI_MODELS, CHATGPT_WEB_MODEL, CHEAPNESS_REFERENCE_INPUT_TOKENS,
    CHEAPNESS_REFERENCE_OUTPUT_TOKENS, CredentialMode, DAANIO_USER_AGENT, DEFAULT_CONTEXT_LIMIT,
    EventStream, ModelCapabilities, ModelCatalogRefreshSummary, ModelRoute, ModelRouteApiMethod,
...
pub use daanio_provider_core::{ProviderFailoverPrompt, parse_failover_prompt_message};
...
/// paths for OpenAI (codex-spark) and Claude (haiku) OAuth, but daanio also runs
...
/// from `[provider] stream_idle_timeout_secs` / `DAANIO_STREAM_IDLE_TIMEOUT_SECS`
...
fn cached_live_models_for_openai_compatible_profile(
    ...
    let cache = daanio_provider_openrouter::load_disk_cache_entry_for_namespace(&resolved.id)?;
    let cache_is_stale = daanio_provider_openrouter::current_unix_secs()
...
fn configured_standard_openrouter_profile_routes() -> Vec<ModelRoute> {
    let Some(cache) = daanio_provider_openrouter::load_disk_cache_entry_for_namespace("openrouter")
    else {
        return Vec::new();
    };
...
pub struct MultiProvider {
    ...
    /// `daanio-provider-copilot-runtime` and is instantiated through
    ...
    /// `daanio-provider-antigravity-runtime` and is instantiated through
    /// `external::instantiate_external_provider`.
    antigravity: RwLock<Option<Arc<dyn Provider>>>,
    /// Gemini provider (hot-swappable after login). Held as `dyn Provider`:
    /// the concrete runtime lives downstream in `daanio-provider-gemini-runtime`
    ...
    /// `dyn Provider`: the concrete runtime lives downstream in
    /// `daanio-provider-cursor-runtime` and is instantiated through
    /// `external::instantiate_external_provider`.
...
impl MultiProvider {
    ...
    fn routes_memo_key(&self) -> String {
        ...
        format!(
            "{}|{}|{}|{:?}|{}|{}|{}|{}",
            // Scope by home so sandboxes (tests, DAANIO_HOME switches) never
            // share catalogs that were built from different credential files.
            std::env::var("DAANIO_HOME").unwrap_or_default(),
    ...
    fn activity_source_key(&self, provider: ActiveProvider) -> String {
        match provider {
            ...
            ActiveProvider::OpenRouter => {
                ...
                // daanio subscription, and direct OpenAI-compatible profiles.
                let label = self
                    .active_openrouter_execution_provider()
                    .map(|execution| execution.runtime_display_name())
                    .unwrap_or_else(|| "OpenRouter".to_string());
                let runtime = std::env::var("DAANIO_RUNTIME_PROVIDER").ok();
        ...
        match provider {
            ActiveProvider::Claude => {
                ...
                if let Some(anthropic) = self.anthropic_provider() {
                    ...
                    anyhow::bail!(
                        "Claude credentials not available. Run `daanio login --provider claude` first."
                    );
                }
            ...
            ActiveProvider::OpenAI => {
                let Some(openai) = self.openai_provider() else {
                    ...
                    anyhow::bail!(
                        "OpenAI credentials not available. Run `daanio login --provider openai` first."
                    );
                };
            ...
            ActiveProvider::Copilot => {
                let Some(copilot) = self.copilot_provider() else {
                    anyhow::bail!(
                        "GitHub Copilot credentials not available. Run `daanio login --provider copilot` first."
                    );
                };
            ...
            ActiveProvider::Antigravity => {
                let Some(antigravity) = self.antigravity_provider() else {
                    anyhow::bail!(
                        "Antigravity credentials not available. Run `daanio login --provider antigravity` first."
                    );
                };
            ...
            ActiveProvider::Gemini => {
                let Some(gemini) = self.gemini_provider() else {
                    anyhow::bail!(
                        "Gemini credentials not available. Run `daanio login --provider gemini` first."
                    );
                };
            ...
            ActiveProvider::Cursor => {
                let Some(cursor) = self.cursor_provider() else {
                    anyhow::bail!(
                        "Cursor credentials not available. Run `daanio login --provider cursor` first."
                    );
                };
            ...
            ActiveProvider::OpenRouter => {
                ...
                let Some(openrouter) = self.openrouter_provider() else {
                    anyhow::bail!(
                        "OpenRouter/OpenAI-compatible credentials not available. Set the configured API key or run `daanio login --provider openrouter` first."
                    );
                };
    ...
    fn set_model_on_openai_compatible_profile(
        ...
        if !crate::provider_catalog::openai_compatible_profile_is_configured(profile) {
            anyhow::bail!(
                "{} credentials not available. Run `daanio login --provider {}` first.",
    ...
    pub(super) fn set_config_default_model(
            ...
            Self::resolve_config_provider_selection(pref, crate::config::config())
        {
            ...
            let pinned = daanio_provider_core::AuthRoute::parse_explicit_credential_prefix(pref);
            let anthropic_credential_mode = pinned.and_then(|route| {
                ...
                .then(|| match route.mode {
                    daanio_provider_core::AuthMode::ApiKey => {
                        anthropic::AnthropicCredentialMode::ApiKey
                    }
                    daanio_provider_core::AuthMode::Oauth => {
            ...
            let openai_credential_mode = pinned.and_then(|route| {
                matches!(
                    route.provider,
                    daanio_provider_core::DualAuthProvider::OpenAI
                )
                .then(|| match route.mode {
                    daanio_provider_core::AuthMode::ApiKey => openai::OpenAICredentialMode::ApiKey,
                    daanio_provider_core::AuthMode::Oauth => openai::OpenAICredentialMode::OAuth,
                })
            });
        ...
        let prefix = match active {
            ActiveProvider::Claude => {
                if let Some(anthropic) = self.anthropic_provider() {
                    ...
                    anthropic
                        .credential_mode()
                        .auth_route(daanio_provider_core::DualAuthProvider::Anthropic)
            ...
            ActiveProvider::OpenAI => {
                if let Some(openai) = self.openai_provider() {
                    openai
                        .credential_mode()
                        .auth_route(daanio_provider_core::DualAuthProvider::OpenAI)
                        .map(|route| route.model_prefix())
                        .unwrap_or("openai")
                } else {
                    "openai"
...
impl Provider for MultiProvider {
        ...
        match self.active_provider() {
            ActiveProvider::Claude => {
                ...
                if let Some(anthropic) = self.anthropic_provider() {
                    ...
                    daanio_provider_core::DEFAULT_CLAUDE_MODEL.to_string()
                }
            }
            ActiveProvider::OpenAI => self
                .openai_provider()
                .map(|o| o.model())
                .unwrap_or_else(|| daanio_provider_core::DEFAULT_OPENAI_MODEL.to_string()),
    ...
    fn active_resolved_credential(&self) -> Option<daanio_provider_core::ResolvedCredential> {
        use daanio_provider_core::ResolvedCredential;
        match self.active_provider() {
            ActiveProvider::Claude => {
    ...
    fn active_explicit_credential(&self) -> Option<daanio_provider_core::ResolvedCredential> {
        use daanio_provider_core::ResolvedCredential;
    ...
    fn set_model(&self, model: &str) -> Result<()> {
            ...
            explicit_model_provider_prefix(requested_model)
        {
            ...
            let pinned = daanio_provider_core::AuthRoute::parse_explicit_credential_prefix(prefix);
            let openai_credential_mode = pinned.and_then(|route| {
                matches!(
                    route.provider,
                    daanio_provider_core::DualAuthProvider::OpenAI
    ...
    fn handles_tools_internally(&self) -> bool {
        match self.active_provider() {
            ActiveProvider::Claude => {
                // Direct API does NOT handle tools internally - daanio executes them
            ...
            ActiveProvider::Cursor => self
                ...
                .map(|o| o.handles_tools_internally())
                .unwrap_or(false),
            ActiveProvider::Bedrock => false, // daanio executes Bedrock tool calls
            ActiveProvider::OpenRouter => false, // daanio executes tools
        }
    }
    ...
    fn supports_compaction(&self) -> bool {
        match self.active_provider() {
            ...
            ActiveProvider::Bedrock => self
                .bedrock_provider()
                .map(|o| o.uses_daanio_compaction())
                .unwrap_or(false),
            ActiveProvider::OpenRouter => self
                .active_openrouter_execution_provider()
                .map(|o| o.supports_compaction())
        ...
        match self.active_provider() {
            ActiveProvider::Claude => {
                if self.anthropic_provider().is_some() {
                    ...
                    self.claude_provider()
                        .map(|c| c.uses_daanio_compaction())
                        .unwrap_or(false)
                }
            }
            ActiveProvider::OpenAI => self
