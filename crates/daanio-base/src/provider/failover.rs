use super::*;
use daanio_provider_core::{FailoverDecision, ProviderFailoverPrompt};
...
impl MultiProvider {
    ...
    pub(super) fn fallback_sequence(active: ActiveProvider) -> Vec<ActiveProvider> {
        daanio_provider_core::fallback_sequence(active)
    }
    ...
    pub(super) fn classify_failover_error(err: &anyhow::Error) -> FailoverDecision {
        daanio_provider_core::classify_failover_error_message(&err.to_string())
    }
