...
pub fn simplified_model_routes_for_picker(
    ...
    let display_models = display_models.into_iter().collect::<Vec<_>>();
    if current_provider_name
        .eq_ignore_ascii_case(crate::subscription_catalog::DAANIO_PROVIDER_DISPLAY_NAME)
    {
        // Every model returned by Daanio's authenticated catalog is routed by
        // the Daanio gateway. Do not reclassify familiar model names as direct
        // OpenAI/Anthropic/Gemini routes or merge their static provider catalogs.
...
pub fn remote_model_routes_fallback(
    ...
    if remote_provider_name.is_some_and(|name| {
        ...
        return remote_available_entries
            ...
            .map(|model| ModelRoute {
                ...
                api_method: crate::subscription_catalog::DAANIO_ROUTE_API_METHOD.to_string(),
                available: true,
                detail: "daanio subscription routing · managed server-side".to_string(),
                cheapness: None,
            })
            .collect();
    }
    ...
    for model in remote_available_entries {
        ...
        if daanio_provider_core::model_id::matches_known_model(model, ALL_OPENAI_MODELS) {
            let availability = model_availability_for_account(model);
...
pub fn remote_model_routes_lightweight_fallback(
    ...
    let is_daanio_subscription = remote_provider_name.is_some_and(|name| {
        name.eq_ignore_ascii_case(crate::subscription_catalog::DAANIO_PROVIDER_DISPLAY_NAME)
    });
...
fn remote_openai_compatible_profile_models(
    ...
    if let Some(cache) =
        daanio_provider_openrouter::load_disk_cache_entry_for_namespace(&resolved.id)
    {
...
mod tests {
    ...
    #[test]
    fn simplified_daanio_picker_uses_only_live_gateway_models() {
        let routes = simplified_model_routes_for_picker(
            crate::subscription_catalog::DAANIO_PROVIDER_DISPLAY_NAME,
            "gpt-5.6-sol",
            vec!["gpt-5.6-sol".to_string(), "server-new-model".to_string()],
        );
        assert_eq!(
            routes
                .iter()
                ...
                .collect::<Vec<_>>(),
            vec!["gpt-5.6-sol", "server-new-model"]
        );
    ...
    impl EnvGuard {
        fn new() -> Self {
            ...
            let vars = vec![
                ("DAANIO_HOME", std::env::var_os("DAANIO_HOME")),
        ...
        fn save_opencode_cache(&self, source_api_base: &str, model_ids: &[&str]) {
            let daanio_home = std::env::var_os("DAANIO_HOME").expect("DAANIO_HOME set");
            let cache_dir = std::path::PathBuf::from(daanio_home).join("cache");
            std::fs::create_dir_all(&cache_dir).expect("create cache dir");
            let cache = daanio_provider_openrouter::DiskCache {
                cached_at: daanio_provider_openrouter::current_unix_secs()
