use anyhow::Result;
use daanio_provider_core::{ActiveProvider, provider_key};
...
impl RuntimeProviderId {
    ...
    pub const fn label(self) -> &'static str {
        match self {
            Self::Daanio => "Daanio Subscription",
...
impl ProviderActivation {
    ...
    pub fn azure_openai(model: Option<String>) -> Self {
        ...
        if let Some(model) = model.filter(|value| !value.trim().is_empty()) {
            activation.with_model_hint("DAANIO_OPENROUTER_MODEL", model)
        } else {
            activation
        }
    }
    ...
    pub fn daanio_subscription(model: impl Into<String>) -> Self {
        Self::locked(RuntimeProviderId::Daanio, ActiveProvider::OpenRouter)
            .with_model_hint("DAANIO_OPENROUTER_MODEL", model)
    }

    pub fn apply_env(&self) -> Result<()> {
        crate::env::set_var("DAANIO_RUNTIME_PROVIDER", self.runtime_id.key());
        match self.runtime_id {
            ...
            RuntimeProviderId::OpenAiCompatible => {
                if std::env::var_os("DAANIO_OPENROUTER_TRANSPORT_STATE").is_none() {
                    crate::env::set_var("DAANIO_OPENROUTER_TRANSPORT_STATE", "direct-api-key");
                }
            }
            _ => {
        ...
        match self.selection {
            RuntimeSelection::Locked(active_provider) => {
                ...
                crate::env::set_var("DAANIO_ACTIVE_PROVIDER", active_key_for_log);
                crate::env::set_var("DAANIO_FORCE_PROVIDER", "1");
            }
...
pub fn lock_runtime_provider_key(provider_key_raw: &str) {
    crate::env::set_var("DAANIO_ACTIVE_PROVIDER", provider_key_raw);
    crate::env::set_var("DAANIO_FORCE_PROVIDER", "1");
    crate::logging::auth_event(
...
mod tests {
    ...
    fn azure_activation_preserves_identity_while_using_openrouter_slot() {
        ...
        // (e.g. anthropic_tests sets DAANIO_RUNTIME_PROVIDER=claude); without
        ...
        assert_eq!(
            std::env::var("DAANIO_OPENROUTER_MODEL").as_deref(),
            Ok("gpt-4.1-mini")
        );
    }
}
