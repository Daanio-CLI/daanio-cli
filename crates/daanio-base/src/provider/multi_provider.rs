    ...
    pub(super) async fn try_same_provider_account_failover(
        ...
        for alternative_label in &alternatives {
            ...
            match attempt {
                ...
                .push(format!(
                        "⚡ Auto-switched {} account: {} → {}. To turn this off, set `[provider].same_provider_account_failover = false` in `~/.daanio/config.toml` or export `DAANIO_SAME_PROVIDER_ACCOUNT_FAILOVER=false`.",
