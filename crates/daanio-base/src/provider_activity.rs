...
//!   1. When daanio last successfully used it (for recency-sorted `/usage`).
...
//!
//! Data persists to `~/.daanio/provider_activity.json` and is shared across
...
//!   - `openrouter`, `daanio`, `copilot`, `gemini`, `cursor`, `bedrock`,
...
pub fn display_name_for_source_key(source_key: &str) -> String {
    ...
    match source_key {
        ...
        "daanio" => "Daanio subscription".to_string(),
...
/// `"NVIDIA NIM"`) plus the optional `DAANIO_RUNTIME_PROVIDER` key onto a
...
mod tests {
    ...
    #[test]
    fn record_use_and_spend_roundtrip_under_daanio_home() {
        let _env_lock = lock_env();
        clear_ledger_cache();
        let temp = tempfile::tempdir().expect("tempdir");
        let _home = EnvVarGuard::set("DAANIO_HOME", temp.path().as_os_str());
