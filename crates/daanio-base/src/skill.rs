...
impl SkillRegistry {
    ...
    ///
    /// Holds GLOBAL skills only (plugins, `~/.daanio/skills/`,
    ...
    /// Only runs if ~/.daanio/skills/ doesn't exist yet.
    fn import_from_external() {
        let daanio_skills = match crate::storage::daanio_dir() {
            Ok(dir) => dir.join("skills"),
            Err(_) => return,
        };

        if daanio_skills.exists() {
        ...
        {
            let count = Self::copy_skills_dir(&claude_skills, &daanio_skills);
            if count > 0 {
                sources.push(format!("{} from Claude Code", count));
                copied.extend(Self::list_skill_names(&daanio_skills));
            }
        }
        ...
        if let Ok(codex_skills) = crate::storage::user_home_path(".codex/skills")
            && codex_skills.is_dir()
        {
            let count = Self::copy_skills_dir(&codex_skills, &daanio_skills);
            if count > 0 {
    ...
    /// `~/.daanio/skills/`, and `~/.agents/skills/`.
    ///
    ...
    pub fn load_global() -> Result<Self> {
        ...
        // first, so explicit daanio/agents skills with the same name win below.
        if let Some(plugins_root) = Self::claude_plugins_root() {
            registry.load_plugin_skills_from_root(&plugins_root);
        }

        // Load from ~/.daanio/skills/ (daanio's own global skills)
        if let Ok(daanio_dir) = crate::storage::daanio_dir() {
            let daanio_skills = daanio_dir.join("skills");
            if daanio_skills.exists() {
                registry.load_from_dir(&daanio_skills)?;
            }
        }
    ...
    /// `./.daanio/skills/`, `./.agents/skills/`, and `./.claude/skills/`.
    ///
    ...
    fn load_project_local_dirs(&mut self, working_dir: Option<&Path>) -> Result<()> {
        // Load from ./.daanio/skills/ (project-local daanio skills)
        let local_daanio = Self::project_local_dir(working_dir, ".daanio");
        if local_daanio.exists() {
            self.load_from_dir(&local_daanio)?;
        }
    ...
    /// plugin never prevents daanio's own skills from loading.
    ...
    /// Reload the shared global skill sources (plugins, `~/.daanio/skills/`,
    ...
    pub fn reload_global(&mut self) -> Result<usize> {
        ...
        if let Ok(daanio_dir) = crate::storage::daanio_dir() {
            let daanio_skills = daanio_dir.join("skills");
            if daanio_skills.exists() {
                count += self.load_from_dir_count(&daanio_skills)?;
            }
        }
...
/// A skill recommended/curated by daanio that the user may want to install.
...
pub struct EndorsedSkill {
    ...
    /// Grouping label used to organize the endorsed list (e.g. "daanio",
    /// "NVIDIA CUDA-X").
...
/// Curated list of skills endorsed by daanio. Used by the `/skills` command to
...
pub const ENDORSED_SKILLS: &[EndorsedSkill] = &[
    EndorsedSkill {
        ...
        category: "daanio",
        source: "bundled in daanio repo (.daanio/skills/optimization)",
        install: None,
    },
    EndorsedSkill {
        ...
        category: "daanio",
        source: "bundled with daanio / Claude Code skills",
        install: None,
    },
    EndorsedSkill {
...
/// Return the curated list of skills endorsed by daanio.
...
mod tests {
    ...
    #[test]
    fn load_for_working_dir_reads_project_local_daanio_skills() {
        let temp = tempfile::tempdir().expect("tempdir");
        write_test_skill(temp.path(), ".daanio", "wd-only");
    ...
    fn project_overlay_is_session_scoped_and_composes_over_globals() {
        let temp = tempfile::tempdir().expect("tempdir");
        write_test_skill(temp.path(), ".daanio", "session-skill");
    ...
    fn registry_contains_reports_loaded_skills() {
        let temp = tempfile::tempdir().expect("tempdir");
        write_test_skill(temp.path(), ".daanio", "present-skill");
    ...
    #[test]
    fn explicit_daanio_skill_wins_over_plugin_skill_with_same_name() {
        let temp = tempfile::tempdir().expect("tempdir");
        ...
        // Explicit daanio skill with the same name.
        write_test_skill(temp.path(), ".daanio", "shared-name");
        ...
        registry.load_plugin_skills_from_root(&plugins_root);
        registry
            .load_from_dir(&temp.path().join(".daanio/skills"))
        ...
        assert_eq!(
            ...
            "explicit daanio skill must override the plugin-provided one"
        );
    }
