//! Embedding facade for daanio.
//!
//! The heavy ONNX/tokenizer implementation lives in the `daanio-embedding`
//! workspace crate so unchanged embedding code can stay cached across self-dev
//! builds. This module keeps daanio's process-wide cache, stats, and local path /
//! logging integration stable.

use anyhow::Result;
use daanio_embedding as backend;
use serde::Serialize;
...
pub fn models_dir() -> Result<PathBuf> {
    let dir = daanio_dir()?.join("models").join(backend::MODEL_NAME);
