...
use cache::*;
pub use daanio_usage_types::{ProviderUsage, ProviderUsageProgress, UsageLimit};
pub use model::*;
use provider_fetch::*;
    ...
    F: FnMut(ProviderUsageProgress) + Send,
{
    ...
    // Public Daanio builds authenticate only to the Daanio gateway. Never
    // inspect or contact upstream providers merely because credentials happen
    // to exist in the user's shell or in another tool's config directory.
...
/// Order reports so the most recently used login comes first. Sources daanio
...
/// Azure, daanio subscription, ...) still show recency + local spend without
