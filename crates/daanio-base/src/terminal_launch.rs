use anyhow::Result;
pub use daanio_terminal_launch::{
...
///
/// Resolution order: `DAANIO_SPAWN_HOOK` env (handled by config env overrides;
...
///
/// When a spawn hook is configured (`[terminal] spawn_hook` / `DAANIO_SPAWN_HOOK`),
/// the hook takes over the spawn: daanio runs `<hook> <program> <args...>` with
/// `DAANIO_SPAWN_*` metadata env vars so external programs (tmux, kitty remote,
/// herd, window managers) control where and how the session appears. If the
/// hook cannot be started, daanio falls back to its built-in terminal detection.
...
mod tests {
    ...
    fn spawn_via_hook_runs_hook_with_metadata_env() {
        ...
        std::fs::write(
            &hook_path,
            format!(
                "#!/bin/sh\nprintf '%s|%s|%s|%s' \"$DAANIO_SPAWN_KIND\" \"$DAANIO_SPAWN_SESSION_ID\" \"$DAANIO_SPAWN_SWARM_ID\" \"$*\" > {}\n",
        ...
        .spawn_env("DAANIO_SPAWN_SWARM_ID", "swarm-7");
        ...
        assert_eq!(
            recorded, "swarm-agent|ses_hooked|swarm-7|/usr/local/bin/daanio --resume ses_hooked",
            "hook should receive metadata env and the daanio command as argv"
        );
    }
}
