...
const ALLOW_LEGACY_AUTH_ENV: &str = "DAANIO_ALLOW_CODEX_LEGACY_AUTH";
...
pub struct DaanioOpenAiAuthFile {
...
fn relabel_accounts(auth: &mut DaanioOpenAiAuthFile) -> bool {
...
fn daanio_auth_path() -> Result<PathBuf> {
    Ok(crate::storage::daanio_dir()?.join("openai-auth.json"))
}
...
pub fn save_auth_file(auth: &DaanioOpenAiAuthFile) -> Result<()> {
    let auth_path = daanio_auth_path()?;
    let clean = DaanioOpenAiAuthFile {
...
fn load_daanio_credentials() -> Result<CodexCredentials> {
    let auth = load_auth_file()?;
    if auth.openai_accounts.is_empty() {
        anyhow::bail!("No OpenAI accounts configured in daanio auth file")
    }
    ...
    let account = auth
        ...
        .context("No OpenAI accounts in daanio auth file")?;
...
/// the raw tokens plus a `last_refresh` timestamp. Without an expiry, daanio
...
pub(crate) fn expires_at_from_access_token(access_token: &str) -> Option<i64> {
    ...
    // `exp` is in seconds since epoch; the rest of daanio tracks expiry in millis.
