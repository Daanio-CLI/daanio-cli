...
pub fn apply_runtime_env() -> Result<()> {
    let endpoint = load_endpoint().ok_or_else(|| {
        anyhow::anyhow!(
            "{} not found in environment or ~/.config/daanio/{}",
            ENDPOINT_ENV,
            ENV_FILE
        )
    })?;

    crate::env::set_var("DAANIO_OPENROUTER_API_BASE", endpoint);
    crate::env::set_var("DAANIO_OPENROUTER_API_KEY_NAME", API_KEY_ENV);
    crate::env::set_var("DAANIO_OPENROUTER_ENV_FILE", ENV_FILE);
    crate::env::set_var("DAANIO_OPENROUTER_CACHE_NAMESPACE", "azure-openai");
    ...
    crate::env::set_var("DAANIO_OPENROUTER_MODEL_CATALOG", "0");

    if uses_entra_id() {
        crate::env::set_var("DAANIO_OPENROUTER_AUTH_HEADER", "authorization-bearer");
        crate::env::set_var("DAANIO_OPENROUTER_DYNAMIC_BEARER_PROVIDER", "azure");
    } else {
        crate::env::set_var("DAANIO_OPENROUTER_AUTH_HEADER", "api-key");
        crate::env::remove_var("DAANIO_OPENROUTER_DYNAMIC_BEARER_PROVIDER");
    }
...
pub async fn get_bearer_token() -> Result<String> {
    daanio_azure_auth::get_bearer_token(COGNITIVE_SCOPE).await
}
