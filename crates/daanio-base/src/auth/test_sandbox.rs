...
impl AuthTestSandbox {
    pub fn new() -> anyhow::Result<Self> {
        ...
        std::fs::create_dir_all(temp.path().join("config").join("daanio"))?;
    ...
    pub fn config_dir(&self) -> PathBuf {
        self.root().join("config").join("daanio")
    }
...
mod tests {
    ...
    #[test]
    fn sandbox_isolates_daanio_home_and_config_dir() {
        let sandbox = AuthTestSandbox::new().expect("sandbox");

        assert_eq!(
            std::env::var("DAANIO_HOME").ok().as_deref(),
