...
use daanio_provider_core::ActiveProvider;
...
fn globally_preferred_model_rank(model: &str) -> (u8, usize) {
    ...
    let openai_default = normalize_model_for_preference(daanio_provider_core::DEFAULT_OPENAI_MODEL);
    let claude_default = normalize_model_for_preference(daanio_provider_core::DEFAULT_CLAUDE_MODEL);

    if normalized == openai_default {
        return (0, 0);
    }
    // Some catalogs expose the clean release id instead of daanio's Sol route.
...
/// order. The Claude/OpenAI subscription default bias mirrors daanio's global
...
fn normalize_model_for_preference(model: &str) -> String {
    let mut id = daanio_provider_core::model_id::canonical(model);
    ...
    id = daanio_provider_core::model_id::strip_date_suffix(&id).to_string();
...
fn route_matches_activation(route: &ModelRoute, activation: &AuthActivationResult) -> bool {
    ...
    match provider_id {
        ...
        "daanio" => {
            // Daanio subscription routes deliberately keep their managed public
            // identity even though the runtime reuses OpenRouter transport code.
            return matches!(
                api_method,
                crate::provider::ModelRouteApiMethod::DaanioSubscription
            );
        }
        "azure-openai" => {
...
/// `daanio_provider_core::auto_default_provider`: Claude wins when both families
...
fn normalized_login_provider_id(provider_id: &str) -> Option<&'static str> {
    match provider_id.trim().to_ascii_lowercase().as_str() {
        ...
        "daanio" | "subscription" | "daanio-subscription" => Some("daanio"),
...
fn api_key_env_bindings_for_provider(provider_id: &str) -> Vec<(String, String)> {
    match provider_id {
        ...
        "daanio" => vec![
            (
                crate::subscription_catalog::DAANIO_API_KEY_ENV.to_string(),
                crate::subscription_catalog::DAANIO_ENV_FILE.to_string(),
            ),
            (
                crate::subscription_catalog::DAANIO_API_BASE_ENV.to_string(),
                crate::subscription_catalog::DAANIO_ENV_FILE.to_string(),
            ),
        ],
        "bedrock" => vec![
...
/// process (issue #453).
///
/// `/login` persists API keys to the per-provider env file under the daanio
...
fn direct_provider_activation(provider_id: &str) -> Option<ProviderActivation> {
    match normalized_login_provider_id(provider_id)? {
        ...
        "daanio" => Some(ProviderActivation::locked(
            RuntimeProviderId::Daanio,
            ActiveProvider::OpenRouter,
        )),
...
mod tests {
    ...
    fn direct_login_provider_ids_are_normalized_with_display_labels() {
        for (hint, normalized, label) in [
            ...
            ("subscription", "daanio", "Daanio Subscription"),
    ...
    fn direct_login_provider_activation_sets_runtime_identity_and_active_provider() {
        // Sandbox DAANIO_HOME so activation's env-file credential sync (#453)
        // cannot read the developer's real ~/.config/daanio/*.env files and
        ...
        for (provider, runtime, active) in [
            ...
            crate::env::remove_var("DAANIO_FORCE_PROVIDER");
            ...
            assert_eq!(
                std::env::var("DAANIO_RUNTIME_PROVIDER").as_deref(),
                Ok(runtime)
            );
            assert_eq!(
                std::env::var("DAANIO_ACTIVE_PROVIDER").as_deref(),
                Ok(active)
            );
            assert_eq!(std::env::var("DAANIO_FORCE_PROVIDER").as_deref(), Ok("1"));
        }
    }

    #[test]
    fn direct_login_provider_descriptor_matrix_has_full_lifecycle_parity() {
        // Sandbox DAANIO_HOME for the same reason as the activation matrix
        ...
        for provider in crate::provider_catalog::login_providers() {
            let Some((normalized, runtime, active, switch_prefix)) = (match provider.target {
                crate::provider_catalog::LoginProviderTarget::Daanio => {
                    Some(("daanio", "daanio", "openrouter", ""))
                }
    ...
    #[test]
    fn daanio_auth_lifecycle_matches_only_managed_subscription_routes() {
        let activation = AuthActivationResult {
            provider_id: Some("daanio".to_string()),
            provider_label: Some("Daanio Subscription".to_string()),
            activated_model: Some("gpt-5.5".to_string()),
            expected_runtime: Some("daanio-subscription".to_string()),
            expected_catalog_namespace: Some("daanio-subscription".to_string()),
        };
        let routes = vec![
            ...
            route(
                "gpt-5.5",
                "Daanio Subscription",
                crate::subscription_catalog::DAANIO_ROUTE_API_METHOD,
                true,
            ),
        ];
        ...
        assert!(
            report.ok(),
            "canonical Daanio route should match: {report:?}"
        );
        assert_eq!(report.selectable_provider_routes, 1);
        assert_eq!(
            report.route_sample,
        ...
        assert_eq!(
            activation.model_switch_request("Daanio Subscription", "gpt-5.5"),
            "gpt-5.5"
        );
    }
    ...
    fn post_auth_model_selection_prefers_newest_live_release_for_unranked_provider() {
        let _env = EnvGuard::new(&["DAANIO_HOME"]);
    ...
    /// Claude flagship over the OpenAI flagship to mirror daanio's default model.
    #[test]
