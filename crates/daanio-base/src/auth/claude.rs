...
/// Represents a named Anthropic OAuth account stored in daanio's auth.json.
...
fn relabel_accounts(auth: &mut DaanioAuthFile) -> bool {
...
pub fn daanio_path() -> Result<PathBuf> {
    Ok(crate::storage::daanio_dir()?.join("auth.json"))
}

// ---- Multi-account helpers ----

/// Read the daanio auth file, auto-migrating from legacy format if needed.
pub fn load_auth_file() -> Result<DaanioAuthFile> {
    let path = daanio_path()?;
    if !path.exists() {
        return Ok(DaanioAuthFile::default());
    }

    crate::storage::harden_secret_file_permissions(&path);

    let mut auth: DaanioAuthFile = crate::storage::read_json(&path)
        .with_context(|| format!("Could not read daanio credentials from {:?}", path))?;
...
/// Write the daanio auth file (multi-account format).
pub fn save_auth_file(auth: &DaanioAuthFile) -> Result<()> {
    let auth_path = daanio_path()?;

    let clean = DaanioAuthFile {
...
/// Falls through Claude Code -> daanio accounts -> OpenCode, preferring non-expired tokens.
pub fn load_credentials() -> Result<ClaudeCredentials> {
    ...
    // prompt, so it is read once at import time and copied into daanio's own
    ...
    if let Ok(creds) = load_daanio_credentials() {
        if creds.expires_at > now_ms {
            return Ok(creds);
        }
        expired_candidates.push(("daanio", creds));
    }
    ...
    anyhow::bail!("No Claude OAuth credentials found (checked Claude Code, daanio, OpenCode)")
}

/// Load credentials for a specific daanio account by label.
...
/// Load credentials from the active daanio account (multi-account aware).
fn load_daanio_credentials() -> Result<ClaudeCredentials> {
    let auth = load_auth_file()?;
    if auth.anthropic_accounts.is_empty() {
        anyhow::bail!("No anthropic accounts configured in daanio auth.json");
    }
    ...
    let account = auth
        ...
        .context("No anthropic accounts in daanio auth.json")?;
...
/// them into a daanio-managed Anthropic account so future loads and token
/// refreshes are served from daanio's own `auth.json`.
///
...
/// rotates into daanio's store on refresh, so the copy stays usable after the
