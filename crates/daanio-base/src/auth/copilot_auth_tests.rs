...
fn gh_cli_fallback_requires_explicit_opt_in() {
    let key = "DAANIO_COPILOT_ALLOW_GH_AUTH_TOKEN";
...
fn save_github_token_creates_config_dir() -> Result<()> {
    ...
    let dir = TempDir::new().map_err(|e| anyhow!(e))?;
    let config_dir = dir.path().join("github-copilot");
    let prev_daanio_home = std::env::var_os("DAANIO_HOME");
    ...
    if let Some(prev) = prev_daanio_home {
...
#[test]
fn legacy_copilot_config_dir_uses_daanio_home_external_dir() -> Result<()> {
    let _guard = crate::storage::lock_test_env();
    let dir = TempDir::new().map_err(|e| anyhow!(e))?;
...
fn live_verify_failure_record_blocks_auto_use() -> Result<()> {
    // Lock the test env and isolate DAANIO_HOME so the validation record we write
