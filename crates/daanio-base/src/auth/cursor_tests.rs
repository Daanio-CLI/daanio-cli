...
#[test]
fn config_file_path_under_daanio() {
    let path = config_file_path().unwrap();
    let path_str = path.to_string_lossy();
    assert!(path_str.contains("daanio"));
...
fn save_and_load_api_key() {
    ...
    let file = dir.path().join("daanio").join("cursor.env");
...
#[test]
fn cursor_auth_file_path_respects_daanio_home() {
    // Regression: on Linux the auth.json path previously used
    // `dirs::config_dir()` directly, ignoring DAANIO_HOME. That leaked the real
    ...
    // provider correctly looked under `$DAANIO_HOME/external/...`.
    ...
    assert!(
        ...
        "cursor auth path should be under DAANIO_HOME/external, got {}",
        path.display()
    );
...
#[test]
fn cursor_vscdb_paths_respect_daanio_home() {
    let _guard = crate::storage::lock_test_env();
    let prev_home = std::env::var_os("DAANIO_HOME");
    let temp = TempDir::new().unwrap();
    crate::env::set_var("DAANIO_HOME", temp.path());
