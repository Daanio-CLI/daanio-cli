...
/// Cached auth status plus the `DAANIO_HOME` it was computed under.
///
/// Auth probes read credential files relative to `DAANIO_HOME`. Tests swap
/// `DAANIO_HOME` to per-test temp dirs, and a status computed under one home
...
/// CLI tools don't get installed/uninstalled while daanio is running, so caching
...
/// location no installed or self-dev daanio binary ever runs from.
///
...
/// multiple browser tabs. Set `DAANIO_ALLOW_BROWSER_IN_TESTS=1` to opt out
...
pub fn running_in_test_harness() -> bool {
    ...
    *IN_TEST_HARNESS.get_or_init(|| {
        if env_truthy("DAANIO_ALLOW_BROWSER_IN_TESTS") {
            return false;
        }
        std::env::current_exe()
            .ok()
...
fn log_auth_status_snapshot(event: &str, status: &AuthStatus) {
    crate::logging::auth_event(
        ...
        "all",
        &[
            ("daanio", auth_state_label(status.daanio)),
...
impl AuthStatus {
    ...
    pub fn has_any_available(&self) -> bool {
        self.anthropic.state == AuthState::Available
            || self.daanio == AuthState::Available
    ...
    /// daanio believes credentials are available/expired/missing without leaking
    ...
    pub fn log_snapshot(&self, surface: &str) {
        crate::logging::event_info(
            ...
            vec![
                ...
                ("daanio", self.daanio.label().to_string()),
    ...
    pub fn state_for_key(&self, key: LoginProviderAuthStateKey) -> AuthState {
        match key {
            ...
            LoginProviderAuthStateKey::Daanio => self.daanio,
        ...
        let (
            ...
            crate::provider_catalog::LoginProviderTarget::Daanio => {
                let (source, detail) = summarize_sources(vec![
                    env_source(crate::subscription_catalog::DAANIO_API_KEY_ENV),
                    config_source(
                        crate::subscription_catalog::DAANIO_API_KEY_ENV,
                        crate::subscription_catalog::DAANIO_ENV_FILE,
                        "~/.config/daanio/daanio.env",
                    ),
                ]);
                (
                    source,
            ...
            crate::provider_catalog::LoginProviderTarget::ClaudeApiKey => {
                ...
                // config file (`~/.config/daanio/anthropic.env`), *not* an env
                // var and *not* `~/.daanio/auth.json` (which holds the separate
            ...
            crate::provider_catalog::LoginProviderTarget::Azure => {
                let (source, detail) = summarize_sources(vec![
                    ...
                    config_source(
                        ...
                        "~/.config/daanio/azure-openai.env",
                    ),
                ]);
                (
                    source,
            ...
            crate::provider_catalog::LoginProviderTarget::Bedrock => {
                let (source, detail) = summarize_sources(vec![
                    ...
                    config_source(
                        ...
                        "~/.config/daanio/bedrock.env",
                    ),
                    env_source("AWS_PROFILE"),
                    env_source("DAANIO_BEDROCK_PROFILE"),
                    ...
                    crate::provider_catalog::active_named_provider_profile_credential_source()
                {
                    ...
                    summarize_sources(vec![
                        ...
                        config_source(
                            &resolved.api_key_env,
                            &resolved.env_file,
                            format!("~/.config/daanio/{}", resolved.env_file),
                        ),
...
fn build_auth_status_uncached(mode: AuthProbeMode) -> (AuthStatus, Vec<(&'static str, u128)>) {
    ...
    record_auth_probe_step(&mut timings, "daanio", || probe_daanio_status(&mut status));
...
fn probe_daanio_status(status: &mut AuthStatus) {
    if crate::subscription_catalog::has_credentials() {
        status.daanio = AuthState::Available;
    }
}
...
fn assessment_for_key(
    ...
    match key {
        ...
        LoginProviderAuthStateKey::Daanio
...
fn anthropic_oauth_source(status: &AuthStatus) -> Option<(AuthCredentialSource, String)> {
    ...
    {
        return Some((
            AuthCredentialSource::DaanioManagedFile,
            "~/.daanio/auth.json".to_string(),
        ));
    }
...
fn openai_oauth_source(status: &AuthStatus) -> Option<(AuthCredentialSource, String)> {
        ...
        .is_empty()
    {
        return Some((
            AuthCredentialSource::DaanioManagedFile,
            "~/.daanio/openai-auth.json".to_string(),
        ));
    }
