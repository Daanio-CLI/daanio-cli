...
#[test]
fn daanio_auth_file_default_is_empty() {
    let auth = DaanioAuthFile::default();
    assert!(auth.anthropic_accounts.is_empty());
    assert!(auth.active_anthropic_account.is_none());
}

#[test]
fn daanio_auth_file_roundtrip() {
    let auth = DaanioAuthFile {
    ...
    let parsed: DaanioAuthFile = serde_json::from_str(&json).unwrap();
...
#[test]
fn daanio_path_respects_daanio_home() {
    let _lock = crate::storage::lock_test_env();
    let temp = tempfile::TempDir::new().unwrap();
    let _home = EnvVarGuard::set("DAANIO_HOME", temp.path());

    assert_eq!(daanio_path().unwrap(), temp.path().join("auth.json"));
    assert_eq!(
...
#[test]
fn daanio_auth_file_multi_account() {
    let auth = DaanioAuthFile {
...
#[test]
fn daanio_auth_file_legacy_migration_format() {
    let legacy_json = r#"{
        "anthropic": {
            "access": "legacy_acc",
            "refresh": "legacy_ref",
    ...
    let parsed: DaanioAuthFile = serde_json::from_str(legacy_json).unwrap();
...
fn update_account_profile_sets_email() {
    let mut auth = DaanioAuthFile::default();
...
///   cargo test -p daanio-base --lib auth::claude::tests::live_keychain -- --ignored --nocapture
#[cfg(target_os = "macos")]
#[test]
