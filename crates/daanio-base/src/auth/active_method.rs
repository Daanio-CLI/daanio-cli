...
//! [`AuthStatus`] plus the `DAANIO_RUNTIME_PROVIDER` override on its own, keyed on
...
impl From<daanio_provider_core::ResolvedCredential> for ActiveCredential {
    fn from(value: daanio_provider_core::ResolvedCredential) -> Self {
        match value {
            daanio_provider_core::ResolvedCredential::Oauth => Self::OAuth,
            daanio_provider_core::ResolvedCredential::ApiKey => Self::ApiKey,
        }
    }
}
...
pub struct ResolvedProviderAuth {
    ...
    /// True when the active credential was pinned via `DAANIO_RUNTIME_PROVIDER`
...
///
/// `runtime_provider` is the raw `DAANIO_RUNTIME_PROVIDER` value (if any); it
...
pub fn resolve_dual_credential_auth(
    ...
    let dual = daanio_provider_core::DualAuthProvider::from_active_provider(provider)?;
    ...
    let forced =
        daanio_provider_core::pinned_mode_for(dual, runtime_provider).map(|mode| match mode {
            daanio_provider_core::AuthMode::Oauth => ActiveCredential::OAuth,
            daanio_provider_core::AuthMode::ApiKey => ActiveCredential::ApiKey,
        });
