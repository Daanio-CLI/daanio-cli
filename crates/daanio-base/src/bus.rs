...
pub use daanio_batch_types::{BatchProgress, BatchSubcallProgress, BatchSubcallState};
...
pub struct ModelRefreshCompleted {
    pub session_id: String,
    pub result: std::result::Result<daanio_provider_core::ModelCatalogRefreshSummary, String>,
}
...
/// background tasks, without re-coupling `daanio-base` to swarm internals.
#[derive(Clone, Debug)]
...
pub enum BusEvent {
    ...
    UsageReport(Vec<daanio_usage_types::ProviderUsage>),
    /// Progressive usage report update while providers are still loading
    UsageReportProgress(daanio_usage_types::ProviderUsageProgress),
