...
/// Default system prompt for daanio (embedded at compile time)
...
/// creating `~/.daanio/swarm-prompt.md` (global) or `./.daanio/swarm-prompt.md`
...
/// project `./.daanio/swarm-prompt.md`, then global `~/.daanio/swarm-prompt.md`,
...
pub fn load_swarm_prompt(working_dir: Option<&Path>) -> String {
    ...
    let candidates = [
        Some(project_dir.join(".daanio").join("swarm-prompt.md")),
...
/// `mission` module in the upper `daanio-app-core` layer; the asset lives here
...
pub fn build_system_prompt_full_with_capabilities(
    ...
    // Add optional prompt overlays from ~/.daanio/ and ./.daanio/
    ...
    // Add optional preferred-tool guidance from ~/.daanio/ and ./.daanio/
...
impl SelfDevProductContext {
    fn from_working_dir(working_dir: Option<&Path>) -> Self {
        ...
        if path.contains("/crates/daanio-desktop") || path.ends_with("crates/daanio-desktop") {
            Self::Desktop
        } else {
            Self::Tui
        }
...
pub fn build_session_context(working_dir: Option<&Path>) -> String {
    ...
    lines.push(format!(
        "Daanio version: {} ({})",
        daanio_build_meta::version(),
        daanio_build_meta::git_hash()
    ));
...
/// Load optional prompt overlay markdown from ~/.daanio/ and ./.daanio/
fn load_prompt_overlay_files_from_dir(working_dir: Option<&Path>) -> (Option<String>, usize) {
    ...
    let project_dir = working_dir.unwrap_or(Path::new("."));
    if let Some((content, size)) = load_file(
        &project_dir.join(".daanio").join("prompt-overlay.md"),
        "Project Prompt Overlay (.daanio/prompt-overlay.md)",
    ) {
        total_chars += size;
        contents.push(content);
    }
    ...
    if let Ok(global_overlay) =
        crate::storage::daanio_dir().map(|dir| dir.join("prompt-overlay.md"))
        && let Some((content, size)) = load_file(
            &global_overlay,
            "Global Prompt Overlay (~/.daanio/prompt-overlay.md)",
        )
    {
        total_chars += size;
        contents.push(content);
...
/// Load optional preferred-tool guidance from ~/.daanio/ and ./.daanio/
fn load_preferred_tools_files_from_dir(working_dir: Option<&Path>) -> (Option<String>, usize) {
    ...
    let project_dir = working_dir.unwrap_or(Path::new("."));
    if let Some((content, size)) = load_file(
        &project_dir.join(".daanio").join("preferred-tools.md"),
        "Project Preferred Tools (.daanio/preferred-tools.md)",
    ) {
        total_chars += size;
        contents.push(content);
    }
    ...
    if let Ok(global_preferred_tools) =
        crate::storage::daanio_dir().map(|dir| dir.join("preferred-tools.md"))
        && let Some((content, size)) = load_file(
            &global_preferred_tools,
            "Global Preferred Tools (~/.daanio/preferred-tools.md)",
        )
    {
        total_chars += size;
        contents.push(content);
