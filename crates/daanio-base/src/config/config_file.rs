use super::*;
use crate::storage::daanio_dir;
use std::path::PathBuf;

impl Config {
    /// Get the config file path
    pub fn path() -> Option<PathBuf> {
        daanio_dir().ok().map(|d| d.join("config.toml"))
    }
    ...
    ///
    /// Scans `~/.daanio/sessions` for the directories the user works in most,
    ...
    pub fn bake_launch_hotkeys_once() -> bool {
        use daanio_import_core::repo_ranking;

        let cfg = Self::load();
        if cfg.launch_hotkeys.imported {
            return false;
        }
        let Ok(daanio_dir) = daanio_dir() else {
            return false;
        };
        let sessions_dir = daanio_dir.join("sessions");
        ...
        let entries: Vec<daanio_config_types::LaunchHotkeyEntry> = plan
            .into_iter()
            .map(|p| daanio_config_types::LaunchHotkeyEntry {
                chord: p.chord,
    ...
    pub fn external_auth_source_allowed_for_path_cached(
        ...
        // a new path-bound trust decision, or before tests switch DAANIO_HOME. Fall
