...
impl Config {
    ...
    pub fn create_default_config_file() -> anyhow::Result<PathBuf> {
        ...
        let default_content = r#"# daanio configuration file
# Location: ~/.daanio/config.toml
#
# Environment variables override these settings.
# Run `/config` in daanio to see current settings.
...
# Spawn a fresh daanio session in a new terminal window, reusing the current
# session's working directory. Companion to the system-wide launch hotkeys.
# `daanio setup-hotkey` installs these three global launch hotkeys on macOS,
...
#   Cmd+;        new daanio in your home directory
#   Cmd+'        new daanio in your last project directory
#   Cmd+Shift+'  new daanio self-dev session (last daanio repo)
...
# How to apply the transcript inside daanio: insert|append|replace|send
mode = "send"
...
# bing_api_key_env = "DAANIO_BING_API_KEY"
# bing_api_key = ""
...
# this. Configure here or via the DAANIO_SEARXNG_URL environment variable, then
...
# extended/full additionally emit ignorable _daanio/* extension notifications.
profile = "standard"
# Tool profile requested when `daanio acp` starts the daemon itself.
...
#   claude        = Claude via OAuth/subscription (token in ~/.daanio/auth.json)
#   anthropic-api = Claude via direct Anthropic API key (ANTHROPIC_API_KEY env
#                   or ~/.config/daanio/anthropic.env). API-key mode does NOT fall
...
# to match this value. Also overridable per-launch via DAANIO_STREAM_IDLE_TIMEOUT_SECS.
...
# Env override: DAANIO_SWARM_MODEL
# swarm_model = "inherit"
#
...
# Env override: DAANIO_SWARM_SPAWN_MODE
...
# Env override: DAANIO_SWARM_MAX_CONCURRENT_AGENTS
...
# Env override: DAANIO_SWARM_STRIP_LAYOUT
...
# Env override: DAANIO_MEMORY_MODEL
...
# dormant instead of degrading to the no-LLM path. Env: DAANIO_MEMORY_SIDECAR_ENABLED
...
# Env override: DAANIO_MEMORY_EMBEDDING_BACKEND
...
# Set DAANIO_TERMINAL to force a supported terminal emulator instead.
...
#
# When set, daanio runs `<spawn_hook> <daanio-binary> <args...>` instead of
# opening a terminal emulator itself. The hook receives DAANIO_SPAWN_* env vars
# describing the spawn so multiplexers/wrappers can decide where it appears:
#   DAANIO_SPAWN_KIND        - "swarm-agent", "resume", "selfdev", "restart", ...
#   DAANIO_SPAWN_SESSION_ID  - session the window will run
#   DAANIO_SPAWN_TITLE       - suggested window/tab title
#   DAANIO_SPAWN_CWD         - session working directory (also the hook's cwd)
#   DAANIO_SPAWN_PROGRAM     - daanio binary path
#   DAANIO_SPAWN_COMMAND     - full shell-escaped command line
#   DAANIO_SPAWN_SWARM_ID / DAANIO_SPAWN_COORDINATOR_SESSION_ID (swarm spawns)
# If the hook fails to start, daanio falls back to built-in terminal detection.
# Env override: DAANIO_SPAWN_HOOK (set empty to disable a config hook).
#
# Examples:
#   spawn_hook = "tmux new-window"                # tmux window per agent
#   spawn_hook = "kitty @ launch --type=tab --"   # kitty tab per agent
#   spawn_hook = "~/bin/daanio-spawn-router"       # custom placement script
# spawn_hook = ""
#
# External command used to focus/raise an existing session window, replacing
# the built-in wmctrl/xdotool title search. Receives DAANIO_FOCUS_SESSION_ID
# and DAANIO_FOCUS_TITLE env vars. Pair with spawn_hook so the program that
# placed the window also brings it to the front.
# Env override: DAANIO_FOCUS_HOOK (set empty to disable a config hook).
#
# Example:
#   focus_hook = "~/bin/daanio-focus-router"
# focus_hook = ""
#
# macOS only: terminal that the Cmd+; launch hotkey and in-app session spawns
# open daanio into. One of: ghostty, iterm2, wezterm, warp, alacritty, vscode,
# terminal (Apple Terminal). Preferred over the legacy
# ~/.daanio/preferred_terminal.json file. After changing this, re-run
# `daanio setup-hotkey` so the generated launcher script (Cmd+;) picks it up.
# preferred = "ghostty"
...
[hooks]
# Lifecycle hooks: external commands daanio runs at well-defined points so other
# programs can observe or gate agent behavior. Commands are parsed shell-style
# (quotes work) but executed directly, with DAANIO_HOOK_* env vars describing
# the event:
#   DAANIO_HOOK_EVENT       - "turn_start", "turn_end", "session_start",
#                            "session_end", "pre_tool", "post_tool"
#   DAANIO_HOOK_SESSION_ID  - the session the event belongs to
#   DAANIO_HOOK_CWD         - session working directory (also the hook's cwd)
#   DAANIO_HOOK_PAYLOAD     - JSON mirror of all fields
# Hook processes get DAANIO_HOOKS_DISABLED=1 so nested daanio calls don't recurse.
#
# All hooks except pre_tool are observers: detached, fire-and-forget, failures
# only logged. Env overrides: DAANIO_HOOK_TURN_START, DAANIO_HOOK_TURN_END,
# DAANIO_HOOK_SESSION_START, DAANIO_HOOK_SESSION_END, DAANIO_HOOK_PRE_TOOL,
# DAANIO_HOOK_POST_TOOL (set empty to disable a config hook).
#
...
# think/stream window before any tool call. Extra fields: DAANIO_HOOK_MODEL,
# DAANIO_HOOK_SOURCE ("chat"/"resume"/"ambient").
# turn_start = "~/bin/daanio-turn-start"
#
# Runs when an agent turn completes. Extra fields: DAANIO_HOOK_STATUS
# ("ok"/"error"), DAANIO_HOOK_DURATION_MS, DAANIO_HOOK_MODEL,
# DAANIO_HOOK_LAST_ASSISTANT_TEXT (first 4000 chars), DAANIO_HOOK_ERROR.
# turn_end = "~/bin/daanio-turn-notify"
#
# Runs when a session becomes active. Extra: DAANIO_HOOK_SOURCE
# ("create"/"attach"/"resume").
# session_start = ""
#
# Runs when a session closes normally. Extra: DAANIO_HOOK_SOURCE ("close").
# session_end = ""
#
# Gate hook before every tool call. Receives DAANIO_HOOK_TOOL_NAME and the tool
# input JSON on stdin (truncated copy in DAANIO_HOOK_TOOL_INPUT). Exit 0 allows
...
# pre_tool = "~/bin/daanio-tool-policy"
#
# Max milliseconds to wait for pre_tool before failing open (default: 5000).
# pre_tool_timeout_ms = 5000
#
# Runs after each tool call. Extra fields: DAANIO_HOOK_TOOL_NAME,
# DAANIO_HOOK_STATUS, DAANIO_HOOK_DURATION_MS, DAANIO_HOOK_OUTPUT_BYTES,
# DAANIO_HOOK_ERROR.
# post_tool = ""
...
[power]
# Prevent automatic system sleep while any daanio session is actively working.
...
# Set DAANIO_DISABLE_POWER_INHIBIT=1 to force-disable regardless of this setting.
...
# ntfy_topic = "daanio-ambient-your-secret-topic"
...
# email_from = "daanio@example.com"
# email_smtp_host = "smtp.gmail.com"
# email_smtp_port = 587
# Password via env: DAANIO_SMTP_PASSWORD (preferred) or config below
# email_password = ""
...
# telegram_bot_token = ""  # From @BotFather (prefer DAANIO_TELEGRAM_BOT_TOKEN env var)
...
# discord_bot_token = ""     # From Discord Developer Portal (prefer DAANIO_DISCORD_BOT_TOKEN env var)
...
# DAANIO_JADE_RELAY_API_BASE, DAANIO_JADE_RELAY_TOKEN, DAANIO_JADE_RELAY_TOKEN_ID,
# DAANIO_JADE_RELAY_USER_ID, DAANIO_JADE_RELAY_SESSION_ID.
...
# Tool partner discovery is disabled until you configure a trusted endpoint.
# enabled = false
# endpoint = "https://your-domain.example/v1/discovery"
	"#;

        // Substitute platform-specific defaults from the keybinding registry.
        let p = daanio_config_types::KeybindingPlatform::current();
        let effort_increase =
            daanio_config_types::default_binding("effort_increase", p).unwrap_or("alt+right");
        let effort_decrease =
            daanio_config_types::default_binding("effort_decrease", p).unwrap_or("alt+left");
