...
const LEDGER_ENV: &str = "DAANIO_LIVE_TEST_LEDGER";
const COVERAGE_ENV: &str = "DAANIO_LIVE_TEST_COVERAGE";
...
pub mod checkpoints {
    ...
    pub const REAL_DAANIO_TOOL_SMOKE: &str = "real_daanio_tool_smoke";
...
const END_TO_END_CHECKPOINTS: &[LiveVerificationCheckpointDefinition] = &[
    ...
    LiveVerificationCheckpointDefinition {
        ...
        required_for_user_ready: true,
        spends_balance: false,
        description: "Daanio can load the credential from the expected env/config source and records only a fingerprint.",
    },
    LiveVerificationCheckpointDefinition {
    ...
    LiveVerificationCheckpointDefinition {
        ...
        required_for_user_ready: true,
        spends_balance: true,
        description: "A full Daanio turn executes a harmless local tool requested by the model.",
    },
    LiveVerificationCheckpointDefinition {
    ...
    LiveVerificationCheckpointDefinition {
        id: checkpoints::REAL_DAANIO_TOOL_SMOKE,
        label: "Real Daanio tool smoke",
        category: "tools",
        required_for_user_ready: true,
        spends_balance: true,
        description: "A normal Daanio agent turn uses the real streamed parser, advertised tool schema, registry execution, tool-result followup, and transcript validation without malformed tool calls.",
    },
    LiveVerificationCheckpointDefinition {
...
pub struct LiveVerificationBuild {
    pub daanio_version: String,
    pub daanio_git_hash: String,
    pub daanio_git_date: String,
    pub daanio_git_dirty: bool,
    pub daanio_semver: String,
    pub os: String,
    pub arch: String,
    pub pid: u32,
}
...
impl LiveVerificationBuild {
    pub fn current() -> Self {
        let version = daanio_build_meta::version().to_string();
        Self {
            daanio_git_dirty: version.contains("dirty"),
            daanio_version: version,
            daanio_git_hash: daanio_build_meta::git_hash().to_string(),
            daanio_git_date: daanio_build_meta::git_date().to_string(),
            daanio_semver: daanio_build_meta::semver().to_string(),
...
pub struct LiveProviderModelCoveragePair {
    ...
    /// daanio version string that produced the most recent entry for this pair.
    #[serde(default)]
    pub latest_daanio_version: String,
...
pub struct LiveProviderModelCoverageSummary {
    ...
    /// Full monitoring roster: every provider daanio knows about (OpenAI-compatible
...
pub struct ProviderMonitorEntry {
    ...
    /// True when `daanio provider-doctor <id>` can drive this provider today
...
fn update_coverage(event: &LiveVerificationEvent, path: &Path) -> Result<()> {
    ...
    coverage.latest.insert(
        ...
        LiveVerificationCoverageEntry {
            ...
            daanio_version: event.build.daanio_version.clone(),
            daanio_git_hash: event.build.daanio_git_hash.clone(),
            daanio_git_dirty: event.build.daanio_git_dirty,
...
pub fn format_provider_test_coverage_report(
    ...
    out.push_str("Developer/live verification evidence recorded by daanio. This is evidence, not a guarantee of future provider availability.\n\n");
    ...
    let (coverage, path) = match load_coverage(coverage_path) {
        ...
        Err(err) => {
            ...
            out.push_str("Once daanio ships a curated developer coverage snapshot, this command should prefer that snapshot and separately show local evidence.\n\n");
            out.push_str(&format!("Ledger error: {}\n\n", err));
            out.push_str("You can generate local evidence with:\n\n");
            out.push_str(&format!(
                "  daanio auth-test --provider {} --model {}",
    ...
    let Some(entry) = matches.last() else {
        out.push_str("Status: Not yet covered by this daanio verification ledger\n\n");
        out.push_str(&format!("Ledger: {}\n\n", path.display()));
        out.push_str("This does not mean the provider/model is broken. It only means daanio has no recorded live verification evidence for this exact provider/model pair.\n");
        return out;
    };
    ...
    out.push_str(&format!(
        ...
        coverage_actor_label(entry.daanio_git_dirty, &entry.daanio_version),
    ));
    ...
    out.push_str(&format!(
        "Tested with: daanio {} ({}){}\n\n",
        entry.daanio_version,
        entry.daanio_git_hash,
        if entry.daanio_git_dirty {
            ", dirty"
        } else {
            ""
        }
    ...
    out.push_str("\n## What this means\n\n");
    out.push_str("These checks exercise real daanio runtime paths, including basic chat and tool-use smoke tests when present. Missing evidence should be read as 'not yet recorded', not as a failure.\n");
    out
}
...
fn provider_test_coverage_checkpoint_label(checkpoint: &str) -> String {
    match checkpoint {
        ...
        checkpoints::REAL_DAANIO_TOOL_SMOKE => "Real daanio tool smoke".to_string(),
...
struct ProviderModelCoverageBuilder {
    ...
    latest_daanio_version: String,
    latest_daanio_dirty: bool,
    entries: usize,
...
impl ProviderModelCoverageBuilder {
    fn record(&mut self, entry: &LiveVerificationCoverageEntry) {
            ...
            .unwrap_or(true)
        {
            ...
            self.latest_daanio_version = entry.daanio_version.clone();
            self.latest_daanio_dirty = entry.daanio_git_dirty;
        }
    ...
    fn finish(self) -> LiveProviderModelCoveragePair {
        ...
        LiveProviderModelCoveragePair {
            ...
            latest_daanio_version: self.latest_daanio_version,
            latest_daanio_dirty: self.latest_daanio_dirty,
...
pub fn strict_live_provider_model_coverage_summary(
        ...
        .map(ProviderModelCoverageBuilder::finish)
    {
        ...
        match pair.checkpoint_status(checkpoints::REAL_DAANIO_TOOL_SMOKE) {
...
const STRICT_PIPELINE_STAGES: &[(&str, &str)] = &[
    ...
    (checkpoints::REAL_DAANIO_TOOL_SMOKE, "real tool smoke"),
];
...
fn doctor_tier_for_stage(stage_id: &str) -> &'static str {
    match stage_id {
        ...
        | checkpoints::TOOL_EXECUTION_LOOP
        | checkpoints::TOOL_RESULT_FOLLOWUP
        | checkpoints::REAL_DAANIO_TOOL_SMOKE => "full",
...
/// the report. Lets `provider-test-coverage` enumerate *every* provider daanio
...
pub fn format_strict_live_provider_model_coverage_summary(
    ...
    out.push_str("A pair is READY only after every stage below passes in a real Daanio runtime.\n");
    ...
    if all_pairs.is_empty() {
        ...
        out.push_str("`daanio provider-doctor <provider> --tier full` to record one.\n\n");
    } else {
        ...
        for pair in &shown_pairs {
            ...
            let tested = format!(
                ...
                coverage_actor_label(pair.latest_daanio_dirty, &pair.latest_daanio_version),
            );
    ...
    // -- Full provider monitoring roster: EVERY provider daanio knows about. ----
    ...
    out.push_str("    daanio provider-doctor <provider> --tier full   # spends balance\n");
    out.push_str(
        "    daanio provider-doctor <provider> --tier offline # wiring only, no key/spend\n",
    );
...
fn pair_fix_hint(provider_id: &str, model: &str, stage_id: &str) -> String {
    if doctor_supports_provider(provider_id) {
        ...
        format!("run `daanio provider-doctor {provider_id} --model {model} --tier {tier}`")
    } else {
...
mod tests {
    ...
    fn coverage_entry(
        ...
        LiveVerificationCoverageEntry {
            ...
            daanio_version: "test".to_string(),
            daanio_git_hash: "test".to_string(),
            daanio_git_dirty: false,
            user_ready: false,
