...
pub struct McpServerConfig {
    /// Command for stdio servers. Empty for HTTP/SSE servers, which daanio does
    ...
    /// URL for HTTP/SSE servers (Claude Code compat). Unused by daanio today.
...
impl McpServerConfig {
    /// daanio currently only supports stdio (command-based) MCP servers. A config
...
pub struct McpConfig {
    ...
    /// daanio's historical `servers` key.
...
impl McpConfig {
    ...
    fn import_from_external() {
        ...
        if !imported.servers.is_empty() {
            if let Err(e) = imported.save_to_file(&daanio_mcp) {
    ...
    /// order: `.daanio/mcp.json`, then `.mcp.json` (Claude Code project config),
    ...
    fn load_project_locals(project_root: &std::path::Path) -> Self {
        ...
        for relative in [".daanio/mcp.json", ".mcp.json", ".claude/mcp.json"] {
    ...
    /// Load from default locations (merges daanio global + local, local overrides),
    ...
    /// (`.daanio/mcp.json`, `.mcp.json`, `.claude/mcp.json`, and the per-project
    ...
    pub fn load_for_dir(project_dir: Option<&std::path::Path>) -> Self {
        ...
        // Load daanio's own global config (~/.daanio/mcp.json)
        if let Ok(daanio_dir) = crate::storage::daanio_dir() {
            let daanio_mcp = daanio_dir.join("mcp.json");
            if daanio_mcp.exists() {
                if let Ok(config) = Self::load_from_file(&daanio_mcp) {
                    merged.servers.extend(config.servers);
                }
            }
        }
        ...
        // daanio only supports stdio servers today. Drop HTTP/SSE entries (common
