...
fn test_load_project_locals_merge_order() {
    // `.daanio/mcp.json` loads first, then `.mcp.json` overrides same-named
    // servers, then `.claude/mcp.json`.
    let temp = tempfile::tempdir().expect("tempdir");
    let project = temp.path();
    std::fs::create_dir_all(project.join(".daanio")).unwrap();
    std::fs::create_dir_all(project.join(".claude")).unwrap();
    std::fs::write(
        project.join(".daanio/mcp.json"),
        r#"{"servers":{"shared-name":{"command":"daanio-bin"},"daanio-only":{"command":"a"}}}"#,
    )
    .unwrap();
    std::fs::write(
        project.join(".mcp.json"),
    ...
    assert_eq!(
        ...
        "claude-bin",
        ".mcp.json must override .daanio/mcp.json for same-named servers"
    );
    assert!(config.servers.contains_key("daanio-only"));
