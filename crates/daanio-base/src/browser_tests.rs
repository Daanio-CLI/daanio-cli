...
fn test_rewrite_command_with_full_path() {
    ...
    if browser_binary_path().exists() {
        ...
        assert!(result.contains(".daanio/browser"));
    } else {
...
fn test_paths() {
    ...
    assert!(bdir.to_string_lossy().contains(".daanio"));
...
async fn test_inspect_browser_status_without_binary() {
    // Hold the test-env lock: this reads DAANIO_HOME-derived paths, and other
    // tests mutate DAANIO_HOME (and write browser fixture files) under the
    // lock. Without it, the status snapshot and the exists() check below can
    // observe different DAANIO_HOME values mid-test.
    let _guard = crate::storage::lock_test_env();
...
async fn test_ensure_browser_ready_noninteractive_without_binary() {
    ...
    // that mutate DAANIO_HOME under the test-env lock.
    let _guard = crate::storage::lock_test_env();
