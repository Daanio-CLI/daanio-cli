...
fn test_skill_prompt_integration() {
    ...
    assert!(prompt.contains("Your name is Daanio."));
...
fn test_session_context_includes_time_timezone_and_system_info() {
    ...
    assert!(context.contains("Daanio version: "));
...
#[test]
fn test_prompt_overlay_files_are_loaded_from_project_and_global_daanio_dirs() {
    let _guard = crate::storage::lock_test_env();
    let prev_home = std::env::var_os("DAANIO_HOME");
    let temp = tempfile::TempDir::new().unwrap();
    crate::env::set_var("DAANIO_HOME", temp.path());
    ...
    let project_dir = tempfile::TempDir::new().unwrap();
    std::fs::create_dir_all(project_dir.path().join(".daanio")).unwrap();
    std::fs::write(
        project_dir.path().join(".daanio/prompt-overlay.md"),
...
#[test]
fn test_preferred_tools_files_are_loaded_from_project_and_global_daanio_dirs() {
    let _guard = crate::storage::lock_test_env();
    let prev_home = std::env::var_os("DAANIO_HOME");
    let temp = tempfile::TempDir::new().unwrap();
    crate::env::set_var("DAANIO_HOME", temp.path());
    ...
    std::fs::create_dir_all(project_dir.path().join(".daanio")).unwrap();
    std::fs::write(
        project_dir.path().join(".daanio/preferred-tools.md"),
    ...
    let direct_content = direct.0.unwrap();
    assert!(
        direct_content.contains("Project Preferred Tools (.daanio/preferred-tools.md)"),
    ...
    assert!(
        direct_content.contains("Global Preferred Tools (~/.daanio/preferred-tools.md)"),
...
fn test_swarm_prompt_prefers_project_then_global_then_default() {
    ...
    std::fs::create_dir_all(project_dir.path().join(".daanio")).unwrap();
    std::fs::write(
        project_dir.path().join(".daanio/swarm-prompt.md"),
    ...
    std::fs::write(project_dir.path().join(".daanio/swarm-prompt.md"), "   \n").unwrap();
    let prompt = load_swarm_prompt(Some(project_dir.path()));
    assert_eq!(prompt, "global swarm routing");
...
fn test_non_selfdev_prompt_leaves_selfdev_guidance_to_the_tool_schema() {
    ...
    assert!(!prompt.contains("You are working on the daanio codebase itself."));
}
...
fn test_selfdev_prompt_uses_full_selfdev_instructions() {
    ...
    assert!(prompt.contains("You are working on the daanio codebase itself."));
    assert!(prompt.contains("launched from the TUI/root daanio context"));
...
fn test_selfdev_prompt_uses_desktop_focus_for_desktop_working_dir() {
    let desktop_dir = std::path::Path::new("/tmp/daanio/crates/daanio-desktop/src");
...
fn test_split_selfdev_prompt_defaults_to_tui_focus_for_repo_root() {
    let repo_dir = std::path::Path::new("/tmp/daanio");
    let (split, _info) = build_system_prompt_split(None, &[], true, None, Some(repo_dir));
    assert!(
        split
            .static_part
            .contains("launched from the TUI/root daanio context")
    );
...
fn test_selfdev_prompt_prefers_publish_flow_for_active_builds() {
    ...
    assert!(prompt.contains("scripts/dev_cargo.sh build --profile selfdev -p daanio --bin daanio"));
