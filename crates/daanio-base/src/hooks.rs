...
//!
//! Hooks are external commands that daanio runs at well-defined lifecycle
//! points so other programs can observe or gate agent behavior without
//! forking daanio. They are configured in `[hooks]` in config.toml (or
//! `DAANIO_HOOK_*` env vars) and follow the same command-line conventions as
//! `[terminal] spawn_hook`: the command is parsed shell-style but executed
//! directly (no shell), with `DAANIO_HOOK_*` metadata env vars describing the
//! event.
//!
//! Two dispatch styles:
//!
...
//! - **Gate** (`pre_tool`): daanio waits (with a timeout) for the hook to
...
//!
//! Hook processes get `DAANIO_HOOKS_DISABLED=1` in their environment so a
//! hook that itself invokes daanio does not recursively trigger hooks.

use std::path::PathBuf;

/// Maximum bytes of JSON payload exported via `DAANIO_HOOK_PAYLOAD`.
...
pub struct HookEvent {
    ...
    /// `DAANIO_HOOK_STATUS=ok` and `"status": "ok"` in the JSON payload.
...
fn hooks_suppressed() -> bool {
    std::env::var_os("DAANIO_HOOKS_DISABLED").is_some()
}
...
/// JSON payload mirroring the env fields, exported as `DAANIO_HOOK_PAYLOAD`.
...
fn apply_event_env(cmd: &mut std::process::Command, event: &HookEvent) {
    cmd.env("DAANIO_HOOKS_DISABLED", "1");
    cmd.env("DAANIO_HOOK_EVENT", event.event);
    if let Some(session_id) = &event.session_id {
        cmd.env("DAANIO_HOOK_SESSION_ID", session_id);
    }
    if let Some(cwd) = &event.cwd {
        cmd.env("DAANIO_HOOK_CWD", cwd);
    }
    for (key, value) in &event.fields {
        cmd.env(format!("DAANIO_HOOK_{key}"), value);
    }
    cmd.env("DAANIO_HOOK_PAYLOAD", payload_json(event));
}
...
/// stdin (and truncated in `DAANIO_HOOK_TOOL_INPUT`). Contract:
///
...
mod tests {
    ...
    fn gate_test_config(hook: &str, timeout_ms: u64) -> impl Drop + use<> {
        ...
        let reset = EnvReset(vec![
            (
                ...
                std::env::var_os("DAANIO_HOOK_PRE_TOOL"),
            ),
            (
                "DAANIO_HOOK_PRE_TOOL_TIMEOUT_MS",
                std::env::var_os("DAANIO_HOOK_PRE_TOOL_TIMEOUT_MS"),
            ),
        ]);
        crate::env::set_var("DAANIO_HOOK_PRE_TOOL", hook);
        crate::env::set_var("DAANIO_HOOK_PRE_TOOL_TIMEOUT_MS", timeout_ms.to_string());
        reset
    }
    ...
    async fn pre_tool_gate_allows_on_exit_zero_and_blocks_on_exit_two() {
        ...
        let block = write_executable_script(
            temp.path(),
            "block.sh",
            "#!/bin/sh\ncat > /dev/null\necho \"dangerous tool: $DAANIO_HOOK_TOOL_NAME\" >&2\nexit 2\n",
        );
        {
    ...
    fn observer_dispatch_runs_hook_with_event_env() {
        ...
        let script = write_executable_script(
            ...
            "observe.sh",
            &format!(
                "#!/bin/sh\nprintf '%s|%s|%s|%s' \"$DAANIO_HOOK_EVENT\" \"$DAANIO_HOOK_SESSION_ID\" \"$DAANIO_HOOK_STATUS\" \"$DAANIO_HOOKS_DISABLED\" > {}\n",
                crate::terminal_launch::sh_escape(&record.to_string_lossy())
            ),
        );

        let prev = std::env::var_os("DAANIO_HOOK_TURN_END");
        crate::env::set_var("DAANIO_HOOK_TURN_END", script.to_string_lossy().to_string());
        ...
        match prev {
            Some(value) => crate::env::set_var("DAANIO_HOOK_TURN_END", value),
            None => crate::env::remove_var("DAANIO_HOOK_TURN_END"),
        }
