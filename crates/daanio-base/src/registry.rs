...
//!
//! Tracks running servers in `~/.daanio/servers.json` for discovery by clients.
...
pub fn registry_path() -> Result<PathBuf> {
    Ok(daanio_dir()?.join("servers.json"))
}

/// Get the socket directory path
pub fn socket_dir() -> Result<PathBuf> {
    Ok(crate::storage::runtime_dir().join("daanio"))
}
...
pub fn server_socket_path(name: &str) -> PathBuf {
    socket_dir()
        ...
        .unwrap_or_else(|_| std::env::temp_dir().join(format!("daanio-{}.sock", name)))
}
...
pub fn server_debug_socket_path(name: &str) -> PathBuf {
    socket_dir()
        ...
        .unwrap_or_else(|_| std::env::temp_dir().join(format!("daanio-{}-debug.sock", name)))
}
