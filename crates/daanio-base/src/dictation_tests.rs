#[cfg(target_os = "linux")]
use super::focused_daanio_session;
use super::{
...
fn select_candidate_prefers_title_match() {
    ...
    let selected = select_candidate(&candidates, Some("🦀 daanio/sleeping Crab [self-dev]"))
...
#[test]
fn extract_session_short_name_from_daanio_window_title() {
    assert_eq!(
        extract_session_short_name_from_window_title("🦢 daanio/cliff Swan [self-dev]"),
        Some("swan".to_string())
    );
    assert_eq!(
        extract_session_short_name_from_window_title("🦊 daanio Fox"),
        Some("fox".to_string())
    );
}
...
fn remember_and_read_last_focused_session() {
    ...
    let prev = std::env::var_os("DAANIO_HOME");
...
#[cfg(target_os = "linux")]
#[test]
fn focused_daanio_session_uses_niri_window_title_when_process_name_is_generic() {
    let _guard = crate::storage::lock_test_env();
    let temp = tempfile::TempDir::new().expect("tempdir");
    ...
    let focused_process = ChildGuard::spawn_named("daanio");
    let bin_dir = temp.path().join("bin");
    install_fake_niri(
        &bin_dir,
        focused_process.pid(),
        "🦢 daanio/cliff Swan [self-dev]",
    );
    ...
    assert_eq!(
        focused_daanio_session().expect("resolve focused session"),
