...
use daanio_session_types::SessionStatus;
use ratatui::{
    Frame,
...
pub use daanio_tui_session_picker::{
...
pub enum PickerResult {
    ...
    /// to Daanio. This is never emitted by ordinary Enter/resume behavior.
...
impl SessionPicker {
    ...
    fn ensure_selected_preview_loading(&mut self) {
        ...
        let _ = std::thread::Builder::new()
            .name("daanio-session-preview-loader".to_string())
            .spawn(move || {
        ...
        let body = vec![
            Line::from(Span::styled(
                format!(
                    ...
                    daanio_core::util::truncate_str(session_id, 12)
                ),
                Style::default()
                    .fg(Color::White)
                    .add_modifier(Modifier::BOLD),
            ...
            Line::from(""),
            Line::from(Span::styled(
                "Daanio will prepare the transcript first, then ask Claude to exit.",
    ...
    pub fn run(mut self) -> Result<Option<PickerResult>> {
        ...
        let result = loop {
            ...
            terminal.draw(|frame| {
                ...
                daanio_tui_style::adapt_buffer_for_theme(frame.buffer_mut());
            })?;
