        ...
        if let Ok(handle) = tokio::runtime::Handle::try_current() {
            ...
            handle.spawn(async move {
                if provider_clone.prefetch_models().await.is_ok() {
                    // The picker may have opened while the authenticated Daanio
                    // catalog was still loading. Invalidate any startup snapshot
                    // so the next `/model` render uses the live routes.
    ...
    fn restore_remote_startup_history(&mut self, session_id: &str) {
        ...
        // message buffers) drop before we strip and retain the session.
        {
            ...
            let display_messages =
                daanio_tui_messages::display_messages_from_rendered_messages(rendered_messages);
