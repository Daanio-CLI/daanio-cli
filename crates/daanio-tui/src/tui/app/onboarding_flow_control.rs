...
impl App {
    ...
    pub(super) fn maybe_begin_onboarding_flow_on_startup(&mut self) {
        ...
        // spawned by developers (e.g. the niri `daanio self-dev` hotkey) and that
    ...
    /// only counts interactive `daanio` launches (TTY-gated) and can be reset
    ...
    pub(super) fn is_new_user_install() -> bool {
        let Ok(dir) = crate::storage::daanio_dir() else {
            return true;
        };
    ...
    fn has_established_native_session_history(daanio_dir: &std::path::Path) -> bool {
        const ESTABLISHED_SESSION_THRESHOLD: usize = 10;
        let Ok(entries) = std::fs::read_dir(daanio_dir.join("sessions")) else {
            return false;
        };
    ...
    ///
    /// These are launched by developers working on daanio itself (for example the
    /// niri `daanio self-dev` hotkey). That launch path bypasses
    ...
    ///
    /// Public Daanio builds never inspect or import upstream-provider
    /// credentials. They offer only Daanio browser sign-in. Internal builds
    /// may retain the legacy import walkthrough for development.
    ///
    ...
    pub(super) fn begin_onboarding_flow_at_login(&mut self) {
        ...
        let first_party_only = std::env::var_os("DAANIO_FIRST_PARTY_ONLY").is_some();
        let import = if first_party_only {
            None
        } else {
    ...
    /// Start the default first-run login using the Daanio gateway.
    ...
    /// Refresh the status notice for the Daanio login prompt.
    ...
    fn onboarding_finish_import_review(&mut self) {
        ...
        // target the agent repair brief at the right `daanio auth-test --provider`.
    ...
    pub(super) fn onboarding_prepare_recent_project_review(&mut self) -> bool {
        let Some(repository) = self.onboarding_recent_project_path() else {
            self.onboarding_show_suggestions();
            self.set_status_notice(
                "No recent Git repository found. Start daanio inside a project to review it.",
            );
            return false;
        };
        self.onboarding_finish();
    ...
    /// 1. A daanio login that exists but expired (they clearly use it).
    ...
    fn onboarding_other_provider_rows(skip: Option<&str>) -> (Vec<String>, Vec<String>) {
        ...
        let providers: [(&str, &str, AuthState); 8] = [
            ...
            ("Daanio subscription", "daanio", status.daanio),
    ...
    /// commands the agent can run (`daanio auth-test --provider daanio` and
    /// `daanio login --provider daanio`), copy it to the clipboard, and
