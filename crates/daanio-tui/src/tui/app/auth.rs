...
impl App {
    ...
    fn record_oauth_preflight(
        ...
        if matches!(callback_available, Some(false)) {
            ...
            if let Some(target) = callback_target {
                notices.push(format!(
                    "Local callback target {} is unavailable, so daanio is using manual-safe paste completion instead.",
                    target
                ));
            } else {
                notices.push(
                    "The local callback listener is unavailable, so daanio is using manual-safe paste completion instead."
                        .to_string(),
                );
            }
        }
        if !notices.is_empty() {
            notices.push(format!(
                "If login still fails, run daanio auth doctor {} for a guided diagnosis.",
                provider_id
            ));
        }
        notices.join("\n")
    ...
    pub(super) fn show_daanio_subscription_status(&mut self) {
        ...
        for model in crate::subscription_catalog::curated_models() {
            ...
            let tier_suffix = if model.min_tier == crate::subscription_catalog::DaanioTier::Plus {
                String::new()
            } else {
        ...
        message.push_str("\nTiers\n\n");
        for tier in crate::subscription_catalog::DaanioTier::ALL.iter().copied() {
            message.push_str(&format!(
        ...
        if configured_key {
            ...
            message.push_str("\nLog in with /login daanio to see account usage and tier.");
        }
                    ...
                    match crate::subscription_api::fetch_subscription_me().await {
                        ...
                        Err(error) => {
                                ...
                                == Some(&crate::subscription_api::AccountApiError::Unauthorized)
                            {
                                ...
                                "Daanio Account Status\n\nThe saved account key was revoked or expired. Local credentials were cleared. Use /account daanio login to sign in again."
                                    .to_string()
                            } else {
                                format!(
                                    "Daanio Account Status\n\nCould not load /v1/me: {}\n\nThe local credential was retained. Retry /account daanio status, open /account daanio manage, or use /account daanio logout.",
                                    error
                                )
                            };
                            crate::bus::Bus::global().publish(crate::bus::BusEvent::UiActivity(
                                crate::bus::UiActivity::background(
                                    Some(session_id),
                                    message,
                                    Some("Daanio account status unavailable"),
                                ),
                            ));
                        }
                    }
    ...
    pub(super) fn show_auth_status(&mut self) {
        ...
        message.push_str(
            "\nUse /login daanio to sign in securely through daanio.com. Upstream provider credentials are managed server-side by the Daanio gateway and are never requested here.",
        );
        self.push_display_message(DisplayMessage::system(message));
    }
    ...
    pub(super) fn start_logout_provider(
        ...
        if matches!(provider.target, LoginProviderTarget::Daanio) {
            self.start_daanio_account_logout();
            return;
        }

        let result: anyhow::Result<String> = (|| match provider.target {
            LoginProviderTarget::Daanio => unreachable!("handled above"),
    ...
    pub(super) fn start_logout_all(&mut self) {
        ...
        Self::clear_api_key_logout_summary(
            &mut summary,
            &mut errors,
            "daanio subscription API key",
            crate::subscription_catalog::DAANIO_API_KEY_ENV,
            crate::subscription_catalog::DAANIO_ENV_FILE,
        );
        for env_key in [
            crate::subscription_catalog::DAANIO_API_BASE_ENV,
            crate::subscription_catalog::DAANIO_ACCOUNT_ID_ENV,
            crate::subscription_catalog::DAANIO_ACCOUNT_EMAIL_ENV,
            crate::subscription_catalog::DAANIO_TIER_ENV,
        ] {
            if let Err(err) = crate::provider_catalog::save_env_value_to_env_file(
                ...
                errors.push(format!("daanio subscription {}: {}", env_key, err));
            }
        }
    ...
    pub(super) fn start_login_provider(
        ...
        if std::env::var_os("DAANIO_FIRST_PARTY_ONLY").is_some() && provider.id != "daanio" {
            self.push_display_message(DisplayMessage::error(
                "Daanio CLI supports only secure Daanio browser sign-in. Upstream provider credentials are managed by the Daanio gateway and cannot be added here."
                    .to_string(),
            ));
            self.set_status_notice("Login blocked: use /login daanio");
            return;
        }
        ...
        match provider.target {
            ...
            crate::provider_catalog::LoginProviderTarget::Daanio => self.start_daanio_login(),
            ...
            crate::provider_catalog::LoginProviderTarget::Google => {
                ...
                self.push_display_message(DisplayMessage::error(
                    "Google/Gmail login is only available from the CLI right now. Run daanio login --provider google."
                        .to_string(),
                ));
            }
        }
    ...
    fn start_daanio_login(&mut self) {
        self.push_display_message(DisplayMessage::system(
            "Daanio Account Login\n\nRequesting a secure browser approval flow. No email or API key will be requested in the terminal."
                .to_string(),
        ));
        self.set_status_notice("Daanio account: requesting browser approval");
        let session_id = self.session.id.clone();
        let Ok(handle) = tokio::runtime::Handle::try_current() else {
            self.push_display_message(DisplayMessage::error(
                "Daanio account login requires the async runtime.".to_string(),
            ));
            return;
        };
        handle.spawn(async move {
            use crate::subscription_api::{AccountApiError, PollingBackoff, TokenPollOutcome};
            ...
            .await
            {
                ...
                Err(error) => {
                    publish(
                        format!(
                            "Daanio Account Login\n\nCould not start browser approval: {}\n\nRetry /account daanio login. No credential was saved.",
                            error
                        ),
                        "Daanio account login failed",
                    );
                    return;
                }
            };
                ...
                loop {
                    ...
                    .await
                    {
                        ...
                        Ok(TokenPollOutcome::Denied) => break Err(
                            "Daanio account login was canceled or denied in the browser."
                                .to_string(),
                        ),
            ...
            let approved = match approved {
                ...
                Err(error) => {
                    publish(
                        format!("Daanio Account Login\n\n{error}\n\nRetry /account daanio login."),
                        "Daanio account login stopped",
                    );
                    return;
                }
            };
            ...
            if let Err(error) = crate::subscription_catalog::persist_account_credentials(
                ...
                publish(
                    ...
                    "Daanio account credential save failed",
                );
                return;
            }
            crate::auth::AuthStatus::invalidate_cache();
            let (success, message, status) = match crate::subscription_api::fetch_subscription_me_with(
                &client,
                &api_base,
                &approved.api_key,
            )
            ...
            {
                Ok(me) => {
                    let _ = crate::subscription_catalog::persist_account_credentials(
                        &approved.api_key,
                        Some(&me.account_id),
                        Some(&me.email),
                        Some(&me.tier),
                    ...
                    (
                        true,
                        format!(
                        "Daanio Account Ready\n\nSigned in as {} ({}; status: {}).\n\nStatus: /account daanio status\nManage: /account daanio manage\nLogout: /account daanio logout",
                        me.email,
                        me.parsed_tier()
                            .map(|tier| tier.display_name().to_string())
                            .unwrap_or(me.tier),
                ...
                Err(AccountApiError::Unauthorized) => {
                    ...
                    (
                        false,
                        "Daanio Account Login\n\nThe newly issued credential was rejected by /v1/me. Local credentials were cleared; start a new login."
                            .to_string(),
                        "Daanio login rejected",
                    )
                }
                Err(error) => (
                    true,
                    format!(
                        "Daanio Account Ready\n\nSigned in as {}. Account status could not be loaded yet: {}\n\nStatus: /account daanio status\nManage: /account daanio manage\nLogout: /account daanio logout",
                        approved.email, error
                    ),
                    "Daanio login ready",
                ),
            };
            publish(message.clone(), status);
    ...
    pub(super) fn open_daanio_account_management(&mut self) {
    ...
    pub(super) fn start_daanio_account_logout(&mut self) {
        self.set_status_notice("Daanio account: logging out");
        let session_id = self.session.id.clone();
        let Ok(handle) = tokio::runtime::Handle::try_current() else {
            let result = crate::subscription_catalog::clear_account_credentials();
            match result {
                Ok(()) => self.push_display_message(DisplayMessage::system(
                    "Daanio account credentials and cache were cleared locally. Remote revocation could not be attempted without the async runtime."
                        .to_string(),
                )),
                Err(error) => self.push_display_message(DisplayMessage::error(format!(
                    "Failed to clear local Daanio account credentials: {error}"
                ))),
            }
            return;
        };
        handle.spawn(async move {
            ...
            crate::bus::Bus::global().publish(crate::bus::BusEvent::UiActivity(
                crate::bus::UiActivity::background(
                    Some(session_id),
                    message,
                    Some("Daanio account logout complete"),
                ),
            ));
        });
    }
    ...
    fn start_api_key_login(
        ...
        let provider_id = openai_compatible_profile
            ...
            .unwrap_or_else(|| match key_name {
                crate::subscription_catalog::DAANIO_API_KEY_ENV => "daanio".to_string(),
    ...
    fn start_azure_login(&mut self) {
        self.push_display_message(DisplayMessage::system(
            "Azure OpenAI Login\n\n\
             daanio uses Azure OpenAI's /openai/v1 API with either Microsoft Entra ID or an API key.\n\n\
    ...
    fn start_cursor_login(&mut self) {
        ...
        self.push_display_message(DisplayMessage::system(
            "Cursor API Key\n\n\
             ...
             daanio will save it securely and use the native Cursor HTTPS transport.\n\n\
    ...
    pub(super) fn handle_login_input(&mut self, pending: PendingLogin, input: String) {
        ...
        match pending {
            ...
            PendingLogin::ApiKeyProfile {
                ...
                // it proves the input was received and which env var/file daanio
                ...
                let save_result: anyhow::Result<()> =
                    ...
                    } else if key_name == crate::subscription_catalog::DAANIO_API_KEY_ENV {
                        (|| {
                    ...
                    Ok(()) => {
                        ...
                        if key_name == crate::provider::bedrock::API_KEY_ENV {
                            ...
                            if let Some(default_model) = default_model.as_deref() {
                                crate::env::set_var("DAANIO_BEDROCK_MODEL", default_model);
                            }
                        }
                        ...
                        {
                            format!(
                                "Use /login daanio to access curated models via your router. If the model list looks stale, run /refresh-model-list.\nDocs: {}",
                                docs_url
                            )
                        } else if let Some(resolved) = resolved_openai_compatible.as_ref() {
                            if resolved.requires_api_key {
                                "Fetching models now. Daanio will switch to an accessible model returned by the live catalog and show the catalog diff when discovery finishes. If the model list looks stale, run /refresh-model-list.".to_string()
                            } else {
                                format!(
                                    "Local endpoint configured at {}. Fetching models now; Daanio will switch to an accessible model returned by the live catalog and show the catalog diff when discovery finishes. If the model list looks stale, run /refresh-model-list.",
                                    endpoint.as_deref().unwrap_or(resolved.api_base.as_str()),
                                )
                            }
                        } else if key_name == crate::provider::bedrock::API_KEY_ENV {
                            "You can now use /model to switch to Bedrock models. TUI onboarding saved region us-east-2; for a different region, run daanio login --provider bedrock from a terminal.".to_string()
                    ...
                    Ok(()) => {
                        ...
                        Bus::global().publish(BusEvent::LoginCompleted(LoginCompleted {
                            ...
                            message: "Cursor API key saved.\n\n\
                             Stored at ~/.config/daanio/cursor.env.\n\
                             daanio will use it with the native Cursor HTTPS transport."
                                .to_string(),
                        }));
                    }
                    Err(e) => {
    ...
    pub(super) fn onboarding_should_prefer_strongest_model(&self) -> bool {
        ...
        let runtime_provider_forced =
            std::env::var("DAANIO_FORCE_PROVIDER")
                .ok()
                .is_some_and(|value| {
                    matches!(
    ...
    pub(super) fn start_openai_compatible_post_login_activation(
        ...
        crate::bus::Bus::global().publish(crate::bus::BusEvent::UiActivity(
            crate::bus::UiActivity::catalog(
                Some(self.session.id.clone()),
                format!(
                    "{} Model Discovery Started\n\nSaved credentials are active. Daanio is fetching the live model catalog, will only switch to a model returned by that catalog, and will show what changed when discovery finishes.",
                    provider_label
                ),
                        ...
                        if let Some(model) = selected {
                            ...
                            crate::bus::Bus::global().publish(crate::bus::BusEvent::UiActivity(
                                crate::bus::UiActivity::catalog(
                                    Some(session_id),
                                    format!(
                                        "{} Model Discovery Still Updating\n\nSaved credentials are active, but this local refresh pass did not find a selectable {} route yet. Daanio is still processing the auth-change catalog refresh and will switch once provider routes are available. If the model list still looks stale after the auth catalog update, run /refresh-model-list.",
                    ...
                    Err(error) => {
                        ...
                        crate::bus::Bus::global().publish(crate::bus::BusEvent::UiActivity(
                            crate::bus::UiActivity::catalog(
                                Some(session_id),
                                format!(
                                    "{} Model Discovery Still Updating\n\nSaved credentials are active, but this local refresh pass failed before the server auth-change catalog refresh finished. Daanio is still processing the auth-change catalog refresh and will switch once provider routes are available. If the model list still looks stale after the auth catalog update, run /refresh-model-list.\n\nLocal refresh error: {}",
