...
fn swarm_prompt_edit_path_prefers_nonblank_project_override() {
    ...
    let daanio_home = tempfile::tempdir().expect("daanio tempdir");
    let project_prompt = project.path().join(".daanio/swarm-prompt.md");
    ...
    std::fs::write(daanio_home.path().join("swarm-prompt.md"), "global routing")
        .expect("write global prompt");

    let path = ensure_swarm_prompt_edit_path(project.path().to_str(), daanio_home.path())
...
fn swarm_prompt_edit_path_falls_back_to_nonblank_global_override() {
    ...
    let global_prompt = daanio_home.path().join("swarm-prompt.md");
...
fn swarm_prompt_edit_path_materializes_builtin_default_globally() {
    ...
    assert_eq!(path, daanio_home.path().join("swarm-prompt.md"));
