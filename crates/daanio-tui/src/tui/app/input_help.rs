...
impl App {
    pub(super) fn command_help(&self, topic: &str) -> Option<String> {
        ...
        let help = match topic.as_str() {
            ...
            | "model-status"
            | "model status" => {
                "/provider-test-coverage\nShow daanio live verification evidence for the current provider/model.\n\n/provider-test-coverage <provider> <model>\nLook up a specific provider/model pair in the live-test coverage ledger.\n\nThe report shows last-tested time, daanio build, passed/missing checkpoints, readiness gaps, and a caveat that missing evidence is not a provider failure."
            }
            "refresh-model-list" => {
            ...
            "swarm-prompt" => {
                "/swarm-prompt\nOpen the active swarm routing prompt in $VISUAL or $EDITOR.\n\nDaanio uses a nonblank project override at ./.daanio/swarm-prompt.md when present, then ~/.daanio/swarm-prompt.md, then the built-in default. If no editable override exists, this command creates the global file from the built-in default. Restart or reload Daanio after editing because running agent tool registries cache the prompt."
            }
            "subagent" => {
            ...
            "triage" => {
                "/triage [focus]\nTriage open GitHub issues for the current repo, then autonomously fix the safe ones.\n\nThe agent lists untriaged issues with gh, classifies each (auto-fix, needs-info, needs-human, duplicate, question), applies existing labels, fixes and verifies the clear-cut bugs, and reports back with a summary table. Every public comment is clearly signed as the Daanio agent, and issues are never closed as wontfix/invalid without your confirmation.\n\nOptional focus text narrows the triage, for example /triage only crash reports."
            }
            "catchup" => {
            ...
            "log" => {
                "/log mark [note]\nWrite a distinctive DAANIO_LOG_MARK line to ~/.daanio/logs/daanio-YYYY-MM-DD.log with the current session, provider, model, working directory, and optional note. Use this to mark a spot for agents to inspect later."
            }
            "goals" => {
            ...
            "restart" => {
                "/restart\nRestart daanio with the current binary. Session is preserved.\nUseful after config changes, MCP server updates, or env var changes."
            }
            "rebuild" => {
                "/rebuild\nRun git pull --ff-only, cargo build --release, and release tests in the background. daanio stays usable and reloads automatically when the build is ready."
            }
            "selfdev" => {
            ...
            "fork" | "split" => {
                "/fork\nFork the current session into a new terminal pane or window. Clones the full conversation history so both sessions continue from the same point. Inside tmux, daanio automatically opens a right-side pane.\n\n/fork <prompt>\nFork the session and start the new pane/window by answering the prompt. The original session keeps working uninterrupted.\n\n/split\nAlias for /fork."
            }
            "resume" | "sessions" => {
            ...
            "subscription" => {
                "/subscription\nShow curated daanio subscription status for this session, including router config, runtime mode, curated models, and planned tier budget scaffolding."
            }
            "version" => "/version\nShow daanio version/build details.",
            ...
            "auth" | "login" => {
                "/auth\nShow Daanio gateway authentication status.\n\n/login\nStart secure Daanio browser sign-in.\n\n/login daanio\nOpen daanio.com for approval and save the issued gateway credential securely.\n\nUpstream provider credentials are managed server-side by Daanio and are never requested by this CLI."
            }
            "account" | "accounts" => {
                "/account\nOpen Daanio gateway account management.\n\n/account daanio status\nShow Daanio gateway status.\n\n/account daanio login\nStart secure browser sign-in at daanio.com.\n\n/account daanio logout\nRevoke and remove the saved Daanio CLI credential.\n\n/account default-model <model|clear>\nSet the preferred Daanio model.\n\nUpstream provider accounts and credentials are managed server-side by Daanio."
            }
            "save" => {
