...
const REGISTERED_COMMANDS: &[RegisteredCommand] = &[
    ...
    RegisteredCommand::public(
        "/skills",
        "Show loaded skills and daanio-endorsed recommendations",
    ),
    ...
    RegisteredCommand::public("/feedback", "Send feedback about daanio"),
    RegisteredCommand::public("/support", "Email support with diagnostics prefilled"),
    RegisteredCommand::public("/subscription", "Show daanio subscription status"),
    RegisteredCommand::public("/config", "Show or edit configuration"),
    RegisteredCommand::public("/log", "Mark the current location in the daanio logs"),
    RegisteredCommand::public(
        "/keys",
    ...
    RegisteredCommand::public("/selfdev", "Open a new self-dev daanio session"),
...
mod external_cli_suggestion_tests {
    ...
    /// Run with:
    ///   cargo test -p daanio-tui --lib onboarding_suggestion_scan_cost -- --ignored --nocapture
    #[test]
    #[ignore]
...
"#,
    ...
    fn parses_codex_input_text_blocks() {
        ...
        std::fs::write(
            &path,
            r#"{"type":"session_meta","payload":{"id":"sid","cwd":"/home/jeremy/daanio"}}
{"type":"response_item","payload":{"type":"message","role":"user","content":[{"type":"input_text","text":"check in on daanio"}]}}
"#,
        )
        .expect("write fixture");
        ...
        assert_eq!(
            ...
            Some("/home/jeremy/daanio")
        );
        assert_eq!(candidate.context.as_deref(), Some("check in on daanio"));
    }
