...
impl App {
    ...
    pub(super) fn update_cost_impl(&mut self) {
        ...
        let pinned_anthropic = daanio_provider_core::pinned_mode_for(
            daanio_provider_core::DualAuthProvider::Anthropic,
            runtime_provider.as_deref(),
        );
        let pinned_openai = daanio_provider_core::pinned_mode_for(
            daanio_provider_core::DualAuthProvider::OpenAI,
            runtime_provider.as_deref(),
        );
        let is_explicit_anthropic_api = matches!(
            pinned_anthropic,
            Some(daanio_provider_core::AuthMode::ApiKey)
        );
        let is_explicit_anthropic_oauth = matches!(
            pinned_anthropic,
            Some(daanio_provider_core::AuthMode::Oauth)
        );
        let is_explicit_openai_api =
            matches!(pinned_openai, Some(daanio_provider_core::AuthMode::ApiKey));
        let is_explicit_openai_oauth =
            matches!(pinned_openai, Some(daanio_provider_core::AuthMode::Oauth));
