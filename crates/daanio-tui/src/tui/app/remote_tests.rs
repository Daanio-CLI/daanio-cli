...
fn create_test_app() -> crate::tui::app::App {
    ...
    // from its own DAANIO_HOME (scheduled reminders read as a notification).
...
/// Point DAANIO_HOME at a per-process temp dir when the environment does not
/// already pin one, so tests never read the developer's real `~/.daanio`
...
fn ensure_test_daanio_home_if_unset() {
    ...
    let path = TEST_HOME.get_or_init(|| {
        let path = std::env::temp_dir().join(format!("daanio-test-home-{}", std::process::id()));
...
fn reload_handoff_inactive_without_flag_or_marker() {
    ...
    // `daanio.reload` left by a live self-dev reload on this machine cannot
...
fn remote_skill_invocation_with_prompt_sends_remote_turn() {
    ...
    let skill_dir = temp.path().join(".daanio/skills/remote-skill");
