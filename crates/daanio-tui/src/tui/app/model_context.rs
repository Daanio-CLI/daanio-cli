...
impl App {
    ...
    fn failover_config_hint() -> &'static str {
        "To turn this off, set [provider].cross_provider_failover = \"manual\" in ~/.daanio/config.toml or export DAANIO_CROSS_PROVIDER_FAILOVER=manual."
    }
    ...
    fn handle_provider_failover_prompt(&mut self, prompt: crate::provider::ProviderFailoverPrompt) {
            ...
            .cross_provider_failover
        {
            ...
            crate::config::CrossProviderFailoverMode::Countdown if !self.is_remote => {
                ...
                self.push_display_message(DisplayMessage::system(format!(
                    "⚠ {} became unavailable - daanio will switch to {} in 3 seconds unless you cancel.\n\nReason: {}\n\nRetrying would send {}. Press Esc to cancel.\n\n{}",
                    prompt.from_label,
                    prompt.to_label,
                    prompt.reason,
                    input_summary,
    ...
    fn current_route_api_method(&self) -> Option<String> {
        ...
        match (provider_name.as_str(), credential) {
            ("claude", Some(daanio_provider_core::ResolvedCredential::Oauth)) => {
                Some("claude-oauth".to_string())
            }
            ("claude", Some(daanio_provider_core::ResolvedCredential::ApiKey)) => {
                Some("claude-api".to_string())
            }
            ("openai", Some(daanio_provider_core::ResolvedCredential::Oauth)) => {
                Some("openai-oauth".to_string())
            }
            ("openai", Some(daanio_provider_core::ResolvedCredential::ApiKey)) => {
    ...
    fn is_guardrail_reroute_model(model: &str) -> bool {
        let canonical = daanio_provider_core::model_id::canonical(model);
        daanio_provider_core::model_id::strip_date_suffix(&canonical) == GUARDRAIL_REROUTE_MODEL
    }
    ...
    pub(super) fn effective_context_tokens_from_usage(
        ...
        // Shared heuristic (daanio-compaction-core): keeps the sidebar figure
