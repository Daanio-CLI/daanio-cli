...
/// (see `daanio_tui_markdown::reasoning_line_markup`). Trailing blank lines left
/// behind are trimmed so the remaining answer renders cleanly.
pub(super) fn strip_reasoning_lines(content: &str) -> String {
    let sentinel = daanio_tui_markdown::REASONING_SENTINEL;
...
impl App {
    ...
    /// Spawn a brand-new daanio session in a new terminal window.
    pub(crate) fn handle_new_terminal_hotkey(&mut self) {
        ...
        match super::spawn_fresh_session_in_new_terminal(&cwd) {
            ...
            Ok(false) => {
                self.set_status_notice("No supported terminal found; run `daanio` manually")
            }
...
pub(super) fn handle_modal_key(
    ...
    if app.copy_selection_mode {
            ...
            && matches!(code, KeyCode::Char('c') | KeyCode::Char('d'))
        {
            ...
            // closed daanio and lost the error (issue #497). Only fall
...
impl App {
    ...
    fn handle_key_core(
        ...
        // Accept an armed "merge the diverged update" offer: spawn a daanio agent
    ...
    pub(super) fn append_reasoning_text(&mut self, text: &str) {
        ...
        for ch in text.chars() {
            if ch == '\n' {
                ...
                committed.push_str(&daanio_tui_markdown::reasoning_line_markup(&line));
            } else {
        ...
        let partial = daanio_tui_markdown::reasoning_partial_markup(&self.reasoning_pending_line);
    ...
    pub(super) fn close_reasoning_region(&mut self, _footer: Option<String>) {
        ...
        if !pending.is_empty() {
            self.streaming
                .streaming_text
                .push_str(&daanio_tui_markdown::reasoning_line_markup(&pending));
        }
    ...
    pub(super) fn submit_input(&mut self) {
        ...
        if let Some(command) = extract_input_shell_command(&input) {
            ...
            if self.is_remote {
                self.push_display_message(DisplayMessage::system(
                    "Input-line ! shell commands are only available in a local daanio TUI session.",
                ));
        ...
        if let Some(invocation) = skill_invocation {
            ...
            // as .daanio/skills/optimization work immediately after reload/build.
            if skill.is_none() {
            ...
            if let Some(skill) = skill {
                ...
                let endorsed_hint = crate::skill::endorsed_skills()
                    ...
                    .map(|endorsed| match endorsed.install {
                        ...
                        None => format!(
                            "Skill /{} is endorsed but not installed (source: {}). Install it into ~/.daanio/skills/{}/SKILL.md.",
