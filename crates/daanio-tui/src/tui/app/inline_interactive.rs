...
struct RemoteModelCatalogCache {
    ...
    origin: String,
    #[serde(flatten)]
    snapshot: daanio_provider_core::ModelCatalogSnapshot,
...
fn model_picker_route_is_current(
    ...
    daanio_provider_core::model_route_provider_labels_match(&route.provider, current_provider)
}
...
fn model_picker_route_is_recommended(model_name: &str, route: &PickerOption) -> bool {
    RECOMMENDED_MODELS.contains(&model_name)
        && daanio_provider_core::model_route_metadata_is_recommended(
            model_name,
            &route.provider,
            &route.api_method,
            route.available,
...
fn model_picker_route_provider_matches_key(
    ...
    daanio_provider_core::model_route_provider_matches_key(
        route_provider_key,
...
fn model_picker_route_is_default(
    ...
    if let Some((bare_model, provider_label)) = default_model.rsplit_once('@') {
        return bare_model == model_name
            && daanio_provider_core::model_route_provider_labels_match(
                &route.provider,
                provider_label,
            );
    }
...
impl App {
    pub(super) fn remote_model_catalog_snapshot(
        &self,
    ) -> daanio_provider_core::ModelCatalogSnapshot {
        daanio_provider_core::ModelCatalogSnapshot::new(
    ...
    fn append_daanio_subscription_routes(
        &self,
        ...
        let existing = routes
            ...
            .filter(|route| {
                route
                    .api_method
                    .eq_ignore_ascii_case(crate::subscription_catalog::DAANIO_ROUTE_API_METHOD)
            })
    ...
    fn extend_remote_routes_for_uncovered_models(
        ...
        // Daanio subscription routes are a complete, server-managed catalog.
        ...
        let provider_is_daanio_subscription =
            self.remote_provider_name.as_deref().is_some_and(|name| {
                name.eq_ignore_ascii_case(crate::subscription_catalog::DAANIO_PROVIDER_DISPLAY_NAME)
            });
        if provider_is_daanio_subscription {
            // The remote server's authenticated catalog is authoritative for
            // the current account. Build one managed route for every returned
            // chat-capable model, including models released after this CLI was
            // built, and discard any stale locally curated routes.
        ...
        let poisoned_by_daanio_subscription = !routes.is_empty()
        ...
        if poisoned_by_daanio_subscription {
            ...
            *routes = crate::provider::remote_model_routes_fallback(
            ...
            self.append_daanio_subscription_routes(routes, false, true);
            return;
        }
    ...
    fn open_cached_model_picker_if_fresh(
        ...
        if std::env::var("DAANIO_LOG_MODEL_PICKER_TIMING").is_ok() {
    ...
    fn open_model_picker_with_routes(
        ...
        fn route_matches_recent_auth(route_provider: &str, login_provider: &str) -> bool {
            daanio_provider_core::model_route_provider_labels_related(
                route_provider,
                login_provider,
            )
        }
        ...
        if total_ms >= 250 || std::env::var("DAANIO_LOG_MODEL_PICKER_TIMING").is_ok() {
            crate::logging::info(&format!(
    ...
    pub(super) fn handle_session_picker_selection(&mut self, targets: &[ResumeTarget]) {
        ...
        if self.session_picker_mode == SessionPickerMode::CatchUp {
            ...
            for target in targets {
                let ResumeTarget::DaanioSession { session_id } = target else {
                    continue;
                };
        ...
        for target in targets {
            ...
            let name = match target {
                ResumeTarget::DaanioSession { session_id } => {
                    crate::id::extract_session_name(session_id)
                        .map(|s| s.to_string())
                        .unwrap_or_else(|| session_id.to_string())
                }
                ResumeTarget::ClaudeCodeSession { session_id, .. } => {
                    format!(
                        "Claude Code {}",
                        daanio_core::util::truncate_str(session_id, 8)
                    )
                }
                ResumeTarget::CodexSession { session_id, .. } => {
                    format!("Codex {}", daanio_core::util::truncate_str(session_id, 8))
                }
                ...
                ResumeTarget::CursorSession { session_id, .. } => {
                    format!("Cursor {}", daanio_core::util::truncate_str(session_id, 8))
                }
            };
            ...
            match spawn_resume_target_in_new_terminal(&resolved_target, &cwd, socket.as_deref()) {
                ...
                Ok(false) | Err(_) => {
                    // No terminal emulator could be spawned. For a single daanio
                    // session, fall back to resuming in the current terminal
                    // instead of dead-ending with a manual command (issue #203).
                    if targets.len() == 1
                        && spawned == 0
                        && matches!(resolved_target, ResumeTarget::DaanioSession { .. })
                    {
    ...
    pub(super) fn handle_session_picker_current_terminal_selection(
        ...
        let ResumeTarget::DaanioSession { session_id } = resolved_target else {
            self.push_display_message(DisplayMessage::error(format!(
    ...
    fn handle_live_claude_takeover(&mut self, target: &ResumeTarget) -> bool {
        ...
        let display_id = daanio_core::util::truncate_str(session_id, 8);
        ...
        let ResumeTarget::DaanioSession { session_id } = resolved else {
            self.push_display_message(DisplayMessage::error(
                "Claude takeover did not produce a Daanio session.",
            ));
            return false;
        };
    ...
    pub(super) fn handle_batch_crash_restore(&mut self, session_ids: &[String]) {
        ...
        if spawned == 0 && recovered.len() == 1 && failed.len() == 1 {
            self.handle_session_picker_current_terminal_selection(&[ResumeTarget::DaanioSession {
        ...
        if spawned > 0 && failed.is_empty() {
            ...
            let manual: Vec<String> = failed
                .iter()
                .map(|id| format!("  daanio --resume {}", id))
                .collect();
            self.push_display_message(DisplayMessage::system(format!(
    ...
    pub(super) fn picker_fuzzy_score(pattern: &str, text: &str) -> Option<i32> {
        daanio_fuzzy::fuzzy_score_tokens(pattern, text)
    }
...
mod tests {
    ...
    fn model_picker_current_route_allows_provider_aliases() {
        assert!(daanio_provider_core::model_route_provider_labels_match(
            "Anthropic",
            "Claude"
        ));
        assert!(daanio_provider_core::model_route_provider_labels_match(
    ...
    fn model_picker_recommended_route_is_provider_aware() {
        ...
        // Current policy (see daanio-provider-core): claude-opus-4-8 is the
    ...
    fn remote_model_catalog_cache_rejects_stale_and_future_timestamps() {
        let snapshot = daanio_provider_core::ModelCatalogSnapshot::new(
    ...
    fn remote_model_catalog_cache_rejects_forged_or_oversized_routes() {
        let safe_snapshot = daanio_provider_core::ModelCatalogSnapshot::new(
