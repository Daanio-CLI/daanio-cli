...
fn with_temp_daanio_home<T>(f: impl FnOnce() -> T) -> T {
...
#[test]
fn tui_openai_compatible_api_base_keeps_daanio_docs_and_remote_endpoint() -> anyhow::Result<()> {
    with_temp_daanio_home(|| {
        ...
        assert!(
            resolved
                .setup_url
                .contains("github.com/Daanio-CLI/daanio-cli")
        );
...
#[test]
fn tui_daanio_subscription_logout_clears_credentials_and_preserves_api_base() -> anyhow::Result<()>
{
    with_temp_daanio_home(|| {
        crate::provider_catalog::save_env_value_to_env_file(
            crate::subscription_catalog::DAANIO_API_KEY_ENV,
            crate::subscription_catalog::DAANIO_ENV_FILE,
            Some("test-daanio-key"),
        )?;
        crate::provider_catalog::save_env_value_to_env_file(
        ...
        assert!(std::env::var_os(crate::subscription_catalog::DAANIO_API_KEY_ENV).is_none());
        assert_eq!(
            std::env::var(crate::subscription_catalog::DAANIO_API_BASE_ENV).as_deref(),
            Ok("https://subscription.example/v1")
        );
        assert!(std::env::var_os(crate::subscription_catalog::DAANIO_ACCOUNT_ID_ENV).is_none());
        assert!(std::env::var_os(crate::subscription_catalog::DAANIO_ACCOUNT_EMAIL_ENV).is_none());
