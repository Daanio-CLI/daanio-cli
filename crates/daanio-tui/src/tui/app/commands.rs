...
pub(super) fn is_auto_poke_connectivity_error(error: &str) -> bool {
    // Delegate to the shared connectivity classifier (daanio-app-core's
        ...
        Ok(result) => {
            ...
            match result {
                Ok(prepared) => {
                    ...
                    let socket = std::env::var("DAANIO_SOCKET").ok();
...
fn handle_subagent_model_command(app: &mut App, trimmed: &str) -> bool {
    ...
    if app.is_remote {
        app.push_display_message(DisplayMessage::error(
            "/subagent-model requires a live daanio server connection in remote mode.".to_string(),
        ));
        return true;
    }
...
fn handle_subagent_command(app: &mut App, trimmed: &str) -> bool {
    ...
    if app.is_remote {
        app.push_display_message(DisplayMessage::error(
            "/subagent requires a live daanio server connection in remote mode.".to_string(),
        ));
        return true;
    }
...
/// and macOS shortcuts, and any conflicts with daanio's own keybindings.
...
pub(super) fn handle_log_command(app: &mut App, trimmed: &str) -> bool {
    ...
    crate::logging::info(&format!(
        "DAANIO_LOG_MARK id={} session={} provider={} model={} cwd={} note={}",
        marker_id,
        app.session.id,
        app.provider_name(),
        app.provider_model(),
    ...
    let mut message = format!(
        "Log mark written: {}\n\nAgents can search ~/.daanio/logs/ for DAANIO_LOG_MARK or this marker id.",
        marker_id
    );
    if !note.is_empty() {
...
Step 1/4: Tell Daanio where to connect.
...
Security model
  - Daanio stores this host/user target so you can run /ssh {} later.
  - Daanio does not ask for or store your SSH password.
  - If a password is needed, it will be typed into your system ssh prompt, not into Daanio.
...
Daanio will ask for the SSH target, then use your system SSH client for authentication. Daanio never stores SSH passwords."
                    .to_string(),
            ));
        }
        Ok(config) => {
            ...
            lines.push("".to_string());
            lines.push("Security: Daanio stores targets only, never SSH passwords.".to_string());
...
Daanio verified that {} is reachable through your system SSH client.

What this means:
  - Authentication is handled by OpenSSH / your SSH agent.
  - Daanio did not see or store your password.
  - The SSH connection setup is ready for remote Daanio tools.

Next implementation step: start the remote Daanio server over this verified SSH connection.",
...
Daanio could not connect without an interactive login, so it opened a separate terminal running your system ssh command.

What to expect in that terminal
  1. OpenSSH may ask for your password or two-factor prompt.
  2. You type credentials into OpenSSH, not into Daanio.
  3. After login, SSH creates a temporary background control socket.
  4. The terminal verifies that socket before closing.

Security model
  - Daanio cannot read what you type in the SSH terminal.
  - Daanio stores only the target {}.
...
Daanio could not open a terminal automatically. Run this command yourself:

  ssh -f -M -S {} -N {}

Type your password into that SSH prompt if asked. Daanio will not see or store it.",
            profile.name,
            crate::ssh_remote::control_socket_path(&profile.name)
                .map(|p| p.display().to_string())
                .unwrap_or_else(|_| "~/.daanio/ssh-control/remote.sock".to_string()),
            profile.ssh_target
        ))),
        Err(error) => app.push_display_message(DisplayMessage::error(format!(
...
fn git_command_repo_dir(app: &App) -> Result<PathBuf, String> {
    if let Some(path) = active_working_dir(app) {
        ...
        return Err(format!(
            "Unable to run /git: session working directory {} is not accessible from this daanio client.",
            path.display()
        ));
    }
...
pub(super) fn build_triage_prompt(focus: &str) -> String {
    let mut prompt = String::from(
        ...
        Hard rules: every public comment, issue reply, or PR description you post MUST end with a clear agent attribution line like '--- *— Daanio agent (automated triage), on behalf of @<repo-owner>*' so it can never be mistaken for the human. \
...
fn build_test_verification_prompt(claim: &str) -> String {
    ...
    format!(
        "Run Daanio's /test verification orchestrator for: {target}\n\n\
...
pub(super) fn handle_dictation_command(app: &mut App, trimmed: &str) -> bool {
    ...
    if trimmed.starts_with("/dictate ") || trimmed.starts_with("/dictation ") {
        app.push_display_message(DisplayMessage::error(
            "Usage: /dictate\nConfigure [dictation] in ~/.daanio/config.toml to customize command, mode, hotkey, and timeout."
                .to_string(),
        ));
        return true;
    }
...
fn ensure_swarm_prompt_edit_path(
    ...
    daanio_dir: &std::path::Path,
) -> std::io::Result<PathBuf> {
    let project_dir = match working_dir {
        Some(path) => PathBuf::from(path),
        None => std::env::current_dir()?,
    };
    let project_path = project_dir.join(".daanio").join("swarm-prompt.md");
    if file_has_nonblank_content(&project_path) {
        return Ok(project_path);
    }

    let global_path = daanio_dir.join("swarm-prompt.md");
    if file_has_nonblank_content(&global_path) {
        return Ok(global_path);
    }

    std::fs::create_dir_all(daanio_dir)?;
...
pub(super) fn handle_swarm_prompt_command(app: &mut App, trimmed: &str) -> bool {
    ...
    let daanio_dir = match crate::storage::daanio_dir() {
        Ok(path) => path,
        Err(error) => {
            app.push_display_message(DisplayMessage::error(format!(
                "Failed to locate the Daanio config directory: {}",
                error
            )));
            return true;
        }
    };
    let path = match ensure_swarm_prompt_edit_path(app.session.working_dir.as_deref(), &daanio_dir)
    {
        Ok(path) => path,
        Err(error) => {
            app.push_display_message(DisplayMessage::error(format!(
        ...
        .spawn()
    {
        Ok(_) => {
            app.push_display_message(DisplayMessage::system(format!(
                "Opening the active swarm routing prompt in {}:\n{}\n\nChanges apply after restarting or reloading Daanio because running agent tool registries cache the prompt.",
                editor,
                path.display()
            )));
...
pub(super) fn handle_config_command(app: &mut App, trimmed: &str) -> bool {
    ...
    if trimmed == "/subscription" || trimmed == "/subscription status" {
        app.show_daanio_subscription_status();
        return true;
    }
    ...
    if trimmed == "/config edit" {
        ...
        if let Some(path) = Config::path() {
            ...
            app.push_display_message(DisplayMessage {
                role: "system".to_string(),
                content: format!(
                    "Opening config in editor...\n{} {}\n\n*Restart daanio after editing for changes to take effect.*",
                    editor,
                    path.display()
                ),
                tool_calls: vec![],
