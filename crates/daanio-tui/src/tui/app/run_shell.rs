...
pub(super) fn status_spinner_only_symbol(app: &App) -> Option<&'static str> {
    ...
    if status_uses_primary_spinner(&app.status) {
        Some(daanio_tui_style::theme::activity_indicator(
            status_spinner_elapsed(app),
            STATUS_SPINNER_FPS,
...
fn render_status_spinner_into_buffer_mut(buffer: &mut Buffer, area: Rect, symbol: &str) {
    buffer.set_stringn(
        ...
        Style::default().fg(daanio_tui_style::adapt_color_for_theme(
            daanio_tui_style::theme::ai_color(),
        )),
    );
}
...
mod tests {
    ...
    fn status_spinner_fast_path_uses_status_elapsed_clock() {
        ...
        let full_status_symbol = daanio_tui_style::theme::activity_indicator(
            full_status_elapsed,
            STATUS_SPINNER_FPS,
            true,
        );
        let old_app_lifetime_symbol = daanio_tui_style::theme::activity_indicator(
            app_lifetime_elapsed,
            STATUS_SPINNER_FPS,
            true,
        );
        let fast_path_symbol = daanio_tui_style::theme::activity_indicator(
