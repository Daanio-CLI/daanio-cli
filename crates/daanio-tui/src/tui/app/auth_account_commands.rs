...
fn parse_account_command(trimmed: &str) -> Option<Result<AccountCommand, String>> {
    ...
    if std::env::var_os("DAANIO_FIRST_PARTY_ONLY").is_some()
        && !matches!(rest, "doctor" | "list" | "ls")
        && !rest.starts_with("daanio")
        && !rest.starts_with("default-model")
    {
        return Some(Err(
            "Daanio CLI manages only its browser-authorized Daanio session. Use /account daanio or /login daanio; upstream provider accounts are managed server-side."
                .to_string(),
        ));
    }
    ...
    if let Some(provider) = resolve_account_provider_descriptor(first) {
        ...
        let parsed = match subcommand {
            ...
            "status" if provider.id == "daanio" => AccountCommand::DaanioStatus,
            "manage" if provider.id == "daanio" => AccountCommand::DaanioManage,
            "logout" if provider.id == "daanio" => AccountCommand::DaanioLogout,
...
pub(crate) fn execute_account_command_local(app: &mut App, command: AccountCommand) {
    match command {
        ...
        AccountCommand::DaanioStatus => app.show_daanio_subscription_status(),
        AccountCommand::DaanioManage => app.open_daanio_account_management(),
        AccountCommand::DaanioLogout => app.start_daanio_account_logout(),
...
fn save_default_provider_setting(app: &mut App, provider: Option<&str>) {
    ...
    let provider = match normalized.as_deref() {
        ...
        Some(other) if daanio_provider_core::AuthRoute::parse(other).is_some() => normalized,
        Some(other) => {
            app.push_display_message(DisplayMessage::error(format!(
...
fn save_openai_compat_setting(app: &mut App, setting: OpenAiCompatSetting, value: Option<&str>) {
    ...
    let (env_key, normalized_value) = match setting {
        OpenAiCompatSetting::ApiBase => {
            ...
            ("DAANIO_OPENAI_COMPAT_API_BASE", normalized)
        }
...
mod tests {
    ...
    #[test]
    fn parse_native_daanio_account_actions() {
        assert!(matches!(
            parse_account_command("/account daanio login"),
            Some(Ok(AccountCommand::Login { provider_id })) if provider_id == "daanio"
        ));
        assert!(matches!(
            parse_account_command("/account daanio status"),
            Some(Ok(AccountCommand::DaanioStatus))
        ));
        assert!(matches!(
            parse_account_command("/account daanio manage"),
            Some(Ok(AccountCommand::DaanioManage))
        ));
        assert!(matches!(
            parse_account_command("/account daanio logout"),
            Some(Ok(AccountCommand::DaanioLogout))
        ));
    }
    ...
    fn render_auth_doctor_markdown_includes_recovery_steps() {
        ...
        assert!(markdown.contains("daanio login --provider openai"));
        assert!(markdown.contains("Review current state: daanio auth status --json"));
    }
}
