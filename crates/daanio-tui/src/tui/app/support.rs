...
pub(super) const SUPPORT_EMAIL_ENV: &str = "DAANIO_SUPPORT_EMAIL";
pub(super) const DEFAULT_SUPPORT_EMAIL: &str = "support@daanio.com";
pub(super) const SUPPORT_SUBJECT: &str = "daanio support";
...
pub(super) fn build_mailto_url(email: &str, body: &str) -> String {
    format!(
        "mailto:{}?subject={}&body={}",
        email,
...
fn build_channel() -> String {
    if std::env::var(daanio_selfdev_types::CLIENT_SELFDEV_ENV).is_ok() {
        return "selfdev".to_string();
    }
    if daanio_build_meta::is_release_build() {
...
fn read_telemetry_id() -> Option<String> {
    let path = crate::storage::daanio_dir().ok()?.join("telemetry_id");
...
fn gather_diagnostics(app: &App) -> SupportDiagnostics {
    ...
    // Account identity is only present when a daanio subscription is
    ...
    let (account_id, account_email, tier) = if has_subscription {
        (
            load_env_value_from_env_or_config(cat::DAANIO_ACCOUNT_ID_ENV, cat::DAANIO_ENV_FILE),
            load_env_value_from_env_or_config(cat::DAANIO_ACCOUNT_EMAIL_ENV, cat::DAANIO_ENV_FILE),
...
pub(super) fn handle_support_command(app: &mut App, trimmed: &str) -> bool {
    ...
    let support_email = std::env::var(SUPPORT_EMAIL_ENV)
        .ok()
        .filter(|email| !email.trim().is_empty())
        .unwrap_or_else(|| DEFAULT_SUPPORT_EMAIL.to_string());
    let mailto = build_mailto_url(&support_email, &body);
    ...
    message.push_str(&format!("To: {support_email}\n"));
...
mod tests {
    ...
    fn support_mailto_url_is_correctly_encoded() {
        ...
        assert!(url.starts_with("mailto:help@example.com?subject=daanio%20support&body="));
