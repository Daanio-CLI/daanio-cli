...
fn tui_mouse_scroll_trace_enabled() -> bool {
    std::env::var_os("DAANIO_TUI_SCROLL_TRACE").is_some()
}
...
impl App {
    ...
    pub(super) fn toggle_swarm_message_expand(&mut self, msg_idx: usize) -> bool {
        ...
        let Some(toggled) = daanio_tui_messages::toggle_collapsible_swarm_content(&message.content)
        else {
            return false;
        };
        let expanded = daanio_tui_messages::parse_collapsible_swarm_content(&toggled)
