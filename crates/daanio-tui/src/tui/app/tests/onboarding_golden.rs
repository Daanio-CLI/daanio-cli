...
//   cargo test -p daanio-tui onboarding_golden -- --nocapture
...
fn onboarding_golden_walks_every_phase() {
    ...
    // failed) points the user at Daanio website-key entry.
    {
        ...
        assert!(
            text.contains("Press Enter to sign in securely through daanio.com"),
            "{text}"
        );
    }
    ...
    // 5. Suggestions (resting state).
    {
        ...
        assert!(text.contains("Welcome to daanio onboarding"), "{text}");
    }
}
...
fn onboarding_golden_walks_failure_and_async_states() {
    ...
    let assert_guided_polish = |title: &str, text: &str| {
        assert!(
            text.contains("Welcome to daanio onboarding"),
