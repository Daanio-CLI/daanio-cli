...
fn reasoning_region_closes_before_normal_output() {
    ...
    assert!(
        !answer_line.contains(daanio_tui_markdown::REASONING_SENTINEL),
    ...
    assert!(
        !text.contains(daanio_tui_markdown::REASONING_SENTINEL),
...
fn reasoning_region_renders_dim_italic_text_without_gutter() {
    ...
    assert!(
        !rendered.contains(daanio_tui_markdown::REASONING_SENTINEL),
...
fn strip_reasoning_lines_removes_reasoning_keeps_answer() {
    ...
    content.push_str(&daanio_tui_markdown::reasoning_line_markup("thinking one"));
    content.push_str(&daanio_tui_markdown::reasoning_line_markup("thinking two"));
    content.push('\n');
    content.push_str("Here is the answer.\n");

    let stripped = strip_reasoning_lines(&content);
    assert_eq!(stripped, "Here is the answer.");
    assert!(!stripped.contains(daanio_tui_markdown::REASONING_SENTINEL));
}
...
fn strip_reasoning_lines_reasoning_only_becomes_empty() {
    ...
    let mut content = String::new();
    content.push_str(&daanio_tui_markdown::reasoning_line_markup("only thinking"));
    let stripped = strip_reasoning_lines(&content);
...
fn multiple_reasoning_blocks_anchor_in_order_and_clear_next_prompt() {
    // Hermetic DAANIO_HOME: these assertions depend on the default
    ...
    with_temp_daanio_home(|| {
        ...
        assert!(
            !app.streaming_text()
                .contains(daanio_tui_markdown::REASONING_SENTINEL),
...
fn anchored_trace_never_moves_and_clears_on_next_prompt() {
    ...
    // not leaking in from the developer's real ~/.daanio (other tests
