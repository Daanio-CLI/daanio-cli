    ...
    fn model_routes(&self) -> Vec<crate::provider::ModelRoute> {
        vec![
            crate::provider::ModelRoute {
                model: daanio_provider_core::DEFAULT_CLAUDE_MODEL.to_string(),
            ...
            crate::provider::ModelRoute {
                model: daanio_provider_core::DEFAULT_OPENAI_MODEL.to_string(),
                provider: "OpenAI".to_string(),
                api_method: "openai-api-key".to_string(),
                available: true,
                detail: String::new(),
...
fn onboarding_strongest_model_only_runs_without_explicit_defaults() {
    with_temp_daanio_home(|| {
        let previous_force = std::env::var_os("DAANIO_FORCE_PROVIDER");
        ...
        if let Some(value) = previous_force {
            crate::env::set_var("DAANIO_FORCE_PROVIDER", value);
        } else {
            crate::env::remove_var("DAANIO_FORCE_PROVIDER");
        }
    });
...
fn openrouter_key_typed_through_full_key_path_does_not_reopen_picker() {
    ...
    with_temp_daanio_home(|| {
        ...
        // It is written to $DAANIO_HOME/config/daanio/openrouter.env and exported
...
fn startup_check_skips_selfdev_canary_session() {
    with_temp_daanio_home(|| {
        ...
        app.onboarding_flow = None;
        app.onboarding_startup_checked = false;
        // Self-dev / canary sessions (e.g. the niri `daanio self-dev` hotkey) take
...
fn import_failure_h_key_prepares_agent_repair_brief() {
    ...
    with_temp_daanio_home(|| {
        ...
        assert!(brief.contains("daanio auth-test --provider daanio --json"), "{brief}");
        ...
        assert!(on_disk.contains("daanio auth-test --provider daanio --json"), "{on_disk}");
...
fn import_continue_reaches_ready_quality_first_openai_model() {
    ...
    with_temp_daanio_home(|| {
        ...
        runtime.block_on(async {
            ...
            assert_eq!(model, daanio_provider_core::DEFAULT_OPENAI_MODEL);
