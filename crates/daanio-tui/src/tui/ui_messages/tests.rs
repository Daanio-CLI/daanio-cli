...
fn render_compact_launch_and_divergence_notices_as_one_line() {
    ...
    let notices = [
        DisplayMessage::system(
            "Configured Daanio launch hotkeys (niri):\nSuper+; → daanio (/home/user/project)\n\nBound system-wide.",
        )
        .with_title("Launch hotkeys"),
        DisplayMessage::system(
            "Update diverged. Press Ctrl+Y to let a daanio agent merge local and upstream (or run `git pull` / `git rebase` yourself).",
        )
        .with_title("Update"),
    ];
...
fn render_assistant_message_plan_card_keeps_nested_fences_inside() {
    ...
    assert!(plain.contains("cargo test -p daanio-tui"), "plain: {plain}");
    ...
    let bash_line = lines
        .iter()
        .map(extract_line_text)
        .find(|line| line.contains("cargo test -p daanio-tui"))
...
fn render_system_message_uses_connection_card_for_reconnect_status() {
    let msg = DisplayMessage::system(
        "⚡ Connection lost - retrying (attempt 2, 7s) - connection reset by server · resume: daanio --resume koala",
    )
    .with_title("Connection");
    ...
    assert!(plain.contains("daanio --resume koala"));
}
...
fn render_swarm_message_centered_mode_caps_wrap_width_for_long_notifications() {
    ...
    let msg = DisplayMessage::swarm(
        "File activity",
        "/home/jeremy/daanio/src/tui/ui_messages.rs - moss just edited this file while you were working nearby, so the notification should still read as centered in wide layouts.",
    );
...
fn render_swarm_message_collapsed_shows_tldr_and_expand_badge_only() {
    let saved = crate::tui::markdown::center_code_blocks();
    crate::tui::markdown::set_center_code_blocks(false);
    let content = daanio_tui_messages::encode_collapsible_swarm_content(
...
fn render_swarm_message_expanded_shows_body_and_collapse_badge() {
    let saved = crate::tui::markdown::center_code_blocks();
    crate::tui::markdown::set_center_code_blocks(false);
    let collapsed = daanio_tui_messages::encode_collapsible_swarm_content(
        "fixed the flaky test",
        "The flaky test was caused by a race in the setup helper.",
    );
    let expanded =
        daanio_tui_messages::toggle_collapsible_swarm_content(&collapsed).expect("toggle");
...
fn render_tool_message_shows_intent_and_technical_preview_on_one_line() {
    ...
    let msg = DisplayMessage {
        ...
        tool_data: Some(crate::message::ToolCall {
            ...
            name: "bash".to_string(),
            input: serde_json::json!({
                "command": "cargo test -p daanio render_background_task --lib",
...
fn render_tool_message_shows_discovery_browse_results_and_rationale() {
    let msg = first_discovery_message(
        "Discoverable tools in 'payments' (Daanio tool directory; recommendations must be based only on fit; details: https://daanio.com/discovery-tools):\n\n- agentcard: prepaid virtual Visa cards for AI agents (https://agentcard.sh/?via=daanio-discovery)\n\nBrowse request ID: `11111111-2222-4333-8444-555555555555`",
        serde_json::json!({
            "action": "browse",
            "category": "payments",
    ...
    assert!(
        plain.contains("Daanio partners with tool providers to make their tools discoverable"),
        "{plain}"
    );
    assert!(
        without_whitespace(&plain).contains("Learnmore:https://daanio.com/discovery-tools"),
        "{plain}"
    );
...
fn batched_discovery_renders_first_use_disclosure_inline_once() {
    ...
    assert_eq!(
        plain
            .matches("Daanio partners with tool providers to make their tools discoverable")
            .count(),
        1,
...
fn render_tool_message_shows_selected_discovery_setup() {
    ...
    assert!(
        !plain.contains("Daanio partners with tool providers"),
...
fn render_tool_message_shows_catalog_suggestion_receipt_and_trust_line() {
    ...
    assert!(plain.contains("Daanio maintainers only"), "{plain}");
...
fn discovery_cards_wrap_within_narrow_transcript_width() {
    let msg = discovery_message(
        "Catalog suggestion submitted.\n\nStatus: received for Daanio maintainer review.",
        serde_json::json!({
            "action": "suggest",
