...
pub(super) fn semver() -> &'static str {
    ...
    SEMVER.get_or_init(|| format!("v{}", daanio_build_meta::semver()))
}

/// True when this process is running from the stable release binary path.
/// Only matches the explicit ~/.daanio/builds/stable/daanio path, NOT
/// ~/.local/bin/daanio launcher path (which now points to current).
...
pub(super) fn binary_age() -> Option<String> {
    let git_date = daanio_build_meta::git_date();
