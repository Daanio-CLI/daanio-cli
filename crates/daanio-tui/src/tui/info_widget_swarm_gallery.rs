...
//! [`daanio_tui_render::swarm_gallery`] module so the live TUI and the
...
use daanio_tui_render::swarm_gallery::{
    ...
    members
        ...
        .map(|member| GalleryMember {
            ...
            todo_items: member
                .todo_items
                .iter()
                .map(|t| daanio_tui_render::swarm_gallery::GalleryTodo {
                    content: t.content.clone(),
                    status: t.status.clone(),
                    tool_intents: t
                        .tool_intents
                        .iter()
                        .map(|tool| daanio_tui_render::swarm_gallery::GalleryToolIntent {
...
pub(crate) fn render_swarm_chat_card_lines(
    ...
    daanio_tui_render::swarm_gallery::render_swarm_chat_cards(&gallery_members, width)
}
...
mod tests {
    use super::*;
    use daanio_tui_render::swarm_gallery::members_to_tiles;
