...
fn copy_selection_text_with_math_targets(
    ...
    let math_targets: Vec<_> = prepared
        ...
        .filter(|target| {
            matches!(
                &target.kind,
                daanio_tui_markdown::CopyTargetKind::Math { .. }
            )
...
mod tests {
    use super::*;
    use daanio_tui_markdown::CopyTargetKind;
    use daanio_tui_messages::{CopyTarget, PreparedChatFrame, PreparedMessages, WrappedLineMap};
