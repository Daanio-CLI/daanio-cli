...
pub(super) fn build_auth_status_line(auth: &AuthStatus, max_width: usize) -> Line<'static> {
    ...
    fn dual_method_label(
        provider: daanio_provider_core::ActiveProvider,
        auth: &AuthStatus,
    ) -> Option<&'static str> {
        use crate::auth::{ActiveCredential, resolve_dual_credential_auth};
        let runtime_provider = std::env::var("DAANIO_RUNTIME_PROVIDER").ok();
    ...
    let anthropic_label = provider_label(
        ...
        dual_method_label(daanio_provider_core::ActiveProvider::Claude, auth),
    );

    let openai_label = provider_label(
        "openai",
        auth.openai,
        dual_method_label(daanio_provider_core::ActiveProvider::OpenAI, auth),
    );
...
fn header_provider_auth_tag(name: &str, auth: &AuthStatus) -> &'static str {
    ...
    if let Some(provider) = daanio_provider_core::parse_provider_hint(name) {
        use crate::auth::{ActiveCredential, resolve_dual_credential_auth};
        match resolve_dual_credential_auth(provider, auth, runtime_provider.as_deref()) {
            ...
            None if matches!(
                provider,
                daanio_provider_core::ActiveProvider::Claude
                    | daanio_provider_core::ActiveProvider::OpenAI
            ) =>
            {
                return "";
            }
...
fn version_display_candidates() -> Vec<String> {
    let full = format!("daanio {}", semver());
    let core = format!("daanio {}", semver_core());
    let minor = format!("daanio {}", semver_minor());
...
fn build_persistent_header_with_auth(
    ...
    let client_version_full = server_name
        .as_ref()
        .map(|_| daanio_build_meta::version().to_string());
    ...
    let version_text = if client_version_label.is_some() {
        ...
        let tag = daanio_build_meta::git_tag();
...
mod tests {
    ...
    fn ensure_test_daanio_home_if_unset() {
        static TEST_HOME: OnceLock<std::path::PathBuf> = OnceLock::new();

        if std::env::var_os("DAANIO_HOME").is_some() {
            return;
        }

        let path = TEST_HOME.get_or_init(|| {
            let path =
                std::env::temp_dir().join(format!("daanio-test-home-{}", std::process::id()));
            let _ = std::fs::create_dir_all(&path);
            path
        });
        crate::env::set_var("DAANIO_HOME", path);
    }

    fn create_test_app() -> crate::tui::app::App {
        ensure_test_daanio_home_if_unset();
    ...
    fn persistent_header_labels_server_and_client_versions() {
        ...
        let client_version = compact_version_label(daanio_build_meta::version());
        assert!(
    ...
    fn persistent_header_keeps_git_hash_when_semvers_match_but_builds_differ() {
        let mut app = create_test_app();
        let client_semver = compact_version_label(daanio_build_meta::version());
        ...
        assert!(
            client_line.contains(&format!("· {}", daanio_build_meta::version())),
    ...
    fn header_model_display_name_sweeps_real_model_catalog() {
        ...
        // prettify_model_id, over the model ids daanio actually routes.
        let cases = [
            // Anthropic
    ...
    fn header_provider_auth_tag_reports_active_credential_for_openai() {
        ...
        let prev = std::env::var_os("DAANIO_RUNTIME_PROVIDER");
        ...
        if let Some(value) = prev {
            crate::env::set_var("DAANIO_RUNTIME_PROVIDER", value);
        }
    }
    ...
    fn header_provider_auth_tag_honors_runtime_selection_and_oauth_first() {
        ...
        crate::env::set_var("DAANIO_RUNTIME_PROVIDER", "claude-api");
        assert_eq!(header_provider_auth_tag("anthropic", &both), "api-key");

        // Explicit OAuth selection.
        crate::env::set_var("DAANIO_RUNTIME_PROVIDER", "claude");
    ...
    fn build_persistent_header_prefers_configured_model_during_remote_connect() {
        let _guard = crate::storage::lock_test_env();
        let prev_model = std::env::var_os("DAANIO_MODEL");
        let prev_provider = std::env::var_os("DAANIO_PROVIDER");
        crate::env::set_var("DAANIO_MODEL", "gpt-5.4");
        crate::env::set_var("DAANIO_PROVIDER", "openai");
        ...
        if let Some(prev_model) = prev_model {
            crate::env::set_var("DAANIO_MODEL", prev_model);
        } else {
            crate::env::remove_var("DAANIO_MODEL");
        }
        if let Some(prev_provider) = prev_provider {
            crate::env::set_var("DAANIO_PROVIDER", prev_provider);
        } else {
            crate::env::remove_var("DAANIO_PROVIDER");
        }
    }
    ...
    fn build_header_lines_omits_placeholder_provider_label_when_unknown() {
        ...
        // sibling test that sets DAANIO_MODEL=gpt-5.4 mid-flight leaks into this
    ...
    fn build_header_lines_hides_secondary_placeholder_during_brief_connecting_phase() {
        // Same env sensitivity as the placeholder test above: DAANIO_MODEL /
        // DAANIO_PROVIDER mutations from sibling tests change what renders.
        let _guard = crate::storage::lock_test_env();
        let app = crate::tui::app::App::new_for_remote(None);
    ...
    fn auth_status_line_marks_active_credential_when_both_configured() {
        ...
        let rendered_with = |runtime: Option<&str>| {
            match runtime {
                Some(value) => crate::env::set_var("DAANIO_RUNTIME_PROVIDER", value),
                None => crate::env::remove_var("DAANIO_RUNTIME_PROVIDER"),
            }
