...
//!
//! 1. `DAANIO_THEME=dark|light` env override (also accepts `auto`).
...
//! 4. Fallback: dark (daanio's native palette).
//!
//! The result is stored in `daanio_tui_style::theme_mode` where the renderer
//! adapts colors for light backgrounds at frame time.

use daanio_tui_style::ThemeMode;
...
pub fn init_theme_mode() -> ThemeMode {
    ...
    daanio_tui_style::set_theme_mode(mode);
    mode
}
...
pub fn current_theme_label() -> &'static str {
    match daanio_tui_style::theme_mode() {
        ThemeMode::Dark => "dark",
...
fn resolve_configured_theme(query_terminal: bool) -> ThemeMode {
    let configured = std::env::var("DAANIO_THEME")
        .ok()
