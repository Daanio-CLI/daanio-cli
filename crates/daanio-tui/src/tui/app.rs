...
pub enum AppRuntimeMode {
    /// Normal product TUI. The client renders state owned by the daanio server.
    RemoteClient,
...
fn active_runtime_provider_key() -> Option<String> {
    std::env::var("DAANIO_RUNTIME_PROVIDER")
        .ok()
...
/// An interactive "let a daanio agent merge the diverged update for you" offer.
///
...
/// daanio session, pre-loaded with a prompt to reconcile the branches, instead of
...
pub struct App {
    ...
    // Interactive "spawn a daanio agent to merge the diverged update" offer shown
    ...
    /// to target the agent repair brief (`daanio auth-test --provider X`). `None`
    /// when unknown.
    ...
    remote_resolved_credential: Option<daanio_provider_core::ResolvedCredential>,
...
// The cache-relevant projection lives in `daanio-message-types` (re-exported
// through `crate::message`) so this local path and the server event path in
// `daanio-app-core::agent::kv_cache_request_event` hash messages identically.
