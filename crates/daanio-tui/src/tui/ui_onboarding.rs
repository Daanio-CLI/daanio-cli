...
//!   3. "Welcome to daanio onboarding" title.
...
/// everywhere. Override with `DAANIO_ROUNDED_PILLS=on|off`.
fn rounded_pill_caps_supported() -> bool {
    ...
    *SUPPORTED.get_or_init(|| {
        ...
        if let Ok(raw) = std::env::var("DAANIO_ROUNDED_PILLS") {
...
fn telemetry_header_lines(width: u16) -> Vec<Line<'static>> {
    ...
    let lines = vec![
        "daanio collects anonymous usage statistics (version, OS, session",
        "activity, and crash reasons). No code, prompts, or personal data.",
        "Opt out anytime: export DAANIO_NO_TELEMETRY=1",
    ];
    lines
        .into_iter()
        .map(|text| {
        ...
        OnboardingWelcomeKind::Login {
            ...
            match import {
                ...
                None if error.is_some() => {
                    ...
                    lines.push(
                        Line::from(Span::styled(
                            "Press Enter to sign in securely through daanio.com.",
                            Style::default().fg(dim_color()),
                        ))
                        .alignment(align),
                    );
                    // If we detected a coding agent the user recently used, offer
                    // to hand the fix to it (it can run daanio's auth-test and add
