...
fn session_scan_limit() -> usize {
    std::env::var("DAANIO_SESSION_PICKER_MAX_SESSIONS")
        .ok()
...
fn include_old_saved_sessions_on_initial_load() -> bool {
    std::env::var("DAANIO_SESSION_PICKER_INCLUDE_OLD_SAVED")
        .ok()
...
fn session_list_disk_cache_path() -> Result<PathBuf> {
    Ok(storage::daanio_dir()?.join("cache/session-picker-list-v1.json"))
}
...
pub fn load_cached_sessions_grouped() -> Option<(Vec<ServerGroup>, Vec<SessionInfo>)> {
    let sessions_dir = storage::daanio_dir().ok()?.join("sessions");
...
fn transcript_paths_for_session(session: &SessionInfo) -> Vec<PathBuf> {
    match &session.resume_target {
        ResumeTarget::DaanioSession { session_id } => {
            let Ok(sessions_dir) = storage::daanio_dir().map(|dir| dir.join("sessions")) else {
                return Vec::new();
            };
            vec![
...
/// Parse a single daanio session snapshot (+ journal) into a [`SessionInfo`],
...
pub fn load_sessions() -> Result<Vec<SessionInfo>> {
    ...
    let (mut sessions, external_sessions) = std::thread::scope(|scope| {
        ...
        // default list appear to jump from a handful of Daanio rows straight to old
        ...
        'fill: while start < candidates.len() {
            ...
            let parsed = parallel_map(batch, move |stem| {
                parse_daanio_session_info(sessions_dir_ref, &stem, catchup_ref)
            });
    ...
    sessions
        ...
        .map(|session| {
            ...
            let short_name = working_dir
                ...
                .unwrap_or_else(|| {
                    format!("claude {}", daanio_core::util::truncate_str(&session_id, 8))
                });
...
fn load_codex_session_stub(path: &Path) -> Result<Option<SessionInfo>> {
    ...
    let working_dir = meta
        ...
        .and_then(|v| v.as_str())
        .map(|s| s.to_string());
    let short_name = format!("codex {}", daanio_core::util::truncate_str(&session_id, 8));
    let title = format!(
        "Codex session {}",
...
fn load_pi_session_stub(path: &Path) -> Result<Option<SessionInfo>> {
    ...
    let working_dir = header
        ...
        .and_then(|v| v.as_str())
        .map(|s| s.to_string());
    let short_name = format!("pi {}", daanio_core::util::truncate_str(&session_id, 8));
    let title = format!(
        "Pi session {}",
        daanio_core::util::truncate_str(&session_id, 8)
    );
...
fn load_cursor_session_stub(path: &Path) -> Result<Option<SessionInfo>> {
    ...
    let short_name = working_dir
        ...
        .and_then(|name| name.to_str())
        .map(|name| name.to_string())
        .unwrap_or_else(|| format!("cursor {}", daanio_core::util::truncate_str(&session_id, 8)));
