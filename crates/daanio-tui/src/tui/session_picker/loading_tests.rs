...
fn cached_grouped_sessions_round_trip_from_disk() {
    ...
    let _scan_limit = EnvVarGuard::set_str("DAANIO_SESSION_PICKER_MAX_SESSIONS", "100");
    let _include_saved = EnvVarGuard::set_str("DAANIO_SESSION_PICKER_INCLUDE_OLD_SAVED", "0");
    ...
    let session = SessionInfo {
        ...
        server_name: None,
        server_icon: None,
        source: SessionSource::Daanio,
        resume_target: ResumeTarget::DaanioSession {
...
fn load_sessions_includes_saved_sessions_beyond_scan_limit() {
    ...
    let temp = tempfile::tempdir().expect("temp dir");
    let _home = EnvVarGuard::set_path("DAANIO_HOME", temp.path());
    let _scan_limit = EnvVarGuard::set_str("DAANIO_SESSION_PICKER_MAX_SESSIONS", "50");
...
#[test]
fn session_matches_query_searches_daanio_transcript_contents() {
    let _env_lock = crate::storage::lock_test_env();
    let temp = tempfile::tempdir().expect("temp dir");
    let _home = EnvVarGuard::set_path("DAANIO_HOME", temp.path());
...
fn benchmark_real_resume_loading_phases() {
    ...
    let sessions_dir = storage::daanio_dir().expect("daanio dir").join("sessions");
    ...
    let daanio_summary_elapsed = phase_start.elapsed();
    ...
    eprintln!(
        concat!(
            ...
            "daanio_summary={}ms daanio_loaded={} skipped_empty={} skipped_imported={} summary_errors={} ",
        ...
        daanio_summary_elapsed.as_millis(),
        sessions.len(),
        skipped_empty,
        skipped_imported,
        summary_errors,
...
#[test]
#[ignore = "developer benchmark: scans the real DAANIO_HOME session directory"]
...
fn onboarding_scoped_loader_returns_only_codex_sessions() {
    ...
    // A daanio session that must NOT appear in the scoped Codex view (the whole
    // point of the scoped loader is to skip parsing these on onboarding).
    let mut daanio_session = Session::create_with_id(
        "session_onboarding_daanio_1780000000000".to_string(),
        Some("/tmp/daanio-onboard".to_string()),
        Some("Daanio Onboarding".to_string()),
    );
    daanio_session.append_stored_message(crate::session::StoredMessage {
        id: "msg-1".to_string(),
        role: crate::message::Role::User,
        content: vec![crate::message::ContentBlock::Text {
    ...
    daanio_session.save().expect("save daanio session");
    ...
    assert!(
        ...
        "scoped Codex load must not include daanio/other-CLI sessions"
    );
}
...
fn hidden_debug_sessions_do_not_consume_default_resume_budget() {
    ...
    // loader stopped after the first 50, leaving no ordinary Daanio sessions for
