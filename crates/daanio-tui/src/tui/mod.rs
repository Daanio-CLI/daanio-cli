...
// `daanio-terminal-image` crate (shared with the `read` tool). Re-exported here
// so existing `crate::tui::image` / `crate::tui::image_metadata` paths keep working.
pub use daanio_terminal_image::display as image;
use daanio_terminal_image::metadata as image_metadata;
pub mod info_widget;
...
pub mod mermaid;
pub mod permissions {
    pub use daanio_tui_permissions::*;
}
mod remote_diff;
pub mod screenshot;
...
pub use daanio_tui_workspace::workspace_map;
pub use daanio_tui_workspace::workspace_map_widget;
...
pub trait TuiState {
    ...
    /// Number of daanio compactions already applied to this session, when known.
...
fn supports_reliable_zero_cache_read_warning(
    ...
    // OpenRouter/Daanio-subscription routes can only be treated as reliable for zero-read
...
pub(crate) const REDRAW_SWARM_SPINNER: Duration =
    Duration::from_millis(daanio_tui_render::swarm_gallery::STRIP_SPINNER_FRAME_MS);
...
fn swarm_spinner_redraw_active(state: &dyn TuiState) -> bool {
    state.client_focused()
        && state
            ...
            .iter()
            .any(|m| daanio_tui_render::swarm_gallery::is_active_status(&m.status))
}
...
pub(crate) fn subscribe_metadata(
    ...
    resolve_subscribe_metadata(
        ...
        remote_working_dir,
        daanio_selfdev_types::client_selfdev_requested(),
    )
}
...
pub(crate) fn resolve_subscribe_metadata(
    ...
    if !selfdev && let Some(dir) = client_working_dir {
        ...
        while let Some(path) = current {
            if crate::build::is_daanio_repo(path) {
                selfdev = true;
                break;
            }
            current = path.parent();
...
pub fn display_messages_from_session(session: &crate::session::Session) -> Vec<DisplayMessage> {
    let mut messages = daanio_tui_messages::display_messages_from_rendered_messages(
