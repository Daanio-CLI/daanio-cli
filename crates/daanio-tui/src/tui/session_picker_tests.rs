...
fn onboarding_external_filter_picks_latest_visible_transcript() {
    ...
    let daanio = make_session("daanio_one", "daanio", false, SessionStatus::Closed);

    let mut picker = SessionPicker::new(vec![older, daanio, newer]);
...
fn onboarding_external_filter_with_no_matches_has_no_target() {
    let daanio = make_session("daanio_only", "daanio", false, SessionStatus::Closed);
    let mut picker = SessionPicker::new(vec![daanio]);
...
fn onboarding_banner_renders_prompt_and_both_action_rows() {
    ...
    assert!(
        text.contains("Welcome to daanio"),
    ...
    let welcome_y = lines
        .iter()
        .position(|line| line.contains("Welcome to daanio"))
        .expect("welcome row");
    let review_y = lines
        .iter()
...
//
//   cargo test -p daanio-tui --lib --release -- --ignored --nocapture benchmark_resume_op
//
