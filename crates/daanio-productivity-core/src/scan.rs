...
//! several GB. We keep a sidecar cache (`~/.daanio/cache/productivity/summaries.json`)
...
fn cache_path() -> Result<PathBuf> {
    let dir = daanio_storage::daanio_dir()?
        .join("cache")
        .join("productivity");
...
fn sessions_dir() -> Result<PathBuf> {
    Ok(daanio_storage::daanio_dir()?.join("sessions"))
}
