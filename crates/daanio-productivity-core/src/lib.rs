//! Productivity report generation for daanio.
//!
...
/// PNG to `~/.daanio/generated-images/productivity-<timestamp>.png`.
...
fn save_png(png: &[u8]) -> Result<PathBuf> {
    let dir = daanio_storage::daanio_dir()?.join("generated-images");
