...
#[tokio::test]
#[ignore = "live smoke: requires ANTHROPIC_API_KEY, or set DAANIO_LIVE_ANTHROPIC_ALLOW_OAUTH=1 to use Claude OAuth credentials"]
async fn live_anthropic_reasoning_smoke() -> Result<()> {
    let _env_lock = daanio_base::storage::lock_test_env();
    let using_api_key = std::env::var_os("ANTHROPIC_API_KEY").is_some();
    let allow_oauth = std::env::var_os("DAANIO_LIVE_ANTHROPIC_ALLOW_OAUTH").is_some();
    if !using_api_key && !allow_oauth {
        eprintln!(
            "skipping live Anthropic smoke: set ANTHROPIC_API_KEY or DAANIO_LIVE_ANTHROPIC_ALLOW_OAUTH=1"
        );
        return Ok(());
    }

    let _max_tokens = EnvVarGuard::set_if_missing("DAANIO_ANTHROPIC_MAX_TOKENS", "2048");
    let model = std::env::var("DAANIO_LIVE_ANTHROPIC_MODEL")
        .or_else(|_| std::env::var("DAANIO_ANTHROPIC_MODEL"))
        .unwrap_or_else(|_| "claude-sonnet-4-6".to_string());
    let effort = std::env::var("DAANIO_LIVE_ANTHROPIC_REASONING_EFFORT")
        .unwrap_or_else(|_| "low".to_string());
    let prompt = std::env::var("DAANIO_LIVE_ANTHROPIC_PROMPT")
        .unwrap_or_else(|_| "Live smoke test: answer exactly OK.".to_string());
    let system = std::env::var("DAANIO_LIVE_ANTHROPIC_SYSTEM").unwrap_or_else(|_| {
        "You are a live provider smoke test. Keep the answer tiny.".to_string()
    });
    let require_thinking = std::env::var_os("DAANIO_LIVE_ANTHROPIC_REQUIRE_THINKING").is_some();
    ...
    if require_thinking {
        assert!(
            thinking_bytes > 0,
            "live Anthropic response did not include thinking deltas despite DAANIO_LIVE_ANTHROPIC_REQUIRE_THINKING"
        );
    }
    Ok(())
}
...
fn credential_mode_runtime_provider_identity_round_trips() {
    let _guard = daanio_base::storage::lock_test_env();
    let previous = std::env::var_os("DAANIO_RUNTIME_PROVIDER");

    daanio_base::env::set_var("DAANIO_RUNTIME_PROVIDER", "claude");
    assert_eq!(
    ...
    daanio_base::env::set_var("DAANIO_RUNTIME_PROVIDER", "claude-api");
    assert_eq!(
        AnthropicCredentialMode::from_runtime_env(
            daanio_provider_core::DualAuthProvider::Anthropic
        ),
    ...
    match previous {
        Some(value) => daanio_base::env::set_var("DAANIO_RUNTIME_PROVIDER", value),
        None => daanio_base::env::remove_var("DAANIO_RUNTIME_PROVIDER"),
    }
}
...
async fn auto_mode_falls_back_to_api_key_when_oauth_is_expired() {
    ...
    let _runtime = EnvVarGuard::set("DAANIO_RUNTIME_PROVIDER", "auto");

    daanio_base::auth::claude::upsert_account(daanio_base::auth::claude::AnthropicAccount {
...
async fn explicit_oauth_mode_does_not_silently_fall_back_to_api_key() {
    ...
    let _home = EnvVarGuard::set("DAANIO_HOME", temp.path());
    let _api_key = EnvVarGuard::set("ANTHROPIC_API_KEY", "test-anthropic-api-key");
    let _runtime = EnvVarGuard::set("DAANIO_RUNTIME_PROVIDER", "claude");
...
fn anthropic_fallback_prefers_best_available_and_skips_tried_and_retired() {
    ...
    let _guard = daanio_base::storage::lock_test_env();
    daanio_base::provider::models::reset_model_catalog_services_for_tests();
    let known = daanio_base::provider::known_anthropic_model_ids();
    assert!(
        !known.is_empty(),
