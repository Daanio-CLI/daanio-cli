...
//! loading, cache-TTL toggles) stay in `daanio_base::provider::anthropic`;
...
use daanio_base::auth;
use daanio_base::auth::oauth;
use daanio_provider_core::{EventStream, NativeToolResultSender, Provider};
fn oauth_beta_headers(model: &str) -> &'static str {
    daanio_provider_core::anthropic_oauth_beta_headers(model)
}

use anyhow::{Context, Result};
use async_trait::async_trait;
use daanio_base::provider::anthropic::{
...
#[cfg(test)]
use daanio_base::provider::anthropic::{OAUTH_BETA_HEADERS, effectively_1m};
#[cfg(test)]
use daanio_message_types::{ContentBlock, Role};
use daanio_message_types::{Message, StreamEvent, ToolDefinition};
#[cfg(test)]
...
pub(crate) const OAUTH_BETA_HEADERS_1M: &str =
    daanio_provider_core::ANTHROPIC_OAUTH_BETA_HEADERS_1M;
...
fn load_official_claude_client_metadata() -> OAuthClientMetadata {
    let path = match daanio_base::storage::user_home_path(".claude.json") {
        Ok(path) => path,
...
async fn oauth_preflight_get(
    ...
    if !resp.status().is_success() {
        ...
        let body = daanio_base::util::http_error_body(resp, "HTTP error").await;
...
async fn ensure_oauth_preflight(
    ...
    let eval = OAuthEvalRequest {
        attributes: OAuthEvalAttributes {
            ...
            subscription_type: daanio_base::auth::claude::get_subscription_type()
    ...
    if all_ok {
        daanio_base::logging::info("Claude OAuth preflight completed successfully");
    }
    Ok(())
}

/// Quality-first default shared with model routing and first-run selection.
const DEFAULT_MODEL: &str = daanio_provider_core::DEFAULT_CLAUDE_MODEL;
...
/// Override with DAANIO_ANTHROPIC_MAX_TOKENS env var.
...
impl AnthropicProvider {
    fn is_usage_exhausted() -> bool {
        let usage = daanio_base::usage::get_sync();
    ...
    /// `DAANIO_RUNTIME_PROVIDER` happens to be in the current process (e.g. a
    /// self-dev session may have it set to `claude-api`). This also updates
    /// `DAANIO_RUNTIME_PROVIDER` so any provider instances the probes build
    ...
    pub async fn fetch_live_model_ids_for_doctor(&self) -> Result<Vec<String>> {
        ...
        let catalog = if is_oauth {
            daanio_base::provider::fetch_anthropic_model_catalog_oauth(&token).await?
        } else {
            daanio_base::provider::fetch_anthropic_model_catalog(&token).await?
        };
        // Persist so the rest of the process benefits from the warm catalog,
        // exactly like the runtime's own prefetch.
        daanio_base::provider::persist_anthropic_model_catalog(&catalog);
        if !catalog.context_limits.is_empty() {
            daanio_base::provider::populate_context_limits(catalog.context_limits.clone());
        }
        if !catalog.available_models.is_empty() {
            daanio_base::provider::populate_anthropic_models(catalog.available_models.clone());
        }
        Ok(catalog.available_models)
    }

    pub fn new() -> Self {
        let model = std::env::var("DAANIO_ANTHROPIC_MODEL").unwrap_or_else(|_| {
        ...
        let _ = tokio::runtime::Handle::try_current().map(|_| {
            tokio::spawn(async {
                let _ = daanio_base::usage::get().await;
            })
        });

        let max_tokens = std::env::var("DAANIO_ANTHROPIC_MAX_TOKENS")
            .ok()
            .and_then(|v| v.trim().parse::<u32>().ok())
            .unwrap_or(DEFAULT_MAX_TOKENS);
        let reasoning_effort = daanio_base::config::config()
            .provider
        ...
        Self {
            ...
            credential_mode: Arc::new(RwLock::new(AnthropicCredentialMode::from_runtime_env(
                daanio_provider_core::DualAuthProvider::Anthropic,
            ))),
            max_tokens,
    ...
    fn model_supports_output_effort(model: &str) -> bool {
        ...
        // generations); see `daanio_provider_core::anthropic_reasoning_caps`.
        // Fable 5 verified live 2026-07-01; Sonnet 5 verified live 2026-07-07.
        daanio_provider_core::anthropic_reasoning_caps(model).output_effort
    }

    fn model_supports_adaptive_thinking(model: &str) -> bool {
        daanio_provider_core::anthropic_reasoning_caps(model).adaptive_thinking
    }

    fn model_supports_manual_thinking(model: &str) -> bool {
        daanio_provider_core::anthropic_reasoning_caps(model).manual_thinking
    }

    fn model_supports_xhigh_effort(model: &str) -> bool {
        daanio_provider_core::anthropic_reasoning_caps(model).xhigh_effort
    }
    ...
    fn model_supports_max_effort(model: &str) -> bool {
        daanio_provider_core::anthropic_reasoning_caps(model).max_effort
    }

    fn model_supports_reasoning_effort(model: &str) -> bool {
        daanio_provider_core::anthropic_reasoning_caps(model).supports_reasoning_effort()
    }
    ...
    fn normalize_reasoning_effort(raw: &str) -> Option<String> {
        ...
        match value.as_str() {
            ...
            other => {
                daanio_base::logging::info(&format!(
    ...
    fn store_effort_for_model(model: &str, effort: &str) -> String {
        if daanio_base::prompt::is_deep_swarm_effort(effort) {
            daanio_base::prompt::SWARM_DEEP_EFFORT.to_string()
        } else if daanio_base::prompt::is_swarm_effort(effort) {
            daanio_base::prompt::SWARM_EFFORT.to_string()
        } else {
    ...
    fn manual_thinking_budget(effort: &str, max_tokens: u32) -> Option<u32> {
        let desired = match effort {
            ...
            "high" => 8_192,
            "xhigh" | "max" => 16_384,
            e if daanio_base::prompt::is_swarm_effort(e) => 16_384,
            _ => return None,
        };
    ...
    fn build_reasoning_request_parts(
        ...
        let show_thinking = daanio_base::config::config().display.show_thinking;
    ...
    async fn get_access_token(&self) -> Result<(String, bool)> {
        ...
        // daanio OAuth account may still deserialize successfully even after its
    ...
    async fn get_oauth_access_token(&self) -> Result<(String, bool)> {
        ...
        {
            anyhow::bail!(
                "Claude OAuth credentials are missing the required user:inference scope (scopes: {}). Run `daanio login --provider claude` to mint a fresh Claude.ai OAuth token, or import/use a fresh Claude Code login.",
        ...
        if fresh_creds.expires_at < now + 300_000 && !fresh_creds.refresh_token.is_empty() {
            ...
            {
                Ok(refreshed) => {
                    daanio_base::logging::info("OAuth token refreshed successfully");
                ...
                Err(e) => {
                    daanio_base::logging::error(&format!("OAuth token refresh failed: {}", e));
        ...
        if fresh_creds.expires_at <= now {
            anyhow::bail!(
                "Claude OAuth token is expired and no usable refresh token is available. Run `daanio login --provider claude` to refresh OAuth credentials"
            );
        }
    ...
    pub(crate) fn set_credential_mode(&self, mode: AnthropicCredentialMode) -> Result<()> {
        ...
        *mode_guard = mode;
        ...
        if let Some(route) = mode.auth_route(daanio_provider_core::DualAuthProvider::Anthropic) {
            daanio_base::env::set_var("DAANIO_RUNTIME_PROVIDER", route.runtime_provider_key());
        }
        ...
        daanio_base::auth::AuthStatus::invalidate_cached_status();
        Ok(())
    }
    ...
    fn format_messages(&self, messages: &[Message], is_oauth: bool) -> Vec<ApiMessage> {
        daanio_provider_anthropic::format_messages(messages, is_oauth)
    }
    ...
    fn format_content_blocks(
        ...
        daanio_provider_anthropic::format_content_blocks(blocks, is_oauth)
    }
    ...
    fn format_tools(&self, tools: &[ToolDefinition], is_oauth: bool) -> Vec<ApiTool> {
        daanio_provider_anthropic::format_tools(tools, is_oauth, is_cache_ttl_1h())
    }
}
...
fn log_anthropic_canonical_input(
    ...
    daanio_provider_core::fingerprint::log_provider_canonical_input(
        "anthropic",
        model,
        format,
        &payload,
...
impl Provider for AnthropicProvider {
    ...
    fn credential_mode(&self) -> daanio_provider_core::CredentialMode {
        self.credential_mode_snapshot()
    }

    fn set_credential_mode(&self, mode: daanio_provider_core::CredentialMode) -> Result<()> {
    ...
    fn set_model(&self, model: &str) -> Result<()> {
        ...
        let model: &str = if is_1m_model(model)
            && matches!(
                daanio_provider_core::anthropic_context_mode(model),
                daanio_provider_core::AnthropicContextMode::Native1M
            ) {
            strip_1m_suffix(model)
        } else {
            model
        };
        if !daanio_base::provider::known_anthropic_model_ids()
            .iter()
    ...
    fn available_models_for_switching(&self) -> Vec<String> {
        daanio_base::provider::cached_anthropic_model_ids()
            .unwrap_or_else(daanio_base::provider::known_anthropic_model_ids)
    }
    ...
    async fn prefetch_models(&self) -> Result<()> {
        ...
        let catalog = if is_oauth {
            match daanio_base::provider::fetch_anthropic_model_catalog_oauth(&token).await {
                ...
                Err(err) if is_oauth_catalog_auth_error(&err.to_string()) => {
                    ...
                    daanio_base::provider::fetch_anthropic_model_catalog_oauth(&refreshed_token)
                        .await
                }
                Err(err) => Err(err),
            }
        ...
        daanio_base::provider::persist_anthropic_model_catalog(&catalog);
        if !catalog.context_limits.is_empty() {
            daanio_base::provider::populate_context_limits(catalog.context_limits);
        }
        if !catalog.available_models.is_empty() {
            daanio_base::provider::populate_anthropic_models(catalog.available_models);
        }
        Ok(())
    }
...
async fn run_stream_with_retries(
    ...
    for attempt in 0..MAX_RETRIES {
        if attempt > 0 {
            ...
            let delay = daanio_provider_core::retry_after::retry_delay(
                attempt,
                RETRY_BASE_DELAY_MS,
            ...
            let _ = tx
                .send(Ok(StreamEvent::ConnectionPhase {
                    phase: daanio_message_types::ConnectionPhase::Retrying {
                        attempt: attempt + 1,
                        max: MAX_RETRIES,
                    },
                }))
        ...
        let (attempt_tx, attempt_guard) =
            daanio_provider_core::attempt_tracker::track_attempt_output(tx.clone());
        ...
        let attempt_client = if attempt == 0 {
            ...
            daanio_provider_core::fresh_transport_client()
        };
        ...
        .await
        {
            ...
            Err(e) => {
                ...
                if is_oauth && is_oauth_auth_error(&error_str) && !attempted_forced_refresh {
                    ...
                    let _ = tx
                        .send(Ok(StreamEvent::ConnectionPhase {
                            phase: daanio_message_types::ConnectionPhase::Authenticating,
                        }))
                        .await;
                        ...
                        Err(refresh_err) => {
                            let _ = tx
                                .send(Err(anyhow::anyhow!(
                                    "{}\n\nAutomatic Claude OAuth refresh failed: {}\nRun `daanio login --provider claude` (preferred) or `claude`, then retry.",
                                    e,
                                    refresh_err
                                )))
                                .await;
                ...
                if is_retryable_error(&error_str) && attempt + 1 < MAX_RETRIES {
                    if saw_output {
                        ...
                        daanio_base::logging::info(&format!("Transient error, will retry: {}", e));
                    }
                    next_retry_delay =
                        daanio_provider_core::retry_after::retry_after_from_error(&e);
                    last_error = Some(e);
                    continue;
                }
                ...
                if is_oauth && is_oauth_auth_error(&error_str) {
                    let _ = tx
                        .send(Err(anyhow::anyhow!(
                            "{}\n\nClaude OAuth authentication failed. Run `daanio login --provider claude` (preferred) or `claude`, then retry.",
                            e
                        )))
                        .await;
                } else {
...
async fn stream_response(
    ...
    use daanio_message_types::ConnectionPhase;
    let requested_model_base = strip_1m_suffix(&request.model).to_ascii_lowercase();
    if std::env::var("DAANIO_ANTHROPIC_DEBUG")
        .map(|v| v == "1")
        .unwrap_or(false)
        && let Ok(json) = serde_json::to_string_pretty(&request)
    {
        daanio_base::logging::info(&format!("Anthropic request payload:\n{}", json));
    }
    ...
    if !response.status().is_success() {
        ...
        let retry_after = daanio_provider_core::retry_after::retry_after(response.headers());
        let error_text = daanio_base::util::http_error_body(response, "HTTP error").await;
        return Err(daanio_provider_core::retry_after::error_with_retry_after(
    ...
    // `[provider] stream_idle_timeout_secs` / `DAANIO_STREAM_IDLE_TIMEOUT_SECS`
    // so slow reasoning models don't trip a premature timeout (issue #434).
    let sse_chunk_timeout = daanio_base::provider::stream_idle_timeout();
...
fn is_retryable_error(error_str: &str) -> bool {
    daanio_provider_core::is_transient_transport_error(error_str)
        // Server errors (5xx)
...
fn anthropic_model_quality_rank(model: &str) -> usize {
    ...
    let normalized = daanio_provider_core::model_id::strip_date_suffix(
        &daanio_provider_core::model_id::canonical(model),
    )
    .to_string();
    daanio_provider_core::ALL_CLAUDE_MODELS
        .iter()
        .position(|candidate| {
            daanio_provider_core::model_id::strip_date_suffix(
                &daanio_provider_core::model_id::canonical(candidate),
            ) == normalized
        })
        ...
        // available.
        .unwrap_or(daanio_provider_core::ALL_CLAUDE_MODELS.len())
}
...
fn anthropic_recommended_model_from_error(error_str: &str) -> Option<String> {
    ...
    daanio_base::provider::known_anthropic_model_ids()
        .into_iter()
...
fn parse_sse_event(buffer: &mut String) -> Option<SseEvent> {
    ...
    for line in event_str.lines() {
        ...
        } else if let Some(rest) = daanio_base::util::sse_data_line(line) {
            data = rest.to_string();
        }
    }
    ...
    match event.event_type.as_str() {
        "message_start" => {
            ...
            if let Ok(parsed) = serde_json::from_str::<MessageStartEvent>(&event.data) {
                ...
                // substitution (and surface it under DAANIO_LOG_SERVED_MODEL).
                if let Some(served) = parsed.message.model.as_deref() {
                    daanio_base::logging::info(&format!("Anthropic served model={}", served));
                    if std::env::var("DAANIO_LOG_SERVED_MODEL").is_ok() {
                ...
                if let Some(usage) = parsed.message.usage {
                    ...
                    if let Some(tier) = usage.service_tier.as_deref() {
                        ...
                        if std::env::var("DAANIO_LOG_SERVICE_TIER").is_ok() {
        ...
        "ping" => {
            ...
            events.push(StreamEvent::ConnectionPhase {
                phase: daanio_message_types::ConnectionPhase::Streaming,
            });
        }
        "error" => {
            daanio_base::logging::error(&format!("Anthropic stream error: {}", event.data));
...
fn build_system_param(system: &str, is_oauth: bool) -> Option<ApiSystem> {
    daanio_provider_anthropic::build_system_param(system, is_oauth, is_cache_ttl_1h())
}
...
fn build_system_param_split(
    ...
    daanio_provider_anthropic::build_system_param_split(
        static_part,
        dynamic_part,
        is_oauth,
        is_cache_ttl_1h(),
...
fn format_messages_with_identity(messages: Vec<ApiMessage>, is_oauth: bool) -> Vec<ApiMessage> {
    daanio_provider_anthropic::format_messages_with_identity(messages, is_oauth, is_cache_ttl_1h())
}

#[cfg(test)]
fn add_message_cache_breakpoint(messages: &mut [ApiMessage]) {
    daanio_provider_anthropic::add_message_cache_breakpoint(messages, is_cache_ttl_1h())
}
