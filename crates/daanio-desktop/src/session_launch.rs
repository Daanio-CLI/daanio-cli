...
pub(super) fn default_desktop_working_dir() -> Option<PathBuf> {
    if let Ok(raw) = std::env::var("DAANIO_DESKTOP_WORKING_DIR") {
        let path = PathBuf::from(raw);
        if is_usable_directory(&path) {
            return Some(path);
        }
        crate::desktop_log::warn(format_args!(
            "daanio-desktop: ignoring DAANIO_DESKTOP_WORKING_DIR because it is not a directory: {}",
            path.display()
        ));
    }
...
pub fn launch_resume_session(session_id: &str, title: &str) -> Result<()> {
    let title = format!("daanio · {}", compact_title(title));
    let candidates = terminal_candidates(&title, &["--resume", session_id]);
    launch_first_available_terminal(candidates, &format!("daanio --resume {session_id}"))
}

pub fn launch_new_session() -> Result<()> {
    let candidates = terminal_candidates("daanio · new session", &["--fresh-spawn"]);
    launch_first_available_terminal(candidates, "daanio")
}

pub fn launch_selfdev_session() -> Result<()> {
    let candidates = terminal_candidates("daanio · self-dev", &["self-dev"]);
    launch_first_available_terminal(candidates, "daanio self-dev")
}
...
pub fn launch_home_session() -> Result<()> {
    ...
    let candidates =
        terminal::terminal_candidates_in_dir("daanio · home", &["--fresh-spawn"], home.as_path());
    launch_first_available_terminal(candidates, "daanio in home directory")
}
...
pub fn send_message_to_session(session_id: &str, _title: &str, message: &str) -> Result<()> {
    ...
    spawn_bounded_desktop_session_worker("daanio-desktop-workspace-message", move || {
        let (_command_tx, command_rx) = mpsc::channel();
        if let Err(error) =
            run_server_session(Some(&session_id), &message, Vec::new(), None, command_rx)
        {
            crate::desktop_log::error(format_args!(
                "daanio-desktop: workspace server message failed session_id={session_id}: {error:#}"
            ));
        }
    })
...
pub fn spawn_fresh_server_session(
    ...
    spawn_bounded_desktop_session_worker("daanio-desktop-fresh-session", move || {
        if let Err(error) =
            run_server_session(None, &message, images, Some(event_tx.clone()), command_rx)
        {
            crate::desktop_log::error(format_args!(
                "daanio-desktop: fresh server session failed: {error:#}"
            ));
...
pub fn spawn_message_to_session(
    ...
    let (command_tx, command_rx) = mpsc::channel();
    let handle = DesktopSessionHandle { command_tx };
    spawn_bounded_desktop_session_worker("daanio-desktop-session-message", move || {
        if let Err(error) = run_server_session(
            ...
            crate::desktop_log::error(format_args!(
                "daanio-desktop: server session message failed session_id={session_id}: {error:#}"
            ));
            send_desktop_event_ref(
                Some(&event_tx),
                DesktopSessionEvent::Error(format!("{error:#}")),
...
pub fn spawn_cycle_model(
    ...
    spawn_bounded_desktop_session_worker("daanio-desktop-cycle-model", move || {
            if let Err(error) = cycle_model(
                ...
                crate::desktop_log::error(format_args!(
                    "daanio-desktop: model cycle failed direction={direction} target_session={}: {error:#}",
...
pub fn spawn_load_model_catalog(
    ...
    spawn_bounded_desktop_session_worker("daanio-desktop-load-model-catalog", move || {
        if let Err(error) = load_model_catalog(target_session_id.as_deref(), Some(event_tx.clone()))
        {
            crate::desktop_log::error(format_args!(
                "daanio-desktop: model catalog load failed target_session={}: {error:#}",
                target_session_id.as_deref().unwrap_or("<current>")
            ));
            send_desktop_event_ref(
                Some(&event_tx),
...
pub fn spawn_set_model(
    ...
    spawn_bounded_desktop_session_worker("daanio-desktop-set-model", move || {
        if let Err(error) = set_model(&model, target_session_id.as_deref(), Some(event_tx.clone()))
        {
            crate::desktop_log::error(format_args!(
                "daanio-desktop: set model failed model={} target_session={}: {error:#}",
...
pub fn spawn_set_compaction_mode(
    ...
    spawn_control_request(
        "daanio-desktop-set-compaction-mode",
        target_session_id,
        event_tx,
...
pub fn spawn_compact_session(
    ...
    spawn_control_request(
        "daanio-desktop-compact-session",
        target_session_id,
        event_tx,
    ...
    F: FnOnce(u64) -> serde_json::Value + Send + 'static,
{
    spawn_bounded_desktop_session_worker(worker_name, move || {
        if let Err(error) = run_control_request(
            ...
            crate::desktop_log::error(format_args!(
                "daanio-desktop: {action_label} failed target_session={}: {error:#}",
                target_session_id.as_deref().unwrap_or("<current>")
            ));
            send_desktop_event_ref(
                Some(&event_tx),
...
fn run_server_session(
    ...
    loop {
        ...
        if reconnected_session_id != session_id {
            anyhow::bail!(
                "daanio server reconnected to unexpected session id: expected {session_id}, got {reconnected_session_id}"
            );
        }
        send_desktop_event(
            &event_tx,
...
pub(super) fn send_desktop_event_ref(
    ...
    if let Some(event_tx) = event_tx {
        ...
        if event_tx.send(event).is_err() {
            crate::desktop_log::warn(format_args!(
                "daanio-desktop: failed to deliver backend event {event_kind}, receiver is closed"
            ));
        }
    }
}
...
pub(super) fn socket_path() -> PathBuf {
    if let Ok(custom) = std::env::var("DAANIO_SOCKET") {
        return PathBuf::from(custom);
    }
    if let Ok(dir) = std::env::var("DAANIO_RUNTIME_DIR") {
        return PathBuf::from(dir).join("daanio.sock");
    }
    if let Ok(dir) = std::env::var("XDG_RUNTIME_DIR") {
        return PathBuf::from(dir).join("daanio.sock");
    }
    std::env::temp_dir()
        .join(format!("daanio-{}", runtime_user_discriminator()))
        .join("daanio.sock")
}
