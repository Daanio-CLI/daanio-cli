...
const DESKTOP_RELOAD_WINDOW_ENV: &str = "DAANIO_DESKTOP_RELOAD_WINDOW";
const DESKTOP_RELOAD_HANDOFF_READY_ENV: &str = "DAANIO_DESKTOP_RELOAD_READY_FILE";
const DESKTOP_RELOAD_HANDOFF_RELEASE_ENV: &str = "DAANIO_DESKTOP_RELOAD_RELEASE_FILE";
const DESKTOP_RELOAD_HANDOFF_PLACEMENT_ENV: &str = "DAANIO_DESKTOP_RELOAD_PLACEMENT_FILE";
...
fn main() {
    ...
    desktop_log::info(format_args!(
        "daanio-desktop: starting pid={} version={} build_hash={}",
        std::process::id(),
    ...
    if let Err(error) = pollster::block_on(run()) {
        desktop_log::error(format_args!("daanio-desktop: fatal error: {error:#}"));
        std::process::exit(1);
    }
}
...
fn install_desktop_diagnostic_hooks() {
    ...
    std::panic::set_hook(Box::new(move |panic_info| {
        desktop_log::error(format_args!("daanio-desktop: panic: {panic_info}"));
        desktop_log::error(format_args!(
            "daanio-desktop: panic backtrace: {}",
        ...
        match event {
                    ...
                    if event.state == ElementState::Pressed =>
                {
                    ...
                    match app.handle_key(key_input) {
                        ...
                        KeyOutcome::OpenSession { session_id, title } => {
                            ...
                            {
                                desktop_log::error(format_args!(
                                    "daanio-desktop: failed to open session {session_id}: {error:#}"
                                ));
                            }
                        }
                        KeyOutcome::SpawnSession => {
                            ...
                            if let Err(error) = session_launch::launch_new_session() {
                                desktop_log::error(format_args!(
                                    "daanio-desktop: failed to spawn session: {error:#}"
                                ));
                            } else {
                                spawn_session_cards_load(
                                    DesktopSessionCardsPurpose::WorkspaceRefresh,
                        ...
                        KeyOutcome::SpawnSelfDevSession => {
                            if let Err(error) = session_launch::launch_selfdev_session() {
                                desktop_log::error(format_args!(
                                    "daanio-desktop: failed to spawn self-dev session: {error:#}"
                                ));
                            }
                        }
                        KeyOutcome::SpawnHomeSession => {
                            if let Err(error) = session_launch::launch_home_session() {
                                desktop_log::error(format_args!(
                                    "daanio-desktop: failed to spawn home session: {error:#}"
                                ));
                            }
                        }
                            ...
                            if app.is_single_session() {
                                ...
                                match session_launch::spawn_message_to_session(
                                    ...
                                    Err(error) => desktop_log::error(format_args!(
                                        "daanio-desktop: failed to send image draft to {session_id}: {error:#}"
                                    )),
                                }
                                ...
                                desktop_log::error(format_args!(
                                    "daanio-desktop: failed to send draft to {session_id}: {error:#}"
                                ));
                            } else {
                                spawn_session_cards_load(
                                    DesktopSessionCardsPurpose::WorkspaceRefresh,
                    ...
                    Err(SurfaceError::Timeout) => {
                        ...
                        if consecutive_timeouts == 1 || delay == SURFACE_TIMEOUT_BACKOFF_MAX {
                            desktop_log::warn(format_args!(
                                "daanio-desktop: surface acquire timed out, retrying in {}ms after {} consecutive timeout(s)",
                                delay.as_millis(),
            ...
            Event::UserEvent(DesktopUserEvent::CanvasReady(result)) => {
                ...
                match canvas {
                    ...
                    Err(message) => {
                        desktop_log::error(format_args!(
                            "daanio-desktop: failed to initialize desktop renderer: {message}"
                        ));
            ...
            Event::UserEvent(DesktopUserEvent::TranscriptHydrated {
                ...
                if app.apply_hydrated_transcript(&session_id, result) {
                    desktop_log::info(format_args!(
                        "daanio-desktop: hydrated resumed transcript for {session_id} in {}ms",
                        loaded_in.as_millis()
                    ));
                    window.set_title(&app.status_title());
...
impl DesktopHostRendererState {
    fn start_gpu_init(
        ...
        std::thread::Builder::new()
            .name("daanio-desktop-gpu-init".to_string())
            .spawn(move || {
                ...
                {
                    desktop_log::warn(format_args!(
                        "daanio-desktop: failed to deliver async canvas initialization result"
                    ));
                }
            })
...
fn load_desktop_preferences() -> Option<workspace::DesktopPreferences> {
    match desktop_prefs::load_preferences() {
        ...
        Err(error) => {
            desktop_log::error(format_args!(
                "daanio-desktop: failed to load desktop preferences: {error:#}"
            ));
            None
        }
    }
...
fn initial_single_session_app(resume_session_id: Option<&str>) -> DesktopApp {
    ...
    match session_data::load_session_card_by_id(session_id) {
        ...
        Err(error) => {
            desktop_log::error(format_args!(
                "daanio-desktop: failed to load resumed session metadata for {session_id}: {error:#}"
            ));
            app.set_status_label(format!("resumed session {session_id}"));
...
fn apply_single_session_error(app: &mut DesktopApp, error: anyhow::Error) {
    desktop_log::error(format_args!("daanio-desktop: UI action failed: {error:#}"));
...
fn desktop_build_hash_label() -> &'static str {
    option_env!("DAANIO_DESKTOP_GIT_HASH").unwrap_or("unknown")
}
