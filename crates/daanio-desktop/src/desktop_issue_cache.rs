            ...
            match fetch_issue_comments(repo, number, runner) {
                ...
                Err(error) => {
                    ...
                    desktop_log::warn(format_args!(
                        "daanio-desktop: failed to refresh comments for GitHub issue {repo}#{number}: {error:#}"
                    ));
                    existing_comments
                }
            }
...
pub(crate) fn issue_cache_root() -> PathBuf {
    if let Some(path) = std::env::var_os("DAANIO_DESKTOP_ISSUE_CACHE_DIR") {
        return PathBuf::from(path);
    }
    daanio_data_dir().join("desktop/github/issues")
}

fn daanio_data_dir() -> PathBuf {
    if let Some(path) = std::env::var_os("DAANIO_HOME") {
        return PathBuf::from(path);
    }
    if let Some(home) = std::env::var_os("HOME") {
        return PathBuf::from(home).join(".daanio");
    }
    PathBuf::from(".daanio")
}
...
mod tests {
    ...
    fn unique_issue_cache_temp_root(name: &str) -> PathBuf {
        ...
        std::env::temp_dir().join(format!(
            "daanio-desktop-issue-cache-{name}-{}-{nanos}",
            std::process::id()
        ))
    }
    ...
    fn parses_common_github_remote_urls() {
        assert_eq!(
            parse_github_repo_from_remote("git@github.com:Daanio-CLI/daanio-cli.git").as_deref(),
            Some("Daanio-CLI/daanio-cli")
        );
        assert_eq!(
