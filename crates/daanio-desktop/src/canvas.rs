...
impl Canvas {
        ...
        let (device, queue) = adapter
            .request_device(
                &wgpu::DeviceDescriptor {
                    label: Some("daanio-desktop-device"),
    ...
    pub(crate) fn suspend_for_zero_size(&mut self, size: PhysicalSize<u32>) {
        if !self.surface_zero_sized {
            desktop_log::info(format_args!(
                "daanio-desktop: suspending surface rendering while window is zero-sized ({}x{})",
    ...
    pub(crate) fn resize(&mut self, size: PhysicalSize<u32>) {
        ...
        if was_zero_sized {
            desktop_log::info(format_args!(
                "daanio-desktop: resuming surface rendering at {}x{}",
                size.width, size.height
            ));
        }
                ...
                .and_then(|loader| match loader.join() {
                    ...
                    Err(_) => {
                        desktop_log::error(format_args!(
                            "daanio-desktop: font system loader thread panicked"
                        ));
                        None
                    }
                })
    ...
    pub(crate) fn ensure_render_pipeline(&mut self) {
        ...
        let shader = self
            ...
            .create_shader_module(wgpu::ShaderModuleDescriptor {
                label: Some("daanio-desktop-primitive-shader"),
                source: wgpu::ShaderSource::Wgsl(SHADER.into()),
            });
        let pipeline_layout = self
            .device
            .create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
                label: Some("daanio-desktop-primitive-pipeline-layout"),
                bind_group_layouts: &[],
                push_constant_ranges: &[],
            });
        self.render_pipeline = Some(self.device.create_render_pipeline(
            &wgpu::RenderPipelineDescriptor {
                label: Some("daanio-desktop-primitive-pipeline"),
    ...
    pub(crate) fn render_boot_frame(
        ...
        let mut encoder = self
            ...
            .create_command_encoder(&wgpu::CommandEncoderDescriptor {
                label: Some("daanio-desktop-boot-frame"),
            });
        frame_profile.checkpoint("frame_setup");
        {
            let _render_pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
                label: Some("daanio-desktop-boot-clear-pass"),
    ...
    pub(crate) fn render_scene(
        ...
        let mut encoder = self
            .device
            .create_command_encoder(&wgpu::CommandEncoderDescriptor {
                label: Some("daanio-desktop-scene-render"),
            });
        frame_profile.checkpoint("frame_setup");
...

        {
            let mut render_pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
                label: Some("daanio-desktop-scene-pass"),
                color_attachments: &[Some(wgpu::RenderPassColorAttachment {
                    view: &view,
                    resolve_target: None,
                    ops: wgpu::Operations {
    ...
    pub(crate) fn render(
        ...
        let mut encoder = self
            .device
            .create_command_encoder(&wgpu::CommandEncoderDescriptor {
                label: Some("daanio-desktop-render-workspace"),
            });
        let now = Instant::now();
        ...
        if self.text_needs_prepare {
            let text_areas = if let DesktopApp::SingleSession(single_session) = app {
                if let Some(viewport) = single_session_viewport.clone() {
                    ...
                    desktop_log::error(format_args!(
                        "daanio-desktop: missing single-session viewport while preparing text"
                    ));
                    Vec::new()
                }
                ...
                match (
                    ...
                    (Some(font_system), Some(text_atlas), Some(text_renderer)) => {
                        if let Err(error) = text_renderer.prepare(
                            ...
                            desktop_log::error(format_args!(
                                "daanio-desktop: failed to prepare text, recreating renderer: {error:?}"
                            ));
                    ...
                    _ => {
                        desktop_log::error(format_args!(
                            "daanio-desktop: text renderer state was incomplete before prepare, recreating renderer"
                        ));
                        self.text_renderer = None;
                        self.text_atlas = None;
                        self.text_needs_prepare = true;
                ...
                match (
                    ...
                    (Some(font_system), Some(text_atlas), Some(text_renderer)) => {
                        if let Err(error) = text_renderer.prepare(
                            ...
                            desktop_log::error(format_args!(
                                "daanio-desktop: failed to prepare streaming text, recreating renderer: {error:?}"
                            ));
                            self.streaming_text_renderer = None;
                            self.streaming_text_atlas = None;
                            self.streaming_text_needs_prepare = true;
                    ...
                    _ => {
                        desktop_log::error(format_args!(
                            "daanio-desktop: streaming text renderer state was incomplete before prepare, recreating renderer"
                        ));
                        self.streaming_text_renderer = None;
                        self.streaming_text_atlas = None;
                        self.streaming_text_needs_prepare = true;
        ...
        {
            let mut render_pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
                label: Some("daanio-desktop-workspace-pass"),
                color_attachments: &[Some(wgpu::RenderPassColorAttachment {
                    view: &view,
                    resolve_target: None,
                    ops: wgpu::Operations {
            ...
            {
                desktop_log::error(format_args!(
                    "daanio-desktop: failed to render text, recreating renderer: {error:?}"
                ));
            ...
            {
                desktop_log::error(format_args!(
                    "daanio-desktop: failed to render streaming text, recreating renderer: {error:?}"
                ));
...
pub(crate) fn upload_primitive_vertices(
    ...
    if primitive_vertex_buffer_should_reallocate(*primitive_vertex_capacity, vertices.len()) {
        ...
        *primitive_vertex_buffer = Some(device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("daanio-desktop-workspace-vertices"),
            size,
