...
-> Result<DesktopReasoningEffortRequestQueue> {
    ...
    std::thread::Builder::new()
        .name("daanio-desktop-effort-queue".to_string())
        .spawn(move || {
    ...
    while let Ok(mut request) = request_rx.recv() {
        ...
        if coalesced > 0 {
            desktop_log::info(format_args!(
                "daanio-desktop: coalesced {coalesced} superseded reasoning effort request(s); applying {}",
    ...
    if still_latest {
        ...
        if let Err(error) = result {
            desktop_log::error(format_args!(
                "daanio-desktop: reasoning effort sync failed generation={} target_session={}: {error:#}",
                request.generation,
        ...
        desktop_log::warn(format_args!(
            "daanio-desktop: stale reasoning effort sync failed generation={} target_session={}: {error:#}",
            request.generation,
            request.target_session_id.as_deref().unwrap_or("<current>")
        ));
    } else {
        ...
        desktop_log::info(format_args!(
            "daanio-desktop: dropped stale reasoning effort response generation={} event_count={dropped}",
            request.generation
        ));
    }
}
