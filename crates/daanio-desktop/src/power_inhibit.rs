...
const MODE_ENV: &str = "DAANIO_DESKTOP_POWER_INHIBIT";

/// Best-effort inhibitor that keeps laptops awake while Daanio is actively
        ...
        match build_inhibit_command(platform).spawn() {
            ...
            Err(error) => {
                crate::desktop_log::error(format_args!(
                    "daanio-desktop: failed to acquire power inhibitor: {error}"
                ));
                self.available = false;
            }
        }
        ...
        if let Some(mut child) = self.child.take() {
            if let Err(error) = child.kill() {
                crate::desktop_log::warn(format_args!(
                    "daanio-desktop: failed to stop power inhibitor process: {error}"
                ));
            }
            if let Err(error) = child.wait() {
                crate::desktop_log::warn(format_args!(
                    "daanio-desktop: failed to reap power inhibitor process: {error}"
                ));
            }
        }
    }
