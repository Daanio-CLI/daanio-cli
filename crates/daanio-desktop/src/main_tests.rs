...
fn desktop_reduced_motion_env_parses_common_flag_values() {
    ...
    assert_eq!(DESKTOP_REDUCED_MOTION_ENV, "DAANIO_DESKTOP_REDUCED_MOTION");
}

#[test]
fn desktop_process_role_parses_internal_flags() {
    assert_eq!(
        desktop_process_role_from_args(["daanio-desktop"].into_iter()),
        DesktopProcessRole::StableHost
    );
    assert_eq!(
        desktop_process_role_from_args(["daanio-desktop", "--desktop-host"].into_iter()),
        DesktopProcessRole::StableHost
    );
    assert_eq!(
        desktop_process_role_from_args(["daanio-desktop", "--desktop-app-worker"].into_iter()),
        DesktopProcessRole::AppWorker
    );
    assert_eq!(
        desktop_process_role_from_args(
            ["daanio-desktop", "--desktop-process-role=stable_host"].into_iter()
        ),
        DesktopProcessRole::StableHost
    );
    assert_eq!(
        desktop_process_role_from_args(
            ["daanio-desktop", "--desktop-process-role", "app-worker"].into_iter()
        ),
        DesktopProcessRole::AppWorker
    );
    assert_eq!(
...
fn desktop_app_worker_relaunch_replaces_existing_process_role() {
    let relaunch = DesktopRelaunch {
        binary: PathBuf::from("/tmp/daanio-desktop"),
        args: vec![
...
fn desktop_hot_reload_rewrites_resume_to_live_session() {
    let relaunch = DesktopRelaunch {
        binary: PathBuf::from("/old/daanio-desktop"),
        args: vec![
    ...
    let updated = relaunch.for_app(&app, PathBuf::from("/new/daanio-desktop"));

    assert_eq!(updated.binary, PathBuf::from("/new/daanio-desktop"));
    assert_eq!(
        updated.args,
        vec![
            OsString::from("--fullscreen"),
...
fn desktop_hot_reload_persists_workspace_focus_before_spawn() -> Result<()> {
    ...
    unsafe {
        std::env::set_var("DAANIO_DESKTOP_STATE", &state_path);
    }
    ...
    unsafe {
        std::env::remove_var("DAANIO_DESKTOP_STATE");
    }
...
fn single_session_issues_slash_toggles_local_issue_browser() {
    ...
    assert!(app.draft.is_empty());
    assert!(app.messages.is_empty());
    assert_eq!(app.side_panel().github_issues.repo, "Daanio-CLI/daanio-cli");
    assert_eq!(
        app.side_panel()
            .github_issues
            .selected_issue()
...
fn single_session_issue_browser_investigate_injects_context() {
    ...
    assert!(message.contains("Repository: Daanio-CLI/daanio-cli"));
...
fn single_session_visual_state_smoke_covers_markdown_spinner_and_switcher() {
    ...
    assert_visual_text_contains(
        &switcher_key,
        "loading recent sessions from ~/.daanio/sessions...",
    );
}
...
fn assistant_inline_code_pills_align_after_unicode_list_markers() {
    ...
    let mut app = SingleSessionApp::new(None);
    app.messages.push(SingleSessionMessage::assistant(
        "- `crates/`: shared modules\n- `daanio-provider-*`: model/provider integrations.",
    ));
...
fn code_block_header_placement_is_stable_across_sizes_and_text_scales() {
    let markdown = "before\n\n```text\ndaanio-desktop native window input uses winit and renders via wgpu\n  indented code stays code\n```\n\nafter";
    let sizes = [
        PhysicalSize::new(520, 420),
        ...
        for scale_step in scale_steps {
            ...
            assert!(
                lines[header_index + 1..]
                    ...
                    .any(|line| line.text.starts_with("  daanio-desktop")),
...
fn single_session_tool_events_expand_context_and_collapse_previous_call() {
    ...
    app.apply_session_event(session_launch::DesktopSessionEvent::ToolInput {
        id: None,
        delta: r#"{"command":"cargo test -p daanio-desktop","timeout":120000,"intent":"Run desktop tests"}"#.to_string(),
    });
    ...
    assert!(body.contains("$ cargo test -p daanio-desktop"));
...
fn single_session_tool_cards_have_native_geometry_and_success_fill() {
    ...
    app.apply_session_event(session_launch::DesktopSessionEvent::ToolInput {
        id: Some("tool-card".to_string()),
        delta: r#"{"command":"cargo check -p daanio-desktop"}"#.to_string(),
    });
    app.apply_session_event(session_launch::DesktopSessionEvent::ToolFinished {
        id: Some("tool-card".to_string()),
        name: "bash".to_string(),
...
fn single_session_hotkey_help_toggles_discoverable_shortcuts() {
    ...
    assert!(help_has_shortcut(
        &help,
        "Super+;",
        "spawn a self-dev daanio session"
    ));
    assert!(help_has_shortcut(
        &help,
        "Super+'",
        "spawn a daanio session in home"
    ));
    assert!(help_has_shortcut(
        &help,
        "Ctrl+Shift+K",
...
fn single_session_session_switcher_renders_tui_style_cards_and_role_preview() {
    ...
    app.apply_session_switcher_cards(vec![workspace::SessionCard {
        ...
        detail: "8 msgs · just now · daanio".to_string(),
...
fn desktop_resume_args_are_parsed() {
    assert_eq!(
        desktop_resume_session_id_from_args(["daanio-desktop", "--resume", "session_beta"]),
        Some("session_beta".to_string())
    );
    assert_eq!(
        desktop_resume_session_id_from_args(["daanio-desktop", "--resume=session_gamma"]),
        Some("session_gamma".to_string())
    );
    assert_eq!(
        desktop_resume_session_id_from_args(["daanio-desktop"]),
        None
    );
}
    ...
    match action {
        ...
        QueueTraceAction::Reloading => {
            app.apply_session_event(session_launch::DesktopSessionEvent::Reloading {
                new_socket: Some("/tmp/daanio-reload-model-test.sock".to_string()),
            });
            None
        }
        QueueTraceAction::Reloaded => {
    ...
    let drained = apply_session_event_batch_then_auto_drain(
        ...
        vec![
            session_launch::DesktopSessionEvent::Reloading {
                new_socket: Some("/tmp/daanio-reload-event-loop-test.sock".to_string()),
            },
...
fn single_session_event_loop_multiple_reloads_preserve_queued_prompt_order() {
    ...
    assert_eq!(
        apply_session_event_batch_then_auto_drain(
            ...
            vec![
                session_launch::DesktopSessionEvent::Reloading {
                    new_socket: Some("/tmp/daanio-reload-one.sock".to_string()),
                },
                session_launch::DesktopSessionEvent::Reloaded {
                    session_id: "reload-model-session".to_string(),
                },
    ...
    assert_eq!(
        apply_session_event_batch_then_auto_drain(
            ...
            vec![
                session_launch::DesktopSessionEvent::Reloading {
                    new_socket: Some("/tmp/daanio-reload-two.sock".to_string()),
                },
                session_launch::DesktopSessionEvent::Reloaded {
                    session_id: "reload-model-session".to_string(),
                },
...
fn single_session_reload_queue_state_space_matches_reference_model() {
    ...
    // test profile; depth 5 is ~111k traces and made `cargo test -p daanio-desktop`
...
fn headless_chat_smoke_message_parses_hidden_flag() {
    ...
    assert_eq!(
        headless_chat_smoke_message(&["daanio-desktop".to_string()]),
        None
    );
}
...
fn desktop_startup_flags_enable_logging_and_benchmark_mode() {
    let args = vec!["daanio-desktop".to_string(), "--startup-log".to_string()];
    ...
    assert!(startup_benchmark_requested(&args));
    assert!(!startup_log_requested(&["daanio-desktop".to_string()]));
...
fn single_session_reload_event_keeps_worker_state_processing() {
    let mut app = SingleSessionApp::new(None);
    app.apply_session_event(session_launch::DesktopSessionEvent::Reloading {
        new_socket: Some("/tmp/daanio-reload.sock".to_string()),
    });
...
fn single_session_without_session_is_native_fresh_draft() {
    ...
    assert_eq!(app.status_title(), "Daanio Desktop (Beta) · fresh session");
...
fn desktop_mode_defaults_to_single_session_and_gates_workspace_prototype() {
    assert_eq!(
        desktop_mode_from_args(["daanio-desktop"]),
        DesktopMode::SingleSession
    );
    assert_eq!(
        desktop_mode_from_args(["daanio-desktop", "--workspace"]),
