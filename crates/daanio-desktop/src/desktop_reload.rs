...
impl DesktopReloadStartup {
    pub(crate) fn from_env() -> Self {
        ...
        let window_placement = raw_window_placement.as_deref().and_then(|raw| {
            ...
            if placement.is_none() {
                desktop_log::warn(format_args!(
                    "daanio-desktop: ignoring invalid reload window placement {raw:?}"
                ));
            }
            placement
        });
        let handoff = match (ready_file, release_file) {
            ...
            _ => {
                desktop_log::warn(format_args!(
                    "daanio-desktop: ignoring incomplete reload handoff environment"
                ));
                None
            }
        };
...
impl DesktopReloadStartupHandoff {
    pub(crate) fn signal_ready_and_wait_for_release(&self) -> Option<DesktopReloadWindowPlacement> {
        if let Err(error) = write_desktop_reload_marker(&self.ready_file) {
            desktop_log::warn(format_args!(
                "daanio-desktop: failed to signal reload readiness: {error:#}"
            ));
            return None;
        }

        desktop_log::info(format_args!(
            "daanio-desktop: reload child ready, waiting for parent release"
        ));
        ...
        desktop_log::warn(format_args!(
            "daanio-desktop: reload parent did not release handoff within {}ms; showing replacement window anyway",
    ...
    pub(crate) fn poll_with_placement(
        ...
        if self.ready_file.exists() {
            if let Some(placement) = placement {
                ...
                {
                    desktop_log::warn(format_args!(
                        "daanio-desktop: failed to synchronize final reload window placement: {error:#}"
                    ));
                }
            }
...
pub(crate) fn cleanup_desktop_reload_handoff_files(ready_file: &Path, release_file: &Path) {
    ...
    if ready_file.parent() == release_file.parent()
        ...
        && parent
            ...
            .is_some_and(|name| name.starts_with("daanio-desktop-reload-"))
    {
...
impl DesktopHotReloader {
    ...
    pub(crate) fn drain_app_worker_messages(&mut self) -> DesktopWorkerDrain {
        ...
        while let Some(message) = worker.try_recv() {
            match message {
                Ok(DesktopWorkerToHostMessage::Ready(ready)) => {
                    desktop_log::info(format_args!(
                        "daanio-desktop: app worker ready; pid={} mode={:?}",
                ...
                Ok(DesktopWorkerToHostMessage::Snapshot(snapshot)) => {
                    desktop_log::info(format_args!(
                        "daanio-desktop: app worker snapshot response {}; mode={}",
                        snapshot.request_id, snapshot.snapshot.mode
                    ));
                }
                Ok(DesktopWorkerToHostMessage::Metrics(metrics)) => {
                    desktop_log::info(format_args!(
                        "daanio-desktop: app worker reported {} metric(s)",
                        metrics.metrics.len()
                    ));
                }
                Ok(DesktopWorkerToHostMessage::Log(log)) => {
                    desktop_log::info(format_args!(
                        "daanio-desktop: app worker log {:?}: {}",
                        log.level, log.message
                    ));
                }
                Ok(DesktopWorkerToHostMessage::Exited(exit)) => {
                    desktop_log::warn(format_args!(
                        "daanio-desktop: app worker exited code={:?} reason={:?}",
                        exit.code, exit.reason
                    ));
                }
                Err(error) => {
                    desktop_log::error(format_args!(
                        "daanio-desktop: failed to read app worker message: {error:#}"
                    ));
        ...
        if !should_drop_worker {
            match worker.try_wait() {
                Ok(Some(status)) => {
                    desktop_log::warn(format_args!(
                        "daanio-desktop: app worker process exited unexpectedly: {status}"
                    ));
                    should_drop_worker = true;
                }
                Ok(None) => {}
                Err(error) => {
                    desktop_log::warn(format_args!(
                        "daanio-desktop: failed to poll app worker process: {error:#}"
                    ));
                    should_drop_worker = true;
                }
            }
        ...
        {
            desktop_log::warn(format_args!(
                "daanio-desktop: failed to clean up stopped app worker: {error:#}"
            ));
        }
        drained
    }
    ...
    pub(crate) fn start_app_worker_for_current_binary(
        ...
        let Some(relaunch) = self.relaunch.clone() else {
            desktop_log::warn(format_args!(
                "daanio-desktop: cannot start app worker for {reason}; current process cannot be relaunched"
            ));
            return;
        };
    ...
    pub(crate) fn force_reload(&mut self, app: &DesktopApp, window: &Window) -> bool {
        ...
        if self.pending_handoff.is_some() {
            desktop_log::warn(format_args!(
                "daanio-desktop: force reload requested while another reload handoff is pending"
            ));
            return false;
        }
        let Some(relaunch) = self.relaunch.clone() else {
            desktop_log::warn(format_args!(
                "daanio-desktop: force reload requested but current process cannot be relaunched"
            ));
            return false;
        };
        let binary = desktop_reload_binary_candidate(&relaunch.binary);
    ...
    pub(crate) fn reload_with_strategy(
        ...
        match self.strategy {
            ...
            DesktopReloadStrategy::AppWorkerRestart => {
                desktop_log::info(format_args!(
                    "daanio-desktop: {reason} requested app-worker restart; keeping stable host window alive"
                ));
                self.restart_app_worker(app, window, relaunch, binary, reason);
                false
            }
    ...
    pub(crate) fn restart_app_worker(
        ...
        {
            desktop_log::warn(format_args!(
                "daanio-desktop: failed to stop previous app worker before {reason}: {error:#}"
            ));
        }
        ...
        match worker_relaunch.spawn_app_worker(self.event_loop_proxy.clone()) {
            Ok(mut worker) => {
                ...
                {
                    desktop_log::error(format_args!(
                        "daanio-desktop: failed to initialize app worker for {reason}: {error:#}"
                    ));
                    if let Err(kill_error) = worker.kill() {
                        desktop_log::warn(format_args!(
                            "daanio-desktop: failed to kill uninitialized app worker: {kill_error:#}"
                        ));
                    }
                    return;
                }
                desktop_log::info(format_args!(
                    "daanio-desktop: app worker restarted for {reason}; pid={}",
                    worker.child_id()
                ));
                self.app_worker = Some(worker);
            }
            Err(error) => desktop_log::error(format_args!(
                "daanio-desktop: failed to restart app worker for {reason}: {error:#}"
            )),
        }
    }
    ...
    pub(crate) fn reload_full_process_handoff(
        ...
        match relaunch.spawn_for_window(window) {
            ...
            Err(error) => {
                desktop_log::error(format_args!(
                    "daanio-desktop: failed to {reason} desktop: {error:#}"
                ));
                false
            }
        }
    ...
    pub(crate) fn poll_pending_handoff(&mut self, window: &Window) -> bool {
        ...
        match pending_handoff.poll_with_placement(DesktopReloadWindowPlacement::from_window(window))
        {
            ...
            Ok(DesktopReloadHandoffPoll::Ready) => {
                desktop_log::info(format_args!(
                    "daanio-desktop: reload replacement is ready; exiting old process"
                ));
                true
            }
            Ok(DesktopReloadHandoffPoll::TimedOut) => {
                desktop_log::warn(format_args!(
                    "daanio-desktop: reload replacement did not become ready within {}ms; keeping old process alive",
            ...
            Err(error) => {
                desktop_log::error(format_args!(
                    "daanio-desktop: failed to release reload replacement: {error:#}"
                ));
                true
            }
        }
...
impl DesktopRelaunch {
    ...
    pub(crate) fn spawn_for_window(
        ...
        let handoff = match DesktopReloadHandoff::new(window) {
            ...
            Err(error) => {
                desktop_log::warn(format_args!(
                    "daanio-desktop: reload handoff unavailable, falling back to immediate relaunch: {error:#}"
                ));
                None
            }
        };
        desktop_log::info(format_args!(
            "daanio-desktop: hot reloading into {} with args {:?}{}",
            self.binary.display(),
            self.args,
            if handoff.is_some() {
                " using handoff"
    ...
    pub(crate) fn for_app(&self, app: &DesktopApp, binary: PathBuf) -> Self {
        ...
        {
            desktop_log::error(format_args!(
                "daanio-desktop: failed to persist workspace state before hot reload: {error:#}"
            ));
        }
    ...
    fn spawn_app_worker(
        ...
        desktop_log::info(format_args!(
            "daanio-desktop: spawning app worker {} with args {:?}",
            self.binary.display(),
            self.args
        ));
        let mut command = Command::new(&self.binary);
    ...
    let has_mode_arg = args.iter().any(|arg| {
        arg == "--workspace"
            ...
            || arg.to_str().is_some_and(|value| {
                value.starts_with("--resume=") || value.starts_with("daanio://")
            })
    });
    if !has_mode_arg {
...
pub(crate) fn find_desktop_repo_in_ancestors(start: &Path) -> Option<PathBuf> {
    start
        .ancestors()
        .find(|candidate| is_daanio_desktop_repo(candidate))
        .map(Path::to_path_buf)
}

pub(crate) fn is_daanio_desktop_repo(candidate: &Path) -> bool {
    if !candidate.join("crates/daanio-desktop/Cargo.toml").is_file() {
        return false;
    }
    std::fs::read_to_string(candidate.join("Cargo.toml"))
        .map(|cargo_toml| cargo_toml.contains("name = \"daanio\""))
        .unwrap_or(false)
}
