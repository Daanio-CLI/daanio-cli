...
fn desktop_event_parser_maps_streaming_server_events() {
    ...
    assert_eq!(
        desktop_event_from_server_value(&json!({
            "type": "reloading",
            "new_socket": "/tmp/daanio-new.sock"
        })),
        Some(DesktopSessionEvent::Reloading {
            new_socket: Some("/tmp/daanio-new.sock".to_string())
        })
    );
    assert_eq!(
        desktop_event_from_server_value(&json!({
...
fn desktop_worker_roundtrips_message_with_fake_server() -> Result<()> {
    ...
    let socket_path = std::env::temp_dir().join(format!(
        "daanio-desktop-worker-smoke-{}-{}.sock",
        std::process::id(),
    ...
    let previous_socket = std::env::var_os("DAANIO_SOCKET");
    unsafe {
        std::env::set_var("DAANIO_SOCKET", &socket_path);
    }
    ...
    restore_env_var("DAANIO_SOCKET", previous_socket);
    let _ = std::fs::remove_file(&socket_path);
...
fn desktop_worker_emits_reloaded_before_real_done_after_fake_reload() -> Result<()> {
    ...
    let nonce = std::time::SystemTime::now()
        ...
        .as_nanos();
    let socket_path = std::env::temp_dir().join(format!(
        "daanio-desktop-worker-reload-old-{}-{nonce}.sock",
        std::process::id(),
    ));
    let new_socket_path = std::env::temp_dir().join(format!(
        "daanio-desktop-worker-reload-new-{}-{nonce}.sock",
        std::process::id(),
    ));
    let _ = std::fs::remove_file(&socket_path);
...
fn desktop_worker_rejects_reconnect_session_id_mismatch() -> Result<()> {
    ...
    let nonce = std::time::SystemTime::now()
        ...
        .as_nanos();
    let socket_path = std::env::temp_dir().join(format!(
        "daanio-desktop-worker-reload-mismatch-old-{}-{nonce}.sock",
        std::process::id(),
    ));
    let new_socket_path = std::env::temp_dir().join(format!(
        "daanio-desktop-worker-reload-mismatch-new-{}-{nonce}.sock",
        std::process::id(),
    ));
    let _ = std::fs::remove_file(&socket_path);
    let _ = std::fs::remove_file(&new_socket_path);
...
fn desktop_worker_rejects_malformed_server_event_lines() -> Result<()> {
    let _guard = ENV_LOCK.lock().unwrap();
    let socket_path = std::env::temp_dir().join(format!(
        "daanio-desktop-worker-malformed-{}-{}.sock",
        std::process::id(),
        std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .unwrap()
    ...
    assert!(
        format!("{error:#}").contains("failed to parse daanio server event"),
        "{error:#}"
    );
    let requests = server.join().unwrap()?;
    assert_eq!(requests[2]["type"], "message");
...
fn validate_reload_socket_path_requires_owned_socket_in_current_directory() -> Result<()> {
    ...
    let current_socket = std::env::temp_dir().join(format!(
        "daanio-desktop-reload-validate-current-{}-{nonce}.sock",
        std::process::id(),
    ));
    let new_socket = std::env::temp_dir().join(format!(
        "daanio-desktop-reload-validate-new-{}-{nonce}.sock",
        std::process::id(),
    ));
    let non_socket = std::env::temp_dir().join(format!(
        "daanio-desktop-reload-validate-file-{}-{nonce}",
        std::process::id(),
    ));
...
fn workspace_send_message_uses_shared_server_instead_of_prompt_argv() -> Result<()> {
    let _guard = ENV_LOCK.lock().unwrap();
    let socket_path = std::env::temp_dir().join(format!(
        "daanio-desktop-workspace-send-{}-{}.sock",
        std::process::id(),
        std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .unwrap()
...
fn desktop_workers_reconnect_independently_across_same_fake_reload() -> Result<()> {
    ...
    let nonce = std::time::SystemTime::now()
        ...
        .as_nanos();
    let socket_path = std::env::temp_dir().join(format!(
        "daanio-desktop-worker-multi-reload-old-{}-{nonce}.sock",
        std::process::id(),
    ));
    let new_socket_path = std::env::temp_dir().join(format!(
        "daanio-desktop-worker-multi-reload-new-{}-{nonce}.sock",
        std::process::id(),
    ));
    let _ = std::fs::remove_file(&socket_path);
    let _ = std::fs::remove_file(&new_socket_path);
