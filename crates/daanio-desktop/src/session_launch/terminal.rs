...
pub(super) fn terminal_candidates(title: &str, daanio_args: &[&str]) -> Vec<Command> {
...
pub(super) fn terminal_candidates_in_dir(
    ...
    terminal_candidates_with_working_dir(title, daanio_args, Some(working_dir))
}
...
fn terminal_candidates_with_working_dir(
    ...
    if let Ok(raw_terminal) = std::env::var("DAANIO_DESKTOP_TERMINAL") {
        match terminal_env_command(&raw_terminal, daanio_args) {
            Ok(mut command) => {
                apply_working_dir(&mut command, working_dir);
                candidates.push(command);
            }
            Err(error) => crate::desktop_log::warn(format_args!(
                "daanio-desktop: ignoring invalid DAANIO_DESKTOP_TERMINAL={raw_terminal:?}: {error:#}"
            )),
        }
    }
...
fn terminal_env_command(raw_terminal: &str, daanio_args: &[&str]) -> Result<Command> {
...
pub(super) fn daanio_bin() -> String {
    std::env::var("DAANIO_BIN").unwrap_or_else(|_| "daanio".to_string())
}
...
mod tests {
    ...
    fn parses_terminal_env_command_with_quotes_and_escapes() -> Result<()> {
        assert_eq!(
            parse_terminal_env_command("kitty --title 'Daanio Desktop' --")?,
            vec!["kitty", "--title", "Daanio Desktop", "--"]
        );
        assert_eq!(
            parse_terminal_env_command(r#"footclient -T daanio\ desktop --"#)?,
            vec!["footclient", "-T", "daanio desktop", "--"]
        );
        assert_eq!(
            parse_terminal_env_command(r#"terminal --class "daanio desktop""#)?,
            vec!["terminal", "--class", "daanio desktop"]
        );
        Ok(())
    }
    ...
    #[test]
    fn terminal_env_command_appends_daanio_invocation_without_shell() -> Result<()> {
        let command = terminal_env_command("kitty --title 'Daanio Desktop'", &["--resume", "abc"])?;
        let args = command
            .get_args()
        ...
        assert_eq!(
            args,
            vec!["--title", "Daanio Desktop", "daanio", "--resume", "abc"]
        );
        Ok(())
    }
}
