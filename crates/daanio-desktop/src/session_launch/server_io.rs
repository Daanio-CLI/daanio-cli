...
pub(super) fn ensure_server_running() -> Result<()> {
    ...
    spawn_daanio_server_with_diagnostics()?;
    connect_server_with_retry_path(&path, SERVER_START_TIMEOUT).map(|_| ())
}

fn spawn_daanio_server_with_diagnostics() -> Result<()> {
    let mut command = Command::new(daanio_bin());
    command
        .arg("serve")
        .stdin(Stdio::null())
        .stdout(Stdio::piped())
    ...
    let mut child = command.spawn().context("failed to spawn daanio serve")?;
    ...
    std::thread::Builder::new()
        .name("daanio-desktop-serve-wait".to_string())
        .spawn(move || match child.wait() {
            Ok(status) if status.success() => crate::desktop_log::info(format_args!(
                "daanio-desktop: daanio serve child pid={pid} exited with {status}"
            )),
            Ok(status) => crate::desktop_log::error(format_args!(
                "daanio-desktop: daanio serve child pid={pid} exited with {status}"
            )),
            Err(error) => crate::desktop_log::warn(format_args!(
                "daanio-desktop: failed to wait for daanio serve child pid={pid}: {error}"
            )),
        })
        .context("failed to spawn daanio serve wait logger")?;
    ...
    R: Read + Send + 'static,
{
    if let Err(error) = std::thread::Builder::new()
        .name(format!("daanio-desktop-serve-{stream_name}"))
        .spawn(move || {
            let reader = BufReader::new(pipe);
            for line in reader.lines() {
                match line {
                    Ok(line) => crate::desktop_log::info(format_args!(
                        "daanio-desktop: daanio serve pid={pid} {stream_name}: {}",
                        crate::desktop_log::truncate_for_log(&line, 4096)
                    )),
                    Err(error) => {
                        crate::desktop_log::warn(format_args!(
                            "daanio-desktop: failed reading daanio serve pid={pid} {stream_name}: {error}"
                        ));
                        break;
                    }
                }
    ...
    {
        crate::desktop_log::warn(format_args!(
            "daanio-desktop: failed to spawn daanio serve {stream_name} logger: {error}"
        ));
    }
}
...
pub(super) fn connect_server_with_retry_path(
    ...
    match last_error {
        Some(error) => Err(error).with_context(|| {
            format!(
                "timed out connecting to daanio server at {}",
                socket_path.display()
            )
        }),
        None => anyhow::bail!("timed out connecting to daanio server"),
    }
}
...
pub(super) fn validate_reload_socket_path(
    ...
    if trimmed.is_empty() {
        anyhow::bail!("daanio server advertised an empty reload socket path");
    }

    let new_socket_path = PathBuf::from(trimmed);
    if !new_socket_path.is_absolute() {
        anyhow::bail!(
            "daanio server advertised non-absolute reload socket path {}",
    ...
    if !metadata.file_type().is_socket() {
        anyhow::bail!(
            "daanio server advertised reload path that is not a socket: {}",
            new_socket_path.display()
        );
    }
    ...
    if metadata.uid() != effective_uid {
        anyhow::bail!(
            "daanio server advertised reload socket owned by uid {}, expected uid {}: {}",
            metadata.uid(),
            effective_uid,
            new_socket_path.display()
        );
    ...
    let current_parent = current_socket_path.parent().with_context(|| {
        format!(
            "current daanio socket path has no parent: {}",
    ...
    let current_parent = current_parent.canonicalize().with_context(|| {
        format!(
            "failed to canonicalize current daanio socket directory {}",
    ...
    if current_parent != new_parent {
        anyhow::bail!(
            "daanio server advertised reload socket outside current socket directory: {} (current directory {})",
            new_socket_path.display(),
            current_parent.display()
        );
    }
...
pub(super) fn subscribe_to_server(
    ...
    let selfdev = working_dir
        .as_deref()
        .and_then(|path| path_contains_daanio_repo(path).then_some(true));
    write_json_line(
        writer,
        json!({
            "type": "subscribe",
...
#[cfg(unix)]
fn path_contains_daanio_repo(path: &str) -> bool {
    let mut current = Some(Path::new(path));
    while let Some(path) = current {
        if path.join("crates/daanio-desktop").is_dir() && path.join("Cargo.toml").is_file() {
            return true;
        }
        current = path.parent();
    }
...
pub(super) fn read_session_id_from_events(
    ...
    while started.elapsed() < timeout {
        ...
        match reader.read_line(&mut line) {
            Ok(0) => anyhow::bail!("daanio server disconnected before assigning a session"),
            Ok(_) => {
                let value = parse_server_event_line(&line, "waiting for session id")?;
                if value.get("type").and_then(Value::as_str) == Some("session") {
                    let Some(session_id) = value.get("session_id").and_then(Value::as_str) else {
                        anyhow::bail!("daanio server sent malformed session event");
                    };
                ...
                if value.get("type").and_then(Value::as_str) == Some("error") {
                    ...
                    crate::desktop_log::error(format_args!(
                        "daanio-desktop: daanio server rejected fresh session: {}",
                        crate::desktop_log::truncate_for_log(message, 2048)
                    ));
                    anyhow::bail!("daanio server rejected fresh session: {message}");
                }
            ...
            Err(error) => return Err(error).context("failed to read daanio server event"),
        }
    }

    anyhow::bail!("timed out waiting for daanio server session id")
}
...
pub(super) fn read_session_id_from_state(
    ...
    while started.elapsed() < timeout {
        line.clear();
        match reader.read_line(&mut line) {
            Ok(0) => anyhow::bail!("daanio server disconnected before returning state"),
            Ok(_) => {
                ...
                {
                    let Some(session_id) = value.get("session_id").and_then(Value::as_str) else {
                        anyhow::bail!("daanio server sent malformed state event");
                    };
                    ...
                    && value.get("id").and_then(Value::as_u64) == Some(state_request_id)
                {
                    let message = value
                        ...
                        .unwrap_or("unknown server error");
                    crate::desktop_log::error(format_args!(
                        "daanio-desktop: daanio server rejected state request id={state_request_id}: {}",
                        crate::desktop_log::truncate_for_log(message, 2048)
                    ));
                    anyhow::bail!("daanio server rejected state request: {message}");
                }
            }
            Err(error)
                if matches!(
    ...
    anyhow::bail!("timed out waiting for daanio server state")
}
...
pub(super) fn read_model_changed(
    ...
    while started.elapsed() < timeout {
        line.clear();
        match reader.read_line(&mut line) {
            Ok(0) => anyhow::bail!("daanio server disconnected before switching model"),
            Ok(_) => {
                    ...
                    && value.get("id").and_then(Value::as_u64) == Some(request_id)
                {
                    let message = value
                        ...
                        .unwrap_or("unknown server error");
                    crate::desktop_log::error(format_args!(
                        "daanio-desktop: daanio server rejected model switch id={request_id}: {}",
                        crate::desktop_log::truncate_for_log(message, 2048)
                    ));
                    anyhow::bail!("daanio server rejected model switch: {message}");
                }
            }
            Err(error)
                if matches!(
    ...
    anyhow::bail!("timed out waiting for daanio server model switch")
}
...
pub(super) fn read_model_catalog(
    ...
    while started.elapsed() < timeout {
        line.clear();
        match reader.read_line(&mut line) {
            Ok(0) => anyhow::bail!("daanio server disconnected before loading model catalog"),
            Ok(_) => {
                    ...
                    && value.get("id").and_then(Value::as_u64) == Some(request_id)
                {
                    ...
                    anyhow::bail!("daanio server returned malformed model catalog");
                }
                forward_non_done_server_event(event_tx, &value);
                if value.get("type").and_then(Value::as_str) == Some("error")
                    && value.get("id").and_then(Value::as_u64) == Some(request_id)
                {
                    let message = value
                        ...
                        .unwrap_or("unknown server error");
                    crate::desktop_log::error(format_args!(
                        "daanio-desktop: daanio server rejected model catalog request id={request_id}: {}",
                        crate::desktop_log::truncate_for_log(message, 2048)
                    ));
                    anyhow::bail!("daanio server rejected model catalog request: {message}");
                }
            }
            Err(error)
                if matches!(
    ...
    anyhow::bail!("timed out waiting for daanio server model catalog")
}
...
pub(super) fn read_control_response(
    ...
    while started.elapsed() < timeout {
        line.clear();
        match reader.read_line(&mut line) {
            Ok(0) => anyhow::bail!("daanio server disconnected while {action_label}"),
            Ok(_) => {
                ...
                if event_type == "error" && id_matches {
                    let message = value
                        ...
                        .unwrap_or("unknown server error");
                    crate::desktop_log::error(format_args!(
                        "daanio-desktop: daanio server rejected {action_label} id={request_id}: {}",
                        crate::desktop_log::truncate_for_log(message, 2048)
                    ));
                    anyhow::bail!("daanio server rejected {action_label}: {message}");
                }
            }
            Err(error)
                if matches!(
    ...
    anyhow::bail!("timed out waiting for daanio server while {action_label}")
}
                    ...
                    match event_type {
                        ...
                        Some("error") => {
                            let message = value
                                ...
                                .unwrap_or("unknown server error");
                            anyhow::bail!(
                                "daanio server rejected cancel request {cancel_request_id}: {message}"
                            );
                        }
                        _ => {}
                    }
...
fn parse_server_event_line(line: &str, context: &str) -> Result<Value> {
    match serde_json::from_str::<Value>(line.trim()) {
        ...
        Err(error) => {
            crate::desktop_log::error(format_args!(
                "daanio-desktop: failed to parse daanio server event while {context}: {error}; line={}",
                crate::desktop_log::truncate_for_log(line.trim(), 512)
            ));
            Err(error).context("failed to parse daanio server event")
        }
    }
}
...
fn validate_server_event_value(value: &Value, context: &str) -> Result<()> {
    let Some(object) = value.as_object() else {
        anyhow::bail!("daanio server sent non-object event while {context}");
    };
    let Some(event_type) = object.get("type").and_then(Value::as_str) else {
        anyhow::bail!("daanio server sent event without string type while {context}");
    };
    ...
    match event_type {
        "stdin_request" => {
            ...
            {
                anyhow::bail!(
                    "daanio server sent stdin_request with non-string prompt while {context}"
                );
            }
            if value
                .get("is_password")
            ...
            {
                anyhow::bail!(
                    "daanio server sent stdin_request with non-boolean is_password while {context}"
                );
            }
        }
        "reloading"
        ...
        {
            anyhow::bail!(
                "daanio server sent reloading event with non-string new_socket while {context}"
            );
        }
        _ => {}
    }
...
fn require_non_empty_event_string(
        ...
        .is_some_and(|value| !value.trim().is_empty())
    {
        ...
        anyhow::bail!(
            "daanio server sent {event_type} event without non-empty string {field} while {context}"
        );
    }
}
