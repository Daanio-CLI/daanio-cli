...
fn latest_external_cli_suggestion_uses_newest_candidate_context() {
    ...
    let new = ExternalCliSessionCandidate {
        ...
        working_dir: Some("/home/user/daanio".to_string()),
    ...
    assert_eq!(
        suggestion,
        "continue the latest Codex session in daanio: implement startup continuation suggestions"
    );
}
...
fn bash_tool_rendering_shows_intent_command_and_background_flag() {
    let lines = rendered_tool_text(
        "▾ bash running\n  input: {\"intent\":\"run the desktop tests\",\"command\":\"cargo test -p daanio-desktop\",\"run_in_background\":true}",
        true,
    );
    assert_eq!(
        lines,
        vec![
            "  ● bash · running · $ cargo test -p daanio-desktop · background: yes",
...
fn active_tool_lines_carry_visual_metadata_for_native_cards() {
    let lines = rendered_tool_lines(
        "▾ bash running\n  input: {\"command\":\"cargo test -p daanio-desktop\"}",
        true,
    );
...
fn tool_result_content_renders_inside_inline_widget() {
    ...
    assert_eq!(
        lines[0],
        "  ✕ bash · failed · tests failed · $ cargo test -p daanio-desktop"
    );
    assert_eq!(
        lines[1],
        "    error[E0425]: cannot find value `foo` in this scope"
...
fn inactive_tool_result_compacts_to_metadata_only() {
    ...
    assert_eq!(
        lines,
        vec!["  ✓ bash · done · tests passed · $ cargo test -p daanio-desktop"]
    );
}
