    ...
    match session_data::load_recent_session_cards() {
        ...
        Err(error) => {
            desktop_log::error(format_args!(
                "daanio-desktop: failed to load session metadata: {error:#}"
            ));
            Vec::new()
        }
    }
...
pub(crate) fn load_crashed_session_cards_for_desktop() -> Vec<workspace::SessionCard> {
    match session_data::load_crashed_session_cards() {
        ...
        Err(error) => {
            desktop_log::error(format_args!(
                "daanio-desktop: failed to load crashed session metadata: {error:#}"
            ));
            Vec::new()
        }
    }
...
pub(crate) fn spawn_recovery_session_count_scan(
    ...
    if let Err(error) = spawn_bounded_desktop_async_job("daanio-desktop-recovery-scan", move || {
            ...
            .is_err()
        {
            desktop_log::warn(format_args!(
                "daanio-desktop: failed to deliver recovery count, event loop is closed"
            ));
        }
    }) {
        desktop_log::error(format_args!(
            "daanio-desktop: failed to start recovery scan: {error:#}"
        ));
    }
}
...
pub(crate) fn spawn_single_session_card_refresh(
    ...
    if let Err(error) =
        spawn_bounded_desktop_async_job("daanio-desktop-session-card-refresh", move || {
            ...
            {
                desktop_log::warn(format_args!(
                    "daanio-desktop: failed to deliver session card refresh, event loop is closed"
                ));
            }
        })
    {
        desktop_log::error(format_args!(
            "daanio-desktop: failed to start session card refresh: {error:#}"
        ));
    }
}
...
pub(crate) fn spawn_session_cards_load(
    ...
    if let Err(error) = spawn_bounded_desktop_async_job(
        format!("daanio-desktop-session-cards-{purpose:?}"),
        move || {
            if !delay.is_zero() {
            ...
            {
                desktop_log::warn(format_args!(
                    "daanio-desktop: failed to deliver session cards load, event loop is closed"
                ));
            }
        },
    ) {
        desktop_log::error(format_args!(
            "daanio-desktop: failed to start session card load: {error:#}"
        ));
    }
}
...
pub(crate) fn spawn_restore_crashed_sessions(event_loop_proxy: EventLoopProxy<DesktopUserEvent>) {
    if let Err(error) = spawn_bounded_desktop_async_job(
        "daanio-desktop-restore-crashed-sessions",
        move || {
            let started = Instant::now();
            ...
            {
                desktop_log::warn(format_args!(
                    "daanio-desktop: failed to deliver crashed-session restore result, event loop is closed"
                ));
            }
        },
    ) {
        desktop_log::error(format_args!(
            "daanio-desktop: failed to start crashed-session restore: {error:#}"
        ));
    }
}
...
pub(crate) fn spawn_github_issue_sync(
    ...
    spawn_bounded_desktop_async_job("daanio-desktop-github-issues-sync", move || {
        let result = desktop_issue_cache::sync_current_repo_issue_cache()
            .map_err(|error| format!("{error:#}"));
        match &result {
            Ok(summary) => desktop_log::info(format_args!(
                "daanio-desktop: synced {} GitHub issue(s) for {} in {}ms to {} (comment_threads={} comment_errors={})",
                summary.issue_count,
                summary.repo,
            ...
            Err(error) => desktop_log::warn(format_args!(
                "daanio-desktop: GitHub issue sync failed: {error}"
            )),
        }
        if event_loop_proxy
        ...
        {
            desktop_log::warn(format_args!(
                "daanio-desktop: failed to deliver GitHub issue sync result"
            ));
        }
    })
}
...
pub(crate) fn start_pending_transcript_hydration(
    ...
    let spawned =
        spawn_bounded_desktop_async_job("daanio-desktop-transcript-hydration", move || {
            let started = Instant::now();
            ...
            {
                desktop_log::warn(format_args!(
                    "daanio-desktop: failed to deliver hydrated transcript"
                ));
            }
        });
    if let Err(error) = spawned {
        desktop_log::warn(format_args!(
            "daanio-desktop: transcript hydration fell back to blocking load: {error:#}"
        ));
...
-> Option<mpsc::Sender<workspace::DesktopPreferences>> {
    ...
    match std::thread::Builder::new()
        .name("daanio-desktop-preferences-saver".to_string())
        .spawn(move || {
        ...
        Err(error) => {
            desktop_log::error(format_args!(
                "daanio-desktop: failed to start preferences saver: {error:#}"
            ));
            None
        }
    }
...
pub(crate) fn queue_desktop_preferences_save(
    ...
    if let Err(error) =
        spawn_bounded_desktop_async_job("daanio-desktop-preferences-save-once", move || {
            save_desktop_preferences_off_ui_thread(preferences, 1, Duration::ZERO);
        })
    {
        desktop_log::error(format_args!(
            "daanio-desktop: failed to queue preferences save: {error:#}"
        ));
    }
}
