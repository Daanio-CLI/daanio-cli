...
fn preferences_path() -> Result<PathBuf> {
    if let Ok(path) = std::env::var("DAANIO_DESKTOP_STATE") {
        return Ok(PathBuf::from(path));
    }

    if let Ok(path) = std::env::var("DAANIO_HOME") {
        return Ok(PathBuf::from(path).join("config/daanio/desktop-state.json"));
    }

    if let Ok(path) = std::env::var("XDG_CONFIG_HOME") {
        return Ok(PathBuf::from(path).join("daanio/desktop-state.json"));
    }

    let home = std::env::var_os("HOME")
        .map(PathBuf::from)
        .context("HOME is not set")?;
    Ok(home.join(".config/daanio/desktop-state.json"))
}
...
mod tests {
    ...
    fn saves_and_loads_preferences() -> Result<()> {
        ...
        let dir =
            std::env::temp_dir().join(format!("daanio-desktop-prefs-test-{}", std::process::id()));
        let _ = fs::remove_dir_all(&dir);
        let path = dir.join("state.json");
        unsafe {
            std::env::set_var("DAANIO_DESKTOP_STATE", &path);
        }
    ...
    fn preference_temp_paths_are_unique_and_hidden_next_to_target() {
        let path = PathBuf::from("/tmp/daanio-desktop-prefs/state.json");
