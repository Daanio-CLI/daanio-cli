...
fn write(level: &str, args: fmt::Arguments<'_>, mirror_to_stderr: bool) {
    ...
    if let Err(error) = file.write_all(line.as_bytes()) {
        eprintln!("daanio-desktop: failed to write desktop log: {error}");
        *guard = None;
        return;
    }
    if let Err(error) = file.flush() {
        eprintln!("daanio-desktop: failed to flush desktop log: {error}");
        *guard = None;
    }
}
...
fn open_log_file() -> Option<File> {
    ...
    {
        eprintln!(
            "daanio-desktop: failed to create desktop log directory {}: {error}",
            parent.display()
        );
        return None;
    }
    match OpenOptions::new().create(true).append(true).open(&path) {
        ...
        Err(error) => {
            eprintln!(
                "daanio-desktop: failed to open desktop log {}: {error}",
                path.display()
            );
            None
        }
...
fn desktop_log_path() -> Option<PathBuf> {
    if std::env::var_os("DAANIO_DESKTOP_LOG").is_some_and(|value| !env_flag_enabled(value)) {
        return None;
    }
    if let Some(path) = std::env::var_os("DAANIO_DESKTOP_LOG_PATH") {
        if path.is_empty() {
            return None;
        }
    ...
    let root = std::env::var_os("DAANIO_HOME")
        .map(PathBuf::from)
        .or_else(|| std::env::var_os("HOME").map(|home| PathBuf::from(home).join(".daanio")))?;
    Some(
        root.join("logs")
            .join(format!("daanio-desktop-{}.log", utc_date_label())),
    )
}
