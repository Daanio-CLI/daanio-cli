...
pub(crate) async fn render_hero_frame_to_image(
    ...
    let (device, queue) = adapter
        .request_device(
            &wgpu::DeviceDescriptor {
                label: Some("daanio-desktop-hero-capture-device"),
    ...
    let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
        label: Some("daanio-desktop-hero-capture-primitive-shader"),
        source: wgpu::ShaderSource::Wgsl(SHADER.into()),
    });
    let pipeline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
        label: Some("daanio-desktop-hero-capture-pipeline-layout"),
        bind_group_layouts: &[],
        push_constant_ranges: &[],
    });
    let render_pipeline = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
        label: Some("daanio-desktop-hero-capture-primitive-pipeline"),
    ...
    let texture = device.create_texture(&wgpu::TextureDescriptor {
        label: Some("daanio-desktop-hero-capture-texture"),
        size: wgpu::Extent3d {
            width: size.width,
            height: size.height,
    ...
    let vertex_buffer = device.create_buffer(&wgpu::BufferDescriptor {
        label: Some("daanio-desktop-hero-capture-vertices"),
    ...
    let output_buffer = device.create_buffer(&wgpu::BufferDescriptor {
        label: Some("daanio-desktop-hero-capture-readback"),
    ...
    let mut encoder = device.create_command_encoder(&wgpu::CommandEncoderDescriptor {
        label: Some("daanio-desktop-hero-capture-encoder"),
    });
    {
        let mut render_pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
            label: Some("daanio-desktop-hero-capture-pass"),
    ...
    buffer_slice.map_async(wgpu::MapMode::Read, move |result| {
        if tx.send(result).is_err() {
            desktop_log::warn(format_args!(
                "daanio-desktop: failed to deliver hero capture readback result"
            ));
        }
    });
...
impl HeroMaskRenderer {
    pub(crate) fn new(device: &wgpu::Device, target_format: wgpu::TextureFormat) -> Self {
        let bind_group_layout = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
            label: Some("daanio-desktop-hero-mask-bind-group-layout"),
            entries: &[
        ...
        let pipeline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("daanio-desktop-hero-mask-pipeline-layout"),
            bind_group_layouts: &[&bind_group_layout],
            push_constant_ranges: &[],
        });
        let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("daanio-desktop-hero-mask-shader"),
            source: wgpu::ShaderSource::Wgsl(HERO_MASK_SHADER.into()),
        });
        let pipeline = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
            label: Some("daanio-desktop-hero-mask-pipeline"),
            layout: Some(&pipeline_layout),
            vertex: wgpu::VertexState {
                module: &shader,
                entry_point: "vs_main",
        ...
        let sampler = device.create_sampler(&wgpu::SamplerDescriptor {
            label: Some("daanio-desktop-hero-mask-sampler"),
        ...
        let vertex_buffer = device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("daanio-desktop-hero-mask-vertices"),
            size: (6 * std::mem::size_of::<HeroMaskVertex>()) as wgpu::BufferAddress,
            usage: wgpu::BufferUsages::VERTEX | wgpu::BufferUsages::COPY_DST,
            mapped_at_creation: false,
        });
        let uniform_buffer = device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("daanio-desktop-hero-mask-uniform"),
    ...
    pub(crate) fn create_resources(
        ...
        let glyph_texture = create_hero_mask_texture(
            device,
            queue,
            "daanio-desktop-hero-alpha-texture",
            mask.width,
            mask.height,
            &mask.glyph_rgba,
        );
        let reveal_texture = create_hero_mask_texture(
            device,
            queue,
            "daanio-desktop-hero-reveal-texture",
            mask.width,
            mask.height,
            &mask.reveal_rgba,
        );
        ...
        let bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some("daanio-desktop-hero-mask-bind-group"),
