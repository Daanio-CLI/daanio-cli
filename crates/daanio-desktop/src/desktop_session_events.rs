...
pub(crate) fn spawn_session_event_forwarder(
    ...
    if let Err(error) = std::thread::Builder::new()
        .name("daanio-desktop-session-event-forwarder".to_string())
        .spawn(move || {
            ...
            while let Ok(first_event) = session_event_rx.recv() {
                    ...
                    .is_err()
                {
                    desktop_log::warn(format_args!(
                        "daanio-desktop: failed to forward session events, event loop is closed"
                    ));
                    break;
                }
            }
        })
    {
        desktop_log::error(format_args!(
            "daanio-desktop: failed to start session event forwarder: {error:#}"
        ));
    }
}
