...
pub(crate) fn log_desktop_session_event_batch_profile(
    ...
    emit_desktop_profile_event(
        "daanio-desktop-session-event-profile",
        serde_json::json!({
...
pub(crate) fn log_desktop_session_card_refresh_profile(
    ...
    emit_desktop_profile_event(
        "daanio-desktop-session-card-refresh-profile",
        serde_json::json!({
...
pub(crate) fn log_desktop_session_cards_load_profile(
    ...
    emit_desktop_profile_event(
        "daanio-desktop-session-cards-load-profile",
        serde_json::json!({
...
pub(crate) fn log_desktop_preferences_save_profile(
    ...
    emit_desktop_profile_event(
        "daanio-desktop-preferences-save-profile",
        serde_json::json!({
...
pub(crate) fn log_desktop_crashed_sessions_restore_profile(
    ...
    emit_desktop_profile_event(
        "daanio-desktop-crashed-sessions-restore-profile",
        serde_json::json!({
            "restored": restored,
            "errors": errors,
...
impl DesktopFrameProfiler {
    pub(crate) fn new() -> Self {
        ...
        let budget = duration_millis_env(
            "DAANIO_DESKTOP_FRAME_BUDGET_MS",
...
impl DesktopInteractionLatencyProfiler {
    pub(crate) fn new() -> Self {
        ...
        let log_all = desktop_frame_profile_log_all(mode.as_deref());
        let budget = duration_millis_env(
            "DAANIO_DESKTOP_INPUT_LATENCY_BUDGET_MS",
    ...
    pub(crate) fn observe_presented(&mut self, frame: &DesktopRenderFrameResult) {
        ...
        eprintln!(
            "daanio-desktop-latency-profile {}",
            serde_json::json!({
                "kind": pending.kind,
...
impl DesktopNoPaintWatchdog {
    ...
    pub(crate) fn new_with_start_and_mode(now: Instant, mode: Option<&str>) -> Self {
        ...
        let budget =
            duration_millis_env("DAANIO_DESKTOP_NO_PAINT_BUDGET_MS", DESKTOP_NO_PAINT_BUDGET);
        Self {
            enabled,
            log_all,
            budget,
    ...
    pub(crate) fn observe_active_tick(
        ...
        if report_due {
            ...
            emit_desktop_profile_event(
                "daanio-desktop-no-paint-profile",
                serde_json::json!({
...
pub(crate) fn desktop_profile_log_path() -> Option<PathBuf> {
    if std::env::var_os("DAANIO_DESKTOP_PROFILE_LOG").is_some_and(|value| !env_flag_enabled(value))
    {
        return None;
    }
    if let Some(path) = std::env::var_os("DAANIO_DESKTOP_PROFILE_LOG_PATH") {
        if path.is_empty() {
            return None;
        }
...
pub(crate) fn desktop_profile_stderr_enabled() -> bool {
    std::env::var_os("DAANIO_DESKTOP_PROFILE_STDERR").is_none_or(env_flag_enabled)
}
    ...
    DESKTOP_PROFILE_LOG_TX
        .get_or_init(|| {
            ...
            match std::thread::Builder::new()
                .name("daanio-desktop-profile-log".to_string())
                .spawn(move || {
                    let mut file = path.and_then(|path| {
                            ...
                            && let Err(error) = std::fs::create_dir_all(parent)
                        {
                            desktop_log::error(format_args!(
                                "daanio-desktop: failed to create profile log directory {}: {error}",
                                parent.display()
                            ));
                            return None;
                        }
                        match OpenOptions::new().create(true).append(true).open(&path) {
                            ...
                            Err(error) => {
                                desktop_log::error(format_args!(
                                    "daanio-desktop: failed to open profile log {}: {error}",
                                    path.display()
                                ));
                                None
                            }
                    ...
                    while let Ok(line) = rx.recv() {
                        ...
                        {
                            desktop_log::error(format_args!(
                                "daanio-desktop: failed to write profile log: {error}"
                            ));
                            file = None;
                        }
                    }
                ...
                Err(error) => {
                    desktop_log::error(format_args!(
                        "daanio-desktop: failed to start profile logger: {error:#}"
                    ));
                    None
                }
            }
    ...
    if let Some(tx) = desktop_profile_log_sender() {
            ...
            .is_err()
        {
            desktop_log::warn(format_args!(
                "daanio-desktop: failed to queue profile event {event}, logger is closed"
            ));
        }
    }
}
...
pub(crate) fn log_desktop_slow_interaction(
    ...
    emit_desktop_profile_event(
        "daanio-desktop-interaction-profile",
        serde_json::json!({
            "kind": kind,
