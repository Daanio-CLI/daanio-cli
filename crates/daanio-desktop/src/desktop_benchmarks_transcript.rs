...
///
/// - `DAANIO_DESKTOP_BENCHMARK_SESSIONS` (default 8)
/// - `DAANIO_DESKTOP_BENCHMARK_MIN_MESSAGES` (default 24)
pub(crate) fn real_transcript_benchmark_selection() -> (usize, usize) {
    let max_sessions = std::env::var("DAANIO_DESKTOP_BENCHMARK_SESSIONS")
        .ok()
        .and_then(|value| value.trim().parse::<usize>().ok())
        .filter(|value| *value > 0)
        .unwrap_or(8);
    let min_messages = std::env::var("DAANIO_DESKTOP_BENCHMARK_MIN_MESSAGES")
        .ok()
        .and_then(|value| value.trim().parse::<usize>().ok())
        .unwrap_or(24);
    ...
    if transcripts.is_empty() {
        println!(
            ...
            serde_json::to_string_pretty(&serde_json::json!({
                "frames": frames,
                "sessions": [],
                "note": "no real transcripts with >=24 messages found under ~/.daanio/sessions",
            }))?
        );
        return Ok(());
    }
...
pub(crate) fn benchmark_real_transcript_scroll(
    ...
    let diagnose = std::env::var_os("DAANIO_DESKTOP_SCROLL_DIAG").is_some();
        ...
        if !single_session_body_text_window_contains(window_start, window_end, &viewport) {
            ...
            if diagnose {
                ...
                if rebuild_us > worst_rebuild_us {
                    ...
                    if let Ok(path) = std::env::var("DAANIO_DESKTOP_SCROLL_DIAG_DUMP") {
                        let text = window
                            .iter()
                            .map(|l| l.text.as_str())
                            .collect::<Vec<_>>()
