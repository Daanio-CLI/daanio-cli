...
//! run explicitly:
//!   cargo test -p daanio-app-core tool::computer::coverage -- --ignored --nocapture --test-threads=1
//!
...
use super::*;
use daanio_tool_core::{ToolContext, ToolExecutionMode};
...
async fn coverage_clipboard_scripting_system() {
    ...
    ok(json!({"action":"notify","text":"daanio coverage test","title":"daanio"})).await;
