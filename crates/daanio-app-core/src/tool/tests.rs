...
async fn registry_execute_pre_tool_hook_blocks_and_allows() {
    ...
    let prev = std::env::var_os("DAANIO_HOOK_PRE_TOOL");
    crate::env::set_var("DAANIO_HOOK_PRE_TOOL", policy.to_string_lossy().to_string());
    // daanio-base is compiled without cfg(test) here, so the config cache only
    ...
    match prev {
        Some(value) => crate::env::set_var("DAANIO_HOOK_PRE_TOOL", value),
        None => crate::env::remove_var("DAANIO_HOOK_PRE_TOOL"),
    }
    crate::config::invalidate_config_cache();
...
async fn gemini_build_tools_from_registry_definitions_omits_const_keywords() {
    // Moved from daanio-base/src/provider/gemini_tests.rs: this is the one test
