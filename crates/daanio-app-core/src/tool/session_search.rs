...
struct SearchInput {
    ...
    /// Restrict Daanio sessions by saved/bookmarked flag.
    #[serde(default)]
    saved: Option<bool>,
    /// Restrict Daanio sessions by debug flag.
    #[serde(default)]
    debug: Option<bool>,
    /// Restrict Daanio sessions by canary flag.
    #[serde(default)]
    canary: Option<bool>,
    /// Restrict source: daanio, claude, codex, pi, opencode, cursor, or all.
    #[serde(default)]
    ...
    /// Scan every available Daanio session instead of the recent indexed subset.
    #[serde(default)]
...
/// Covers the daanio store plus the external stores (claude/codex/pi/cursor).
pub fn spawn_recent_index_warmup() {
    tokio::task::spawn_blocking(|| {
        let start = std::time::Instant::now();
        remove_legacy_index();
        let empty_query = QueryProfile::new("__daanio_index_warmup__");

        let daanio_count = (|| -> Result<usize> {
            let sessions_dir = storage::daanio_dir()?.join("sessions");
            let collection = collect_session_files(&sessions_dir, DEFAULT_MAX_SCAN_SESSIONS)?;
            if collection.files.is_empty() {
                return Ok(0);
            }
            let _ = daanio_index_candidates(&collection.files, &empty_query)?;
            Ok(collection.files.len())
        })()
        .unwrap_or_else(|err| {
            crate::logging::info(&format!("daanio session index warmup skipped: {err}"));
            0
        });
        ...
        crate::logging::info(&format!(
            "Session search index warmup completed for {daanio_count} daanio + {external_count} external session(s) in {}ms",
    ...
    fn parameters_schema(&self) -> Value {
        json!({
            ...
            "properties": {
                ...
                "saved": {
                    "type": "boolean",
                    "description": "Restrict Daanio sessions by saved/bookmarked flag."
                },
                "debug": {
                    "type": "boolean",
                    "description": "Restrict Daanio sessions by debug/test flag."
                },
                "canary": {
                    "type": "boolean",
                    "description": "Restrict Daanio sessions by canary flag."
                },
                "source": {
                    "type": "string",
                    "enum": ["all", "daanio", "claude", "codex", "pi", "opencode", "cursor"],
                ...
                "exhaustive": {
                    "type": "boolean",
                    "description": "Search every available Daanio session instead of the recent indexed subset. Slower, but useful for deep recall."
                }
            },
            "required": ["query"]
        })
...
fn normalize_source_filter(raw: Option<&str>) -> std::result::Result<Option<String>, String> {
    ...
    match normalized.as_str() {
        "all" => Ok(None),
        "daanio" | "claude" | "claude-code" | "codex" | "pi" | "opencode" | "cursor" => {
            Ok(Some(normalized.replace("claude-code", "claude")))
        }
        _ => Err(format!(
            "source must be one of all, daanio, claude, codex, pi, opencode, or cursor; received {source}."
        )),
    }
}
...
fn search_sessions_blocking(
    ...
    if source_matches_filter("daanio", options) {
        ...
        if !files.is_empty() {
            ...
            report.scanned_daanio_sessions = files.len();
            ...
            if !files.is_empty() {
                ...
                let mut candidates = if options.exhaustive {
                    ...
                    match daanio_index_candidates(&files, query) {
                ...
                report.candidate_daanio_sessions = candidates.len();
                if using_index {
...
fn index_dir() -> Result<PathBuf> {
    Ok(storage::daanio_dir()?.join("cache"))
}
...
/// Build/update the incremental daanio session index and return the candidate
/// subset of `files` that plausibly match `query`.
fn daanio_index_candidates(
    files: &[SessionFileCandidate],
    query: &QueryProfile,
) -> Result<Vec<SessionFileCandidate>> {
    let index_path = index_dir()?.join("session_search_daanio_index_v2.bin");
...
fn collect_external_jsonl_source(
    ...
    // Like the daanio path, cap how many candidate files get fully parsed.
...
fn claude_index_candidates(
    sessions: &[daanio_import_core::ClaudeCodeSessionInfo],
    query: &QueryProfile,
) -> Vec<daanio_import_core::ClaudeCodeSessionInfo> {
    let index = index_dir()
                ...
                for session in chunk {
                    ...
                    records.push(ExternalSessionRecord {
                        ...
                        short_name: Some(format!(
                            "claude {}",
                            daanio_core::util::truncate_str(&session.session_id, 8)
                        )),
                        title: Some(title),
...
fn external_text_matches_query(text: &str, query: &QueryProfile) -> bool {
    daanio_session_types::normalized_session_search_text_matches(&text.to_lowercase(), query)
}
...
fn append_session_results(
    ...
    if !daanio_session_matches_filters(session, options) {
        return;
    }
    ...
    for (message_index, msg) in session.messages.iter().enumerate() {
        ...
        results.push(SearchResult {
            ...
            matched_terms: match_score.matched_terms,
            exact_match: match_score.exact_match,
            context: build_daanio_context(&session.messages, message_index, options),
        });
    }
}
...
fn daanio_session_matches_filters(session: &Session, options: &SearchOptions) -> bool {
    if !source_matches_filter("daanio", options) {
        return false;
    }
    if !provider_matches(session.provider_key.as_deref(), "daanio", options) {
        return false;
    }
