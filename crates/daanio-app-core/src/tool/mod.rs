...
pub(crate) use daanio_tool_core::intent_schema_property;
pub use daanio_tool_core::{StdinInputRequest, Tool, ToolContext, ToolExecutionMode};
pub use daanio_tool_types::{ToolImage, ToolOutput};
...
impl Registry {
    ...
    /// correctly.
    ///
    /// The canonical mapping lives in `daanio-tool-types::resolve_tool_name` so
    ...
    pub(crate) fn resolve_tool_name(name: &str) -> &str {
        daanio_tool_types::resolve_tool_name(name)
    }
    ...
    /// (`.mcp.json`, `.daanio/mcp.json`, `.claude/mcp.json`) against
