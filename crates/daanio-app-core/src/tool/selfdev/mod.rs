...
//! Self-development tool - manage canary builds when working on daanio itself
...
/// Source used when cloning the Daanio repository for self-development.
pub fn configured_repo_url() -> Result<String> {
    std::env::var("DAANIO_REPO_URL")
...
impl BuildRequest {
    ...
    fn requests_dir() -> Result<PathBuf> {
        let dir = storage::daanio_dir()?.join("selfdev-build-requests");
    ...
    fn terminal_history_limit() -> usize {
        std::env::var("DAANIO_SELFDEV_REQUEST_HISTORY_LIMIT")
            .ok()
...
impl SelfDevTool {
    ...
    pub fn description_for(is_selfdev: bool) -> &'static str {
        if is_selfdev {
            "Manage self-dev builds, tests, and reloads while working on daanio itself."
        } else {
            "Enter self-dev mode to work on daanio itself. Also sets up the dev \
             environment, reloads daanio to a newer build, and locates daanio config/paths."
        }
    }
            ...
            json!({
                ...
                "properties": {
                    ...
                    "target": {
                        ...
                        "description": "Build target for action=build. auto chooses from changed paths; tui builds daanio; desktop builds daanio-desktop; all builds both."
                    },
                    "command": {
                        "type": "string",
            ...
            json!({
                ...
                "properties": {
                    ...
                    "action": {
                        ...
                        "description": "Action. `enter` spawns a self-dev session (optionally seeded with `prompt`); `setup` checks/installs the dev prerequisites (rust toolchain, git, repo clone); `reload` restarts daanio into a newer installed build; `status` shows build/version state; `find-config` locates daanio config and key paths."
                    },
                    "prompt": {
                        "type": "string",
...
impl Tool for SelfDevTool {
    ...
    async fn execute(&self, input: Value, ctx: ToolContext) -> Result<ToolOutput> {
        ...
        let result = match action.as_str() {
            ...
            // access only make sense once you are working on daanio itself.
            "build" => {
                self.do_build(
                    params.reason,
                    params.target,
...
impl SelfDevTool {
    fn is_test_session() -> bool {
        std::env::var("DAANIO_TEST_SESSION")
            .map(|value| {
    ...
    fn reload_timeout_secs() -> u64 {
        std::env::var("DAANIO_SELFDEV_RELOAD_TIMEOUT_SECS")
            .ok()
    ...
    fn build_reload_wait_secs() -> u64 {
        std::env::var("DAANIO_SELFDEV_BUILD_WAIT_SECS")
            .ok()
            .and_then(|raw| raw.trim().parse::<u64>().ok())
            .filter(|secs| *secs > 0)
            .unwrap_or(1800)
    ...
    fn resolve_repo_dir(working_dir: Option<&std::path::Path>) -> Option<std::path::PathBuf> {
        if let Some(dir) = working_dir {
            for ancestor in dir.ancestors() {
                if build::is_daanio_repo(ancestor) {
    ...
    fn launch_binary() -> Result<std::path::PathBuf> {
        build::client_update_candidate(true)
            ...
            .ok_or_else(|| anyhow::anyhow!("Could not resolve daanio executable to launch"))
    }
    ...
    fn build_lock_path(worktree_scope: &str) -> Result<PathBuf> {
        let dir = storage::daanio_dir()?.join("selfdev-build-locks");
        storage::ensure_dir(&dir)?;
