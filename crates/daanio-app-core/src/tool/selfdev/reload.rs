use super::*;
pub use daanio_selfdev_types::ReloadRecoveryDirective;
...
impl ReloadContext {
    ...
    pub fn path_for_session(session_id: &str) -> Result<std::path::PathBuf> {
        ...
        Ok(storage::daanio_dir()?.join(format!("reload-context-{}.json", sanitized)))
    }

    fn legacy_path() -> Result<std::path::PathBuf> {
        Ok(storage::daanio_dir()?.join("reload-context.json"))
    }
...
impl SelfDevTool {
    pub(super) async fn do_reload(
        ...
        // a local debug/selfdev checkout may not have target/release/daanio).
        ...
        if !SelfDevTool::is_test_session() && !target_binary.exists() {
            return Ok(ToolOutput::new(
                format!(
                    "No binary found at {}.\n\
                     Run 'daanio self-dev --build' first, or build with 'scripts/dev_cargo.sh build --profile selfdev -p daanio --bin daanio' and then try reload again.",
        ...
        let version_before = daanio_build_meta::version().to_string();
