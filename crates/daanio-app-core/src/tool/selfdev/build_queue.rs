...
impl SelfDevTool {
  ...
  case "${{1:-}}" in
    test|check|build|clippy|bench)
      if [[ "${{DAANIO_IN_DEV_CARGO:-0}}" == "1" || -z "${{DAANIO_DEV_CARGO_SCRIPT:-}}" ]]; then
        command cargo "$@"
      else
        DAANIO_IN_DEV_CARGO=1 "${{DAANIO_DEV_CARGO_SCRIPT}}" "$@"
      fi
      ;;
    *) command cargo "$@" ;;
  esac
...
{command}"#
    ...
    fn build_command_is_desktop_only(command: &SelfDevBuildCommand) -> bool {
        command.display.contains("-p daanio-desktop") && !command.display.contains("-p daanio ")
    }
    ...
    pub(super) async fn do_build(
        ...
        let repo_dir =
            SelfDevTool::resolve_repo_dir(ctx.working_dir.as_deref()).ok_or_else(|| {
                anyhow::anyhow!("Could not find the daanio repository directory for selfdev build")
            })?;
    ...
    pub(super) async fn do_test(
        ...
        let repo_dir =
            SelfDevTool::resolve_repo_dir(ctx.working_dir.as_deref()).ok_or_else(|| {
                anyhow::anyhow!("Could not find the daanio repository directory for selfdev test")
            })?;
        let requested_source = SelfDevTool::requested_source_state(&repo_dir)?;
