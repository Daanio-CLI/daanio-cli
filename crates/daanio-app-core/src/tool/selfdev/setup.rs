...
impl SelfDevTool {
    ...
    /// building daanio from source: the rust toolchain, git, and a local repo
    ...
    pub(super) async fn do_setup(&self, ctx: &ToolContext) -> Result<ToolOutput> {
        ...
        // Rust toolchain: cargo + rustc are required to build daanio.
        ...
        if repo_dir.is_none() {
            ...
            if SelfDevTool::is_test_session() {
                clone_note = Some("Test mode: skipped cloning the daanio source.".to_string());
            } else if git_available {
                match Self::clone_selfdev_source() {
                    Ok(path) => {
                        clone_note = Some(format!("Cloned daanio source into {}.", path.display()));
                        repo_dir = Some(path);
                    }
                    Err(err) => {
                        clone_note = Some(format!(
                            "Could not clone daanio source automatically: {err}",
                        ));
                    }
                }
            }
        ...
        match &repo_dir {
            ...
            None => {
                let target = Self::selfdev_clone_dir()
                    ...
                    .unwrap_or_else(|_| "~/.daanio/source/daanio".to_string());
                checks.push(SetupCheck::missing(
                    "repository",
                    "no local daanio checkout found",
                    match super::configured_repo_url() {
                        Ok(repo_url) => {
                            format!("Clone the source manually: `git clone {repo_url} {target}`.")
                        }
                        Err(_) => format!(
                            "Set `DAANIO_REPO_URL` to your fork's Git URL, then clone it into {target}."
                        ),
                    },
                ));
            }
        ...
        if let Some(repo) = repo_dir.as_deref() {
            match build::find_dev_binary(repo) {
                ...
                None => checks.push(SetupCheck::missing(
                    ...
                    "Build it once with `daanio self-dev --build`, or inside a \
        ...
        if !failing.is_empty() {
            ...
            output.push_str(
                "\nAll prerequisites satisfied. Use `selfdev enter` to start working on daanio.\n",
            );
        }
    ...
    /// `selfdev find-config`: report the key daanio paths (config file, home,
    ...
    pub(super) async fn do_find_config(&self, ctx: &ToolContext) -> Result<ToolOutput> {
        let daanio_home = storage::daanio_dir().ok();
        let config_path = daanio_home.as_ref().map(|home| home.join("config.toml"));
        ...
        let mut output = String::from("## daanio config & paths\n\n");
        output.push_str(&format!(
            "**Config file:** {}\n",
        ...
        output.push_str(&format!(
            "**daanio home:** {}\n",
            format_path(daanio_home.as_deref())
        ));
        output.push_str(&format!(
            "**Logs dir:** {}\n",
        ...
        let metadata = json!({
            ...
            "daanio_home": daanio_home.as_ref().map(|p| p.display().to_string()),
    ...
    pub(super) async fn do_reload_to_newer_build(&self, _ctx: &ToolContext) -> Result<ToolOutput> {
        ...
        if !server::server_has_newer_binary() {
            return Ok(ToolOutput::new(
                "Already running the newest installed daanio build; no reload needed.",
            ));
        }

        let hash = daanio_build_meta::git_hash().to_string();
    ...
    fn selfdev_clone_dir() -> Result<std::path::PathBuf> {
        Ok(storage::daanio_dir()?.join("source").join("daanio"))
    }

    /// Clone the daanio source into the default self-dev source directory.
    fn clone_selfdev_source() -> Result<std::path::PathBuf> {
        let repo_url = super::configured_repo_url()?;
        ...
        if repo_dir.exists() {
            ...
            anyhow::bail!(
                "{} exists but is not a daanio repository; move it aside and retry",
                repo_dir.display()
            );
        }
        ...
        if !build::is_daanio_repo(&repo_dir) {
            anyhow::bail!(
                "cloned source at {} is not a valid daanio repository",
                repo_dir.display()
            );
        }
        Ok(repo_dir)
