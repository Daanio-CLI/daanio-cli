...
use daanio_swarm_core::validate_swarm_tldr;
...
impl RunPlanReporter {
    ...
    /// Record an explicit checkpoint (a DAANIO_CHECKPOINT-style milestone) on
...
/// The `daanio login` invocation most likely to fix a credential wave for
...
fn credential_login_fix_hint(provider: Option<&str>) -> String {
    ...
    format!("`daanio login --provider {target}`")
}
...
fn plan_status_budget_line(summary: &PlanGraphStatus, deep_cap: usize) -> Option<String> {
    ...
    let budget_label = if budget == usize::MAX {
        format!("{} (member cap)", daanio_swarm_core::MAX_SWARM_MEMBERS)
    } else {
        budget.to_string()
    };
    ...
    let effective_budget = if budget == usize::MAX {
        daanio_swarm_core::MAX_SWARM_MEMBERS
    } else {
        budget
    };
...
fn format_swarm_model_list(
    ...
    model_routes: &[daanio_provider_core::ModelRoute],
) -> String {
...
impl CommunicateTool {
    pub fn new() -> Self {
        ...
        let description = if swarm_prompt.is_empty() {
            ...
            format!(
                "{BASE_DESCRIPTION}\n\nSwarm prompt (user-tunable via ~/.daanio/swarm-prompt.md):\n{swarm_prompt}"
            )
        };
        Self { description }
    }
