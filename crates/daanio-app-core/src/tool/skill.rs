...
struct SkillInput {
    ...
    /// `list` shows both loaded skills and the daanio-endorsed catalog.
...
impl SkillTool {
        ...
        let skill = registry.get(&name).ok_or_else(|| {
                ...
                .find(|endorsed| endorsed.name == name)
            {
                match endorsed.install {
                    ...
                    None => anyhow::anyhow!(
                        "Skill '{}' is endorsed but not installed (source: {}). Install it into ~/.daanio/skills/{}/SKILL.md, then run skill_manage reload_all.",
                        name,
                        endorsed.source,
                        name
                    ),
    ...
    async fn list_skills(&self, working_dir: Option<&std::path::Path>) -> Result<ToolOutput> {
        ...
        let mut output = if skills.is_empty() {
            ...
            - ~/.daanio/skills/<skill-name>/SKILL.md (global)\n\
            - ./.daanio/skills/<skill-name>/SKILL.md (project-local)\n\
...
/// Append the curated daanio-endorsed skill catalog to `output`, grouped by
...
mod tests {
    ...
    fn create_test_tool_with_skill(name: &str) -> (SkillTool, tempfile::TempDir) {
        ...
        let skill_dir = temp_dir.path().join(".daanio").join("skills").join(name);
