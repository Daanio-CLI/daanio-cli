...
async fn communicate_assign_task_can_spawn_fallback_agent() {
    ...
    let socket_path = runtime_dir.path().join("daanio.sock");
    let _runtime = EnvGuard::set("DAANIO_RUNTIME_DIR", runtime_dir.path());
    let _socket = EnvGuard::set("DAANIO_SOCKET", &socket_path);
    let _debug = EnvGuard::set("DAANIO_DEBUG_CONTROL", "1");
