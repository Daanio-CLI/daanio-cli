...
fn with_temp_home<T>(f: impl FnOnce(&Path) -> T) -> T {
    ...
    let previous_home = std::env::var("DAANIO_HOME").ok();
...
#[test]
#[ignore = "local performance benchmark over the real ~/.daanio session corpus"]
fn bench_real_session_search_corpus() {
    if std::env::var("DAANIO_SESSION_SEARCH_BENCH_REAL")
        .ok()
        .as_deref()
        != Some("1")
    {
        eprintln!("set DAANIO_SESSION_SEARCH_BENCH_REAL=1 to run against the real session corpus");
        return;
    }
    ...
    for query in ["session_search", "optimization", "nonexistentneedle123"] {
        ...
        eprintln!(
            ...
            report.scanned_daanio_sessions,
            report.candidate_daanio_sessions,
            report.results.len(),
            report.truncated
        );
    }
    ...
    for query in ["session_search", "nonexistentneedle123"] {
        ...
        .expect("search succeeds");
        eprintln!(
            "BENCH_EXTERNAL query={query} elapsed_ms={} scanned_daanio={} scanned_external={} sources={:?} results={} truncated={}",
            start.elapsed().as_millis(),
            report.scanned_daanio_sessions,
...
#[test]
fn external_codex_sessions_are_searchable_without_daanio_session_dir() {
    with_temp_home(|home| {
        ...
        std::fs::remove_dir_all(home.join("sessions")).expect("remove daanio sessions dir");
        ...
        assert_eq!(report.scanned_daanio_sessions, 0);
...
#[test]
fn external_cursor_sessions_are_searchable_without_daanio_session_dir() {
    with_temp_home(|home| {
