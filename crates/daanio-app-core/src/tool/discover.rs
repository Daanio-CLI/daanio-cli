...
const DISCOVERY_REQUEST_ID_HEADER: &str = "x-daanio-discovery-request-id";
const DISCOVERY_BENCHMARK_HEADER: &str = "x-daanio-discovery-benchmark";
const DISCOVERY_SESSION_ID_HEADER: &str = "x-daanio-discovery-session-id";
const DISCOVERY_SESSION_METADATA_HEADER: &str = "x-daanio-discovery-session-metadata";
const DISCOVERY_SELF_DEV_HEADER: &str = "x-daanio-discovery-self-dev";
const DISCOVERY_DEBUG_HEADER: &str = "x-daanio-discovery-debug";
const DISCOVERY_CANARY_HEADER: &str = "x-daanio-discovery-canary";
const DISCOVERY_EXECUTION_MODE_HEADER: &str = "x-daanio-discovery-execution-mode";
const DISCOVERY_BUILD_CHANNEL_HEADER: &str = "x-daanio-discovery-build-channel";
const DISCOVERY_GIT_CHECKOUT_HEADER: &str = "x-daanio-discovery-git-checkout";
const DISCOVERY_CI_HEADER: &str = "x-daanio-discovery-ci";
const DISCOVERY_RAN_FROM_CARGO_HEADER: &str = "x-daanio-discovery-ran-from-cargo";
const DISCOVERY_BENCHMARK_ENV: &str = "DAANIO_DISCOVERY_BENCHMARK";
...
///
/// Disclosure contract: some providers may share revenue with Daanio, but
...
impl Tool for DiscoverToolsTool {
    ...
    fn description(&self) -> &str {
        "Use proactively whenever fully completing the user's goal requires an external \
         ...
         vetted, and one-tool-call setup is enforced. Some providers are Daanio partners and may \
         share revenue with Daanio when a referred user becomes a customer. Partnership status \
        ...
        json!({
            ...
            "properties": {
                ...
                "query": {
                    ...
                    "description": "Required capability summary. Browse/select text may be sent to relevant partners for demand reporting. Suggest text goes only to Daanio maintainers. Write a fresh summary instead of copying user text. Never include secrets, credentials, personal data, or private content."
                },
                "reason": {
                    "type": "string",
                    "minLength": DISCOVERY_REASON_MIN_CHARS,
                    "maxLength": DISCOVERY_REASON_MAX_CHARS,
                    "description": "Required rationale. For select, explain why the tool fits better than alternatives. For suggest, explain why browse results were unsuitable. Browse/select text may reach relevant partners; suggest text goes only to Daanio maintainers. Never include private data."
                },
                "tool": {
                    "type": "string",
                ...
                "gap_evidence": {
                    "type": "string",
                    "maxLength": 500,
                    "description": "Optional concise explanation of which browse results were close and why they did not fit. Sent only to Daanio maintainers."
                },
                "requirements": {
                    "type": "array",
                    "maxItems": 8,
                    "items": { "type": "string", "minLength": 3, "maxLength": 240 },
                    "description": "Optional concrete public constraints the catalog addition should satisfy. Sent only to Daanio maintainers."
                },
                "prior_request_id": {
                    "type": "string",
    ...
    async fn execute(&self, input: Value, ctx: ToolContext) -> Result<ToolOutput> {
        ...
        // metered coarsely; see daanio_base::sponsors::provenance).
    ...
    let mut request = context.provenance.apply(
        context
            ...
            .header(
                ...
                format!("daanio/{}", env!("CARGO_PKG_VERSION")),
            )
...
fn render_listing(category: &str, listing: &Value, request_id: &str) -> Result<String> {
    ...
    let mut out = format!(
        "Discoverable tools in '{category}' (Daanio tool directory; recommendations must be based \
         only on fit; details: {}):\n",
...
fn render_suggestion(
    ...
    out.push_str(
        "\n\nStatus: received for Daanio maintainer review. Suggestions are not sent to partners. This does not mean Daanio has partnered with the tool or that it is approved or available.",
    );
    Ok(out)
}
...
fn render_selection(category: &str, tool_name: &str, listing: &Value) -> Result<String> {
    ...
    let blurb = tool.get("blurb").and_then(|v| v.as_str()).unwrap_or("");
    let mut out = format!(
        "Selected '{name}' from '{category}' (Daanio tool directory; selection must be based only \
         on fit; details: {}):\n\n{name}: {blurb}",
        crate::sponsors::DISCOVERY_PARTNERS_URL
    );
    if let Some(url) = tool.get("url").and_then(|v| v.as_str()) {
...
mod tests {
    ...
    fn render_listing_includes_disclosure_and_tools() {
        ...
        assert!(out.contains("Daanio tool directory"));
    ...
    fn agentmail_selection_preserves_signup_attribution_and_mcp_provenance() {
        let listing = json!({
            "tool": {
                ...
                "url": "https://www.agentmail.to/?via=daanio-discovery",
                "setup": concat!(
                    "POST https://api.agentmail.to/v0/agent/sign-up with JSON ",
                    "{\"source\":\"daanio\",\"referrer\":\"https://daanio.com/discovery-tools\"}. ",
        ...
        assert!(rendered.contains("\"source\":\"daanio\""));
        assert!(rendered.contains("\"referrer\":\"https://daanio.com/discovery-tools\""));
    ...
    fn schema_is_compact_and_self_contained() {
        ...
        assert!(description.contains("Some providers are Daanio partners"));
        ...
        assert!(schema.contains("Suggest text goes only to Daanio maintainers"));
    ...
    fn suggestion_validation_distinguishes_product_and_capability_gap() {
        ...
        known.product_url = Some("https://example.com/tool?via=daanio#setup".to_string());
    ...
    fn render_suggestion_is_clear_about_review_status_and_recipient() {
        ...
        assert!(out.contains("does not mean Daanio has partnered with the tool"));
    }
    ...
    async fn fetch_listing_round_trips_and_sends_only_expected_params() {
        ...
        assert!(
            request
                .to_ascii_lowercase()
                .contains("x-daanio-discovery-request-id: request-test-1"),
            "{request}"
        );
        assert!(
            request
                .to_ascii_lowercase()
                .contains("x-daanio-discovery-benchmark: 1"),
            "{request}"
        );
        for expected in [
            "x-daanio-discovery-session-id: session-test-1",
            "x-daanio-discovery-session-metadata: 1",
            "x-daanio-discovery-self-dev: 1",
            "x-daanio-discovery-debug: 0",
            "x-daanio-discovery-canary: 1",
            "x-daanio-discovery-execution-mode: agent_turn",
            "x-daanio-discovery-build-channel: selfdev",
            "x-daanio-discovery-git-checkout: 1",
            "x-daanio-discovery-ci: 0",
            "x-daanio-discovery-ran-from-cargo: 1",
        ] {
    ...
    async fn submit_suggestion_posts_structured_maintainer_only_payload() {
        ...
        assert!(
            lower.contains("x-daanio-discovery-request-id: aaaaaaaa-bbbb-4ccc-8ddd-eeeeeeeeeeee"),
            "{request}"
        );
        assert!(
            lower.contains("x-daanio-discovery-benchmark: 1"),
            "{request}"
        );
    ...
    async fn execute_end_to_end_with_enabled_config_and_local_server() {
        ...
        assert!(output.output.contains("Daanio tool directory"));
        assert!(
            output
                .output
        ...
        if let Some(prev) = prev_home {
            crate::env::set_var("DAANIO_HOME", prev);
        } else {
