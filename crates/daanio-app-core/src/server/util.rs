...
pub(crate) fn debug_control_allowed() -> bool {
    ...
    if std::env::var("DAANIO_DEBUG_CONTROL")
        .ok()
    ...
    if let Ok(daanio_dir) = crate::storage::daanio_dir()
        && daanio_dir.join("debug_control").exists()
    {
        return true;
    }
    false
...
pub(crate) fn embedding_idle_unload_secs() -> u64 {
    parse_embedding_idle_unload_secs(
        std::env::var("DAANIO_EMBEDDING_IDLE_UNLOAD_SECS")
            .ok()
            .as_deref(),
    )
}
...
pub(crate) fn swarm_id_for_dir(dir: Option<PathBuf>) -> Option<String> {
    if let Ok(sw_id) = std::env::var("DAANIO_SWARM_ID") {
...
pub(crate) fn startup_headless_recovery_test_delay() -> Option<std::time::Duration> {
    let raw = std::env::var("DAANIO_TEST_HEADLESS_STARTUP_RECOVERY_DELAY_MS").ok()?;
...
mod newer_binary_tests {
    ...
    fn reports_update_when_candidate_is_strictly_newer() {
        let candidates = vec![(PathBuf::from("/x/stable/daanio"), Some(t(200)))];
        assert!(newer_binary_available(
            Some(t(100)),
            Some(std::path::Path::new("/x/current/daanio")),
            candidates,
        ));
    }
    ...
    fn ignores_candidate_that_is_not_newer() {
        let candidates = vec![(PathBuf::from("/x/stable/daanio"), Some(t(100)))];
        assert!(!newer_binary_available(
            Some(t(100)),
            Some(std::path::Path::new("/x/current/daanio")),
            candidates,
    ...
    fn never_reloads_into_self_even_if_paths_were_equal() {
        ...
        let candidates = vec![(PathBuf::from("/x/current/daanio"), Some(t(999)))];
        assert!(!newer_binary_available(
            Some(t(100)),
            Some(std::path::Path::new("/x/current/daanio")),
            candidates,
    ...
    fn suppresses_update_when_candidate_mtime_unavailable() {
        ...
        let candidates = vec![(PathBuf::from("/x/stable/daanio"), None)];
        assert!(!newer_binary_available(
            Some(t(100)),
            Some(std::path::Path::new("/x/current/daanio")),
            candidates,
    ...
    fn reports_update_if_any_candidate_is_newer() {
        let candidates = vec![
            (PathBuf::from("/x/stable/daanio"), None),
            (PathBuf::from("/x/shared/daanio"), Some(t(300))),
        ];
        assert!(newer_binary_available(
            Some(t(100)),
            Some(std::path::Path::new("/x/current/daanio")),
    ...
    fn newer_server_is_not_outdated_by_older_channel_binary() {
        ...
        assert!(!newer_binary_available(
            Some(t(300)),
            Some(std::path::Path::new("/x/builds/versions/dev/daanio")),
            candidates,
        ));
    }
...
mod reload_target_tests {
    ...
    fn same_binary_is_always_used() {
        ...
        let decision = guarded_reload_target(
            candidate("/x/current/daanio"),
            Path::new("/x/current/daanio"),
            Some(Path::new("/x/current/daanio")),
            Some(Path::new("/x/current/daanio")),
            Some(t(200)),
            Some(t(100)),
        );
    ...
    fn newer_candidate_is_used() {
        ...
        let decision = guarded_reload_target(
            candidate("/x/shared-server/daanio"),
            Path::new("/x/builds/versions/new/daanio"),
            Some(Path::new("/x/builds/versions/old/daanio")),
            Some(Path::new("/x/builds/versions/old/daanio")),
            Some(t(100)),
            Some(t(200)),
        );
        match decision {
            ReloadTargetDecision::UseCandidate((path, _)) => {
                assert_eq!(path, PathBuf::from("/x/shared-server/daanio"));
            }
    ...
    fn equal_mtime_candidate_is_used() {
        ...
        let decision = guarded_reload_target(
            candidate("/x/shared-server/daanio"),
            Path::new("/x/builds/versions/same/daanio"),
            Some(Path::new("/x/builds/versions/current/daanio")),
            Some(Path::new("/x/builds/versions/current/daanio")),
            Some(t(100)),
            Some(t(100)),
        );
    ...
    fn strictly_older_candidate_is_blocked_and_uses_current_exe() {
        ...
        let decision = guarded_reload_target(
            candidate("/x/shared-server/daanio"),
            Path::new("/x/builds/versions/old-0.14.3/daanio"),
            Some(Path::new("/x/builds/versions/new/daanio")),
            Some(Path::new("/x/builds/versions/new/daanio")),
            Some(t(300)),
            Some(t(100)),
        );
        match decision {
            ReloadTargetDecision::DowngradeBlockedUseCurrent((path, _)) => {
                assert_eq!(path, PathBuf::from("/x/builds/versions/new/daanio"));
            }
    ...
    fn unreadable_candidate_mtime_is_treated_as_downgrade() {
        let decision = guarded_reload_target(
            candidate("/x/shared-server/daanio"),
            Path::new("/x/builds/versions/unknown/daanio"),
            Some(Path::new("/x/builds/versions/new/daanio")),
            Some(Path::new("/x/builds/versions/new/daanio")),
            Some(t(300)),
            None,
    ...
    fn downgrade_without_current_exe_falls_back_to_candidate() {
        ...
        let decision = guarded_reload_target(
            candidate("/x/shared-server/daanio"),
            Path::new("/x/builds/versions/old/daanio"),
            None,
            None,
            None,
            Some(t(100)),
...
mod pick_newest_candidate_tests {
    ...
    fn other_flavor_wins_when_strictly_newer() {
        ...
        let chosen = pick_newest_candidate([
            entry(
                "/x/versions/old-selfdev/daanio",
                "shared-server",
                Some(t(100)),
            ),
            entry("/x/versions/new-release/daanio", "stable", Some(t(200))),
        ])
        .expect("a candidate");
        assert_eq!(chosen.0, PathBuf::from("/x/versions/new-release/daanio"));
    }
    ...
    fn own_flavor_wins_on_tie() {
        ...
        let chosen = pick_newest_candidate([
            entry("/x/versions/selfdev/daanio", "shared-server", Some(t(200))),
            entry("/x/versions/release/daanio", "stable", Some(t(200))),
        ])
        .expect("a candidate");
        assert_eq!(chosen.0, PathBuf::from("/x/versions/selfdev/daanio"));
    }
    ...
    fn own_flavor_wins_when_strictly_newer() {
        let chosen = pick_newest_candidate([
            entry(
                "/x/versions/fresh-selfdev/daanio",
                "shared-server",
                Some(t(300)),
            ),
            entry("/x/versions/release/daanio", "stable", Some(t(200))),
        ...
        .expect("a candidate");
        assert_eq!(chosen.0, PathBuf::from("/x/versions/fresh-selfdev/daanio"));
    }
    ...
    fn unknown_other_mtime_never_displaces_preferred() {
        ...
        let chosen = pick_newest_candidate([
            entry("/x/versions/selfdev/daanio", "shared-server", Some(t(100))),
            entry("/x/versions/release/daanio", "stable", None),
        ])
        .expect("a candidate");
        assert_eq!(chosen.0, PathBuf::from("/x/versions/selfdev/daanio"));
    }
    ...
    fn duplicate_canonical_paths_collapse() {
        ...
        let chosen = pick_newest_candidate([
            entry("/x/versions/same/daanio", "shared-server", Some(t(100))),
            entry("/x/versions/same/daanio", "stable", Some(t(999))),
        ])
        .expect("a candidate");
...
mod newest_reload_candidate_integration_tests {
    ...
    //! a temp `DAANIO_HOME`. This reproduces the field "/update -> new client,
    ...
    fn selfdev_daemon_reloads_into_fresh_release_after_update() {
        ...
        if let Some(prev_home) = prev_home {
            crate::env::set_var("DAANIO_HOME", prev_home);
        } else {
    ...
    /// Install a release-archive-style version dir: a tiny `daanio` wrapper
    /// script plus the real `daanio-linux-x86_64.bin` payload, with independently
    ...
    fn install_release_style_binary(
        ...
        let dir = build::builds_dir()
            ...
            .join(version);
        std::fs::create_dir_all(&dir).expect("create version dir");
        let payload = dir.join("daanio-linux-x86_64.bin");
...
mod deleted_suffix_tests {
    ...
    fn strips_linux_deleted_marker() {
        let p = PathBuf::from("/home/u/.daanio/builds/versions/abc/daanio (deleted)");
        assert_eq!(
            strip_deleted_suffix(p),
            PathBuf::from("/home/u/.daanio/builds/versions/abc/daanio")
        );
    }

    #[test]
    fn leaves_normal_paths_untouched() {
        let p = PathBuf::from("/home/u/.daanio/builds/versions/abc/daanio");
    ...
    fn only_strips_trailing_marker() {
        ...
        let p = PathBuf::from("/home/u/daanio (deleted)/daanio");
        assert_eq!(strip_deleted_suffix(p.clone()), p);
    }
}
