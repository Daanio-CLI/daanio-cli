...
fn sibling_socket_path_roundtrip() {
    let main = std::path::PathBuf::from("/tmp/daanio.sock");
    let debug = std::path::PathBuf::from("/tmp/daanio-debug.sock");
...
fn cleanup_socket_pair_removes_main_and_debug_files() {
    ...
    let main = dir.join(format!("daanio-test-{}.sock", stamp));
    let debug = dir.join(format!("daanio-test-{}-debug.sock", stamp));
...
async fn reap_stale_socket_removes_dead_socket_pair_and_lock() {
    ...
    let socket = temp.path().join("daanio.sock");
    let debug = temp.path().join("daanio-debug.sock");
...
fn existing_server_start_errors_are_detected() {
    assert!(server_start_matches_existing_server(
        "Error: Another daanio server process is already running for runtime dir /run/user/1000"
    ));
    assert!(server_start_matches_existing_server(
        "Error: Refusing to replace active server socket at /run/user/1000/daanio.sock"
    ));
    assert!(!server_start_matches_existing_server(
