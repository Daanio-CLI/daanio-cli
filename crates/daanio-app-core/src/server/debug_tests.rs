...
mod transcript_routing_tests {
    ...
    async fn resolve_transcript_target_session_prefers_last_focused_live_session() {
        ...
        let daanio_dir = crate::storage::daanio_dir().expect("daanio dir");
        let active_dir = daanio_dir.join("active_pids");
    ...
    async fn resolve_transcript_target_session_prefers_current_niri_focused_session_over_last_focused()
     {
        ...
        let focused_process = ChildGuard::spawn_named("daanio:d:swan");
...
mod debug_execution_tests {
    ...
    fn debug_message_timeout_secs_reads_valid_env_values() {
        let _guard = EnvVarGuard::set("DAANIO_DEBUG_MESSAGE_TIMEOUT_SECS", "17");
        assert_eq!(debug_message_timeout_secs(), Some(17));
    }

    #[test]
    fn debug_message_timeout_secs_ignores_missing_empty_invalid_and_zero() {
        let _guard = EnvVarGuard::remove("DAANIO_DEBUG_MESSAGE_TIMEOUT_SECS");
        assert_eq!(debug_message_timeout_secs(), None);
        drop(_guard);

        let _guard = EnvVarGuard::set("DAANIO_DEBUG_MESSAGE_TIMEOUT_SECS", "   ");
        assert_eq!(debug_message_timeout_secs(), None);
        drop(_guard);

        let _guard = EnvVarGuard::set("DAANIO_DEBUG_MESSAGE_TIMEOUT_SECS", "abc");
        assert_eq!(debug_message_timeout_secs(), None);
        drop(_guard);

        let _guard = EnvVarGuard::set("DAANIO_DEBUG_MESSAGE_TIMEOUT_SECS", "0");
        assert_eq!(debug_message_timeout_secs(), None);
    }
}
