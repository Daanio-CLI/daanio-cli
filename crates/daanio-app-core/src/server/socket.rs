...
pub fn socket_path() -> PathBuf {
    ...
    crate::storage::runtime_dir().join("daanio.sock")
}
...
pub async fn connect_socket(path: &std::path::Path) -> Result<Stream> {
    match Stream::connect(path).await {
        ...
        Err(err) if err.kind() == std::io::ErrorKind::ConnectionRefused && path.exists() => {
            anyhow::bail!(
                "Socket exists but refused the connection at {}. Retry, or remove it after confirming no daanio server is running.",
                path.display()
            )
        }
...
///
/// Safety: a live daemon holds an exclusive `flock` on `daanio-daemon.lock` for
...
pub async fn reap_stale_socket_if_dead(path: &std::path::Path) -> bool {
    ...
    crate::logging::warn(&format!(
        "Reaping stale daanio socket with no live listener at {}",
        path.display()
    ));
...
pub(super) fn daemon_lock_path() -> PathBuf {
    crate::storage::runtime_dir().join("daanio-daemon.lock")
}
...
pub(super) fn acquire_daemon_lock() -> Result<DaemonLockGuard> {
    ...
    try_acquire_daemon_lock(&path)?.ok_or_else(|| {
        anyhow::anyhow!(
            "Another daanio server process is already running for runtime dir {}",
...
pub fn set_socket_path(path: &str) {
    crate::env::set_var("DAANIO_SOCKET", path);
}
...
/// `DAANIO_READY_FD`, and awaits a single byte on the read end. The server
...
pub async fn spawn_server_notify(cmd: &mut std::process::Command) -> Result<std::process::Child> {
    ...
    cmd.env("DAANIO_READY_FD", write_fd.to_string());
...
pub(super) fn server_start_matches_existing_server(stderr_output: &str) -> bool {
    stderr_output.contains("Another daanio server process is already running")
...
pub(super) fn format_server_start_error(
    ...
    if stderr_output.trim().is_empty() {
        format!(
            "Server exited before signalling ready ({}). Check logs at ~/.daanio/logs/",
            status
        )
    } else {
        format!(
...
/// Write a single byte to the fd in `DAANIO_READY_FD` and close it.
...
pub(super) fn signal_ready_fd() {
    #[cfg(unix)]
    {
        ...
        if let Ok(fd_str) = std::env::var("DAANIO_READY_FD") {
            crate::env::remove_var("DAANIO_READY_FD");
