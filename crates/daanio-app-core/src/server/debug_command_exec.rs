...
use anyhow::Result;
use daanio_agent_runtime::{InterruptSignal, SoftInterruptSource};
...
pub(super) fn debug_message_timeout_secs() -> Option<u64> {
    let raw = std::env::var("DAANIO_DEBUG_MESSAGE_TIMEOUT_SECS").ok()?;
...
pub(super) async fn execute_debug_command(
    ...
    if trimmed == "mcp" || trimmed == "mcp:servers" {
        ...
        let config_path = if let Ok(daanio_dir) = crate::storage::daanio_dir() {
            let path = daanio_dir.join("mcp.json");
            if path.exists() {
    ...
    if trimmed == "reload" {
        let repo_dir = crate::build::get_repo_dir()
            .ok_or_else(|| anyhow::anyhow!("Could not find daanio repository directory"))?;

        let target_binary = crate::build::find_dev_binary(&repo_dir)
            .unwrap_or_else(|| build::release_binary_path(&repo_dir));
        if !target_binary.exists() {
            return Err(anyhow::anyhow!(format!(
                "No binary found at {}. Run 'daanio self-dev --build' first, or build with 'scripts/dev_cargo.sh build --profile selfdev -p daanio --bin daanio' and publish current.",
        ...
        let daanio_dir = crate::storage::daanio_dir()?;
        let info_path = daanio_dir.join("reload-info");
...
mod tests {
    ...
    use daanio_agent_runtime::InterruptSignal;
    ...
    async fn debug_tool_selfdev_reload_returns_promptly_for_direct_execution() {
        ...
        let _test_session = EnvGuard::set("DAANIO_TEST_SESSION", "1");
        let _debug_control = EnvGuard::set("DAANIO_DEBUG_CONTROL", "1");
