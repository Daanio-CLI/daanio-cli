...
/// (`DAANIO_SPAWN_KIND`, swarm/coordinator ids, ...) to spawn hooks and
...
fn explicit_route_for_configured_model(model: &str) -> Option<SwarmSpawnSelection> {
    if let Some((prefix, bare)) = model.split_once(':')
        ...
        && !bare.trim().is_empty()
    {
        return Some(SwarmSpawnSelection {
            model: Some(bare.trim().to_string()),
            provider_key: Some("daanio".to_string()),
            route_api_method: Some("daanio-subscription".to_string()),
        });
    }
...
fn resolve_swarm_spawn_selection(
    ...
    // Daanio subscription workers always follow the coordinator's active
    // model. The `model` field is generated by the coordinating LLM, so it is
    // not a trustworthy user override: a stale/default `gpt-5.6-sol` value
    // could otherwise move a Claude coordinator's workers onto GPT. Users who
    // need a different model can switch the coordinator before spawning.
...
fn clear_headed_startup_message(session_id: &str) {
    if let Ok(daanio_dir) = crate::storage::daanio_dir() {
        let path = daanio_dir.join(format!("client-input-{}", session_id));
    ...
    let visible_spawn = match resolved_spawn_mode {
        ...
        SwarmSpawnMode::Visible | SwarmSpawnMode::Auto => prepare_visible_spawn_session(
            ...
            |session_id, cwd, selfdev_requested, provider_key| {
                ...
                // and terminals can identify and reroute it (DAANIO_SPAWN_*).
                let context = crate::session_launch::SessionSpawnContext::kind("swarm-agent")
                    .env("DAANIO_SPAWN_SWARM_ID", swarm_id)
                    .env("DAANIO_SPAWN_COORDINATOR_SESSION_ID", req_session_id)
