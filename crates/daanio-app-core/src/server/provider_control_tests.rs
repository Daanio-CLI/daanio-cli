...
impl EnvGuard {
    /// Save and clear the given env vars, and redirect `DAANIO_HOME` to a fresh
    /// empty temp dir for the lifetime of the guard.
    ///
    /// The temp home keeps these tests hermetic: provider activation reads
    /// on-disk model catalog caches (`~/.daanio/cache/<profile>_models.json`) to
    ...
    fn save(keys: &[&'static str]) -> Self {
        ...
        if !all_keys.contains(&"DAANIO_HOME") {
            all_keys.push("DAANIO_HOME");
        }
        let saved = all_keys
            .iter()
        ...
        let temp_home = tempfile::tempdir().expect("create temp DAANIO_HOME");
