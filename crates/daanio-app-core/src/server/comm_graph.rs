...
//! `daanio_plan::dag`. Each handler lifts the swarm's current `VersionedPlan` into
//! a `TaskGraph` (via `daanio_plan::bridge`), applies the engine op (which enforces
...
use daanio_plan::MAX_PLAN_ITEMS;
use daanio_plan::bridge::{apply_task_graph, parse_kind, to_task_graph};
use daanio_plan::dag::{self, HandoffArtifact, NodeSpec, NodeStatus, TaskGraph};
