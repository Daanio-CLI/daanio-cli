...
use chrono::Utc;
use daanio_agent_runtime::{SoftInterruptMessage, SoftInterruptQueue, SoftInterruptSource};
use std::sync::Arc;
...
impl AmbientRunnerHandle {
    ...
    pub async fn log_json(&self) -> String {
        let transcripts_dir = match crate::storage::daanio_dir() {
    ...
    async fn run_cycle_visible(
        ...
        let daanio_bin =
            std::env::current_exe().unwrap_or_else(|_| std::path::PathBuf::from("daanio"));

        // Spawn kitty with `daanio ambient run-visible`
        logging::info("Ambient visible: spawning kitty with daanio TUI");
        let child = std::process::Command::new("kitty")
            .args([
                "--title",
                "🤖 daanio ambient cycle",
                "-e",
                &daanio_bin.to_string_lossy(),
                "ambient",
                "run-visible",
            ])
