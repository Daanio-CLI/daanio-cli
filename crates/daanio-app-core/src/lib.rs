...
//! Application core for daanio (upper layer).
//!
...
//! message, memory, telemetry, ...) lives in the `daanio-base` crate and is
//! re-exported here via `pub use daanio_base::*`, so every existing
//! `crate::<module>` path (e.g. `crate::config`, `crate::provider`) keeps
//! resolving unchanged across this crate and the root `daanio` crate, which in
//! turn re-exports this crate via `pub use daanio_app_core::*`.

// Foundational layer: re-export every `daanio-base` module so `crate::<module>`
