...
const DAANIO_NATIVE_TOOLS: &[&str] = &["selfdev", "communicate"];
static RECOVERED_TEXT_WRAPPED_TOOL_CALLS: std::sync::atomic::AtomicU64 =
    std::sync::atomic::AtomicU64::new(0);
static DAANIO_REPO_SOURCE_STATE: LazyLock<(Option<String>, Option<bool>)> = LazyLock::new(|| {
...
impl Agent {
    fn should_track_client_cache(&self) -> bool {
        match std::env::var("DAANIO_TRACK_CLIENT_CACHE") {
            Ok(value) => {
            ...
            match compaction.try_write() {
                Ok(mut manager) => {
                    ...
                    let messages = {
                        ...
                        if self.provider.uses_daanio_compaction() {
                            let action =
    ...
    fn record_client_cache_request(&mut self, messages: &[Message]) {
        ...
        let fast_snapshot =
            if !self.provider.uses_daanio_compaction() && self.session.compaction.is_none() {
