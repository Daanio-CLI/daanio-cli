...
impl MessageChannel for JadeRelayChannel {
    ...
    fn is_reply_enabled(&self) -> bool {
        ...
        // Daanio session directly. Keep this channel for outbound notifications
    ...
    async fn reply_loop(&self, runner: AmbientRunnerHandle) {
        ...
        let device_id = format!("daanio-{}", host);
        logging::info(&format!(
...
mod tests {
    ...
    ///   cargo test -p daanio-app-core relay_live -- --ignored --nocapture
    #[tokio::test]
    ...
    async fn test_relay_live_roundtrip() {
        ...
        ch.heartbeat("daanio-test-device").await;
