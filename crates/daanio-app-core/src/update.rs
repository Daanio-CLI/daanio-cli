...
const GITHUB_REPOSITORY_ENV: &str = "DAANIO_GITHUB_REPOSITORY";
const OFFICIAL_GITHUB_REPOSITORY: &str = "Daanio-CLI/daanio-cli";
...
pub fn updates_configured() -> bool {
    configured_github_repository().is_some()
}

fn configured_github_repository() -> Option<String> {
    match std::env::var(GITHUB_REPOSITORY_ENV) {
        Ok(repository) => {
            let repository = repository.trim();
            (!repository.is_empty()).then(|| repository.to_string())
        }
        Err(_) if is_release_build() => Some(OFFICIAL_GITHUB_REPOSITORY.to_string()),
        Err(_) => None,
    }
}

fn github_repository() -> Result<String> {
    let repository = configured_github_repository().context(
        "Daanio updates are not configured; set DAANIO_GITHUB_REPOSITORY=owner/repository",
    )?;
    if repository.is_empty() || !repository.contains('/') {
        anyhow::bail!("DAANIO_GITHUB_REPOSITORY must use the owner/repository format");
    }
    Ok(repository)
}
...
pub fn is_release_build() -> bool {
    daanio_build_meta::is_release_build()
}

fn current_update_semver() -> &'static str {
    daanio_build_meta::update_semver()
}
...
fn metadata_path() -> Result<PathBuf> {
    Ok(storage::daanio_dir()?.join("update_metadata.json"))
}

fn source_build_root() -> Result<PathBuf> {
    Ok(storage::daanio_dir()?.join("builds").join("source"))
}

fn source_build_repo_dir() -> Result<PathBuf> {
    Ok(source_build_root()?.join("daanio"))
}
...
pub fn should_auto_update() -> bool {
    if std::env::var("DAANIO_NO_AUTO_UPDATE").is_ok() {
        return false;
    }
...
pub fn fetch_latest_release_blocking() -> Result<GitHubRelease> {
    let github_repo = github_repository()?;
    let url = format!(
...
fn github_api_request(
    ...
    if let Some(token) = daanio_base::github::github_public_api_token() {
...
fn synthetic_main_release(latest_sha: &str) -> GitHubRelease {
    let github_repo = github_repository().unwrap_or_else(|_| "unconfigured/daanio-cli".to_string());
    GitHubRelease {
...
fn prepare_stable_update_blocking() -> Result<PreparedUpdate> {
    let current_version = daanio_build_meta::version();
...
/// Build daanio from source by cloning/pulling the repo and running cargo build
fn build_from_source() -> Result<PathBuf> {
    let started = Instant::now();
    let build_dir = source_build_root()?;
    fs::create_dir_all(&build_dir)?;

    let repo_dir = build_dir.join("daanio");
    ...
    if repo_dir.join(".git").exists() {
        ...
        let clone_url = format!("https://github.com/{}.git", github_repository()?);
        let output = std::process::Command::new("git")
            .args([
                "clone", "--depth", "1", "--branch", "main", &clone_url, "daanio",
            ])
...
pub fn download_and_install_blocking_with_progress(
    ...
    let temp_path = temp_dir.join(format!("daanio-update-{}", std::process::id()));
    ...
    if asset.name.ends_with(".tar.gz") {
        ...
        for entry in archive.entries()? {
            ...
            entry.unpack(&dest)?;
            if file_name.starts_with("daanio") && !file_name.ends_with(".bin") {
                extracted_binary = Some(dest);
            }
        }
        let Some(extracted_binary) = extracted_binary else {
            anyhow::bail!("Could not find daanio binary inside tar.gz archive");
        };
...
pub fn check_and_maybe_update(auto_install: bool) -> UpdateCheckResult {
    ...
    if !updates_configured() || !should_auto_update() {
    ...
    match check_for_update_blocking() {
        Ok(Some(release)) => {
            let current = daanio_build_meta::version().to_string();
...
mod tests {
    use super::*;
    use daanio_update_core::parse_sha256sums;
    ...
    fn test_asset_name() {
        ...
        assert!(name.starts_with("daanio-"));
    }
    ...
    fn test_parse_sha256sums_accepts_standard_and_binary_lines() {
        ...
        assert_eq!(
            parsed.get("daanio-linux-x86_64.tar.gz").map(String::as_str),
            Some(digest_a.as_str())
        );
        assert_eq!(
            parsed.get("daanio-windows-x86_64.exe").map(String::as_str),
    ...
    fn test_verify_asset_checksum_text_accepts_matching_digest() {
        ...
        let contents = format!("{}  daanio-linux-x86_64.tar.gz\n", digest);
        verify_asset_checksum_text(&contents, "daanio-linux-x86_64.tar.gz", bytes).unwrap();
    }
    ...
    fn test_verify_asset_checksum_text_rejects_mismatch() {
        ...
        let contents = format!("{}  daanio-linux-x86_64.tar.gz\n", wrong);
        let err = verify_asset_checksum_text(&contents, "daanio-linux-x86_64.tar.gz", b"actual")
            .unwrap_err()
            .to_string();
    ...
    fn test_parse_sha256sums_rejects_invalid_digest() {
        let err = parse_sha256sums("not-a-sha  daanio-linux-x86_64.tar.gz\n")
            .unwrap_err()
            .to_string();
    ...
    fn test_summarize_git_pull_failure_diverged() {
        ...
        assert_eq!(
            ...
            daanio_update_core::GIT_PULL_DIVERGED_SUMMARY
        );
        assert!(daanio_update_core::summary_is_divergence(
            &summarize_git_pull_failure(stderr)
        ));
    }
