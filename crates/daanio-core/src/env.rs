...
/// Mutate the process environment for daanio runtime configuration.
///
/// Rust 2024 makes environment mutation unsafe because it can race with
/// concurrent environment access in foreign code. daanio intentionally mutates
...
where
    ...
    V: AsRef<OsStr>,
{
    // SAFETY: daanio treats these mutations as process-global configuration.
...
/// Remove a process environment variable used by daanio runtime configuration.
