...
fn make_test_provider(fetched: Vec<String>) -> CopilotApiProvider {
    CopilotApiProvider {
        client: daanio_base::provider::shared_http_client(),
...
fn context_window_handles_dot_and_dash_names() {
    assert_eq!(
        daanio_base::provider::context_limit_for_model_with_provider(
            "claude-opus-4.6",
            Some("copilot")
        ),
        Some(200_000)
    ...
    assert_eq!(
        daanio_base::provider::context_limit_for_model_with_provider("gpt-5.4", Some("copilot")),
        Some(128_000)
    );
    assert_eq!(
        daanio_base::provider::context_limit_for_model_with_provider(
