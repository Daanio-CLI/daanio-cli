...
//! tier detection, premium request modes), moved out of `daanio-base` so
...
//! root registers [`CopilotApiProvider`] with `daanio_base::provider::external`
//! at startup.

use anyhow::Result;
use async_trait::async_trait;
use chrono::Utc;
use daanio_base::auth::copilot as copilot_auth;
use daanio_message_types::{
    ContentBlock, Message as ChatMessage, Role, StreamEvent, ToolDefinition,
};
#[cfg(test)]
use daanio_provider_copilot::max_token_parameter_for_model as copilot_max_token_parameter_for_model;
use daanio_provider_copilot::{
...
use daanio_provider_copilot::{DEFAULT_MODEL, FALLBACK_MODELS};
...
impl CopilotApiProvider {
    ...
    fn persisted_catalog_path() -> Result<std::path::PathBuf> {
        Ok(daanio_base::storage::app_config_dir()?.join("copilot_models_cache.json"))
    }

    fn load_persisted_catalog() -> Option<PersistedCatalog> {
        let path = Self::persisted_catalog_path().ok()?;
        daanio_base::storage::read_json(&path)
            .ok()
    ...
    fn persist_catalog(models: &[String]) {
        ...
        if let Err(error) = daanio_base::storage::write_json(&path, &payload) {
    ...
    pub fn new() -> Result<Self> {
        ...
        let model =
            std::env::var("DAANIO_COPILOT_MODEL").unwrap_or_else(|_| DEFAULT_MODEL.to_string());
    ...
    fn env_premium_mode() -> u8 {
        match std::env::var("DAANIO_COPILOT_PREMIUM").ok().as_deref() {
    ...
    fn startup_prefetch_grace_ms() -> u64 {
        std::env::var("DAANIO_COPILOT_PREFETCH_STARTUP_GRACE_MS")
            .ok()
    ...
    pub fn set_premium_mode(&self, mode: PremiumMode) {
        ...
        if mode != PremiumMode::Normal {
            daanio_base::logging::info(&format!("Copilot premium mode set to {:?}", mode));
        }
    }
    ...
    /// If DAANIO_COPILOT_MODEL is set, this is a no-op (user override).
    pub async fn detect_tier_and_set_default(&self) {
        let detect_start = std::time::Instant::now();
        if std::env::var("DAANIO_COPILOT_MODEL").is_ok() {
            daanio_base::logging::info(
                "Copilot model overridden via DAANIO_COPILOT_MODEL, skipping tier detection",
            );
            self.mark_init_done();
            return;
        }
    ...
    async fn stream_request(
        ...
        for attempt in 0..MAX_RETRIES {
            if attempt > 0 {
                let delay = daanio_provider_core::attempt_tracker::retry_backoff_delay(
                    attempt,
                    RETRY_BASE_DELAY_MS,
                );
                daanio_base::logging::info(&format!(
            ...
            if Self::is_auth_error(status) && !attempted_auth_refresh {
                ...
                *self.bearer_token.write().await = None;
                daanio_base::logging::info("Copilot bearer token expired, refreshing...");
                last_error = Some(anyhow::anyhow!("Copilot auth error (HTTP {})", status));
                continue;
            }

            if !status.is_success() {
                let body_text = daanio_base::util::http_error_body(resp, "HTTP error").await;
                let error_str =
    ...
    async fn process_sse_stream(
        ...
        loop {
            ...
            while let Some(line_end) = buffer.find('\n') {
                ...
                if let Some(data) = daanio_base::util::sse_data_line(&line) {
                    if data.trim() == "[DONE]" {
                        ...
                        daanio_base::copilot_usage::record_request(
                            input_tokens,
                            output_tokens,
                            true,
                        );
...
impl Provider for CopilotApiProvider {
    ...
    fn context_window(&self) -> usize {
        daanio_provider_core::context_limit_for_model_with_provider(
            &self.model(),
            Some(self.name()),
        )
        .unwrap_or(128_000)
