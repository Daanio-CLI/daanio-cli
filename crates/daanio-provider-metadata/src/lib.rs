...
fn login_providers_for_surface(surface: LoginProviderSurface) -> Vec<LoginProviderDescriptor> {
    ...
    let mut providers = login_providers()
        .iter()
        .copied()
        .filter(|provider| !first_party_only || provider.id == DAANIO_LOGIN_PROVIDER.id)
...
pub fn resolve_login_provider(input: &str) -> Option<LoginProviderDescriptor> {
    ...
    login_providers().iter().copied().find(|provider| {
        (std::env::var_os("DAANIO_FIRST_PARTY_ONLY").is_none()
            || provider.id == DAANIO_LOGIN_PROVIDER.id)
...
pub fn resolve_login_provider_loose(input: &str) -> Option<LoginProviderDescriptor> {
    ...
    login_providers().iter().copied().find(|provider| {
        (std::env::var_os("DAANIO_FIRST_PARTY_ONLY").is_none()
            || provider.id == DAANIO_LOGIN_PROVIDER.id)
            && provider.display_name.to_ascii_lowercase() == normalized
    })
}
...
mod tests {
    ...
    #[test]
    fn daanio_profile_matches_official_getting_started_docs() {
        assert_eq!(DAANIO_PROFILE.id, "daanio");
        assert_eq!(DAANIO_PROFILE.api_base, "https://api.daanio.com/v1");
        assert_eq!(DAANIO_PROFILE.api_key_env, "DAANIO_API_KEY");
        assert_eq!(DAANIO_PROFILE.env_file, "daanio.env");
        assert_eq!(
            DAANIO_PROFILE.setup_url,
            "https://daanio.com/docs/getting-started"
        );
        assert_eq!(DAANIO_PROFILE.default_model, Some("gpt-5.6-sol"));
        assert!(DAANIO_PROFILE.requires_api_key);
        assert!(DAANIO_LOGIN_PROVIDER.recommended);
        assert_eq!(
            DAANIO_LOGIN_PROVIDER.auth_kind,
            LoginProviderAuthKind::DeviceCode
        );
        assert_eq!(DAANIO_LOGIN_PROVIDER.target, LoginProviderTarget::Daanio);
    }

    #[test]
    fn matrix_login_provider_aliases_resolve_to_canonical_ids() {
        assert_eq!(
            resolve_login_provider("daanio-api").map(|provider| provider.id),
            Some("daanio")
        );
        assert_eq!(
    ...
    #[test]
    fn matrix_cli_login_selection_prioritizes_daanio() {
