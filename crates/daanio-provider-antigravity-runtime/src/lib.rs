...
//! Gemini/Claude/gpt-oss upstreams), moved out of `daanio-base` so provider
...
//! [`AntigravityProvider`] with `daanio_base::provider::external` at startup.

use anyhow::{Context, Result};
use async_trait::async_trait;
use daanio_base::auth::antigravity as antigravity_auth;
use daanio_message_types::{ConnectionPhase, Message, StreamEvent, ToolDefinition};
use daanio_provider_antigravity::{
...
impl AntigravityProvider {
    fn load_persisted_catalog() -> Option<PersistedCatalog> {
        daanio_base::provider::antigravity::load_persisted_catalog()
    }

    fn persist_catalog(snapshot: &CatalogSnapshot) {
        daanio_base::provider::antigravity::persist_catalog(snapshot);
    }
    ...
    pub fn new() -> Self {
        let model =
            std::env::var("DAANIO_ANTIGRAVITY_MODEL").unwrap_or_else(|_| DEFAULT_MODEL.into());
    ...
    /// by `daanio login --provider antigravity`; there is no API-key path. This
    ...
    fn resolve_model_for_request(&self, model: &str) -> String {
        ...
        // Claude models on the Cloud Code backend currently reject daanio's tool
    ...
    async fn fetch_available_models(&self) -> Result<CatalogSnapshot> {
        daanio_base::provider::antigravity::fetch_catalog_snapshot(&self.client).await
    }
    ...
    async fn generate_content(
        ...
        let mut tools = daanio_provider_gemini::build_tools(tools);
        ...
        // construct. Gemini-native accepts everything daanio emits, so Gemini
        ...
        let request = CodeAssistGenerateRequest {
            ...
            request: VertexGenerateContentRequest {
                contents: daanio_provider_gemini::build_contents(messages),
                system_instruction:
                    daanio_provider_gemini::build_system_instruction_with_tool_guard(
                        system,
                        !tools_is_empty,
                    ),
                tools,
...
impl Provider for AntigravityProvider {
    async fn complete(
        ...
        let (tx, rx) = mpsc::channel::<Result<daanio_message_types::StreamEvent>>(100);
        ...
        tokio::spawn(async move {
            ...
            if let Some(content) = candidate.content {
                ...
                // calls through the standard ToolUseStart/End path so daanio
                ...
                for part in content.parts {
                    ...
                    if let Some(function_call) = part.function_call {
                        ...
                        let call_id = daanio_message_types::sanitize_tool_id(&raw_call_id);
                        let _ = tx
            ...
            if !produced_output {
                ...
                if abnormal {
                    ...
                    let detail = candidate
                        ...
                        .map(|msg| {
                            format!(": {}", daanio_base::util::truncate_str(msg.trim(), 300))
                        })
                        .unwrap_or_default();
                    let _ = tx
                        .send(Err(anyhow::anyhow!(
    ...
    fn model_routes(&self) -> Vec<daanio_provider_core::ModelRoute> {
        let catalog = self.fetched_catalog();
        if !catalog.is_empty() {
            return catalog
                .into_iter()
                .map(|model| daanio_provider_core::ModelRoute {
