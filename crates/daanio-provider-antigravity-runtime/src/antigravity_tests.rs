...
use daanio_provider_core::Provider;
...
fn available_models_display_seeds_from_persisted_catalog() {
    ...
    let previous = std::env::var_os("DAANIO_HOME");
    daanio_base::env::set_var("DAANIO_HOME", temp.path());

    let path = daanio_base::provider::antigravity::persisted_catalog_path().expect("catalog path");
    daanio_base::storage::write_json(
        &path,
        &PersistedCatalog {
    ...
    if let Some(previous) = previous {
        daanio_base::env::set_var("DAANIO_HOME", previous);
    } else {
        daanio_base::env::remove_var("DAANIO_HOME");
    }
}
...
fn resolve_model_for_request_default_prefers_gemini_catalog_model() {
    ...
    *provider.fetched_catalog.write().expect("catalog lock") = vec![
    ...
    // Cloud Code backend. Claude on this backend rejects daanio's tool schemas.
    assert_eq!(
        provider.resolve_model_for_request("default"),
        "gemini-3-flash"
    );
...
fn antigravity_compatible_schema_passes_gemini_through_unchanged() {
    // Gemini is the native backend path; it accepts everything daanio emits, so
