...
use super::*;
use daanio_base::provider::{ModelCatalogRefreshSummary, summarize_model_catalog_refresh};
...
impl Provider for OpenRouterProvider {
    ...
    async fn complete(
        ...
        let api_messages = daanio_provider_openrouter::request::build_chat_messages(
            &effective_messages,
            system,
            allow_reasoning,
            ...
            .map(|t| {
                serde_json::json!({
                    ...
                    "function": {
                        ...
                        "parameters": daanio_provider_openrouter::request::sanitize_tool_parameters_schema(&t.input_schema),
                    }
                })
            })
            .collect();
        ...
        if let Some(effort) = reasoning_effort.as_deref() {
            if self.supports_deepseek_reasoning_effort() {
                let effort = self.wire_reasoning_effort(effort);
                if effort != "none" || self.is_daanio_subscription_runtime() {
            ...
            {
                // Reasoning models on compatible gateways take the standard
                // top-level `reasoning_effort` field. This also covers Claude
                // models routed through Daanio's managed subscription gateway.
                let effort = self.wire_reasoning_effort(effort);
                if effort != "none" || self.is_daanio_subscription_runtime() {
                    request["reasoning_effort"] = serde_json::json!(effort);
                    sent_reasoning_config = true;
                ...
                let effort = if daanio_base::prompt::is_swarm_effort(effort) {
                    "xhigh"
                } else {
                    effort
                };
        ...
        // daanio-generated field with the same key (issue #341).
        ...
        daanio_base::logging::info("OpenRouter transport: HTTPS (SSE)");
    ...
    fn set_model(&self, model: &str) -> Result<()> {
        ...
        let (model_id, provider) = if self.supports_provider_features {
            ...
            let model_id = if provider.is_some() {
                daanio_base::provider::openrouter_catalog_model_id(&model_id).unwrap_or(model_id)
            } else {
                model_id
            };
            (model_id, provider)
        ...
        if let Some(stored_effort) = stored_effort
            && !daanio_base::prompt::is_swarm_effort(&stored_effort)
    ...
    fn set_reasoning_effort(&self, effort: &str) -> Result<()> {
        if !self.supports_any_reasoning_effort() {
            anyhow::bail!(
                "Reasoning effort is not supported by the current model/profile. It works for Daanio-advertised Claude models, OpenRouter, DeepSeek-family and GPT-family reasoning models, and profiles with supports_reasoning_effort = true."
            );
        }
    ...
    fn available_efforts(&self) -> Vec<&'static str> {
            ...
            || self.supports_openai_reasoning_effort()
        {
            ...
            daanio_provider_core::OPENROUTER_SELECTABLE_EFFORTS.to_vec()
        } else {
            vec![]
        }
    }
    ...
    fn model_routes(&self) -> Vec<daanio_provider_core::ModelRoute> {
        ...
        self.available_models_display()
            .into_iter()
            .filter(|model| daanio_base::provider::is_listable_model_name(model))
            .map(|model| {
                ...
                daanio_provider_core::ModelRoute {
                    model,
    ...
    fn context_window(&self) -> usize {
        ...
        if let Some(profile_id) = self.profile_id.as_deref()
            && let Some(limit) =
                daanio_base::provider_catalog::openai_compatible_profile_context_limit(
                    profile_id, &model_id,
                )
        {
            return limit;
        }
        daanio_provider_core::context_limit_for_model_with_provider(&model_id, Some(self.name()))
