...
//! NVIDIA NIM), moved out of `daanio-base` so provider edits compile only this
...
//! with `daanio_base::provider::external::register_openrouter_factory`.
...
//! - Manual pinning: Set DAANIO_OPENROUTER_PROVIDER or use model@Provider syntax

use anyhow::{Context, Result};
use async_trait::async_trait;
use daanio_base::provider_catalog::{
...
use daanio_message_types::{
    CacheControl, ContentBlock, Message, Role, StreamEvent, ToolDefinition,
};
use daanio_provider_core::{EventStream, Provider};
pub use daanio_provider_openrouter::{
...
use daanio_provider_openrouter::{
...
const OPENROUTER_TRANSPORT_STATE_ENV: &str = "DAANIO_OPENROUTER_TRANSPORT_STATE";
...
fn explicit_openrouter_runtime_configured() -> bool {
    [
        ...
        "DAANIO_OPENROUTER_DYNAMIC_BEARER_PROVIDER",
    ]
    .iter()
...
-> Option<daanio_base::provider_catalog::ResolvedOpenAiCompatibleProfile> {
    ...
    let mut matches = openai_compatible_profiles()
        ...
        .filter_map(|profile| {
            ...
            if daanio_base::provider_catalog::openai_compatible_profile_is_configured(*profile) {
                Some(resolved)
            } else {
                None
            }
...
fn configured_api_base() -> String {
    let raw = std::env::var("DAANIO_OPENROUTER_API_BASE")
        .ok()
    ...
    normalize_api_base(&raw).unwrap_or_else(|| {
        daanio_base::logging::warn(&format!(
            "Ignoring invalid DAANIO_OPENROUTER_API_BASE '{}'; using {}",
...
fn configured_api_key_name() -> String {
    let raw = std::env::var("DAANIO_OPENROUTER_API_KEY_NAME")
        .ok()
        .map(|v| v.trim().to_string())
        .filter(|v| !v.is_empty())
    ...
    if is_safe_env_key_name(&raw) {
        ...
        daanio_base::logging::warn(&format!(
            "Ignoring invalid DAANIO_OPENROUTER_API_KEY_NAME '{}'; using {}",
...
fn configured_env_file_name() -> String {
    let raw = std::env::var("DAANIO_OPENROUTER_ENV_FILE")
        .ok()
        .map(|v| v.trim().to_string())
        .filter(|v| !v.is_empty())
    ...
    if is_safe_env_file_name(&raw) {
        ...
        daanio_base::logging::warn(&format!(
            "Ignoring invalid DAANIO_OPENROUTER_ENV_FILE '{}'; using {}",
...
fn load_named_profile_api_key(
    env_key: &str,
    profile: &daanio_base::config::NamedProviderConfig,
) -> Option<String> {
...
fn provider_features_enabled(api_base: &str) -> bool {
    if let Ok(raw) = std::env::var("DAANIO_OPENROUTER_PROVIDER_FEATURES") {
        if let Some(value) = parse_env_bool(&raw) {
            return value;
        }
        daanio_base::logging::warn(&format!(
            "Ignoring invalid DAANIO_OPENROUTER_PROVIDER_FEATURES '{}'; expected true/false",
            raw
        ));
    }
...
fn model_catalog_enabled() -> bool {
    if let Ok(raw) = std::env::var("DAANIO_OPENROUTER_MODEL_CATALOG") {
        if let Some(value) = parse_env_bool(&raw) {
            return value;
        }
        daanio_base::logging::warn(&format!(
            "Ignoring invalid DAANIO_OPENROUTER_MODEL_CATALOG '{}'; expected true/false",
            raw
        ));
    }
    true
...
fn configured_auth_header_mode() -> AuthHeaderMode {
    let Some(raw) = std::env::var("DAANIO_OPENROUTER_AUTH_HEADER")
        .ok()
    ...
    match raw.as_str() {
        ...
        other => {
            daanio_base::logging::warn(&format!(
                "Ignoring invalid DAANIO_OPENROUTER_AUTH_HEADER '{}'; expected authorization-bearer or api-key",
                other
            ));
...
fn configured_auth_header_name() -> HeaderName {
    let raw = std::env::var("DAANIO_OPENROUTER_AUTH_HEADER_NAME")
        .ok()
        .map(|v| v.trim().to_string())
        .filter(|v| !v.is_empty())
    ...
    HeaderName::from_bytes(raw.as_bytes()).unwrap_or_else(|_| {
        daanio_base::logging::warn(&format!(
            "Ignoring invalid DAANIO_OPENROUTER_AUTH_HEADER_NAME '{}'; using api-key",
            raw
        ));
...
fn configured_dynamic_bearer_provider() -> Option<String> {
    std::env::var("DAANIO_OPENROUTER_DYNAMIC_BEARER_PROVIDER")
        .ok()
        .map(|v| v.trim().to_ascii_lowercase())
        .filter(|v| !v.is_empty())
}
...
fn configured_allow_no_auth() -> bool {
    std::env::var("DAANIO_OPENROUTER_ALLOW_NO_AUTH")
        .ok()
...
pub enum OpenRouterTransportState {
    ...
    OpenRouterApiKey,
    /// Daanio subscription access currently reuses the OpenRouter HTTP slot, but is
...
impl OpenRouterTransportState {
    pub fn from_current_env(runtime_provider: Option<&str>) -> Self {
        ...
        if matches!(runtime_provider.as_deref(), Some("daanio")) {
            return Self::DaanioSubscription;
        }
    ...
    fn from_env_marker() -> Option<Self> {
        ...
        match value.as_str() {
            ...
            "daanio" | "daanio-subscription" | "subscription" => Some(Self::DaanioSubscription),
            ...
            other => {
                daanio_base::logging::warn(&format!(
                    "Ignoring invalid {} '{}'; expected openrouter-api-key, daanio-subscription, direct-api-key, or direct-no-auth",
    ...
    fn runtime_provider_is_direct_compatible(runtime_provider: Option<&str>) -> bool {
        matches!(runtime_provider, Some("openai-compatible" | "azure-openai"))
            || runtime_provider
                .and_then(daanio_base::provider_catalog::openai_compatible_profile_by_id)
                .is_some()
    }
...
impl ProviderAuth {
    async fn apply(&self, req: reqwest::RequestBuilder) -> Result<reqwest::RequestBuilder> {
        match self {
            ...
            Self::AzureEntra { .. } => {
                let token = daanio_base::auth::azure::get_bearer_token().await?;
...
async fn fetch_models_from_api(
    ...
    if !response.status().is_success() {
        ...
        let body = daanio_base::util::http_error_body(response, "HTTP error").await;
        anyhow::bail!(
    ...
    let models = parse_openai_compatible_models_response(&raw_body).with_context(|| {
            format!(
                ...
                url,
                auth.label(),
                daanio_base::util::truncate_str(&raw_body.trim().replace('\n', "\\n"), 1200)
            )
        })?;
...
pub fn maybe_schedule_openai_compatible_profile_catalog_refresh(
    profile: daanio_base::provider_catalog::OpenAiCompatibleProfile,
    context: &'static str,
) -> bool {
    let resolved = daanio_base::provider_catalog::resolve_openai_compatible_profile(profile);
    ...
    let previous_fingerprint =
        daanio_provider_openrouter::load_disk_cache_entry_for_namespace(&profile_id)
    ...
    handle.spawn(async move {
        ...
        match fetch_models_from_api(
            daanio_provider_core::shared_http_client(),
            api_base,
            auth,
            models_cache,
        ...
        .await
        {
            Ok(models) => {
                ...
                if updated {
                    ...
                    daanio_base::bus::Bus::global().publish_models_updated();
                } else {
                    daanio_base::logging::info(&format!(
            ...
            Err(error) => daanio_base::logging::info(&format!(
...
pub fn maybe_schedule_standard_openrouter_catalog_refresh(context: &'static str) -> bool {
    ...
    // when DAANIO_OPENROUTER_* env vars are set by an active named profile
    // (e.g. NVIDIA NIM via `[providers.mynvidia]`, which sets
    // DAANIO_OPENROUTER_API_BASE to the NVIDIA endpoint): that profile owns the
    ...
    let cache_is_fresh = current_unix_secs()
        .zip(daanio_provider_openrouter::load_disk_cache_entry_for_namespace(namespace))
        .map(|(now, cache)| {
    ...
    let previous_fingerprint =
        daanio_provider_openrouter::load_disk_cache_entry_for_namespace(namespace)
            .map(|cache| models_fingerprint(&cache.models))
            .unwrap_or_default();
    handle.spawn(async move {
        let models_cache = Arc::new(RwLock::new(ModelsCache::default()));
...
pub struct OpenRouterProvider {
    ...
    /// `DAANIO_OPENAI_EXTRA_BODY` env/env-file value.
...
impl OpenRouterProvider {
    ...
    fn configured_max_tokens(profile_id: Option<&str>) -> Option<u32> {
        if let Ok(raw) = std::env::var("DAANIO_OPENROUTER_MAX_TOKENS") {
            ...
            match trimmed.parse::<u32>() {
                ...
                Err(_) => daanio_base::logging::warn(&format!(
                    "Ignoring invalid DAANIO_OPENROUTER_MAX_TOKENS '{}'; expected a positive integer or auto",
                    raw
                )),
            }
        }
    ...
    /// 2. The `DAANIO_OPENAI_EXTRA_BODY` env var (or the same key inside the
    ...
    fn resolve_extra_body(
        ...
        if let Some(value) = config {
            match value.as_object() {
                ...
                None => daanio_base::logging::warn(
                    "Ignoring provider `extra_body`: expected a table/object of top-level request fields",
                ),
            }
        }

        if let Some(raw) = load_env_value_from_env_or_config("DAANIO_OPENAI_EXTRA_BODY", env_file) {
            match serde_json::from_str::<Value>(&raw) {
                ...
                Ok(_) => daanio_base::logging::warn(
                    "Ignoring DAANIO_OPENAI_EXTRA_BODY: expected a JSON object string, e.g. {\"chat_template_kwargs\":{\"thinking\":true}}",
                ),
                Err(err) => daanio_base::logging::warn(&format!(
                    "Ignoring invalid DAANIO_OPENAI_EXTRA_BODY JSON: {err}"
                )),
            }
        }
    ...
    ///
    /// Unlike the env-var based [`daanio_base::provider_catalog::runtime_provider_display_name`],
    ...
    pub fn runtime_display_name(&self) -> String {
        if self.is_daanio_subscription_runtime() {
            return daanio_base::subscription_catalog::DAANIO_PROVIDER_DISPLAY_NAME.to_string();
        }
        ...
        if !self.supports_provider_features {
            if let Some(profile_id) =
                daanio_base::provider_catalog::openai_compatible_profile_id_for_api_base(
                    &self.api_base,
                )
            ...
            if std::env::var("DAANIO_RUNTIME_PROVIDER")
                .ok()
    ...
    pub fn direct_openai_compatible_route_parts(&self) -> Option<(String, String, String)> {
        ...
        if self.is_daanio_subscription_runtime() {
            return Some((
                daanio_base::subscription_catalog::DAANIO_PROVIDER_DISPLAY_NAME.to_string(),
                daanio_base::subscription_catalog::DAANIO_ROUTE_API_METHOD.to_string(),
                self.api_base.clone(),
            ));
        }
    ...
    /// scoped `DAANIO_API_KEY`, then routes through the OpenAI-compatible
    ...
    fn is_daanio_subscription_runtime(&self) -> bool {
        !self.supports_provider_features
            && self.api_base.trim_end_matches('/')
                == daanio_base::subscription_catalog::DEFAULT_DAANIO_API_BASE.trim_end_matches('/')
            && self
                .auth
                .label()
                .eq_ignore_ascii_case(daanio_base::subscription_catalog::DAANIO_API_KEY_ENV)
    }
    ...
    pub fn new_named_openai_compatible(
        ...
        daanio_base::env::set_var("DAANIO_OPENROUTER_CACHE_NAMESPACE", profile_name);
        ...
        let auth = match profile.auth {
            daanio_base::config::NamedProviderAuth::None => ProviderAuth::None {
                label: "local endpoint (no auth)".to_string(),
            },
            daanio_base::config::NamedProviderAuth::Bearer => ProviderAuth::AuthorizationBearer {
                token: key
                    .ok_or_else(|| anyhow::anyhow!("{} not found in environment", key_label))?,
                label: key_label,
            },
            daanio_base::config::NamedProviderAuth::Header => ProviderAuth::HeaderValue {
        ...
        Ok(Self {
            ...
            supports_provider_features: matches!(
                ...
                daanio_base::config::NamedProviderType::OpenRouter
    ...
    fn is_kimi_model(model: &str) -> bool {
        daanio_provider_openrouter::is_kimi_model(model)
    }
    ...
    fn strip_session_profile_prefix<'a>(&self, model: &'a str) -> &'a str {
        ...
        if daanio_provider_core::explicit_model_provider_prefix(model).is_some() {
            return model;
        }
        let rest = rest.trim();
        if rest.is_empty() {
    ...
    fn thinking_override() -> Option<bool> {
        let raw = std::env::var("DAANIO_OPENROUTER_THINKING").ok()?;
        ...
        match value.as_str() {
            ...
            other => {
                daanio_base::logging::info(&format!(
                    "Warning: Unsupported DAANIO_OPENROUTER_THINKING '{}'; expected enabled/disabled/auto",
                    other
                ));
                None
            }
    ...
    pub fn new() -> Result<Self> {
        ...
        let profile_id = std::env::var("DAANIO_OPENROUTER_CACHE_NAMESPACE")
            .ok()
            .map(|value| value.trim().to_ascii_lowercase())
            .filter(|value| !value.is_empty())
        ...
        let static_context_limits = profile_id
            ...
            .unwrap_or_default();
        let static_models = std::env::var("DAANIO_OPENROUTER_STATIC_MODELS")
            .ok()
            .map(|raw| {
                raw.lines()
                    .map(str::trim)
        ...
        if std::env::var_os("DAANIO_OPENROUTER_CACHE_NAMESPACE").is_none()
            && let Some(profile) = autodetected_profile.as_ref()
        {
            daanio_base::env::set_var("DAANIO_OPENROUTER_CACHE_NAMESPACE", &profile.id);
        }

        let model = std::env::var("DAANIO_OPENROUTER_MODEL")
            .ok()
    ...
    pub fn new_openrouter_api_key_runtime() -> Result<Self> {
        let api_key = load_api_key_from_env_or_config(DEFAULT_API_KEY_NAME, DEFAULT_ENV_FILE)
            .ok_or_else(|| {
                let path = daanio_base::storage::app_config_dir()
    ...
    pub fn new_openai_compatible_profile_runtime(
        ...
        let auth = match load_api_key_from_env_or_config(&resolved.api_key_env, &resolved.env_file)
        {
            ...
            None => {
                ...
                anyhow::bail!(
                    "{} credentials not available. {} not found in environment or {}. Run `daanio login --provider {}` first.",
    ...
    pub(crate) fn filter_profile_chat_supported_models(&self, models: Vec<String>) -> Vec<String> {
        ...
        models
            .into_iter()
            .filter(|model| {
                daanio_base::provider_catalog::openai_compatible_profile_model_supports_chat(
                    profile_id, model,
                )
            })
            .collect()
    ...
    fn model_disk_cache_source_matches(
        &self,
        cache_entry: &daanio_provider_openrouter::DiskCache,
    ) -> bool {
    ...
    pub(crate) fn load_usable_model_disk_cache_entry(
        &self,
    ) -> Option<daanio_provider_openrouter::DiskCache> {
    ...
    fn maybe_schedule_model_catalog_refresh(&self, cache_age_secs: u64, context: &'static str) {
        ...
        handle.spawn(async move {
            match fetch_models_from_api(client, api_base, auth, models_cache, None).await {
                ...
                Err(e) => daanio_base::logging::info(&format!(
    ...
    fn parse_provider_routing() -> ProviderRouting {
        daanio_provider_openrouter::parse_provider_routing_from_env()
    }
    ...
    fn rank_providers_from_endpoints(endpoints: &[EndpointInfo]) -> Vec<String> {
        daanio_provider_openrouter::rank_providers_from_endpoints(endpoints)
    }
    ...
    pub fn has_credentials() -> bool {
        if matches!(
            ...
            return daanio_base::auth::azure::has_configuration();
        }
        if configured_allow_no_auth() {
            return true;
        }
    ...
    fn resolve_auth() -> Result<ProviderAuth> {
        if let Some(provider) = configured_dynamic_bearer_provider() {
            return match provider.as_str() {
                "azure" => {
                    if daanio_base::auth::azure::has_configuration() {
                        Ok(ProviderAuth::AzureEntra {
                            label: "Azure OpenAI Entra ID".to_string(),
                        })
                    } else {
                        anyhow::bail!(
                            "Azure OpenAI is configured for Entra ID, but Azure settings are incomplete. Run `daanio login --provider azure`."
                        )
                    }
                }
                other => anyhow::bail!(
                    "Unsupported DAANIO_OPENROUTER_DYNAMIC_BEARER_PROVIDER '{}'.",
                    other
                ),
            };
        }
