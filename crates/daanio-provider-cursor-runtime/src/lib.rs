...
//! `daanio_base::provider::cursor` because base's model-routing logic needs it
//! without a runtime.

use anyhow::{Context, Result};
use async_trait::async_trait;
use chrono::Utc;
use daanio_base::auth::cursor as cursor_auth;
use daanio_base::provider::cursor::{AVAILABLE_MODELS, DEFAULT_MODEL};
...
fn runtime_cursor_api_key() -> Option<String> {
    daanio_base::auth::cursor::load_api_key().ok()
}
...
impl CursorCliProvider {
    fn persisted_catalog_path() -> Result<std::path::PathBuf> {
        Ok(daanio_base::storage::app_config_dir()?.join("cursor_models_cache.json"))
    }
    ...
    pub fn new() -> Self {
        let model = std::env::var("DAANIO_CURSOR_MODEL").unwrap_or_else(|_| DEFAULT_MODEL.into());
        let provider = Self {
