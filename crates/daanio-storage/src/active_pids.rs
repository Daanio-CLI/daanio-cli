//! Tracking of active session process IDs under `~/.daanio/active_pids`.
//!
...
//! lives in the storage crate because it only needs [`daanio_dir`] and is a
...
use crate::daanio_dir;
use std::path::PathBuf;

/// Directory holding one file per active session ID (`~/.daanio/active_pids`).
pub fn active_pids_dir() -> Option<PathBuf> {
    daanio_dir().ok().map(|d| d.join("active_pids"))
}
...
pub fn streaming_pids_dir() -> Option<std::path::PathBuf> {
    daanio_dir().ok().map(|d| d.join("streaming_pids"))
}
...
pub fn internal_pids_dir() -> Option<std::path::PathBuf> {
    daanio_dir().ok().map(|d| d.join("internal_pids"))
}
...
/// List active session IDs currently tracked in `~/.daanio/active_pids`.
...
/// Live snapshot of how many daanio sessions are running, and how many of those
/// are actively streaming a model response right now. Used by the menu bar
/// indicator (`daanio menubar`) and any other presence UI.
...
mod tests {
    ...
    /// Serialize tests that mutate `DAANIO_HOME`.
