...
mod tests {
    ...
    fn asset_name_is_supported() {
        assert_ne!(get_asset_name(), "daanio-unknown");
    }
    ...
    fn sha256sums_accepts_standard_and_binary_lines() {
        let checksums = parse_sha256sums(
            "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa  daanio-linux-x86_64\n\
             bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb *daanio-macos-aarch64\n",
        )
        .unwrap();
        assert_eq!(
            checksums.get("daanio-linux-x86_64").map(String::as_str),
            Some("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa")
        );
        assert_eq!(
            checksums.get("daanio-macos-aarch64").map(String::as_str),
    ...
    fn checksum_verification_accepts_matching_digest() {
        ...
        let sums = format!("{}  daanio-linux-x86_64\n", digest);
        verify_asset_checksum_text(&sums, "daanio-linux-x86_64", bytes).unwrap();
    }

    #[test]
    fn checksum_verification_rejects_mismatch() {
        let sums = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa  daanio-linux-x86_64\n";
        let err = verify_asset_checksum_text(sums, "daanio-linux-x86_64", b"hello")
            .unwrap_err()
            .to_string();
    ...
    fn sha256sums_rejects_invalid_digest() {
        let err = parse_sha256sums("not-a-digest  daanio\n")
            .unwrap_err()
            .to_string();
