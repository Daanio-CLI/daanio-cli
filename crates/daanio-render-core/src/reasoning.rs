...
//! consumes them. These live in `daanio-render-core` (a backend-neutral, pure
//! crate) rather than in `daanio-tui-markdown` so the foundation/streaming layer
//! can format reasoning lines without depending on any `daanio-tui-*` crate.
...
///
/// Lives here (a backend-neutral, pure crate) rather than in `daanio-tui-markdown`
/// so the foundation/streaming layer can format the summary without depending on
/// any `daanio-tui-*` crate. Re-exported from `daanio-tui-markdown` for the
/// existing `daanio_tui_markdown::reasoning_summary_line_markup` path.
