#!/usr/bin/env bash
#
# Stress test: spawn 40 daanio TUI client instances rapidly
# Measures startup time, memory usage, CPU, fd count, socket health
#
set -euo pipefail

NUM_INSTANCES=${1:-40}
DAANIO_BIN="${DAANIO_BIN:-$(which daanio)}"
LOG_DIR="/tmp/daanio-stress-test-$(date +%s)"
mkdir -p "$LOG_DIR"

MAIN_SOCK="/run/user/$(id -u)/daanio.sock"
DEBUG_SOCK="/run/user/$(id -u)/daanio-debug.sock"

echo "========================================="
echo " daanio Stress Test: $NUM_INSTANCES instances"
echo "========================================="
echo "Binary: $DAANIO_BIN"
...
snapshot() {
    ...
    # daanio process count and total RSS
    local daanio_procs=$(pgrep -c daanio 2>/dev/null || echo 0)
    local total_rss=0
    local total_vms=0
    for pid in $(pgrep daanio 2>/dev/null); do
    ...
    echo "daanio_processes: $daanio_procs" >> "$LOG_DIR/snapshots.log"
    ...
    echo "[$label] procs=$daanio_procs rss=${total_rss}KB($(( total_rss / 1024 ))MB) server_rss=$(awk '/^VmRSS:/{print $2}' /proc/${server_pid:-0}/status 2>/dev/null || echo '?')KB fds=$(ls /proc/${server_pid:-0}/fd 2>/dev/null | wc -l) threads=$(ls /proc/${server_pid:-0}/task 2>/dev/null | wc -l)"
}
...
if ! [ -S "$MAIN_SOCK" ]; then
    echo "ERROR: No daanio server running at $MAIN_SOCK"
    echo "Start one with: daanio serve &"
    exit 1
fi
...
BASELINE_RSS=$(pgrep daanio 2>/dev/null | while read pid; do awk '/^VmRSS:/{print $2}' /proc/$pid/status 2>/dev/null; done | paste -sd+ | bc 2>/dev/null || echo 0)
BASELINE_PROCS=$(pgrep -c daanio 2>/dev/null || echo 0)
...
(
    while true; do
        ts=$(date +%s)
        daanio_procs=$(pgrep -c daanio 2>/dev/null || echo 0)
        total_rss=0
        for pid in $(pgrep daanio 2>/dev/null); do
        ...
        echo "$ts,$daanio_procs,$total_rss,$server_rss,$server_fds,$server_threads,$cpu_load"
        sleep 1
    done
...
# --- Spawn instances ---
echo "=== Spawning $NUM_INSTANCES daanio instances ==="
PIDS=()
SPAWN_TIMES=()

# Use script to give each instance a pty (daanio requires tty)
for i in $(seq 1 $NUM_INSTANCES); do
    ...
    script -q -c "$DAANIO_BIN --no-update --no-selfdev" /dev/null \
...
POST_PROCS=$(pgrep -c daanio 2>/dev/null || echo 0)
POST_RSS=0
for pid in $(pgrep daanio 2>/dev/null); do
    rss=$(awk '/^VmRSS:/{print $2}' /proc/$pid/status 2>/dev/null || echo 0)
