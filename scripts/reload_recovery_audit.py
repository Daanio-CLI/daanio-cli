...
server but not every interrupted session continues. It correlates three sources:
  ...
  1. ~/.daanio/reload-recovery/*.json durable recovery intent records
  2. ~/.daanio/logs/daanio-*.log recovery lifecycle messages
  3. ~/.daanio/sessions/<session>.json coarse transcript markers
...
def daanio_home() -> pathlib.Path:
    return pathlib.Path(os.environ.get("DAANIO_HOME", pathlib.Path.home() / ".daanio"))


def log_files(log_dir: pathlib.Path, max_log_files: int) -> list[pathlib.Path]:
    files = sorted(log_dir.glob("daanio-*.log"), key=lambda p: p.stat().st_mtime, reverse=True)
...
def parse_trace_files(records: dict[tuple[str, str], IntentRecord], trace_dir: pathlib.Path) -> None:
    """Merge structured ~/.daanio/reload-traces/*.jsonl lifecycle events."""
    phase_to_kind = {
...
def main() -> int:
    ...
    parser.add_argument("--home", type=pathlib.Path, default=daanio_home(), help="DAANIO_HOME path")
    ...
    parser.add_argument("--max-log-files", type=int, default=3, help="Newest daanio logs to scan")
