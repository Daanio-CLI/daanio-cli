...
1. **`DaanioPhoneOperator`** for normal deployments and maintenance of the existing stack.
2. **`DaanioPhoneProvisioner`** for infrequent rebuilds, MFA-gated, time-limited, restricted to `us-east-1`, `daanio-phone-*` resources, and bounded runtime roles.
...
- `breaker-lambda.py` calls `ec2:DescribeInstances`, `ec2:StopInstances`, and `sns:Publish` to `arn:aws:sns:us-east-1:302154194530:daanio-guard-warn`.
  ...
  "Statement": [
    {
      ...
      "Resource": [
        "arn:aws:iam::302154194530:role/daanio-phone/DaanioPhoneOperator",
        "arn:aws:iam::302154194530:role/daanio-phone/DaanioPhoneProvisioner"
      ]
    }
  ]
}
...
Set `DaanioPhoneProvisioner` maximum session duration to one hour. Do not allow either role to change its own trust or permissions.
...
```json
{
  ...
  "Statement": [
    ...
    },
    {
      ...
      "Resource": "arn:aws:logs:us-east-1:302154194530:log-group:/aws/lambda/daanio-phone-wake:*"
    }
  ]
}
```
...
```json
{
  ...
  "Statement": [
    ...
    },
    {
      ...
      "Effect": "Allow",
      "Action": "sns:Publish",
      "Resource": "arn:aws:sns:us-east-1:302154194530:daanio-guard-warn"
    },
    {
      "Sid": "WriteOwnLogs",
      "Effect": "Allow",
      ...
      "Resource": "arn:aws:logs:us-east-1:302154194530:log-group:/aws/lambda/daanio-guard-breaker:*"
    }
  ]
}
```
...
```json
{
  ...
  "Statement": [
    ...
    },
    {
      ...
      "Resource": [
        "arn:aws:lambda:us-east-1:302154194530:function:daanio-phone-wake",
        "arn:aws:lambda:us-east-1:302154194530:function:daanio-phone-wake:*",
        "arn:aws:lambda:us-east-1:302154194530:function:daanio-guard-breaker",
        "arn:aws:lambda:us-east-1:302154194530:function:daanio-guard-breaker:*"
      ]
    },
    {
    ...
    },
    {
      ...
      "Effect": "Allow",
      "Action": "iam:PassRole",
      "Resource": "arn:aws:iam::302154194530:role/daanio-phone/runtime/DaanioPhoneInstance",
      "Condition": {
        "StringEquals": {
    ...
    },
    {
      ...
      "Action": "iam:PassRole",
      "Resource": [
        "arn:aws:iam::302154194530:role/daanio-phone/runtime/DaanioPhoneWakeLambda",
        "arn:aws:iam::302154194530:role/daanio-phone/runtime/DaanioPhoneBreakerLambda"
      ],
      "Condition": {
        "StringEquals": {
    ...
    },
    {
      ...
      "Resource": [
        "arn:aws:logs:us-east-1:302154194530:log-group:/aws/lambda/daanio-phone-wake:*",
        "arn:aws:logs:us-east-1:302154194530:log-group:/aws/lambda/daanio-guard-breaker:*"
      ]
    },
    {
      ...
      "Resource": [
        "arn:aws:sns:us-east-1:302154194530:daanio-guard-stop",
        "arn:aws:sns:us-east-1:302154194530:daanio-guard-warn"
      ]
    },
    {
      "Sid": "ReadNamedBudget",
      ...
      "Resource": "arn:aws:budgets::302154194530:budget/daanio-dev-monthly-cost"
    }
  ]
}
```
...
- Human `DaanioPhoneProvisioner`: CloudFormation stack operations on `daanio-phone-server*`, read-only diagnostics, and `iam:PassRole` only for `DaanioPhoneCloudFormationExecution` with `iam:PassedToService = cloudformation.amazonaws.com`.
- `DaanioPhoneCloudFormationExecution`: service permissions below, usable only by CloudFormation.
- Every created resource is tagged `Project=daanio-phone-server` and `ManagedBy=cloudformation`.
- Every created runtime role is under path `/daanio-phone/runtime/` and must carry the `DaanioPhoneRuntimeBoundary` permissions boundary.
  ...
  "Statement": [
    {
      ...
      "Resource": [
        "arn:aws:cloudformation:us-east-1:302154194530:stack/daanio-phone-server*/*",
        "arn:aws:cloudformation:us-east-1:302154194530:changeSet/daanio-phone-server*/*"
      ]
    },
    {
    ...
    },
    {
      ...
      "Effect": "Allow",
      "Action": "iam:PassRole",
      "Resource": "arn:aws:iam::302154194530:role/daanio-phone/DaanioPhoneCloudFormationExecution",
      "Condition": {
        "StringEquals": {
...
|---|---|---|
| EC2 | Run/terminate the one server; create/tag volume and ENI; create/manage one SG; allocate/associate/release one EIP; modify shutdown behavior; describe AMIs/subnets/VPCs | `us-east-1`; request/resource tag `Project=daanio-phone-server`; approved instance types only; IMDSv2 required; approved VPC/subnet |
| IAM | Create/update/delete the three runtime roles and one instance profile; put/delete inline policies; pass roles | Path `/daanio-phone/runtime/` only; require boundary ARN on `CreateRole`; never allow changing deploy/provisioner/emergency roles |
| Lambda | Create/update/delete the two named functions; versions/aliases; permissions | Function ARN prefix `daanio-phone-*` and `daanio-guard-breaker*`; project tag |
| API Gateway v2 | Create/update/delete one HTTP API, integration, route, and stage | Project tag and stack ownership |
| SNS | Create/manage/delete `daanio-guard-stop` and `daanio-guard-warn`; subscriptions | Exact topic-name ARNs |
...
| Budgets | Create/update/delete `daanio-dev-monthly-cost` | Exact budget ARN |
...
aws lambda get-function --function-name daanio-phone-wake --region us-east-1
aws lambda get-function --function-name daanio-guard-breaker --region us-east-1
aws apigatewayv2 get-api --api-id 8c3wp4cbag --region us-east-1
aws sns list-topics --region us-east-1
aws cloudwatch describe-alarms --alarm-name-prefix daanio- --region us-east-1
aws budgets describe-budget --account-id 302154194530 --budget-name daanio-dev-monthly-cost
...
8. **Canary mutation paths without touching production.** Through the provisioner, deploy and remove a tiny tagged canary stack using the same resource classes where practical. For the operator, update a disposable Lambda alias or canary function rather than invoking `daanio-guard-breaker`, which stops production. Do not use the breaker invocation as an IAM test.
...
- Daily deployment and maintenance succeed through `DaanioPhoneOperator`.
