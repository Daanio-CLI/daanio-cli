...
closes that gap by driving the **real daanio binary** under a pseudo-terminal and
...
Everything runs in a throwaway DAANIO_HOME / runtime dir / socket. The user's
...
# protocol daanio turns on at startup):
...
# ──────────────────────────────────────────────────────────────────────────────
# A PTY-driven daanio client. This is the real live-render input path; bytes
...
def launch_client(binary: str, env: dict, session_id: str, name: str,
    ...
    cenv["DAANIO_DEBUG_CMD_PATH"] = str(debug_cmd)
    cenv["DAANIO_DEBUG_RESPONSE_PATH"] = str(debug_resp)
    proc = subprocess.Popen(
        [
            binary,
            "--no-update",
            "--no-selfdev",
            "--socket", env["DAANIO_SOCKET"],
            "--resume", session_id,
        ],
...
def main() -> int:
    ...
    default_bin = Path.home() / ".daanio" / "builds" / "current" / "daanio"
    ...
    root = Path(tempfile.mkdtemp(prefix="daanio-expand-edit-"))
    home = root / "home"
    run = root / "run"
    ...
    env = os.environ.copy()
    env["DAANIO_HOME"] = str(home)
    env["DAANIO_RUNTIME_DIR"] = str(run)
    env["DAANIO_SOCKET"] = str(run / "daanio.sock")
    env["DAANIO_NO_TELEMETRY"] = "1"
    env["DAANIO_DEBUG_CONTROL"] = "1"
    env["DAANIO_TEMP_SERVER"] = "1"
    env["DAANIO_SERVER_OWNER_PID"] = str(os.getpid())
    debug_sock = run / "daanio-debug.sock"
    ...
    server = subprocess.Popen(
        [binary, "serve", "--socket", env["DAANIO_SOCKET"], "--debug-socket",
    ...
    rc = 3
    try:
        wait_for_socket(Path(env["DAANIO_SOCKET"]))
