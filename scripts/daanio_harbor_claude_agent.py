...
IN_CONTAINER_HOME = "/tmp/daanio-home"
IN_CONTAINER_RUNTIME = "/tmp/daanio-runtime"
IN_CONTAINER_INPUT = "/tmp/daanio-input"
IN_CONTAINER_OUTPUT = "/tmp/daanio-output"
IN_CONTAINER_BINARY = "/usr/local/bin/daanio"
IN_CONTAINER_LIB_DIR = f"{IN_CONTAINER_RUNTIME}/lib"
IN_CONTAINER_CA_BUNDLE = f"{IN_CONTAINER_HOME}/ca-certificates.crt"
DEFAULT_BINARY_PATH = "/tmp/daanio-compat-dist/daanio-linux-x86_64.bin"
DEFAULT_CLAUDE_AUTH_PATH = "~/.daanio/auth.json"
DEFAULT_OPENROUTER_ENV_PATH = "~/.config/daanio/openrouter.env"
DEFAULT_ANTHROPIC_ENV_PATH = "~/.config/daanio/anthropic.env"
CA_BUNDLE_CANDIDATES = (
    os.environ.get("DAANIO_HARBOR_CA_BUNDLE"),
...
def _load_anthropic_key() -> str | None:
    return _load_key_from_env_file(
        os.environ.get("DAANIO_HARBOR_ANTHROPIC_ENV", DEFAULT_ANTHROPIC_ENV_PATH),
        "ANTHROPIC_API_KEY",
        "ANTHROPIC_API_KEY",
        "DAANIO_HARBOR_ANTHROPIC_KEY",
    )


def _load_openrouter_key() -> str | None:
    # Priority: explicit env, then the daanio openrouter.env file (raw key per line).
    direct = os.environ.get("OPENROUTER_API_KEY") or os.environ.get("DAANIO_HARBOR_OPENROUTER_KEY")
    if direct and direct.strip():
        return direct.strip()
    path = Path(os.environ.get("DAANIO_HARBOR_OPENROUTER_ENV", DEFAULT_OPENROUTER_ENV_PATH)).expanduser()
    if path.exists() and path.is_file():
        for line in path.read_text().splitlines():
            line = line.strip()
            if not line or line.startswith("#"):
...
DAANIO_BINARY = _resolve_existing_file(
    env_name="DAANIO_HARBOR_BINARY",
    default_path=DEFAULT_BINARY_PATH,
)
CA_BUNDLE = _resolve_existing_file(
    env_name="DAANIO_HARBOR_CA_BUNDLE",
...
OPENSSL_RUNTIME_LIBS = tuple(
    ...
    for lib in (
        _resolve_optional_existing_file(
            candidates=(
                os.environ.get("DAANIO_HARBOR_LIBSSL"),
                *_sibling_runtime_lib_candidates(DAANIO_BINARY, "libssl"),
                "/usr/lib/libssl.so.3",
        ...
        _resolve_optional_existing_file(
            candidates=(
                os.environ.get("DAANIO_HARBOR_LIBCRYPTO"),
                *_sibling_runtime_lib_candidates(DAANIO_BINARY, "libcrypto"),
...
def _benchmark_instruction_preamble() -> str:
    return os.environ.get("DAANIO_HARBOR_EXTRA_PREAMBLE", "")
...
class DaanioClaudeHarborAgent(BaseAgent):
    """Harbor adapter that runs daanio with Opus 4.8.
    ...
    OAuth (provider=claude via ~/.daanio/auth.json) are also supported.
    """
    ...
    def __init__(self, logs_dir: Path, model_name: str | None = None, *args, **kwargs):
        ...
        if "/" in self._model_arg:
            self._provider_arg, self._daanio_model = self._model_arg.split("/", 1)
        else:
            self._provider_arg, self._daanio_model = "anthropic-api", self._model_arg
        ...
        if self._provider_arg == "claude":
            self._claude_auth = _resolve_existing_file(
                env_name="DAANIO_HARBOR_CLAUDE_AUTH",
                default_path=DEFAULT_CLAUDE_AUTH_PATH,
            )

    @staticmethod
    def name() -> str:
        return "daanio-harbor-claude"
    ...
    async def setup(self, environment: BaseEnvironment) -> None:
        ...
        # "error setting certificate file") so tests never run. daanio itself
        ...
        await environment.upload_file(DAANIO_BINARY, IN_CONTAINER_BINARY)
        ...
        version_result = await environment.exec(
            ...
            env={
                ...
                "DAANIO_HOME": IN_CONTAINER_HOME,
                "DAANIO_RUNTIME_DIR": IN_CONTAINER_RUNTIME,
    ...
    async def run(self, instruction: str, environment: BaseEnvironment, context: AgentContext) -> None:
        ...
        env = {
            ...
            "DAANIO_RUNTIME_DIR": IN_CONTAINER_RUNTIME,
            "DAANIO_NO_TELEMETRY": "1",
            "DAANIO_PROVIDER": self._provider_arg,
            "DAANIO_MODEL": self._daanio_model,
            "DAANIO_ANTHROPIC_REASONING_EFFORT": os.environ.get("DAANIO_ANTHROPIC_REASONING_EFFORT", "high"),
        ...
        result = await environment.exec(
            command=(
                ...
                '--provider "$DAANIO_PROVIDER" --model "$DAANIO_MODEL" '
        ...
        try:
            await environment.download_dir(IN_CONTAINER_OUTPUT, self.logs_dir / "daanio-output")
        ...
        metadata: dict[str, Any] = {
            ...
            "model": self._daanio_model,
            "daanio_binary": str(DAANIO_BINARY),
        }

        output_dir = self.logs_dir / "daanio-output"
        ...
        if payload is not None:
            ...
            metadata["daanio_result"] = payload
