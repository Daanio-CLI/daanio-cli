...
Runs a full-feature selfdev daanio build with Cargo timings enabled and summarizes
...
Options:
  ...
  --package <name>          Cargo package to build (default: daanio)
  --bin <name>              Cargo binary to build (default: daanio)
  --feature-profile <name>  DAANIO_DEV_FEATURE_PROFILE for dev_cargo.sh (default: default)
...
Notes:
  ...
  - The "daanio serial stack" summary is not a formal Cargo critical path. It is a
    focused view of the known long-pole crates: daanio-base, daanio-app-core,
    daanio-tui, root daanio lib, and daanio bin.
USAGE
}
...
PY
  ...
  DAANIO_DEV_FEATURE_PROFILE="$feature_profile" \
...
def is_daanio_stack_unit(unit: dict[str, Any]) -> bool:
    name = unit.get("name")
    target = unit.get("target") or ""
    if name in {"daanio-base", "daanio-app-core", "daanio-tui"} and "build script" not in target:
        return True
    if name == "daanio" and (target == "" or f'bin "{bin_name}"' in target):
        return True
    return False

daanio_stack = sorted([unit for unit in units if is_daanio_stack_unit(unit)], key=lambda unit: float(unit.get("start", 0.0)))
stack_span = None
if daanio_stack:
    stack_span = max(float(unit["end"]) for unit in daanio_stack) - min(float(unit.get("start", 0.0)) for unit in daanio_stack)
stack_sum = sum(float(unit.get("duration", 0.0)) for unit in daanio_stack)
stack_frontend_sum = sum(float(unit.get("frontend_duration") or 0.0) for unit in daanio_stack)
stack_codegen_sum = sum(float(unit.get("codegen_duration") or 0.0) for unit in daanio_stack)
