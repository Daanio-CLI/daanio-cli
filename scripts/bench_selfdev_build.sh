...
#   2. leaf touch        - 1-line change in the root `daanio` bin crate
#   3. tui touch         - 1-line change in daanio-tui (largest UI crate)
#   4. core touch        - 1-line change in daanio-app-core (mid-stack crate)
#   5. commit blast      - simulate HEAD moving (DAANIO_BUILD_GIT_HASH change),
#                          which reruns daanio-build-meta's build script and
...
# `scripts/dev_cargo.sh build --profile selfdev -p daanio --bin daanio` succeeds.
#
...
build_cmd=(scripts/dev_cargo.sh build --profile selfdev -p daanio --bin daanio)
...
tui_file="crates/daanio-tui/src/lib.rs"
echo "3. tui touch (${tui_file}):"
touch_file "$tui_file"
run_build "tui" || true
revert_file "$tui_file"

core_file="crates/daanio-app-core/src/lib.rs"
echo "4. core touch (${core_file}):"
touch_file "$core_file"
run_build "core" || true
revert_file "$core_file"

echo "5. commit blast radius (simulated HEAD move via DAANIO_BUILD_GIT_HASH):"
fake_hash="bench$(date +%s | tail -c 4)"
start=$(date +%s.%N)
if DAANIO_BUILD_GIT_HASH="$fake_hash" "${build_cmd[@]}" >/tmp/bench_selfdev_build_commit-blast.log 2>&1; then
    end=$(date +%s.%N)
...
echo "    daanio-build-meta embeds GIT_HASH via env! and daanio-base/app-core/"
