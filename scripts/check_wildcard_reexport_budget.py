...
Counts production Rust occurrences of `pub use daanio_<crate>::*;` (cross-crate
...
`daanio_*` cross-crate wildcards are budgeted.
...
PATTERN = re.compile(r"^\s*pub\s+use\s+(?:::)?daanio_[a-z0-9_]+(?:::[A-Za-z0-9_]+)*::\*\s*;")
