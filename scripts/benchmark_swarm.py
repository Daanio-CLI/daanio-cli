...
Compares daanio's swarm (multi-agent coordination) with single-agent performance
...
Environment:
    Requires daanio server running with debug_control enabled:
        touch ~/.daanio/debug_control
        daanio serve
"""
...
def main():
    ...
    if not os.path.exists(DEBUG_SOCKET):
        ...
        print("Make sure daanio server is running with debug_control enabled:")
        print("  touch ~/.daanio/debug_control")
        print("  daanio serve")
        sys.exit(1)
