...
DEFAULT_BINARY_PATH = "/tmp/daanio-compat-dist/daanio-linux-x86_64"
DEFAULT_OPENAI_AUTH_PATH = "~/.daanio/openai-auth.json"
...
OPENAI_AUTH = _resolve_existing_file(
    env_name="DAANIO_HARBOR_OPENAI_AUTH",
...
def _benchmark_instruction_preamble() -> str:
    # Keep Harbor runs aligned with normal TUI/daanio-run prompting by default.
    # The legacy preamble can still be enabled explicitly for reproducing older
    # runs, but new benchmark runs should rely on daanio's normal system prompt
    # and the official Terminal-Bench task instruction.
    if os.environ.get("DAANIO_HARBOR_LEGACY_PREAMBLE"):
        return LEGACY_BENCHMARK_INSTRUCTION_PREAMBLE
    return os.environ.get("DAANIO_HARBOR_EXTRA_PREAMBLE", "")


def _load_task_hint() -> str:
    if not os.environ.get("DAANIO_HARBOR_ENABLE_HINTS"):
        return ""
    task_name = os.environ.get("DAANIO_HARBOR_CURRENT_TASK", "").strip()
    hints_path = os.environ.get("DAANIO_HARBOR_TASK_HINTS_FILE", "").strip()
    extra = os.environ.get("DAANIO_HARBOR_EXTRA_PREAMBLE", "").strip()
    parts: list[str] = []
    if extra:
        parts.append(extra)
...
class DaanioHarborAgent(BaseAgent):
    def __init__(self, logs_dir: Path, model_name: str | None = None, *args, **kwargs):
        ...
        if "/" in self._model_arg:
            ...
            self._provider_arg, self._daanio_model = "openai", self._model_arg

    @staticmethod
    def name() -> str:
        return "daanio-harbor"
    ...
    async def run(self, instruction: str, environment: BaseEnvironment, context: AgentContext) -> None:
        ...
        env = {
            ...
            "DAANIO_OPENAI_REASONING_EFFORT": os.environ.get("DAANIO_OPENAI_REASONING_EFFORT", "high"),
            "DAANIO_OPENAI_SERVICE_TIER": os.environ.get("DAANIO_OPENAI_SERVICE_TIER", "priority"),
