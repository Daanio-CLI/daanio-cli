#!/usr/bin/env python3
"""Report compile-time isolation risks in the Daanio crate graph.
...
WATCHED_CRATES = ["daanio-base", "daanio-app-core", "daanio-tui", "daanio"]
...
def target_state_violations(stats_by_name: dict[str, CrateStats]) -> list[str]:
    ...
    tui = stats_by_name.get("daanio-tui")
    if tui and "daanio-app-core" in tui.normal_workspace_deps:
        violations.append("target-state: daanio-tui still directly depends on daanio-app-core")

    app_core = stats_by_name.get("daanio-app-core")
    if app_core and "daanio-base" in app_core.normal_workspace_deps:
        violations.append("target-state: daanio-app-core still directly depends on daanio-base")

    base = stats_by_name.get("daanio-base")
    if base:
        for dep in base.normal_workspace_deps:
            if dep in {
                "daanio-azure-auth",
                "daanio-provider-gemini",
                "daanio-provider-openai",
                "daanio-provider-openrouter",
                "daanio-notify-email",
                "daanio-build-support",
            }:
                violations.append(f"target-state: daanio-base still depends on leaf/runtime crate {dep}")
        for dep in base.normal_external_deps:
            if dep.startswith("aws-") or dep in {"aws-types"}:
                violations.append(f"target-state: daanio-base still depends directly on AWS crate {dep}")

    for crate in stats_by_name.values():
        for glob in crate.glob_reexports:
            if crate.name in {"daanio", "daanio-app-core", "daanio-tui"}:
