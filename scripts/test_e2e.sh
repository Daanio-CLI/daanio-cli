#!/bin/bash
# End-to-end test script for daanio
...
echo "=== E2E Testing Script for daanio ==="
echo ""

# Test 1: Check binary exists and runs
echo "Test 1: Check daanio binary..."
if command -v daanio &> /dev/null; then
    echo "✓ daanio binary found"
    daanio --version
else
    echo "✗ daanio binary not found"
    exit 1
fi
...
if [[ "${DAANIO_REAL_PROVIDER:-0}" == "1" ]]; then
    echo ""
    echo "Test 7: Real provider smoke (DAANIO_REAL_PROVIDER=1)..."
    scripts/real_provider_smoke.sh
    echo "✓ Real provider smoke passed"
fi

if [[ "${DAANIO_REAL_AUTH_TEST:-0}" == "1" ]]; then
    echo ""
    echo "Test 8: Auth E2E validation (DAANIO_REAL_AUTH_TEST=1)..."
...
echo "  daanio        # Start TUI mode"
echo "  daanio server # Start server mode"
echo "  daanio client # Connect to server"
