...
fixture_root=${DAANIO_AUTH_FIXTURE_DIR:-$fixture_root_default}
...
Commands:
  ...
  save <name>                  Save the current sandbox DAANIO_HOME as a fixture
  load <name>                  Replace the sandbox DAANIO_HOME with a saved fixture
  reset-sandbox                Remove only the current sandbox DAANIO_HOME
  delete <name>                Delete a saved fixture
  env <name>                   Print exports for running against a loaded fixture
  run <name> -- <args...>      Load fixture, then run daanio with args in sandbox
  help                         Show this help

Environment overrides:
  DAANIO_ONBOARDING_SANDBOX     Sandbox name to load into/from (default: default)
  DAANIO_ONBOARDING_DIR         Explicit onboarding sandbox directory
...
run_daanio() {
  ...
  (
    ...
    if [[ -x "$binary_path" ]]; then
      env DAANIO_HOME="$daanio_home" DAANIO_RUNTIME_DIR="$runtime_dir" "$binary_path" "$@"
    else
      env DAANIO_HOME="$daanio_home" DAANIO_RUNTIME_DIR="$runtime_dir" cargo run --bin daanio -- "$@"
    fi
  )
}
...
save_fixture() {
  ...
  if [[ ! -d "$daanio_home" ]]; then
    echo "sandbox DAANIO_HOME does not exist: $daanio_home" >&2
    exit 1
  fi
  mkdir -p "$(dirname "$dst")"
  copy_dir_contents "$daanio_home" "$dst"
  chmod -R go-rwx "$(dirname "$dst")" 2>/dev/null || true
  cat > "$meta" <<EOF
name=$name
saved_at=$(date -u +%Y-%m-%dT%H:%M:%SZ)
sandbox_name=$sandbox_name
source_daanio_home=$daanio_home
warning=May contain real local auth tokens. Do not commit or share.
EOF
  echo "Saved auth fixture '$name' from $daanio_home"
...
load_fixture() {
  ...
  copy_dir_contents "$src" "$daanio_home"
  chmod -R go-rwx "$daanio_home" 2>/dev/null || true
...
case "$command" in
  ...
  reset-sandbox)
    ...
    echo "Reset sandbox DAANIO_HOME: $daanio_home"
    ;;
  delete)
    name=$(require_name "${1:-}")
    delete_fixture "$name"
