...
def state_path() -> Path:
    configured = os.environ.get("DAANIO_DEMO_SHOP_STATE")
    if configured:
        return Path(configured).expanduser()
    return Path(f"/tmp/daanio-demo-shop-{os.getuid()}.json")
...
def command_search(args: argparse.Namespace) -> int:
    ...
    print("Add one with: daanio-demo-shop add <sku>")
    return 0
...
def command_add(args: argparse.Namespace) -> int:
    ...
    print("Review it with: daanio-demo-shop cart")
    print("Continue with: daanio-demo-shop checkout")
    print(
