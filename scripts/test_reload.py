...
def daanio_debug_socket():
    """Find the active daanio debug socket path."""
    runtime_dir = os.environ.get("XDG_RUNTIME_DIR") or f"/run/user/{os.getuid()}"
    return os.path.join(runtime_dir, "daanio-debug.sock")
...
def dbg(command, session_id=None, timeout=TIMEOUT_SECS):
    ...
    return _send_recv(daanio_debug_socket(), req, timeout=timeout)
...
def test_reload_context_roundtrip():
    daanio_dir = pathlib.Path.home() / ".daanio"
    ctx_path = daanio_dir / "reload-context.json"
...
def test_reload_info_file():
    daanio_dir = pathlib.Path.home() / ".daanio"
    info_path = daanio_dir / "reload-info"
...
def test_canary_binary_path():
    ...
    manifest_path = home / ".daanio" / "build-manifest.json"
    ...
    if canary_hash:
        canary_binary = home / ".daanio" / "builds" / "canary" / "daanio"
...
def test_multiple_sessions_responsive():
    ...
    try:
        for i in range(N):
            s = create_session(f"/tmp/daanio-test-{i}")
            sessions.append(s)
...
def check_server_up():
    dbg_sock = daanio_debug_socket()
    if not os.path.exists(dbg_sock):
        raise RuntimeError(
            f"Debug socket not found at {dbg_sock}.\n"
            "  Start daanio server first:\n"
            "    daanio serve   (or just launch daanio in the repo)"
        )
    try:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Test daanio selfdev reload mechanism",
    ...
    print("\n╔══════════════════════════════════════════════════════════╗")
    print("║      daanio selfdev reload test suite                    ║")
    print("╚══════════════════════════════════════════════════════════╝\n")
