...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Profile a single resumed daanio spawn")
    parser.add_argument("--binary", default="./target/selfdev/daanio")
...
def latest_log_file(log_dir: Path) -> Path:
    logs = sorted(log_dir.glob("daanio-*.log"), key=lambda p: p.stat().st_mtime)
    if not logs:
...
def profile_single_spawn(binary: str, cwd: str, timeout_s: float) -> dict:
    root = Path(tempfile.mkdtemp(prefix="daanio-single-profile-"))
    home = root / "home"
    runtime_dir = root / "run"
    socket_path = runtime_dir / "daanio.sock"
    debug_socket_path = runtime_dir / "daanio-debug.sock"
    home.mkdir(parents=True, exist_ok=True)
    runtime_dir.mkdir(parents=True, exist_ok=True)
    env = os.environ.copy()
    env.update(
        {
            "DAANIO_HOME": str(home),
            "DAANIO_RUNTIME_DIR": str(runtime_dir),
            "DAANIO_SOCKET": str(socket_path),
            "DAANIO_DEBUG_SOCKET": str(debug_socket_path),
            "DAANIO_SWARM_ENABLED": "0",
            "DAANIO_NO_TELEMETRY": "1",
            "DAANIO_TRACE": "1",
            "DAANIO_TEMP_SERVER": "1",
            "DAANIO_SERVER_OWNER_PID": str(os.getpid()),
        }
    )
    ...
    try:
        ...
        while time.perf_counter() < deadline:
            ...
            if rlist:
                ...
                lower = buffer.lower()
                if b"loading session" in lower or b"daanio" in lower or len(buffer) > 4096:
