...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Profile resumed daanio PTY burst startup")
    p.add_argument("--binary", default="./target/release/daanio")
    p.add_argument("--burst", type=int, default=20)
    p.add_argument("--timeout", type=float, default=15.0)
    p.add_argument("--stagger-ms", type=float, default=0.0)
    p.add_argument("--json-out", default="/tmp/daanio_remote_burst_profile.json")
    return p.parse_args()
...
def run_burst(
    ...
    while time.perf_counter() < deadline and (
        ...
        for fd in rlist:
            ...
            if b"loading session" in lower or b"daanio" in lower or len(client.buffer) > 2048:
                client.done = True
...
def main() -> None:
    args = parse_args()
    root = Path(tempfile.mkdtemp(prefix="daanio-remote-burst-"))
    home = root / "home"
    run = root / "run"
    ...
    debug_socket = run / "daanio-debug.sock"

    server = subprocess.Popen(
        [args.binary, "serve", "--socket", env["DAANIO_SOCKET"], "--debug-socket"],
        env=env,
