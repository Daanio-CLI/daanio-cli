...
pub async fn run_debug_command(
    ...
    if !crate::transport::is_socket_path(&debug_socket) {
        ...
        eprintln!("\nMake sure:");
        eprintln!("  1. A daanio server is running (daanio or daanio serve)");
        eprintln!("  2. debug_socket is enabled in ~/.daanio/config.toml");
        eprintln!("     [display]");
        eprintln!("     debug_socket = true");
        eprintln!("\nOr use 'daanio debug start' to start a server.");
        eprintln!("Use 'daanio debug list' to see running servers.");
...
async fn debug_list_servers() -> Result<()> {
    ...
    if servers.is_empty() {
        println!("No running daanio servers found.");
        println!("\nStart one with: daanio debug start");
        return Ok(());
    }

    println!("Running daanio servers:\n");
    ...
    println!("  daanio debug -s /path/to/socket.sock sessions");
...
async fn debug_start_server(arg: &str, socket_path: Option<String>) -> Result<()> {
            ...
            .is_ok()
    {
        ...
        eprintln!("Use 'daanio debug list' to see all servers.");
        return Ok(());
    }
    ...
    eprintln!("Starting daanio server...");
    ...
    if crate::transport::is_socket_path(&debug_socket) {
        ...
        eprintln!("⚠ Debug socket not enabled. Add to ~/.daanio/config.toml:");
        eprintln!("  [display]");
        eprintln!("  debug_socket = true");
    }
