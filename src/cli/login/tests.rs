...
fn scriptable_resume_command_matches_input_kind() {
    assert_eq!(
        ...
        "daanio login --provider openai --callback-url '<url-or-query>'"
    );
    assert_eq!(
        scriptable_resume_command("gemini", "auth_code"),
        "daanio login --provider gemini --auth-code '<code>'"
    );
    assert_eq!(
        scriptable_resume_command("copilot", "complete"),
        "daanio login --provider copilot --complete"
    );
}
...
fn load_pending_login_removes_expired_record() {
    ...
    set_or_clear_env("DAANIO_HOME", prev_home);
}
