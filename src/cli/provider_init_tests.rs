...
fn test_provider_choice_arg_values() {
    assert_eq!(ProviderChoice::Daanio.as_arg_value(), "daanio");
...
async fn explicit_anthropic_api_choice_pins_api_key_over_available_oauth() {
    ...
    assert_eq!(
        std::env::var("DAANIO_RUNTIME_PROVIDER").ok().as_deref(),
        Some("claude-api")
    );
...
#[test]
fn test_init_provider_daanio_uses_browser_credential_and_managed_gateway() {
    let _guard = lock_env();
    let _env_guard = crate::storage::lock_test_env();
    // Sandbox DAANIO_HOME: with the real home, persisted auth/credential state
    // (e.g. a pinned anthropic api-key route) re-pins DAANIO_RUNTIME_PROVIDER
    // during MultiProvider construction and breaks the assertions below.
    let dir = TempDir::new().expect("temp dir");
    let saved_home = std::env::var("DAANIO_HOME").ok();
    crate::env::set_var("DAANIO_HOME", dir.path());
    crate::subscription_catalog::clear_runtime_env();
    crate::env::remove_var("DAANIO_OPENROUTER_MODEL");
    ...
    assert!(profile_for_choice(&ProviderChoice::Daanio).is_none());
    assert_eq!(
        provider_catalog::DAANIO_LOGIN_PROVIDER.auth_kind,
        provider_catalog::LoginProviderAuthKind::DeviceCode
    );
    assert_eq!(
        provider_catalog::DAANIO_LOGIN_PROVIDER.target,
        provider_catalog::LoginProviderTarget::Daanio
    );
    ...
    assert_eq!(
        std::env::var("DAANIO_OPENROUTER_CACHE_NAMESPACE")
            .ok()
            .as_deref(),
        Some(crate::subscription_catalog::DAANIO_CACHE_NAMESPACE)
    );
    ...
    crate::env::remove_var("DAANIO_FORCE_PROVIDER");
    match saved_home {
        Some(home) => crate::env::set_var("DAANIO_HOME", home),
        None => crate::env::remove_var("DAANIO_HOME"),
    }
}
...
fn choice_for_login_provider_round_trips_core_targets() {
    assert_eq!(
        choice_for_login_provider(provider_catalog::DAANIO_LOGIN_PROVIDER),
        Some(ProviderChoice::Daanio)
    );
    assert_eq!(
...
fn resolved_profile_default_model_uses_openai_compatible_override() {
    ...
    crate::env::set_var("DAANIO_OPENAI_COMPAT_DEFAULT_MODEL", "llama3.2");
...
fn apply_login_provider_profile_env_locks_compatible_profile_for_auto_spawn() {
    ...
    assert_eq!(
        std::env::var("DAANIO_PROVIDER_PROFILE_ACTIVE")
            .ok()
            .as_deref(),
        Some("1")
    );
...
async fn init_provider_for_ollama_reapplies_local_compat_runtime_env_after_disabling_subscription_mode()
 {
    ...
    assert_eq!(
        std::env::var("DAANIO_FORCE_PROVIDER").ok().as_deref(),
        Some("1")
    );
    assert_eq!(
        std::env::var("DAANIO_ACTIVE_PROVIDER").ok().as_deref(),
        Some("openrouter")
    );
    assert_eq!(
        std::env::var("DAANIO_RUNTIME_PROVIDER").ok().as_deref(),
...
async fn auto_provider_uses_config_default_named_no_auth_provider() {
    ...
    let saved: Vec<(String, Option<String>)> = [
        ...
        "DAANIO_OPENROUTER_ALLOW_NO_AUTH",
        "DAANIO_OPENROUTER_ENV_FILE",
        "DAANIO_OPENROUTER_DEFAULT_MODEL",
        "DAANIO_OPENROUTER_CACHE_NAMESPACE",
        "DAANIO_PROVIDER_PROFILE_ACTIVE",
        "DAANIO_PROVIDER_PROFILE_NAME",
        "DAANIO_NAMED_PROVIDER_PROFILE",
...
async fn auto_provider_noninteractive_skips_untrusted_external_auth_instead_of_blocking() {
    ...
    let saved: Vec<(String, Option<String>)> = [
        "DAANIO_HOME",
        "DAANIO_NON_INTERACTIVE",
        "DAANIO_DEFERRED_AUTH_BOOTSTRAP",
        "ANTHROPIC_API_KEY",
        "OPENAI_API_KEY",
        "OPENROUTER_API_KEY",
        "GITHUB_TOKEN",
