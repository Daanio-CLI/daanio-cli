...
pub fn hot_reload(session_id: &str) -> Result<()> {
    ...
    if let Ok(migrate_binary) = std::env::var("DAANIO_MIGRATE_BINARY") {
        ...
        if binary_path.exists() {
            ...
            cmd.arg("--resume")
                .arg(session_id)
                .arg("--no-update")
                .env_remove("DAANIO_MIGRATE_BINARY")
                .current_dir(cwd);
            let err = crate::platform::replace_process(&mut cmd);
...
pub fn hot_update(session_id: &str) -> Result<()> {
    ...
    match update::check_for_update_blocking() {
        Ok(Some(release)) => {
            let current = daanio_build_meta::version();
...
/// Minimum interval between `git fetch` update probes across all daanio
...
fn claim_update_fetch_slot() -> bool {
    let Ok(base) = crate::storage::daanio_dir() else {
...
pub fn run_auto_update() -> Result<()> {
    ...
    let repo_dir =
        get_repo_dir().ok_or_else(|| anyhow::anyhow!("Could not find daanio repository"))?;
...
pub fn run_update() -> Result<()> {
    if update::is_release_build() {
        ...
        match update::check_for_update_blocking() {
            Ok(Some(release)) => {
                ...
                reload_server_after_update("installed update");
                update::print_centered("Restart daanio to use the new version.");
            }
            Ok(None) => {
                if repair_stale_shared_server_after_update_check() {
                    reload_server_after_update("repaired stale server target");
    ...
    update::print_centered(&format!("Updating daanio from {}...", repo_dir.display()));
...
fn reload_server_after_update(reason: &str) {
    ...
    let Some(exe) = exe else {
        crate::logging::warn("update: could not find daanio binary to reload stale server");
        return;
    };
