use anyhow::Result;
use clap::Parser;
use daanio::id::new_id;
use daanio::message::{Message, ToolDefinition};
use daanio::provider::{EventStream, Provider};
use daanio::tool::{Registry, ToolContext, ToolExecutionMode};
use serde_json::json;
use std::path::PathBuf;
use std::sync::Arc;

#[derive(Parser)]
#[command(name = "daanio-harness")]
...
fn create_temp_workspace() -> Result<PathBuf> {
    ...
    path.push(format!("daanio-harness-{}", new_id("run")));
    Ok(path)
}
