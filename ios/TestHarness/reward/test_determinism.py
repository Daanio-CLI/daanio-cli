...
SOURCE_ROOT = str(IOS / "Sources" / "DaanioMobile")
...
def find_a_screenshot() -> str | None:
    ...
    for base in (os.environ.get("TMPDIR", "/tmp"), "/tmp"):
        candidates += glob.glob(os.path.join(base, "daanio-ui-matrix", "*.png"))
        candidates += glob.glob(os.path.join(base, "daanio_ios_*.png"))
