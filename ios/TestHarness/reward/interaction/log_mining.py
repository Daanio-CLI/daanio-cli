"""Ground the daanio-mobile user model in the user's REAL TUI usage logs.

This module streams the last N daily logs under ``~/.daanio/logs`` and mines
...
_DATE_RE = re.compile(r"daanio-(\d{4}-\d{2}-\d{2})\.log$")
...
def _select_log_files(log_dir: str, days: int) -> list[tuple[str, str]]:
    ...
    pattern = os.path.join(log_dir, "daanio-*.log")
...
def mine_usage(log_dir: str = "~/.daanio/logs", days: int = 7) -> UsageProfile:
    ...
    if not files:
        return _default_profile(
            f"No daanio-YYYY-MM-DD.log files under {resolved!r}; using defaults."
        )
