...
A deterministic, no-LLM harness for developing and validating the daanio iOS
client (`DaanioMobile`) end-to-end. It replaces the role of the old Rust
...
- **`mock_gateway.py`** - a self-contained (stdlib-only) mock of the daanio
  server gateway. Speaks the exact wire protocol from
  `crates/daanio-base/src/gateway.rs` on one TCP port:
...
```bash
# Full pipeline, screenshot lands in $TMPDIR/daanio-ios-e2e/chat.png
...
python3 TestHarness/mock_gateway.py &        # or run a real `daanio` gateway
...
`Library/Application Support/daanio-servers.json` when the Keychain is
...
`DaanioKit` (the platform-free client core) is fully unit-tested with `swift
