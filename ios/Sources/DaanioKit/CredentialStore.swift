...
public struct KeychainCredentialStore: CredentialStore {
    private static let service = "com.daanio.mobile.servers"
    ...
    public func loadAll() -> [ServerCredential] {
        ...
        if status != errSecItemNotFound {
            NSLog("DaanioKit: keychain read failed (%d), trying file fallback", status)
        }
    ...
    private func persist(_ credentials: [ServerCredential]) {
        ...
        if status != errSecSuccess {
            NSLog("DaanioKit: keychain write failed (%d), using file fallback", status)
            persistToFile(data)
        }
    }
    ...
    private func persistToFile(_ data: Data) {
        ...
        do {
            ...
            NSLog("DaanioKit: file fallback write failed: %@", error.localizedDescription)
        }
    }
    ...
    private static var fallbackURL: URL {
        ...
        return base.appendingPathComponent("daanio-servers.json")
    }
