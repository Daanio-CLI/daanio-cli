...
@Test func pairURIParsesQRPayload() {
    let payload = PairURI.parse("daanio://pair?host=mybox.ts.net&port=7643&code=123456")
...
@Test func pairURIDefaultsPort() {
    let payload = PairURI.parse("daanio://pair?host=mybox&code=987654")
...
@Test func pairURIRejectsGarbage() {
    ...
    #expect(PairURI.parse("daanio://pair?host=&code=1") == nil)
    #expect(PairURI.parse("daanio://pair?host=x") == nil)
    #expect(PairURI.parse("not a uri") == nil)
}

// MARK: - Request encoding (must match crates/daanio-protocol/src/wire.rs)
...
@Test func decodesServerLifecycleEvents() throws {
    ...
    #expect(
        try ServerEvent.decode(line: #"{"type":"reloading","new_socket":"/tmp/daanio.sock"}"#)
            == .reloading(newSocket: "/tmp/daanio.sock"))
    #expect(
        try ServerEvent.decode(
