...
fn test_ambient_state_lifecycle() {
    use daanio::ambient::{AmbientCycleResult, AmbientState, AmbientStatus, CycleStatus};
...
fn test_ambient_scheduled_queue() {
    use daanio::ambient::{Priority, ScheduledItem, ScheduledQueue};

    let tmp = std::env::temp_dir().join("daanio-test-queue.json");
    ...
    queue.push(ScheduledItem {
        ...
        target: daanio::ambient::ScheduleTarget::Ambient,
...
fn test_adaptive_scheduler_intervals() {
    use daanio::ambient_scheduler::{AdaptiveScheduler, AmbientSchedulerConfig};
...
async fn test_ambient_end_cycle_tool() -> Result<()> {
    ...
    let result = daanio::tool::ambient::take_cycle_result();
...
async fn test_ambient_request_permission_tool() -> Result<()> {
    ...
    daanio::tool::ambient::register_ambient_session(ambient_session_id.clone());

    let response = agent.run_once_capture("Request permission").await?;
    daanio::tool::ambient::unregister_ambient_session(&ambient_session_id);
...
async fn test_ambient_runner_status() {
    use daanio::ambient_runner::AmbientRunnerHandle;
...
async fn test_ambient_runner_trigger_and_stop() {
    use daanio::ambient::AmbientStatus;
    use daanio::ambient_runner::AmbientRunnerHandle;
    use daanio::safety::SafetySystem;
...
fn test_memory_reinforcement_provenance() {
    use daanio::memory::{MemoryCategory, MemoryEntry};
...
fn test_ambient_config_defaults() {
    use daanio::config::AmbientConfig;
...
fn test_ambient_lock() {
    use daanio::ambient::AmbientLock;
    let _env = setup_test_env().expect("failed to setup isolated DAANIO_HOME");
...
async fn test_full_ambient_cycle_simulation() -> Result<()> {
    ...
    agent.set_system_prompt("You are the daanio ambient maintenance agent.");
