...
fn test_safety_classification() {
    use daanio::safety::SafetySystem;

    let safety = SafetySystem::new();

    // Tier 1: auto-allowed
    assert!(safety.classify("read") == daanio::safety::ActionTier::AutoAllowed);
    assert!(safety.classify("glob") == daanio::safety::ActionTier::AutoAllowed);
    assert!(safety.classify("grep") == daanio::safety::ActionTier::AutoAllowed);
    assert!(safety.classify("memory") == daanio::safety::ActionTier::AutoAllowed);
    assert!(safety.classify("todoread") == daanio::safety::ActionTier::AutoAllowed);
    assert!(safety.classify("todowrite") == daanio::safety::ActionTier::AutoAllowed);

    // Tier 2: requires permission
    assert!(safety.classify("bash") == daanio::safety::ActionTier::RequiresPermission);
    assert!(safety.classify("edit") == daanio::safety::ActionTier::RequiresPermission);
    assert!(safety.classify("write") == daanio::safety::ActionTier::RequiresPermission);
    assert!(
        safety.classify("create_pull_request") == daanio::safety::ActionTier::RequiresPermission
    );
    assert!(safety.classify("send_email") == daanio::safety::ActionTier::RequiresPermission);
...
fn test_safety_permission_flow() {
    use daanio::safety::{PermissionRequest, PermissionResult, SafetySystem, Urgency};
...
fn test_safety_transcript() {
    use daanio::safety::{AmbientTranscript, SafetySystem, TranscriptStatus};
...
fn test_safety_summary_generation() {
    use daanio::safety::{ActionLog, ActionTier, SafetySystem};
